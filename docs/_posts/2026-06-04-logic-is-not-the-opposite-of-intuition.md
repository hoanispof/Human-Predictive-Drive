---
layout: post
title: "Logic Is Not the Opposite of Intuition: One Process, Two Observer Labels — and Why Neither Can Verify Itself"
date: 2026-06-04
...
version_history: |
  v0.1 (2026-06-04): Initial draft. 9 sections, ~4,631 words.
  v0.2 (2026-06-05): Rewrite.
    +Damasio vmPFC evidence (§3.3) — fills gap: body-direct essential for ALL decisions including "logical" ones.
    +Expert logic = compiled evidence (§3.2) — fixes evidence imbalance (v0.1 only proved intuition side).
    +Explicit "same process, different chunks" (§5) — aligns with core framework refinement.
    +§8 What This Changes (NEW) — "knowing but not doing" reframe + AI implication.
    Compressed Evans & Stanovich (§4) — same substance, 30% shorter.
    Cut social proof tangent (§7). Removed scattered deep-read boxes (links consolidated in §10).
    ~3,900 words. Shorter, tighter, more complete.
---
...
1. **Evidence that expert intuition and expert logic run the same process** — compiled body-direct pattern recognition, built through domain-specific feedback (Klein 1998, Chase & Simon 1973, Damasio 1994)
2. **A reframe of Kahneman's System 1/2** — the key variable is compilation level, not speed or content type
3. **Why the observer sees "two modes"** — the label ("logic" vs "intuition") tracks domain shareability, not a processing difference. Same process inside; different chunks compiled from different domains.
...
5. **Why neither logic nor intuition can verify itself** — and why domain reality is the only arbiter
...
1. A **high-validity environment** — stable, learnable regularities
...
The full framework — 200+ files, CC0 licensed — is available at **[GitHub — Human Predictive Drive](https://github.com/hoanispof/Human-Predictive-Drive)**.
...
- **[Compile-Taxonomy.md](https://github.com/hoanispof/Human-Predictive-Drive/blob/main/Core-Deep-Dive/Body-Base/Chunk/Compile-Taxonomy.md)** — compilation taxonomy, Type 1/Type 2 transition evidence
