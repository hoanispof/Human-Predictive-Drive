...
updated: 2026-05-17 (v3.0 — Major refine: +Architecture B, +2-Stream, +by-product match, +PFC=Lawyer, +Compiled/Fresh, +Domain=Arbiter, F1/F2 label fix, Entity-Compiled, all version refs updated)
...
scope: |
  ...
  v2.0 KEY CHANGE:
    - Reframe: empathy = observation-level result CỦA connection mechanism
      (Connection = mechanism deeper, Empathy = 1 observable output khi ❸ positive)
    - F1/F2 integrated: empathy = chủ yếu F1 (body simulate), F2 hỗ trợ
    - 3 Generative Primitives context (❶ × ❷ × ❸ từ Connection.md v4.0)
    - 2-luồng organic: L1 (SPM-owned, momentary) + L2 (Entity-Compiled, structural)
      weave xuyên suốt, KHÔNG bolted-on
    - Cross-references updated: Agent-Mechanism.md, SPM v3.0, VP v2.0
  v3.0 KEY CHANGE:
    - F1/F2 label: F1 Compiled + F2 Fresh (NOT Feeling/Logic — observer labels only)
    - Entity-Compiled → Entity-Compiled (per-entity valence compiled through experience)
    - +Architecture B: empathy = general-purpose output (not specialized module)
    - +2-Stream empathy: Stream 1 (automatic, habituates) + Stream 2 (mutual SPM, deepens)
    - +By-product match: empathy fires khi B's by-product matches A's SPM template
    - +PFC=Lawyer: stated empathy ≠ actual body empathy
    - +Compiled/Fresh empathy: cost differential → burnout prediction
    - +Domain=Arbiter: empathy can be wrong, domain feedback = final check
    - All dependency versions updated to current
  Tích hợp: Connection v4.0 + SPM v3.0 + Resonance v1.0 + Inter-Body-Mechanism v1.0
  + VP v2.0 Body-Base Extension + AM §12.2b 2-luồng.
purpose: |
  Central reference file cho toàn bộ Empathy trong framework.
  Thay thế Empathy.md v1.1 (→ backup/Empathy-v1.1-backup.md).
...
dependencies:
  - Connection.md v4.0 — 3 Generative Primitives, 2-Stream, by-product match, PFC=Lawyer
  - Agent-Mechanism.md v2.0 — unified mechanism file, §12 body-need feeder, §12.2b 2-luồng
  - Self-Pattern-Match.md v3.0 — F1 Compiled/F2 Fresh, context-dependent, PFC=Lawyer, 3-cost
  - By-Product-Gap-Resonance.md v1.0 — 2-Stream Architecture, by-product match, anti-match
  - Inter-Body-Mechanism.md v1.0 — Architecture B, 5-Channel, Entity-Compiled, 3-Layer Evolution
  - Valence-Propagation.md v2.0 — per-entity valence, Body-Base Extension, Entity-Compiled
...
>
> Đây là **Self-Pattern-Match F1** — body simulate trạng thái NGƯỜI KHÁC
...
⭐ EMPATHY = OBSERVATION OUTPUT CỦA CONNECTION MECHANISM:
  ...
  Connection (Connection.md v4.0) = observable pattern khi body's social mechanisms
  ...
  ⭐ EMPATHY CÓ 2 LUỒNG REWARD ĐỒNG THỜI:
    ...
    Luồng 2 — Entity-Compiled (structural):
      ...
      = THUỘC VỀ per-agent valence (VP.md v2.0 §2)

    2 luồng sống ở 2 NƠI KHÁC NHAU trong architecture:
      L1 → SPM mechanism (Self-Pattern-Match.md v3.0 §2.1)
      L2 → Entity valence (Valence-Propagation.md v2.0 §2)
  ...
  FLOW CHÍNH:
    ...
    Chunk substrate (Chunk.md v2.0)
      → Agent-Mechanism (detect + model + simulate)
        → Self-Pattern-Match F1 (body-level simulate state agent)
  ...
  SO SÁNH REFERENCE FILES:
    ...
    │ Connection.md v4.0       │ Body ENGAGE agents THẾ NÀO (mechanism)  │
  ...
  FILE NÀY THAY THẾ:
    ...
    Lý do: v1.1 nói "Empathy RỘNG HƠN Connection" — SAI sau Connection v3.0.
    v2.0 reframe: Empathy = observation output CỦA connection mechanism.
    Insights từ v1.1 được tích hợp + refined + organic 2-luồng.

    Empathy-Mirror.md (2026-04-06, ~1,055L → backup/Empathy-Mirror.md)
    Insights tích hợp từ v1.0 trở đi.


  TIỀN ĐỀ BẮT BUỘC ĐỌC:

    1. Chunk.md v2.0 — chunk substrate, compile mechanism
    2. Agent-Mechanism.md — unified model, SPM preview, body-need feeder
    3. Self-Pattern-Match.md v3.0 — F1/F2, context-dependent, valence-gated
    4. Connection.md v4.0 — 3 Generative Primitives, 2-luồng, 2-tầng
  ...
  SCOPE IN:
    ...
    - 7 failure modes + burnout = f(L1/L2)
    - AI era significance

  SCOPE OUT:
    - Connection mechanism chi tiết → Connection.md v4.0
    - SPM mechanism chi tiết → Self-Pattern-Match.md v3.0
    - Giving dynamics chi tiết → 05-Theme-E (Deep-Analysis-Draft/)
    - Education ứng dụng → Empathy-Education.md
    - Resonance chi tiết → By-Product-Gap-Resonance.md
```
...
⭐ ĐỊNH NGHĨA v2.0:
  ...
  Reframe từ v1.1:
    v1.1: "Empathy = Self-Pattern-Match function applied to other agents"
  ...
  Tại sao reframe:
    ...
    → Connection v4.0 làm rõ: empathy = 1 CỤ THỂ output,
  ...
  2 LUỒNG REWARD ĐỒNG THỜI (organic từ cơ chế):
    ...
    Luồng 2 (Entity-Compiled, structural) — CÓ THỂ có hoặc không:
      = Compiled valence SÂU → agent = body-base extension
      = NẾU CÓ: agent's wellbeing = MY wellbeing → sustained drive
      = NẾU KHÔNG: chỉ L1 momentary → burnout nếu L1 negative kéo dài

    → "Nhìn con ăn ngon mà sướng": L1+ (opioid) + L2+ (con = body-ext) = compound
    → "Chăm con ốm dù mệt": L1- (cascade) + L2+ → L2 > L1 → vẫn chăm
    → "Bác sĩ burnout": L1- (cascade) + L2 ≈ 0 → L1 tích lũy → fatigue
  ...
  TẠI SAO "OBSERVABLE OUTPUT" CHỨ KHÔNG PHẢI "MODULE":
    ...
    Bằng chứng:
      → SPM dùng CÙNG chunks cho empathy, giving, teaching, planning
        (Self-Pattern-Match.md v3.0 §11 — multiple use cases)
...
🟡 FRAMEWORK BÁC BỎ 3 CLAIMS CŨ:
  ...
  ❌ REJECTION 2 — Empathy = separate system / emotion / channel:
    ...
    Framework position:
      ...
      → Connection.md v4.0 §1: empathy = SPM F1 + ❸ positive
...
⭐ EMPATHY BUỘC PHẢI CÓ SELF-CHUNKS TÍCH LŨY ĐỦ:
  ...
  Chain kiến trúc (Self-Pattern-Match.md v3.0 §7):
...
🟢 BIRD & COOK 2013 — BẰNG CHỨNG QUYẾT ĐỊNH:
  ...
  CASCADE (Self-Pattern-Match.md v3.0 §12):
...
⭐ EMPATHY = CHỦ YẾU F1, F2 HỖ TRỢ:
  ...
  CONTEXT-DEPENDENT CHUNK SELECTION (SPM v3.0 §4):
  ...
  ⭐ COMPILED/FRESH EMPATHY — COST DIFFERENTIAL (SPM v3.0 §2, Inter-Body-Mechanism.md §3):
...
⭐ EMPATHY = BIOCHEMISTRY THẬT TRONG BODY MÌNH:
  ...
  ⭐ 2 LUỒNG ĐỒNG THỜI — ORGANIC TRONG CƠ CHẾ:
    ...
    LUỒNG 2 — Entity-Compiled (compiled valence):
      = Agent ĐÃ compiled thành body-base extension (VP v2.0 §2)
    ...
    2 LUỒNG GIẢI THÍCH CASES QUAN TRỌNG:
      ...
      "Nhìn con ăn ngon mà sướng":
        L1+ (opioid — thấy con vui) + L2+ (con = body-ext) = COMPOUND REWARD

      "Chăm con ốm dù mệt":
        L1- (cascade ①-④ — thấy con đau) + L2+ (con = body-ext)
        → L2 > L1 → VẪN CHĂM dù body khó chịu

      "Bác sĩ burnout":
        L1- (cascade ①-④ — thấy bệnh nhân đau) + L2 ≈ 0 (chưa compiled)
    ...
    → 2 luồng KHÔNG phải "2 hệ thống" — là 2 OUTPUTS cùng architecture:
      L1 = SPM mechanism output (per-episode)
      L2 = Entity valence compiled output (structural)
...
```
⭐ EMPATHY FIRE QUA 2 STREAMS CỦA RESONANCE (By-Product-Gap-Resonance v1.0 §2):
  ...
  STREAM 1 — HARDWARE/AUTOMATIC EMPATHY:
    ...
    = HABITUATES: thấy lần đầu → mạnh. Thấy lần thứ 100 → yếu đi
      (Weber-Fechner, hedonic treadmill — By-Product-Gap-Resonance v1.0 §2.1)
  ...
  STREAM 2 — SPM MUTUAL EMPATHY (bidirectional, deepens):
    ...
    = ANTI-HABITUATION: Hebbian → more use → stronger → cost GIẢM → empathy TĂNG
      (By-Product-Gap-Resonance v1.0 §2.2)
  ...
  🟡 2-Stream empathy mapping = framework synthesis (Resonance v1.0 + Empathy integration)
...
🟡 4 QUALITY AXES — TỪ AGENT-MECHANISM.MD §7, APPLIED TO EMPATHY:
  ...
  AXES INTERACTION:
    ...
    → High depth + low similarity → mechanical prediction (dog trainer)
    → High feedback + low depth → rapid learning possible (talkative stranger)
...
🟡 EMPATHY-DRIVEN BEHAVIOR = SPECTRUM — 2-LUỒNG LÀ TRỤC PHÂN TÍCH:
  ...
  │ OVERRIDE    │ L2 structural + gene protection  │ cost │ body │ (CÓ       │
  │ (gene)      │ → double drive CÓ CEILING        │      │ ext  │ ceiling)  │
  ├─────────────┼──────────────────────────────────┼──────┼──────┼───────────┤
  │ SCHEMA      │ Cúng dường tới khánh kiệt        │ + có │ ≈ 0  │ ❌ Bug    │
  │ HIJACK      │ KHÔNG có gene carrier bị đe dọa  │ L1   │ ⭐   │ (KHÔNG    │
  │             │ Schema promise drive không L2     │ only │ thiếu│ ceiling)  │
  ├─────────────┼──────────────────────────────────┼──────┼──────┼───────────┤
  ...
  ⭐ KEY — 2-LUỒNG LÀ TRỤC PHÂN BIỆT:
    → Justified override CÓ CEILING vì CÓ L2 (body-level feedback loop)
    → Schema hijack KHÔNG CEILING vì THIẾU L2 (chỉ có L1 + schema promise)
    → = Phân biệt CĂN BẢN hơn "L0 > L1" — là L2 có hay không
```

### §6.2 — Phân biệt justified vs hijack qua 2-luồng

```
⭐ 2 LOẠI OVERRIDE KHÁC CĂN BẢN — L2 QUYẾT ĐỊNH CEILING:

  JUSTIFIED OVERRIDE (evolution DESIGNED):
    → Có gene carrier bị đe dọa (con đói)
    → L2 STRUCTURAL: con = body-base extension (compiled SÂU)
    → L2 + gene protection = DOUBLE drive → override MẠNH
    → NHƯNG: L2 là body-level → có feedback loop TỰ NHIÊN:
      "Nếu mẹ chết → con mất source → L2 TỰ NÓ cảnh báo"
      → "Ăn ít thôi, KHÔNG nhịn chết" = L2 self-regulate
    → = Ceiling TỰ NHIÊN vì L2 = body-level, self-regulating
    → Recovery KHẢ THI: tạm thời hy sinh → phục hồi khi con ổn

  SCHEMA HIJACK (evolution BUG):
    → KHÔNG CÓ gene carrier bị đe dọa
    → L2 THIẾU: sư thầy / tổ chức ≠ body-base extension
      (valence ++ nhưng CHƯA compiled đủ sâu thành structural,
       hoặc schema-based KHÔNG phải interaction-compiled)
    → Drive chủ yếu từ L1 empathy reward + schema promise
  ...
  RANH GIỚI RÕ:
    ...
    Body-base THIẾU + share VÌ gene (L2 có) = JUSTIFIED, có ceiling
    Body-base THIẾU + share VÌ SCHEMA (L2 thiếu) = HIJACK, KHÔNG ceiling
...
🟡 3 PATHWAYS CÓ THỂ STACK — TRIPLE OVERRIDE:
  ...
  VD — Nghiện quyên góp tôn giáo (L2 THIẾU):
    ① Empathy L1: thấy sư/thầy vui → L1 empathy reward
    ② Schema: "cúng = phước, đời sau tốt" → imagine reward
    ③ Status: "người mộ đạo = được tôn trọng" → belonging reward
    → Triple override NHƯNG không có L2 structural backing
...
🟡 TẠI SAO EVOLUTION HARD-WIRE EMPATHY VÀO MỌI ĐỘNG VẬT XÃ HỘI:
  ...
  ⭐ EMPATHY L1 = CƠ CHẾ PHÂN PHỐI TÀI NGUYÊN TỰ NHIÊN:
...
### §7.2 — L2 thêm tầng sustained motivation

```
⭐ L1 GIẢI THÍCH SHARING KHI THỪA — L2 GIẢI THÍCH SUSTAINED CARE:

  L1 = resource distribution khi THỪA (nhóm level):
    = "Share → họ improve → empathy reward"
    = Momentary, per-episode, hoạt động TỐT khi L1 = positive

  L2 = sustained care khi CẦN (kin level):
    → Agent đã compiled thành body-base extension
    → Agent's wellbeing = MY wellbeing (structural)
    → Share VỚI AGENT NÀY = feed body-base MÌNH
    → KHÔNG CẦN đợi "thấy họ improve" (L1 visual input)
    → SUSTAINED drive BẤT KỂ L1 output

  Kin selection (Hamilton 1964) giải thích TẠI SAO evolution select L2:
    = Gene dùng L2 structural drive cho gene carriers (con, ruột thịt)
    → Mẹ share NGAY CẢ KHI L1 = negative (con ốm, con khóc)
    → KHÔNG CẦN "thấy con improve" → structural drive ĐỦ
    → = L2 = evolved extension of kin selection logic

  MIRROR REWARD OVERRIDE (Agent-Mechanism.md §12.3):
    Cây xoài scenario = L1 only (share khi thừa → mirror reward > keep)
    Mẹ chăm con ốm = L2 required (L1 negative → L1 alone KHÔNG incentive)
    → L1 giải thích sharing surplus. L2 giải thích sustained care despite cost.

  → CẢ 2 tồn tại song song, complement nhau:
    L1 (empathy reward) = PULL khi thừa
    L2 (structural drive) = SUSTAIN khi cần

  🟡 L2 as evolved extension of kin selection = framework synthesis
```
...
🟡 EMPATHY + SCARCITY = 2 MẶT CỦA CÙNG ENGINE:
  ...
  ĐIỀU KIỆN ĐỂ L1 SHARING TỐI ƯU:

    ① Body-base CỦA MÌNH ĐÃ ĐỦ:
      → Item dư thật → share = pure L1 reward, không có cost
...
⭐ BURNOUT = f(L1 COST TÍCH LŨY / L2 COMPENSATION):
  ...
  ⭐ L2 COMPENSATION QUYẾT ĐỊNH OUTCOME:
    ...
    ├─────────────────────┼──────────┼───────────┼──────────────────────────┤
    │ Mẹ + con ốm        │ Negative │ STRONG +  │ L2 BÙ L1 cost            │
    │ (cùng intensity)    │ (cascade │ (con =    │ → KHÔNG burnout          │
    │                     │ ①-④)    │ body-ext) │ → chăm tiếp dù mệt      │
    ├─────────────────────┼──────────┼───────────┼──────────────────────────┤
    │ Cha mẹ trẻ khuyết   │ Negative │ STRONG +  │ L2 BÙ NHƯNG L1 cực cao  │
    │ tật (kéo dài)       │ (CỰC    │ (con =    │ → L2 CÓ THỂ không đủ bù │
    │                     │ nhiều)   │ body-ext) │ → fatigue DÙ có L2       │
    └─────────────────────┴──────────┴───────────┴──────────────────────────┘

    ⭐ BURNOUT FORMULA:
      Burnout = f(L1 cost tích lũy / L2 compensation)
      → L2 CAO + L1 vừa → KHÔNG burnout (mẹ chăm con ốm thường)
      → L2 ≈ 0 + L1 bất kỳ → burnout (bác sĩ, social worker)
      → L2 CAO + L1 CỰC CAO kéo dài → VẪN CÓ THỂ burnout
        (cha mẹ trẻ khuyết tật = L2 có nhưng L1 vượt ngưỡng)


  PROFESSIONAL ANCHOR = PROXY L2:

    Bác sĩ có L2 ≈ 0 với bệnh nhân lạ. NHƯNG có thể có:
      → Professional identity anchor: "tôi là bác sĩ, chữa bệnh = nhiệm vụ"
      → = Schema-based proxy cho L2 (anchor provide structural drive)
      → NHƯNG proxy YẾU hơn L2 thật (schema ≠ compiled body-base extension)
      → = Tại sao professional anchor GIẢM burnout nhưng KHÔNG loại bỏ
    Chi tiết: Connection.md v4.0 §3.3, Self-Pattern-Match.md v3.0 §10.4


  NGHỀ NGHIỆP CHỊU NHIỀU NHẤT:
    → Y tá, bác sĩ: L1 cost hàng ngày × nhiều bệnh nhân × L2 ≈ 0
    → Social worker: L1 cost hàng ngày × nhiều case × L2 ≈ 0
    → Giáo viên (môi trường khó): L1 cost × perceived ability thấp
  ...
  🟡 L1/L2 compensation model = framework synthesis
...
```
⭐ MORAL INJURY = L1 COST TÍCH LŨY KHI L2 KHÔNG ĐỦ BÙ:

  (Self-Pattern-Match.md v3.0 §10.4)

  Moral injury khác burnout ở THÊM dimension:
    → Burnout = L1 cost > L2 compensation → kiệt sức
    → Moral injury = L1 cost + PFC nhận ra "mình ĐANG làm hại"
  ...
  Mẹ (KHÔNG moral injury dù cùng F1 cost):
    ...
    → = L1 cost + L2 structural → moral injury KHÔNG xảy ra
      (vì L2 provide moral anchor: "tôi đang feed body-base MÌNH")

  🟡 Moral injury qua L1/L2 = framework synthesis
```
...
🟡 6 FAILURE MODES CÒN LẠI:
  ...
  ④ PARASOCIAL (feedback axis failure):
    ...
    → Connection.md v4.0 §3.4: rich SPM nhưng no Resonance
...
⭐ CASE ĐẶC TRƯNG NHẤT CỦA 2-LUỒNG: L2 CÓ ≠ F1 CÓ
  ...
  ⭐ CƠ CHẾ:
    ...
    Bố mẹ CÓ L2 rất mạnh — con = body-base extension THẬT SỰ:
      ...
      → L2 KHÔNG giả — đây là yêu thương structural, body-level
    ...
    Khi F1 yếu hoặc bị schema chặn:
      ...
      → Bố mẹ PROJECT schema CỦA MÌNH lên con:
        ...
        "Con PHẢI như thế này" (vì L2 drive tôi muốn con tốt nhất)
  ...
  ⭐ KẾT QUẢ — CẢ HAI BÊN ĐỀU ĐÚNG:
    ...
    Bố mẹ perspective:
      → "Tôi yêu con hết mình" → ĐÚNG (L2 thật, structural)
      → "Tôi hy sinh cho con" → ĐÚNG (L2 drive → real sacrifice)
  ...
  ⭐ CONNECTION PATHWAY ANALYSIS — CON NHẬN GÌ VÀ THIẾU GÌ:
    ...
    (Connection.md v4.0 §5 — 8 pathways)
...
⭐ PFC = LAWYER, NOT JUDGE — ÁP DỤNG CHO EMPATHY:
  ...
  (Inter-Body-Mechanism.md §7, SPM v3.0 §3.7)
  ...
  3 CÁCH PFC=LAWYER ẢNH HƯỞNG EMPATHY:
    ...
    ③ PFC AMPLIFY empathy narrative:
      ...
      → Ví dụ: "nghĩa cử cao đẹp" = PFC narrative amplify L1 moderate → override
...
🟡🔴 AI KHÔNG CÓ BODY → KHÔNG CÓ EMPATHY THẬT:
  ...
  EMPATHY = CẦN BODY + CHUNKS TÍCH LŨY TỪ TRẢI NGHIỆM THẬT:
    ...
    Cơ chế con người (SPM v3.0 §2.1, F1):
    ...
    AI KHÔNG CÓ bước nào trong chain này:
      ...
      → Nhưng AI KHÔNG kích hoạt chunks buồn CỦA MÌNH (vì KHÔNG CÓ)
        → không có L1 reward/cost → không có drive thật từ body
      → Và AI KHÔNG CÓ L2 structural connection với bất kỳ ai
...
```
  ESTABLISHED (🟢):
    ...
    🟢 Neonatal cry asymmetry (Dondi 1999)
  ...
  FRAMEWORK SUY LUẬN (🟡):
    ...
    🟡 "Burnout = f(L1 cost / L2 compensation)"
       — Consistent with Figley 2002. 2-luồng formula chưa test trực tiếp.
    🟡 "L2 structural = body-base extension"
    ...
    🟡 "Empathy = observation output CỦA connection mechanism"
       — v2.0 hierarchy: Connection ⊃ Empathy. Logically consistent với
         Connection v4.0 3 primitives model.
    🟡 "Justified override ceiling vì L2, hijack no ceiling vì thiếu L2"
    ...
    🟡 "F1-Resonance vs F2-Resonance quality" (§11.5 v2.2)
       — F1+F1 = deep empathy, F2 alone = "biết nhưng không cảm."
         Masking = chronic F2 compensation. By-Product-Gap-Resonance.md v1.0 §7.10.
    ...
    🟡 "Compiled/Fresh empathy cost differential" (§4.1 v3.0)
       — Compiled empathy (expert, family) cost ≈ 0 → sustainable.
         Fresh empathy (stranger, cross-cultural) cost HIGH → burnout.
         3-cost model (PFC draft + suppress + uncertainty) applied.
    🟡 "Domain=Arbiter for empathy accuracy" (§8.6 v3.0)
       — Empathy can be wrong. Only domain outcome verifies.
  ...
  HYPOTHESIS (🔴):
    ...
    🔴 "Moral injury = L1 cost + moral dimension + L2 ≈ 0"
...
→ Valence-Propagation.md v2.0 — body evaluation + §2 Body-Base Extension dimension
   (NƠI Luồng 2 structural SINH RA).
→ Body-Coupling.md v2.0 — coupling mechanism deep-dive: HOW L2 compile,
...
```
→ Inter-Body-Mechanism.md v1.0 — SOURCE OF TRUTH cho drill insights (v3.0 NEW):
   §1.2 Architecture B (general-purpose empathy). §3 Compiled/Fresh axis.
   §4 3-cost model. §5.4 By-product match. §7 PFC=Lawyer. §8 Entity-Compiled.
→ Agent-Mechanism.md v2.0 — unified mechanism file.
   §12 body-need feeder. §12.2b 2-luồng reward.
   §12.3 mirror reward override + L2 reframe.
   §12.6 grief = L2 structural loss.
→ Mirror-Neuron-Rejection.md (Agent-Mechanism/) — TẠI SAO reject hardware mirror.
   7 bằng chứng, 3 positions. Alternative model.
→ Self-Pattern-Match.md v3.0 — deep drill:
   §2 F1 Compiled/F2 Fresh (v3.0 reframe), §2.1 F1=Luồng 1, §2.5 valence gate,
   §3.7 PFC=Lawyer, §4 context-dependent selection,
   §10 reversed mapping + professional SPM, §10.4 moral injury, §12 alexithymia.
→ By-Product-Gap-Resonance.md v1.0 — 2-Stream Architecture (v3.0 NEW):
   Stream 1 (hardware, habituates) + Stream 2 (SPM mutual, deepens).
   By-product match, anti-match, per-pair topology.
   §7.10 F1/F2 resonance quality + divergence spectrum.
→ Cortisol-Baseline.md v2.0 — full stress cascade.
   §1.5 NE → cortisol mechanism. Baseline drift. Recovery asymmetry.
→ Threat.md — 3 origin sources, NE α1/α2 PFC disconnect.
...
→ 07-Social-Recognition-Arc (Child-Chunk-Development/) — developmental events
   E9-E31. Chunk-based NOT hardware mirror. E9 cry contagion = pattern matching.
```
...
→ Status.md v2.0 — Resource Access Map. Status calibrate vs Connection maintain.
→ Protect.md v1.0 — ownership chunks, loss aversion. L2 loss = grief.
```
...
```
  ...
  Cross-refs:
    → By-Product-Gap-Resonance.md v1.0 §7.10 (F1/F2 resonance quality + divergence spectrum)
...
→ Empathy.md v1.1 → backup/Empathy-v1.1-backup.md
   Lý do supersede: v1.1 nói "Empathy RỘNG HƠN Connection" — SAI sau Connection v3.0.
   v2.0 reframe: empathy = observation output CỦA connection mechanism.
   2-luồng organic (không bolted-on). F1/F2 integrated. Cross-refs updated.
...
```
STATUS: v3.0 COMPLETE — OBSERVATION PARAMETER FILE

  Rewritten 2026-04-24. Major refine 2026-05-17.
  Empathy = observable output khi SPM F1 + ❸ positive valence.
  Connection ⊃ Empathy (mechanism ⊃ observation).
  2-luồng organic: L1 SPM-owned + L2 Entity-Compiled xuyên suốt.
  F1 Compiled / F2 Fresh integrated (v3.0: Compiled/Fresh, NOT Feeling/Logic).
  3 Generative Primitives context từ Connection v4.0.
  6 drill concepts integrated: Architecture B, 2-Stream, by-product match,
  PFC=Lawyer, Compiled/Fresh empathy, Domain=Arbiter.

  CHANGES v2.2 → v3.0 (2026-05-17):
    1. F1/F2 labels: "F1 Feeling + F2 Logic" → "F1 Compiled + F2 Fresh"
       (observer labels only — mechanism = compilation level)
    2. Entity-owned → Entity-Compiled (per-entity valence compiled through experience)
    3. +Architecture B: empathy = general-purpose output (§1.1)
    4. +2-Stream empathy: Stream 1 habituates, Stream 2 deepens (§4.5 NEW)
    5. +By-product match: empathy fire từ B's by-product → A's template (§1.1, §7.1)
    6. +PFC=Lawyer × empathy: fabricate/suppress/amplify (§8.6 NEW)
    7. +Compiled/Fresh empathy: cost differential → burnout prediction (§4.1)
    8. +Domain=Arbiter: empathy can be wrong, domain = final check (§8.6)
    9. All dependency versions updated to current
    10. +Inter-Body-Mechanism.md v1.0 as new dependency
    11. Honest Assessment: +7 🟡 items for new concepts
    12. "logic predict" → "deliberate predict" (4 locations)

  CHANGES v1.1 → v2.0:
    1. Hierarchy: "Empathy RỘNG HƠN Connection" → Connection ⊃ Empathy
    2. Definition: "SPM function" → "observable output khi F1 + ❸ positive"
    3. F1/F2: SPM 6 steps → F1 (core) + F2 (support) tách rõ
    4. 2-luồng: bolted-on blocks → organic xuyên suốt
    5. Burnout: mô tả → formula f(L1/L2) + moral injury
    6. Cross-refs: Agent.md → Agent-Mechanism.md, versions updated
    7. 3 primitives: THIẾU → ❶ × ❷ × ❸ context in §0

  PRESERVED FROM v1.1:
    - Core: 3 mechanisms, developmental bootstrap, biochemistry cascade
    - Bird & Cook 2013 decisive evidence
    - 4 strength factors × 4 quality axes
    - 7 failure modes
    - AI era significance
    - Honest assessment structure
    - ALL 18+ research citations
