...
⭐ OBLIGATION TRONG HIERARCHY OBSERVATION PARAMETERS:
  ...
  OBLIGATION KHÁC CÁC PARAMETER KHÁC:
    ...
    KEY DISTINCTIONS:
      Status = WHAT you have → Obligation = WHAT IT COSTS to keep it
      Threat = FEAR of loss → Obligation = ACCEPTED ONGOING COST
      Autonomy = CAN do alone → Obligation = MUST rely on others
  ...
  OBLIGATION = TRACKING MECHANISM CHO TRAO ĐỔI:
    ...
    🟡 Obligation as TRACKING mechanism (framework synthesis)
    🟡 Obligation as distinct observation parameter (framework claim)
```
...
⭐ OBLIGATION KHÔNG CHỈ TIÊU CỰC:
  ...
  POSITIVE obligation (tự nguyện, L2 buffer cao):
    → Chăm mẹ: obligation CAO nhưng L2 CAO → net = VUI (§7)
    → Giúp bạn thân: obligation THẤP + L2 CAO → gần pure reward
    → Nhà khoa học cống hiến vì yêu nghề: identity obligation + meaning → fulfilling
    → = CHỌN obligation (efference copy) + L2 buffer → positive experience
  ...
  ⭐ KEY: CÓ 3 YẾU TỐ quyết định valence:
    ...
    Bị ép + L2 thấp + Cost mờ → NEGATIVE obligation
    → = Tại sao "cùng obligation, khác trải nghiệm HOÀN TOÀN"

  🟡 Obligation valence = f(choice, L2, cost_clarity) = framework synthesis
...
OBLIGATION CÓ BODY SIGNALS THẬT:
  ...
  🟡 Specific body tension patterns = framework claim (not measured)
```
...
### §3.1 — 5-Factor Formula

```
⭐ OBLIGATION INTENSITY = f(5 factors):

  Obligation_intensity = Agent_value
                         × Replaceability⁻¹
                         × Cost_clarity⁻¹
  ...
  🟢 Reciprocity × replaceability documented (Trivers 1971)
```
...
### §4.3 — TYPE 2: EXCHANGE (symmetric, ongoing)
...
### §4.6 — TYPE 5: IDENTITY (self-imposed, no external enforcement)
...
TYPE 5 — IDENTITY:
  ...
  🟢 Self-concept maintenance (Steele 1988)
```

### §4.7 — TYPE 6: COMPOUND (multiple types overlap)
...
⭐ TIỀN = CÔNG NGHỆ TỐI ƯU HÓA OBLIGATION:
  ...
  ⭐ NHƯNG TIỀN KHÔNG THỂ DÙNG CHO MỌI THỨ:
    ...
    L2 RELATIONSHIPS (bố mẹ, con, bạn thân):
      ...
      → Agent detect signal → L2 bond bị PHỦ NHẬN → hurt
  ...
  ⭐⭐ PARADOX — ÂN TÌNH TỒN TẠI NƠI TIỀN BỊ CẤM:
    ...
    → Vì monetize = phá hủy L2 bond = phá hủy value của relationship
    → = Ân tình là COST CỦA việc có L2 bonds
    → = Trade-off: L2 cao → obligation nặng → nhưng L2 buffer cũng cao → net OK
...
ACCESS COST QUÁ CAO → GIẢM COOPERATION:
  ...
  🟡 Access cost as cooperation barrier = framework synthesis
```
...
L2 (ENTITY-OWNED) = BUFFER GIẢM FELT-COST CỦA OBLIGATION:
  ...
  Mechanism:
    ...
    → = Cost CỦA obligation ĐƯỢC BÙ bởi L2 reward
  ...
  VÍ DỤ:
    ...
    Chăm mẹ ốm:
      ...
      L2: CAO (mẹ = body-base extension, compiled deep qua decades)
      → Net = CAO − CAO = THẤP → "vất vả nhưng VUI"
      → = L2 BUFFER làm obligation BEARABLE

    Chăm bệnh nhân lạ (bác sĩ):
      Obligation: CAO (role obligation TYPE 4)
      L2: ≈ 0 (bệnh nhân mới, chưa compiled)
      → Net = CAO − 0 = CAO → "mệt, burnout risk"
      → = KHÔNG CÓ L2 buffer → obligation FULL WEIGHT

    Giúp bạn thân:
      Obligation: THẤP (friends = exchange, manageable)
      L2: CAO (bạn thân = body-base extension)
      → Net = THẤP − CAO = NEGATIVE → "giúp mà vui, gần pure reward"
      → = L2 EXCEED obligation → experience = POSITIVE

  ⭐ KEY INSIGHT:
    L2 KHÔNG THAY ĐỔI obligation calculation
    L2 THAY ĐỔI felt-cost
    → Cùng obligation, khác L2 → KHÁC trải nghiệm HOÀN TOÀN
    → = Tại sao "chăm mẹ vất vả nhưng vui" vs "chăm bệnh nhân mệt mỏi"

  🟡 L2 as independent buffer = framework synthesis
...
⭐ COMMUNITY-LEVEL OBLIGATION = O(1) TRACKING:
  ...
  Thay vì track từng agent → track COMMUNITY:
    ...
    → = L2 at collective level (Agent-Mechanism §12.5)

    "1 cá nhân vi phạm không làm tôi suy nghĩ lại"
    → Community-level L2 ROBUST to individual failure
  ...
  MECHANISM:
    ...
    Transition happens khi:
      ...
      → Body compile community-level L2 (collective = body-base extension)
...
COUNTRY/HUMANITY LEVEL:
  ...
  → = Compiled: community-L2 so deep → body drive giving WITHOUT PFC rationale
  ...
  OBLIGATION AT THIS SCALE ≈ 0 PER-ACTION:
    ...
    → = CHỌN (TYPE 5 identity) + Community L2 + no specific obligation
  ...
  🟡 Community-level obligation as O(1) solution = framework synthesis
  🟡 Country-level → meaning gateway = framework insight
...
### §10.1 — × Gratitude (INVERSE relationship)
...
### §10.3 — × Threat (obligation failure → threat)
...
```
OBLIGATION × STATUS — MODE RELATIONSHIP:
...
Case 5: Vợ ở nhà nội trợ, chồng đi làm
  ...
  → Có thể THÍCH nội trợ → L2 buffer (con = body-ext) → bearable
  → Có thể KHÔNG thích → no L2 buffer → trapped → resentment
...
Case 10: "Tôi thích cộng đồng, mọi người tốt bụng, tôi giúp khi có thể"
  ...
  → L2: MODERATE-HIGH (community = body-base extension)
  → Efference copy: PRESENT (tự nguyện)
  → = Net experience: POSITIVE (obligation nhẹ + L2 buffer + efference copy)

  "1 cá nhân vi phạm → không suy nghĩ lại":
    → Community L2 ROBUST: N-1 members vẫn positive
  ...
  "Ai đó đòi nợ cost cao → từ chối → cộng đồng vẫn chấp nhận":
    ...
    → = Healthy community = obligation LIGHT + L2 STRONG
```
...
🟢 ESTABLISHED (research support):
  ...
  → Reciprocal altruism (Trivers 1971): cheater detection, reciprocity expectation
  → Social exchange theory (Homans 1958, Blau 1964): cost-benefit social calculation
  → Equity theory (Adams 1963): imbalance → distress
  → Prospect theory (Kahneman & Tversky 1979): certainty premium
  → Conditional regard effects (Rogers 1959, Deci & Ryan 2000)
  → Attachment patterns affect help-seeking (Bowlby 1969)
  → Ecological systems theory (Bronfenbrenner 1979): multiple socialization sources
  → Piaget/Kohlberg: reciprocity understanding develops with age
  → Help-seeking avoidance (DePaulo 1983, Lee 1997)
  → Psychological safety (Edmondson 1999): reduce fear → performance ↑
  → Crowding out effect (Frey & Jegen 2001): monetary → intrinsic ↓
  → Social capital (Putnam 2000): community cooperation → prosperity
  → Power dynamics in exchange (Emerson 1962, Blau 1964)
  → Role conflict → stress (Kahn et al. 1964)
  → Division of labor (Smith 1776, Durkheim 1893)
...
🟡 FRAMEWORK SYNTHESIS (logical, consistent, chưa verified trực tiếp):
  ...
  → Status gap as MULTIPLIER (not 6th factor)
...
🔴 HYPOTHESIS (logical nhưng chưa có evidence trực tiếp):
  ...
  → Community-level L2 as "O(1) obligation" — conceptual, not measured
```
...
8 CÂU HỎI MỞ:
  ...
  Q2: 6 types exhaustive?
      ...
      → Hay parent→child = L2-driven (not obligation)?
  ...
  Q5: Obligation × Gratitude — full model?
      ...
      → Nhưng interaction model cần refine: 
        obligation ↓ gratitude vs L2 buffer moderation
  ...
  Q7: Community-level obligation → how does it break down?
      → When community FAILS (corruption, betrayal at scale):
        → Community L2 collapse → back to individual tracking → O(n) overload
...
PFC FILES:
  → PFC-Function.md — monitoring, prediction, planning = PFC functions in obligation
  → Logic-Feeling-Balance.md — obligation often LOGIC-driven, body feeling may DISAGREE

DOMAIN FILES:
  → Collective-Purpose.md — community/nation-level obligation → meaning

DEVELOPMENT FILES:
  → Child-Chunk-Development.md — reciprocity development, parenting impact

BACKUP:
  → backup/Obligation-Outline-v0.1-backup.md — original outline (2026-04-25)


KEY RESEARCH CITATIONS:
  → 🟢 Trivers 1971 — Reciprocal altruism
  → 🟢 Homans 1958, Blau 1964 — Social exchange theory
  → 🟢 Adams 1963 — Equity theory
  → 🟢 Kahneman & Tversky 1979 — Prospect theory, certainty premium
  → 🟢 Deci & Ryan 2000 — Self-Determination Theory, conditional regard
  → 🟢 Rogers 1959 — Conditional positive regard
  → 🟢 Bowlby 1969 — Attachment theory
  → 🟢 Bronfenbrenner 1979 — Ecological systems theory
  → 🟢 Piaget, Kohlberg — Moral development stages
  → 🟢 Edmondson 1999 — Psychological safety
  → 🟢 Frey & Jegen 2001 — Crowding out effect
  → 🟢 Putnam 2000 — Social capital
  → 🟢 Emerson 1962 — Power-dependence relations
  → 🟢 Kahn et al. 1964 — Role conflict
  → 🟢 Dickerson & Kemeny 2004 — Social evaluative threat → cortisol
  → 🟢 DePaulo 1983, Lee 1997 — Help-seeking avoidance
  → 🟢 Baumrind 1966 — Parenting styles
  → 🟢 Smith 1776, Durkheim 1893 — Division of labor
  → 🟢 Miller 1956, Chase & Simon 1973 — Chunking
  → 🟢 Tajfel 1979 — Social identity theory
...
DEFINITION (§1):
  ...
  → Valence: not always negative — f(choice, L2, cost_clarity)
