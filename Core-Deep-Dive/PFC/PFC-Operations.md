...
purpose: |
  ...
  File NÀY = HOW PFC operates trên Compiled/Fresh spectrum.
  = MECHANISM file cho Hold/Suppress operations, outcomes, quality, pathways.
...
# PFC-Operations — Hold + Suppress trên Compiled/Fresh Spectrum

> **PFC có 2 operations trên Compiled/Fresh spectrum:**
> **HOLD = amplify pattern mới. SUPPRESS = block pattern cũ.**
>
...
- §1 — COMPILED/FRESH SPECTRUM (brief — xem Logic-Feeling.md cho full treatment)
...
> Logic-Feeling.md v2.1 = full treatment. Section này = brief context cho §2+.
...
TRỤC THẬT BÊN TRONG BODY: COMPILATION LEVEL
  ...
  SPECTRUM, KHÔNG PHẢI BINARY:
    ...
    "Bán-compiled": steps 1-3 automatic, step 4 cần fresh draft.
    Expert "partially stuck" = compiled phần quen, fresh phần mới.

  "Logic" vs "Feeling" = OBSERVER LABELS cho CÙNG spectrum.
  Phân tích mechanism: BẮT BUỘC dùng Compiled/Fresh.
  Mô tả cho người ngoài: "Logic/Feeling" OK (approximate).
...
COMPILED → FRESH (Disruption):
  New context, error, betrayal → phải FRESH lại.
...
### §2.3 — HOLD ≠ SUPPRESS: Phân biệt nền tảng
...
│                │ VD: Lái xe quen đường, quên tên người quen.  │
...
> Khi cần thay đổi (tổ hợp ②), 3 outcomes có thể xảy ra.
...
⭐ HOLD + SUPPRESS → NEW COMPILED → OLD FADES:
  ...
  CƠ CHẾ:
    ...
    ② Pattern cũ compiled nhưng KHÔNG CỰC MẠNH.
    ③ Lặp lại → Hebbian compile pattern mới.
    ④ Pattern mới cho BODY REWARD (genuine) → body CONFIRM.
...
⭐ HOLD + SUPPRESS → COMPILE "HOW TO SUPPRESS" → BURIED + FLAT:
  ...
  CƠ CHẾ:
    ① PFC HOLD new pattern ("học toán theo mẹ").
...
⭐ 3 TRẠNG THÁI KHÁC CĂN BẢN:
  ...
  ⚠️ Nhầm lẫn phổ biến:
    ...
    Phân biệt: body response to trigger (not verbal report).
...
> Cùng "compiled" nhưng quality KHÁC CĂN BẢN — phụ thuộc HOW chunks được compiled.
...
> Khi Hold + Suppress xảy ra, CÁI GÌ quyết định Outcome B vs C?
...
⭐ 7 FACTORS:
  ...
  │   + PFC budget nhỏ/damaged + External force + Sleep kém      │
...
```
⭐ TẠI SAO BỐ MẸ HIỂU CON = HIGHEST PFC COST:
