...
LOGIC-FEELING v4.0 TRONG KIẾN TRÚC FRAMEWORK:
  ...
  v1.0 (2026-04-13): "Logic + Feeling = 2 processing modes khác nhau"
    ...
    → Framed as "rules vs body feedback" — đúng NHƯNG chưa đủ sâu

  v2.0 (2026-05-17): "Compiled/Fresh = trục thật. Logic/Feeling = observer labels"
...
```
⭐ OBSERVER PERSPECTIVE TẠO RA LABELS:
...
⭐ NEITHER "LOGIC" NOR BODY-KNOWING = NEUTRAL TRUTH (Inter-Body §7):
  ...
  "LOGIC" output can be LAWYERING:
    → "Tôi nghỉ việc vì career growth" (actual: escape toxic boss)
...
⭐ CRITICAL: Step ③ trong Loop = DOMAIN, not "Logic"
  ...
  TẠI SAO QUAN TRỌNG:
    ...
    → Nhưng PFC = Lawyer (§2.3) → PFC verify ĐÃ có thể CONFABULATE
...
⭐ BODY-KNOWING / FRESH ANNOTATION:
  ...
  Domain verify:
    → NE + Adrenaline = INITIATORS (spike tức thì)
    → Cortisol = SUSTAINER (tới sau, giữ mobilized)
...
⭐ BODY-KNOWING / FRESH ANNOTATION:
  ...
  Domain verify:
    → Einstein + Grossmann: tensor calculus FORMALIZE
...
### 5.7 — Pattern thống nhất từ 6 Cases
