...
**Vô thức KHÔNG thể**: kiểm tra độ chính xác domain, mô phỏng tương lai, so sánh với ký ức. Vô thức **chỉ đo body-base** → body sướng → vô thức "OK" → dù domain có thể sai (nghiện, limerence, comfort zone).
...
| H5 | Lờ mờ = hội tụ multi-weak-signal | F4 | — |
...
| **H9** | **Agent = Self-Pattern-Match thống nhất** | **Agent ✅** | **— (standalone N+2 drilled)** |
...
**F4 Chunk-Internal-Processing** xử lý hình thành chunk nội bộ mới — kết nối logic, trực giác, insight, chuỗi + anchor decay. 7 files. Giải quyết H2 (loại kết nối thứ 4), H5 (hội tụ multi-weak-signal = "lờ mờ"), H6 (chunk decay Ebbinghaus). Cũng hình thức hóa H12 tentative (language evolution driver qua post-verbal bypass).
