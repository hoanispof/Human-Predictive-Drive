...
purpose: |
  ...
  ② Convergence Zone concept — what "lờ mờ" IS at mechanism level
...
> từ tích lũy → phát hiện lờ mờ → label → logic-planning enablement.
...
FRAMEWORK CHƯA CÓ:
  ...
  ❌ CONVERGENCE ZONE concept:
     → "Cái lờ mờ" ở level mechanism = GÌ?
...
STEP 1 — TÍCH LŨY CHUNKS:
  ...
  PRECONDITION FOR CYCLE:
    ...
    → = "Trẻ sơ sinh không có cái lờ mờ" — vì database chưa đủ
...
STEP 2 — CÁI LỜ MỜ XUẤT HIỆN:
  ...
  ⭐ CONVERGENCE ZONE = cái mà PFC detect là "lờ mờ":
...
STEP 3 — TÒ MÒ TỰ NHIÊN:
  ...
  ⭐ CRITICAL CONDITION — TÍCH CỰC vs TIÊU CỰC:
    ...
    IF tích lũy TIÊU CỰC (bị ép, Direction B forced under threat):
      ...
      → = Học sinh bị ép: "thấy toán lờ mờ → SỢ, không phải TÒ MÒ"
...
STEP 4 — LÀM RÕ:
  ...
  MECHANISM — 3 strategies (often combined):
    ...
    STRATEGY C — EXTERNAL CATALYST (AI or collaborator):
      ...
      → = Somatic-Articulation Loop (SAL) WITH EXTERNAL AMPLIFIER
...
STEP 7 — VÒNG TIẾP (deeper + wider):
  ...
  SELF-REINFORCING PROPERTIES:
    ...
    ② SÂU HƠN mỗi vòng:
       ...
       → = "Hiểu A → thấy B lờ mờ → hiểu B → thấy C lờ mờ..."

    ③ RỘNG HƠN mỗi vòng:
       → Cross-domain chunks → cross-domain convergence zones
       → = "Hiểu physics + hiểu philosophy → thấy lờ mờ ở GIAO ĐIỂM"
...
⭐ CLAIM (formalized from §2.7):
  ...
  AFTER LABEL:
    → Same content (label ≠ new content — NT6 confirmed)
...
⭐ IF CONVERGENCE ZONE KHÔNG ĐƯỢC LABEL:
  ...
  FATE A — PERSISTENT VAGUE (nếu surrounding chunks vẫn active):
    ...
    → PFC: persistent vague signal → "lờ mờ khó chịu"
...
═══════════════════════════════════════
  ...
  AI as external crystallization catalyst:     Novel — consistent with ZPD + SAL
...
WITHIN CHUNK-ANALYSIS:
  → F1 NT6: verbal-as-handle → §3.1 label = logic-planning prerequisite
...
✅ DRILLED v1.0 (N+9, 2026-04-17)
  ...
  NEW FRAMEWORK CONTRIBUTIONS:
    ...
    ⑦ AI as external crystallization catalyst (SAL + AI variant)
