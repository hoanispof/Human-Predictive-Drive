...
changes_v1.1: |
  ...
  §7 cross-ref added: chi tiết → Entity-Access-Excess.md §5
  §12 updated: Entity-Access-Excess.md added to cross-references
scope: |
  GRADIENT MODEL cho Entity-Access:
...
purpose: |
  Entity-Compiled.md = DEEP MECHANISM (brain compile agent vào body-base).
  File NÀY = ACCESS MODEL: predict access to entity, gradient Mức 0-5,
...
dependencies:
  ...
  - Autonomy-Hardware.md v1.1 — vmPFC + DRN, autonomy violation detection
  - Gap-Distribution-Profile.md v1.0 — portfolio risk, diversification
sources:
...
> **Cùng cơ chế ở MỌI mức. "Bạn thân" và "mẹ→con" = CÙNG mechanism, KHÁC depth.**
...
```
  ...
  REFRAME TỪ Entity-Owned (binary) → Entity-Access (gradient):
    CŨ: Entity-Owned = emergent state, binary ("có" hoặc "không")
    MỚI: Entity-Access = continuous gradient (Mức 0-5), Entity-Owned = PFC label at Mức 4-5

  3 CONCEPTS RIÊNG BIỆT (không lẫn):
...
⭐⭐⭐ Compiled — ENGINE MODE: Tool ↔ Agent (DYNAMIC per-moment)
  ...
  ⭐⭐ ENGINE MODE = DYNAMIC (shifts MOMENT-TO-MOMENT):
    ...
    Mẹ SÁNG (ngủ đủ, rảnh): Agent-mode deep → "con hôm nay có vẻ lo"
    Mẹ TỐI (mệt, deadline): Tool-mode → "ăn đi!" → predict FUNCTION only
...
⭐⭐ Fresh — GAP-NEED PROFILE: WHY I need access to entity
  ...
  3 DIMENSIONS:
    ...
    ① GAP SOURCE (A:B:C:D ratio) — chi tiết §4:
    ...
    ② BREADTH (bao nhiêu loại gap entity fill?):
      Single-function: bác sĩ (health) → narrow
...
⭐⭐ 3 FACTORS → COMPILATION → ENTITY-ACCESS:
  ...
  ENTITY-ACCESS = 2 LAYERS:
    ...
    Layer 2 — GAP EXPECTATION (implicit): "available ĐỂ FILL NHỮNG GAP CỤ THỂ NÀY"
...
```
⭐⭐⭐ ENTITY-ACCESS = GRADIENT, KHÔNG PHẢI BINARY:

  MỨC 0 — NO ENTITY-ACCESS (3 sub-types):

    0a — MACHINE/SYSTEM: target = máy, hệ thống → KHÔNG CÓ internal state
      → ATM, máy bán hàng, website, app
      → Entity-access impossible BY DEFINITION

    0b — SERVICE ACCESS: target = DỊCH VỤ, cá nhân = invisible interchangeable component
      → Bưu tá, tài xế taxi, nhân viên chuỗi
      → Cá nhân vui hay buồn = IRRELEVANT → đổi cá nhân khác → KHÔNG ảnh hưởng
      → "Xã giao" = 0b WITH COMPILED SCRIPTS, NOT Mức 1
        (tài xế scripted + khách scripted = protocol exchange, KHÔNG predict STATE)
      → CÓ THỂ shift sang Mức 1 nếu cùng cá nhân lặp lại đủ (rare)

    0c — TOOL-MODE TOWARD SPECIFIC INDIVIDUAL: biết entity nhưng chỉ predict function
      → Boss deadline: "nhân viên A làm xong chưa?" → function only
      → Mẹ mệt tối: "con ăn đi!" → BIẾT là con, nhưng lúc này = Tool
      → KHÁC 0b: entity ĐÃ ĐƯỢC NHẬN DIỆN → shift sang Agent DỄ HƠN
      → Mẹ mệt: 0c = PAUSE (entity-access đã có, tạm offline)

  MỨC 1 — AGENT-MODE SHALLOW (gate open, early):
    "Người thú vị" / "nói chuyện hay"
    → Người mới gặp ở party, đồng nghiệp mới, hàng xóm mới

  MỨC 2 — PARTIAL COMPILE (developing):
    "Bạn tốt" / "muốn gặp lại" / "quen quen"
    → Bạn quen vài tháng, barista quen, hàng xóm hay trò chuyện

  MỨC 3 — DEEP COMPILE, B-DOMINANT (friendship):
    "Bạn thân" / "quan trọng" / "nhớ"
    → Entity-access STRONG nhưng KHÔNG "ownership" feeling
    → Exit easy → PFC KHÔNG gọi "sở hữu"

  MỨC 4 — DEEP COMPILE + HARDWARE/LOW EXIT (Entity-Owned):
    "Phần đời tôi" / "KHÔNG THỂ MẤT"
    → PFC label: "Entity-Owned" — mẹ→con, vợ/chồng lâu năm
    → CAN be healthy (A+B dominant) hoặc heading toward excess (C+D rising)

  MỨC 5 — COMPILE + C DOMINANT + NO EXIT (Excess):
    "CỦA TÔI" / "PHẢI ở đây" / "PHẢI theo ý tôi"
    → Entity-access MAXIMUM + distorted → chi tiết §6
...
```
⭐⭐ PHẦN LỚN ENTITY-ACCESS = UNLABELED BY PFC:

  PFC CHỈ LABEL RÕ Ở HIGH-END:
    Mức 3+: "bạn thân", "gia đình", "người yêu" → PFC có TÊN (cultural schema)
    Mức 0-2: barista, hàng xóm, bác nhà thuốc → PFC KHÔNG label

  TẠI SAO:
    ① Compilation chưa đủ deep → PFC chưa register
    ② Cultural schema không cung cấp category
    ③ Hardware không fire → không có automatic label

  NHƯNG CƠ CHẾ VẪN CHẠY:
    Barista quen nghỉ việc → MỘT CHÚT tiếc → confirm: entity-access CÓ ở đây
...
```
⭐⭐⭐ ENTITY-OWNED = PFC LABEL, KHÔNG PHẢI MECHANISM:

  CŨ: Entity-Owned = emergent state: "entity này là phần đời tôi"
  MỚI: Entity-Owned = PFC OBSERVATION khi entity-access compiled deep
...
```
⭐⭐ 4 CON ĐƯỜNG KHỞI NGUỒN ENTITY-ACCESS:
  ...
  MODE 1 — HARDWARE (kin, đặc biệt mẹ→con):
    ...
    → Con→mẹ = HYBRID: hardware (proximity need) + interaction (sensitivity shapes WHO)
...
⭐⭐⭐ B-DOMINANT = OPTIMAL DESTINATION, NOT UNIVERSAL ORIGIN:
  ...
  HEALTHY TRAJECTORY: BẤT KỂ starting mode → CONVERGE VỀ B-DOMINANT:
    ...
    Hardware (A)   ──→ A stays base + B GROWS = "làm bạn với con"
    Limerence (A)  ──→ A fades + B REPLACES = "bạn đời"
    Interaction (B)──→ B stays (ALREADY optimal)
    Schema (D)     ──→ D stays + B DEVELOPS = "bạn đồng nghiệp"

  PATHOLOGICAL = KHÔNG CONVERGE VỀ B:

    Hardware → STUCK A+C: "con phải nghe lời" dù con đã lớn → helicopter
    Limerence → A fades → NO B → C fills: "em phải ở đây" → anxious
    Interaction → C shift (rare): exit easy → bạn exit → self-correcting
...
```
⭐⭐⭐ 4 NGUỒN GAP MÀ ENTITY-ACCESS "EXPECT" FILL:
  ...
  A — HARDWARE EXISTENCE (kin-specific):
    ...
    VD: "Nhìn con ngủ là thấy bình yên" → hardware reward fires
  ...
  C — SELF-REFERENTIAL (entity phục vụ MY gap direction):
    ...
    VD: "Con phải giỏi" = MẸ's gap → con điểm cao → MẸ vui, con's state = irrelevant
  ...
  D — SCHEMA-INSTALLED (obligation):
    Cultural schema install → PFC adopt → compile → obligation.
    Fill = comply → RELIEF tag (NOT opioid).
    VD: "Vợ chồng phải gặp thường xuyên" → gặp → relief → remove schema → WEAKENS
...
⭐⭐ THỰC TẾ: ENTITY-ACCESS = MIX CỦA CẢ 4 SOURCES:
  ...
  ⭐ "SAME BEHAVIOR, DIFFERENT MECHANISM":
    ...
    CASE 2: "Gặp nhau cho yên tâm" → C (anxiety) → relief → ASYMMETRIC
    CASE 3: "Gặp vì phải" → D (schema) → relief → FRAGILE
    → Bề ngoài GIỐNG → mechanism HOÀN TOÀN KHÁC → sustainability KHÁC
  ...
  C JUSTIFIED vs C PROBLEMATIC:
    ...
    → Lifecycle: C justified → B organic (parent shift C→B theo development)
...
⭐ KEY OBSERVATIONS:
  ...
  HARDWARE SUBSIDY ≠ QUALITY:
    ...
    → = WHY mẹ→con excess PERSISTS (subsidy + C + no exit)
    → = WHY bạn excess RARE (no subsidy + exit easy)

  UNLABELED (Mức 1-2) = MOST NUMEROUS:
...
```
→ CHI TIẾT: Entity-Access-Excess.md v1.0 (~1,120L, 65 citations)
...
⭐⭐ ENTITY-ACCESS EXCESS = MỨC 5 TERRITORY:
  ...
  5 BOUNDARY INDICATORS (healthy Mức 4 vs excess Mức 5):
    ① Entity có DRIVE RIÊNG không? (YES = healthy)
    ② Separation = manageable? (YES = healthy)
    ③ Domain drives khác ACTIVE? (YES = healthy)
    ④ Entity's autonomy RESPECTED? (YES = healthy)
    ⑤ PFC CAN articulate WHY beyond "vì thương"? (YES = healthy)
...
⭐⭐ PARENTING + ROMANTIC DÙNG CÙNG CIRCUITS VỚI ADDICTION:
  ...
  🟢 Strathearn et al. 2009: mẹ nhìn con cười → VENTRAL STRIATUM fire = cocaine reward
...
⭐⭐ MẸ→CON EXCESS: DESIGNED vs OVERDRIVEN:
  ...
  ROMANTIC EXCESS:
    Bilateral trap: cả 2 excess → self-coherence loop → no domain → stagnation
    Unilateral: A excess → B "khó thở" → A cling → B withdraw → LOOP
    Schema-maintained: "vợ chồng là một" → compliance, NOT genuine

  EMPTY NEST: severity = f(gap distribution). Excess → SEVERE. Diversified → manageable.

  HIKIKOMORI = 4-mechanism compound:
    ① Entity-access excess (parent) + ② Compiled suppress (child competence)
...
⭐ 3 CON ĐƯỜNG DẪN ĐẾN EXCESS:
  ...
  ① HARDWARE CHAIN:
    ...
    = Contributing factor, not sufficient condition. 3-6% prevalence.
    🟢 Noftle & Shaver 2006, Meta-analysis 2025

  ② CHILDHOOD TRAUMA:
    Inconsistent caregiver → entity-access compile under THREAT cortisol
  ...
  ③ SOCIAL SCHEMA (NO trauma needed):
    "Mẹ phải hy sinh" / "vợ chồng là một" → compile → excess = automated
    → Cultural schema = SUFFICIENT condition
    → Remove schema (change culture) → schema entity-access RELEASED
    🟢 Fuller & Warner 2000
...
```
→ CHI TIẾT: Entity-Access-Excess.md §5 (per-relationship mapping, compound model)
...
⭐⭐ 2 MECHANISM RIÊNG BIỆT, CÓ THỂ ĐỒNG THỜI:
  ...
  COMPILED SUPPRESS (active):
    PFC actively BLOCK gap riêng → lâu → compile → automated suppress
    → ACTIVE, CÓ THỂ SUDDEN, SUPPRESS (gaps cũ bị BURIED)
    → Driven by: fear ("theo gap riêng → mất partner") / schema ("phải hy sinh")

  CẢ 2 ĐỒNG THỜI = COMPOUND (most dangerous → Mức 5):
    Gap shift PULL toward entity + compiled suppress PUSH away from domain
    → Entity = ONLY remaining gap source

  WHICH COMES FIRST?
    Case A — Gap shift FIRST → suppress follows (most common romantic)
    Case B — Compiled suppress FIRST → gap shift follows (cultural enmeshment)
...
## §8 — CALIBRATION ARCHITECTURE → CHI TIẾT: Entity-Access-Calibration.md v1.0 (~1,070L, 25 citations)
...
⭐⭐⭐ CALIBRATION CẦN 3 LAYERS ĐỒNG THỜI:
  ...
  FAILURE KHI BẤT KỲ LAYER NÀO FAIL:
    Layer 1 fail: blind to own gap source → C invisible
    Layer 2 fail: cannot predict entity experience → ép without understanding
    Layer 3 fail: entity responds nhưng compiled override → repeat
...
⭐⭐⭐ TẠI SAO BẠN BÈ TỰ CALIBRATE NHƯNG PARENT→CHILD THÌ KHÔNG:
  ...
  ├──────────────┼───────────┼──────────────┼────────────────────────────┤
  │ Bạn bè      │ LOW       │ HIGH (effect)│ EXTERNAL: ignore → friend  │
  │ (Mức 3)      │           │              │ exits → FORCED adjust     │
  ├──────────────┼───────────┼──────────────┼────────────────────────────┤
  │ Romantic     │ MODERATE  │ MODERATE     │ MIX: some room for excess  │
  │ (Mức 4)      │           │              │ before correction kicks in │
  ├──────────────┼───────────┼──────────────┼────────────────────────────┤
  │ Parent→child │ VERY HIGH │ LOW (ineffect)│ INTERNAL ONLY: child can't│
  │ nhỏ (Mức 4-5)│           │              │ leave → Self-Awareness =   │
...
⭐⭐ OPTIMAL ENTITY-ACCESS ≠ ZERO C:
  ...
  3 RESEARCH LINES CONFIRM:
    ① Baumrind 1966: Authoritative (high demand + high response) > Permissive (zero demand)
  ...
  ENTITY-ACCESS SPECTRUM:
    Under: "mặc kệ, ai lo nấy" → NEGLECTED (no bond)
    Mild: call hỏi thăm, bạn bận thì thôi → A+B dominant, entity autonomy INTACT
    Justified: ép uống thuốc + explain + empathy → temporarily override for entity's benefit
    Excessive: ép ăn khi con no, monitor → C dominant, entity's "no" ignored
    Extreme: con PHẢI học bác sĩ → C+D dominant, entity's drive REPLACED by mine

  ENGINE USE QUALITY (determines calibration outcome):
    CURIOSITY MODE: "tại sao mình muốn con ăn thêm nhỉ?" → observe body → insight → adjust
      → Calibration BECOMES source of Generative reward → "thú vị"
    THREAT MODE: "mình có phải bad parent không?" → self-judgment → ruminate → freeze
      → Paradox: trying to calibrate → anxiety → calibration FAILS
      → = WHY parenting advice as THREAT → counterproductive
  ...
  CANNOT PRESCRIBE (Logic-Feeling-Balance §6-7):
    ...
    → File này helps: IDENTIFY which layer fails, which mode active, what dynamics
...
⭐⭐ 5 FAILURE MODES — KHÁC SOURCE, KHÁC FIX:
  ...
  MODE 4 — DEPLETED (Cascading): PFC resource gone → all 3 layers drop → auto-pilot.
    Fix: resource restoration (sleep, support). Timeline: days-weeks.
  MODE 5 — THREAT-MODE (Engine quality): calibrate with threat cortisol → rumination.
  ...
  COMPOUND (most common): depleted (M4) → triggers M1+M2+M3 simultaneously.
    Must address RESOURCE first, then other modes become accessible.
  ...
  VICIOUS SPIRAL: calibrate poorly → self-blame → worse → break needs external intervention
...
```
⭐⭐ 4-TRỤC: CÔNG CỤ QUAN SÁT ENTITY-ACCESS BEHAVIOR:
  ...
  TRỤC 2 — SELF-AWARENESS (★ INDEPENDENT from Trục 1):
    Blind ←──────── Aware
  ...
  COMPARE BELSKY 1984 (4400+ citations):
    ...
    Framework 4-trục: DECOMPOSES "parent personality" into:
      Trục 1 (cognitive) + Trục 2 (meta-cognitive) + Trục 3 (motivational) + Trục 4 (state)
  ...
  🟡 4-Trục decomposes Belsky = framework synthesis
```
...
```
⭐⭐ VĂN HÓA INSTALL ENTITY-ACCESS QUA SCHEMA:

  COLLECTIVIST (East Asian, Latin American, South Asian):
    Schema: filial piety, familismo, "child = extension of parents"
    → D STRONG → (C+D) proportion HIGHER → entity-access excess = EXPECTED
    → "Guan" (Chao 1994) = care, NOT control (culturally distinct meaning)

  INDIVIDUALIST (US, UK, Northern Europe):
    → D WEAKER → higher burnout 5x (Roskam 2021) nhưng more autonomy
    → "Sang nước ngoài thấy thoải mái" = D released, A+B remain

  COLLECTIVIST PARADOX: lower quality + higher stability
    Maintained by SYSTEM (schema + economic + legal + community), not satisfaction
  ...
  3 CULTURAL-ECONOMIC MODELS:
    A — Family Collectivism: entity-access excess, schema-maintain → BRITTLE stability
    B — Individualism: moderate entity-access, genuine match → RESILIENT but costly
    C — Craft Collectivism (Đức): domain pride + moderate family → MOST balanced
...
═══════════════════════════════════════
  ...
  Calibration × Mức level quantification
  Bilateral trap timeline prediction (5-8yr to stagnation)

CÁI FRAMEWORK THÊM vs CÁI ĐÃ CÓ:
  Research đã biết: attachment styles, parenting outcomes, addiction circuits,
  mentalization trainable, burnout hurts parenting.
  Framework THÊM: unifying MODEL — 3-Factor + Gradient + A:B:C:D + Calibration
  3-Layer + per-Mức analysis, connecting to broader architecture.
...
| R5 | Feldman 2007, 2012 — Oxytocin, bio-behavioral synchrony | §3.1, §4.1 | 🟢 |
...
| R27 | Soenens et al. 2015 — Self-worth contingency ("Trophy Kids") | §9 | 🟢 |
...
| R36 | Coan & Sbarra 2015 — Social Baseline Theory (neural evidence) | §8.5 | 🟢 |
...
| R40 | Zeki & Romaya 2008 — Love/hate shared circuits (putamen+insula) | §5.1 | 🟢 |
| R41 | Maccoby 1992 — Power asymmetry in parent-child | §8.3 | 🟢 |
| R42 | Daly & Wilson 1988 — Cinderella Effect (stepchild 40x) | §10.1 | 🟢 |
...
| R47 | Carnes 1997 — Betrayal bond (neurologically similar to addiction) | §6.4 | 🟢 |
| R48 | Fuller & Warner 2000 — Family stressors → codependency | §6.4 | 🟢 |
| R49 | Chao 1994, 2001 — "Guan" culturally distinct from Western "authoritarian" | §10.1 | 🟢 |
| R50 | Roskam et al. 2021 — Parental burnout 5x individualist (42 countries, N=17,409) | §10.1 | 🟢 |
