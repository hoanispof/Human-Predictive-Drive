...
scope: |
  ...
  FILE NÀY = WHAT CONDITIONS predict sustained vs decaying resonance:
    ...
    → WHEN verbal necessary vs body-level sufficient
  v1.0 KEY CONTENT:
...
- §12 — EMPATHIC ACCURACY + MOTIVATED INACCURACY
...
⭐⭐ RESONANCE SUSTAINABILITY v1.0:
  ...
  FILE NÀY = CÁI GÌ CÒN THIẾU:
    ...
    HOW DOES trajectory change over time?
  ...
  🟢 Individual research claims: each established (32 citations)
  🟡 4-Layer integration model = framework synthesis
...
⭐⭐ RESONANCE TRUYỀN QUA 3 MODALITY ĐỘC LẬP:
  ...
  MODALITY ② — NON-VERBAL (bids + cues):
    ...
    🟢 Gottman: non-verbal bid response 86% vs 33% (turning toward/away).
...
⭐⭐ IM LẶNG CÓ 4 LOẠI — CHỈ 1 LOẠI BỀN VỮNG:
  ...
  🟡 Silence × Limerence vs Mature = framework synthesis.
...
⭐⭐ CẢM THẤY ĐƯỢC HIỂU > THỰC TẾ ĐƯỢC HIỂU:
  ...
  🟢 Laurenceau et al. 1998, 2005 — diary studies:
     PPR = PARTIAL MEDIATOR of intimacy.
     Confirmed in both new AND ongoing relationships.
...
⭐⭐ KHI SELF-EXPANSION DỪNG → RELATIONSHIP BOREDOM → SATISFACTION ↓:
  ...
  🟢 Tsapelas, Aron & Orbuch 2009 (Psychological Science):
     ...
     Controlling for year 7 satisfaction. = 9-year lag effect.

  🟢 Reissman, Aron & Bergen 1993:
     53 couples, 10-week RCT.
     Exciting activities > Pleasant activities > Control.
     = Not just "doing something together" — NOVELTY matters.

  🟢 Aron et al. 2000 (JPSP):
...
⭐⭐ META-ANALYSIS: MAINTENANCE TỰ GIẢM THEO THỜI GIAN:
  ...
  🟢 Ogolsky & Bowers 2013 (Journal of Social and Personal Relationships):
     ...
     BUT: duration NEGATIVELY associated with positivity + assurances.
  ...
  🟢 Ogolsky & Bowers 2013: maintenance meta-analysis (N=12,273).
  🟢 Canary & Stafford 1992: 5 maintenance behaviors.
  🟡 Decline × compilation stage mapping = framework synthesis.
  🟡 Compilation visibility trap = framework synthesis.
```
...
⭐ PHYSIOLOGICAL SYNCHRONY FOLLOWS NON-LINEAR TRAJECTORY:
  ...
  🟢 Reed et al. 2013:
     N=32 couples: LOWER satisfaction → STRONGER cortisol sync.
  ...
  🟡 U-curve × compilation stage = framework synthesis.
...
```
  ...
  → PFC-Operations.md v1.0 — Hold/Suppress, compiled quality, PFC budget
  → Body-Coupling.md v2.0 — coupling mechanism, depth × direction
  → Gap-Direction.md v2.0 — gap = f(surrounding chunks), flexibility basis
  → Valence-Propagation.md v3.0 — per-entity valence, structural vs current
  → Reward-Calibration.md v1.1 — domain feedback = only arbiter
  → Logic-Feeling-Balance.md v2.1 — infinite regress, cannot prescribe
```
