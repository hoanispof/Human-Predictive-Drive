...
> **Trạng thái:** v1.1 — refined 2026-04-18 (cross-refs aligned with v2.0 reference files)
> **Ngày:** 2026-03-25 (v1.0 DRAFT) | 2026-04-18 (v1.1 refine)
> **Mục đích:** Phân tích toàn diện Schema — patterns hành vi xuyên suốt Body→Imagine
> **Reference (v1.1 updated):**
>   Chunk.md v2.0 (2026-04-17) — chunk substrate chi tiết, activation dynamics
>   Feeling.md v2.0 (2026-04-17) — feeling = PFC observation of body-chunk interaction
>   Valence-Propagation.md v1.0 (2026-04-18) — valence per-entity + qua schema chain
>   Drive.md v1.1 (2026-04-18) — drive integration, 6 PFC Modes
>   Agent.md v1.0 — agent = function on chunk substrate
>   Anchor-Schema.md v1.1 — anchor sync point, trust binding
...
> **⚠️ Chunk mechanism chi tiết → xem Chunk.md v2.0 (file NÀY chỉ overview)**
...
```
  ⚠️ v1.1: CHI TIẾT chunk mechanism → xem Chunk.md v2.0 (~1,322L, 14 sections).
  Section NÀY chỉ giữ OVERVIEW để hiểu schema.

  Chunk = nhóm neurons fire đồng bộ = 1 "mảnh" thông tin
  Schema = NHIỀU chunks kết nối = 1 pattern hoàn chỉnh

  Chunk = GẠCH, Schema = NGÔI NHÀ (nhiều gạch ghép lại)
    → "Lửa" = 3 chunks kết nối: [đỏ] + [nóng] + [nguy hiểm]
    → "Mẹ" = multi-modal chunk: visual + auditory + somatic + emotional
  ...
  → CHI TIẾT: Chunk.md v2.0 §1 (definition), §2 (compile 5 mechanisms),
    §3 (lifecycle), §4 (activation dynamics), §5 (anchor-decay model)
  ...
  ⭐ SCHEMA = CHUNKS + LINKS + PURPOSE:
    ...
    3 TRẠNG THÁI schema:
      ...
      ACTIVE (PFC đang dùng):
        ...
        → VD: PFC hold [stickman]+[combat] → activation spread trong [game design]
        → VD: PFC hold [ý nghĩa]+[cuộc đời] → activation spread across nhiều schemas
        → = "Suy nghĩ" = PFC HOLD + BIAS activation, không phải "search database"
        → = Chunk.md v2.0 §4.1: spreading activation probability-weighted
...
⭐ INSIGHT QUAN TRỌNG:
  ...
  Architecture priority: L0 Alive > L1 Body-inputs > L3 Pattern
  (⚠️ v1.1: L2 removed — merged into L1, xem Body-Input-Enumeration.md)
  ...
  Trung bình:
    → Workaholic quên ngủ/gia đình: Threat-{Status} override L1
    → Nghiện game bỏ bê body: Novelty override L1
  ...
  Nặng:
    ...
    → Cờ bạc phá sản: Novelty loop override L1+L3
  ...
  → Priority L0>L1>L3 = ĐÚNG khi hệ thống healthy
...
```
  Surface fix (nhanh, TẠM thời):
    ...
    Đổi hoàn cảnh (context): L1 switch → nhưng body mang schema cũ theo
...
S2: Schema COMPILED có thể UNCOMPILE hoàn toàn?
    ...
    → ⚠️ v1.1: Chunk.md v2.0 §4.3 đã address — Competitive re-linking:
      link mới CẠNH TRANH link cũ, link cũ NEVER fully gone, stress can reactivate.
      🟢 Retrieval-induced forgetting (Anderson 1994), 🟢 Stress relapse (Sinha 2001).
...
S3: AI có thể detect schema không?
    ...
    → ⚠️ v1.1: Valence-Propagation.md §8 đã address — "Công cụ NAVIGATE, không GPS":
      AI detect PATTERN (verbal, behavioral), chuyên gia feel-check, client self-verify.
      3 tầng: AI detect → Chuyên gia verify → Client body-check.
      Approximate + verify = ĐỦ (không cần chính xác tuyệt đối).
      → Chi tiết ứng dụng: AI-Schema-Detection.md (planned).
...
```
⚠️ v1.1 — cross-refs updated 2026-04-18 to align with v2.0 reference files.

REFERENCE FILES (v2.0 — đọc song song):
  → Chunk.md v2.0: chunk substrate CHI TIẾT — §1 definition, §2 compile 5 mechanisms,
    §3 lifecycle, §4 activation dynamics (spreading activation, 7-factor link strength,
    competitive re-linking, trigger surface), §5 anchor-decay model
  → Feeling.md v2.0: feeling = PFC observation of body-chunk interaction —
    §2 7-layer fidelity gradient, §5 8-step flow, §6 reward (dopamine ≠ reward)
  → Valence-Propagation.md v1.0: valence per-entity + propagation qua schema chain —
    §4 propagation (4 mechanisms), §7 PFC blindness, §8 fundamental limits + 3 nguyên tắc

MECHANISM FILES:
  → Drive.md v1.1: drive integration, 5 inputs vô thức, 6 PFC Modes, melody lens
  → Agent.md v1.0: agent = function on chunk substrate, Self-Pattern-Match, Pattern-Resonance
  → Anchor-Schema.md v1.1: anchor sync point, trust binding, 4 nguồn fill
  → Body-Input-Enumeration.md: L0 Alive / L1 Body-inputs / L3 Pattern (reframe)
  → Body-Dissonance.md: 14+ dạng dissonance signal

ANALYSIS FILES:
  → PFC-Analysis.md: PFC functions, cognitive parameters, VTA mechanism
  → Imagination-Analysis-v2.md: Imagine = PFC engine, 5 types
  → Modality-Analysis.md: modality encoding per schema
  → Neural-Processing-Flow.md: hardware flow sensor→cortex→binding→chunk→PFC

SCHEMA-SPECIFIC:
  → Schema-Operations.md: gradient attention, parallel multi-position, PFC simulate
  → Schema-Example.md: ví dụ chi tiết (tình yêu, career, trauma)
  → Schema-Diagnostic.md: diagnostic tools
  → Anchor-Schema-Example.md: anchor cases
  → Schema/backup/Chunk-Search-Advanced.md: ĐÃ superseded bởi Chunk.md v2.0 §4

ỨNG DỤNG:
  → Education-Principles.md: threat-learn vs novelty-learn
  → AI-Schema-Detection.md (planned): AI-assisted schema pattern detection
```
