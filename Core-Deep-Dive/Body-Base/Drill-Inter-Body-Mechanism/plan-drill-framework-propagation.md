...
> **Ngày tạo:** 2026-05-22
> **Trạng thái:** IN PROGRESS — 26/28 done (Phase A+B+T COMPLETE, C1-C9 DONE = PHASE C COMPLETE)
...
### D1. SPM Rename Propagation

```
Self-Pattern-Match → Self-Pattern-Modeling
~70 active files, ~300+ occurrences
Abbreviation SPM KHÔNG ĐỔI

SCOPE:
  - "Self-Pattern-Match" → "Self-Pattern-Modeling" (full name only)
  - File rename: Self-Pattern-Match.md → Self-Pattern-Modeling.md (done in B2)
  - Header/title/reference updates in ~70 files
  - Backup files: KHÔNG sửa

NOTE: Mechanical. Sau khi B2 (SPM v3.1) hoàn thành.
      Nên đặt SAU Phase C để tránh double-touching files.
```
...
① 01-File-Index.md update:
  + Add 4 new files (PFC-Operations, Simulation-Engine, Entity-Compiled, Entity-Access)
  + Update version numbers for all rewritten/refined files
  + Update folder structure nếu thay đổi

② PFC-Hold-Dimensions.md v1.0 → v1.1 (optional):
  + Currently DRAFT. Enrich with Simulation Engine insights.
  + Lower priority. Có thể skip nếu không cần thiết.

③ Verification pass:
  + Spot-check cross-refs across 5-10 key files
  + Verify terminology consistency
  + Verify no "over-clone" bias remains
  + Verify Entity-Owned → Entity-Access alignment
```
...
TOTAL: 28 sessions (9 done + 19 remaining)
NEW CONTENT: ~27,000-31,000 lines estimated
FILES TOUCHED: ~100+
```
...
**Status**: IN PROGRESS — 26/28 sessions done. PHASE A+B+T+C ALL COMPLETE. **NEXT = D1 SPM Rename (~70 files).**
