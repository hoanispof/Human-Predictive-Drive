# Drill: PFC Label Convention — Vocabulary Reference
...
**Mục tiêu**: Formalize toàn bộ PFC-related vocabulary. Standardize across framework. Deprecate inconsistent terms.
**Phạm vi**: PFC roles × Processing spectrum × Operations × Cost × Quality × Regions × Hardware × Simulation Engine × Cognitive ops × Failure modes × Deprecated terms
**Companion**: Body-Feedback-Label v2.0 (body-feedback vocabulary — §8 covers Compiled/Fresh processing, §9C covers 3-cost)
**Phân biệt**: Body-Feedback-Label = body SIGNAL vocabulary. File này = PFC OPERATION vocabulary. Overlap ở §8/§9C của BFL = bridge zone.
**Dependencies**:
  - Drill-Compiled-Fresh-Mechanics v2.0 — §0-§12 (SOURCE cho §2-§5 file này)
  - Inter-Body-Mechanism v1.0 — §3 Compiled/Fresh, §4 3-cost, §7 PFC=Lawyer
  - PFC-Function.md v1.2 — 24 functions, 5 categories
  - PFC-Hardware.md v1.1 — COMT, DRD4, NE, capacity/quality
  - PFC-Hold-Dimensions.md — ~4±1 slots
  - Drill-Simulation-Engine v1.0 — 1 engine, 3 components, 3 axes
  - Logic-Feeling.md v2.1 — observer labels vs mechanism labels
  - Logic-Feeling-Balance.md — PFC = Lawyer, domain = final arbiter
  - Body-Feedback-Label v2.0 — §8 Compiled/Fresh, §9C 3-cost
...
- §1 — 3-TIER LABEL SYSTEM (tương tự BFL)
...
⭐ PFC VOCABULARY = PHÂN TÁN + KHÔNG NHẤT QUÁN:
  ...
  VẤN ĐỀ 3 — PFC CÓ NHIỀU VAI TRÒ:
    Observer (Feeling.md), Lawyer (Inter-Body §7),
    Director (Neural-Processing-Flow §6),
    Universal Resource (Compiled-Fresh §10),
    Quality Controller (Ask-AI §6.1)
  ...
  VẤN ĐỀ 4 — PFC FILES PHÂN TÁN:
    ...
    Compiled-Fresh-Mechanics v2.0, Simulation-Engine v1.0
...
⭐ TRỤC THẬT CỦA PROCESSING = COMPILATION LEVEL:
  ...
  │ (chung) │                         │ somewhere nhưng không biết level.│
  │         │                         │ Dùng: khi chỉ biết "đang xử lý."│
  │         │                         │                                  │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │         │                         │                                  │
  │ TIER 2  │ Compiled processing     │ Automatic. Body-direct. Cost ≈ 0.│
  │ (level) │                         │ Hebbian reinforced qua nhiều lần.│
  │         │                         │ "Cảm thấy biết" / "trực giác."  │
  │         │                         │ Dùng: khi processing KHÔNG cần  │
  │         │                         │ PFC active involvement.          │
  │         │                         │                                  │
  │         │ Fresh processing        │ PFC-mediated. Deliberate.        │
  │         │                         │ Cost > 0. Mỗi lần = effort.     │
  │         │                         │ "Phải nghĩ" / "phải phân tích." │
  │         │                         │ Dùng: khi PFC đang chain novel  │
  │         │                         │ paths actively.                  │
  │         │                         │                                  │
  │         │ Semi-compiled           │ Partially automatic, partially   │
  │         │                         │ fresh. Steps 1-3 compiled,       │
  │         │                         │ step 4 needs PFC.                │
  │         │                         │ Dùng: khi mixed — "biết phần    │
  │         │                         │ nào, phải nghĩ phần nào."        │
  │         │                         │                                  │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │         │                         │                                  │
  │ TIER 2  │ TRANSITION LABELS:      │                                  │
  │ (trans.)│                         │                                  │
  │         │ Fresh → Compiled        │ LEARNING: lặp lại + verify OK → │
  │         │ (learning)              │ Hebbian strengthen → automatic.  │
  │         │                         │ = "Logic → feeling" (cho person).│
  │         │                         │                                  │
  │         │ Compiled → Fresh        │ DISRUPTION: context mới, error → │
  │         │ (disruption)            │ phải re-evaluate deliberately.   │
  │         │                         │ = "Feeling → logic" (phải nghĩ  │
  │         │                         │ lại cái đã tưởng biết).         │
  │         │                         │                                  │
  └─────────┴─────────────────────────┴──────────────────────────────────┘
  ...
  ⚠️ "Compiled/Fresh" = TRỤC MECHANISM. "Logic/Feeling" = OBSERVER LABELS.
     ...
     Xem §12 file này cho full distinction.

  ⚠️ Spectrum, KHÔNG binary:
     Mỗi chunk nằm SOMEWHERE trên spectrum.
     "Expert partially stuck" = compiled phần quen, fresh phần mới.

  (Body-Feedback-Label §8 = CÙNG labels. File này §3 = BỔ SUNG context.)
  (Compiled-Fresh-Mechanics v2.0 §1 = SOURCE mechanism chi tiết.)
```
...
```
⭐ PFC CÓ 2 OPERATIONS TRÊN SPECTRUM — HOLD + SUPPRESS:

  ┌─────────┬─────────────────────────┬──────────────────────────────────┐
  │ Tier    │ Label                   │ Nghĩa + khi nào dùng            │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │         │                         │                                  │
  │ TIER 1  │ PFC operation           │ Chung nhất. PFC đang thực hiện  │
  │ (chung) │                         │ hành động nào đó trên spectrum.  │
  │         │                         │ Dùng: khi chỉ biết "PFC đang    │
  │         │                         │ làm gì đó."                     │
  │         │                         │                                  │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │         │                         │                                  │
  │ TIER 2  │ HOLD                    │ PFC AMPLIFY pattern mới/yếu.     │
  │ (opera.)│ (= PFC Amplify)         │ Sustain attention + working mem. │
  │         │                         │ "Tập trung vào cái này."         │
  │         │                         │ Direction: BOOST new.            │
  │         │                         │ CAN compile → sustainable.       │
  │         │                         │ Brain region: dlPFC, FEF.        │
  │         │                         │ Cost: ① PFC draft.               │
  │         │                         │ Dùng: khi PFC đang amplify.      │
  │         │                         │                                  │
  │         │ SUPPRESS                │ PFC BLOCK pattern đã compiled.   │
  │         │ (= PFC Inhibit)         │ Actively inhibit output route.   │
  │         │                         │ "Kìm lại / ép xuống."            │
  │         │                         │ Direction: BLOCK existing.       │
  │         │                         │ CANNOT compile "not" → unsustain.│
  │         │                         │ Brain region: rIFG, vlPFC.       │
  │         │                         │ Cost: ② Suppress (efference).    │
  │         │                         │ Dùng: khi PFC đang block.        │
  │         │                         │                                  │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │         │                         │                                  │
  │ TIER 2  │ 4 COMBINATIONS:         │                                  │
  │ (combo) │                         │                                  │
  │         │ ① Hold only             │ Learn new, no interference.      │
  │         │                         │ Cost: ① only. EASIEST.           │
  │         │                         │ VD: học ngôn ngữ mới.            │
  │         │                         │                                  │
  │         │ ② Hold + Suppress       │ Learn new WHILE block old.       │
  │         │                         │ Cost: ① + ②. DOUBLE.             │
  │         │                         │ → 3 outcomes: A/B/C (§4.1).      │
  │         │                         │ VD: thay đổi thói quen.          │
  │         │                         │                                  │
  │         │ ③ Suppress only         │ Block old WITHOUT replacement.   │
  │         │                         │ Cost: ② only. WORST strategy.    │
  │         │                         │ Wegner ironic process: rebound.  │
  │         │                         │ VD: "đừng nghĩ con gấu trắng."  │
  │         │                         │                                  │
  │         │ ④ Neither               │ Compiled runs automatic.         │
  │         │   (compiled auto)       │ Cost: ≈ 0. No PFC involvement.  │
  │         │                         │ VD: lái xe quen đường.           │
  │         │                         │                                  │
  └─────────┴─────────────────────────┴──────────────────────────────────┘


  ┌─────────────────────────────────────────────────────────────────────┐
  │ §4.1 — 3 OUTCOMES CỦA HOLD + SUPPRESS (khi cần thay đổi):         │
  │                                                                     │
  │  Outcome A (Genuine Shift):                                         │
  │    New pattern HOLD đủ lâu + domain verify →                        │
  │    compile genuine → suppress TỰ GIẢM (new > old).                 │
  │    = "Thay đổi thật"                                                │
  │                                                                     │
  │  Outcome B (Compiled Suppress):                                     │
  │    SUPPRESS compiled thành meta-pattern → flat, buried.             │
  │    Original VẪN CÒN nhưng bị bury dưới suppress layer.             │
  │    = "Chịu đựng được — nhưng không thay đổi thật"                  │
  │    = "Ngoan" từ ngoài → flat từ trong                               │
  │                                                                     │
  │  Outcome C (Failure/Burst):                                         │
  │    PFC KHÔNG ĐỦ budget → suppress BREAK →                          │
  │    original BURST (thường khi stress/mệt/rượu).                    │
  │    = "Bùng nổ" sau thời gian kìm nén                               │
  │                                                                     │
  │  Label use: specify outcome khi phân tích behavior trajectory.      │
  │  (Compiled-Fresh-Mechanics v2.0 §3-§5 = SOURCE chi tiết.)          │
  └─────────────────────────────────────────────────────────────────────┘
...
⭐ 3 INDEPENDENT COST SOURCES + BUDGET:
  ...
  │ (chung) │                         │ resource nào đó.                 │
  │         │                         │ Dùng: khi chỉ biết "PFC mệt."   │
  │         │                         │                                  │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │         │                         │                                  │
  │ TIER 2  │ ① PFC draft cost        │ Metabolic cost từ HOLD.          │
  │ (source)│                         │ PFC chain novel paths → glucose. │
  ...
  │         │                         │ Body expect X → PFC block →      │
  │         │                         │ efference copy ≠ output → ĐAU.   │
  ...
  │         │                         │ = "Không biết chọn gì" → stress. │
  │         │                         │ Brain region: ACC (conflict).    │
  │         │                         │ GIẢM khi commit (chọn 1 → hold).│
  │         │                         │ Dùng: khi mô tả "không chắc."   │
  │         │                         │                                  │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │         │                         │                                  │
  ...
  │         │                         │ Imagine-Final.                   │
  │         │                         │ Dùng: khi phân tích trade-off   │
  │         │                         │ "dùng PFC cho A → hết cho B."   │
  │         │                         │                                  │
  │         │ Total cost              │ = ① + ② + ③ cộng lại.           │
  │         │                         │ Dùng: khi tính tổng PFC load.   │
  │         │                         │                                  │
  └─────────┴─────────────────────────┴──────────────────────────────────┘
  ...
  ⚠️ ① CAN GIẢM (compile → automatic). ② CANNOT GIẢM (pattern stays).
     ...
     (Body-Feedback-Label §9C = CÙNG labels. File này §5 = BỔ SUNG.)
     (Compiled-Fresh-Mechanics v2.0 §10, §12 = SOURCE chi tiết.)
```
...
⭐ COMPILED CHUNKS CÓ QUALITY DIMENSION — PHÂN BIỆT 3 LOẠI:
  ...
  ⚠️ 3 quality types KHÔNG đổi được → phải BUILD NEW (genuine) bên cạnh.
    ...
    (Compiled-Fresh-Mechanics v2.0 §6 = SOURCE chi tiết.)
```
...
⭐ CÁC VÙNG PFC VÀ MAPPING TRONG FRAMEWORK:
  ...
  (Core-v7.8-Draft.md, Neural-Architecture.md §2 = SOURCE anatomy chi tiết.)
```
...
⭐ 1 ENGINE, 3 COMPONENTS, N APPLICATIONS:
  ...
  (Drill-Simulation-Engine v1.0 = SOURCE chi tiết.)
```
...
⭐ "LOGIC/FEELING" vs "COMPILED/FRESH" — 2 TẦNG KHÁC NHAU:
  ...
  (Body-Feedback-Label §8 = CÙNG distinction. Logic-Feeling.md = SOURCE.)
```
...
```
  ...
  ├──────────────────────────────────┼──────────────────────────────────────┤
  │ Drill-Compiled-Fresh-Mechanics   │ SOURCE: §0-§12 mechanism chi tiết.   │
  │ v2.0                             │ File này formalize LABELS from it.   │
  ├──────────────────────────────────┼──────────────────────────────────────┤
  │ Inter-Body-Mechanism v1.0       │ §3 Compiled/Fresh axis. §4 3-cost.   │
  │                                  │ §7 PFC = Lawyer. SOURCE-OF-TRUTH.    │
  ├──────────────────────────────────┼──────────────────────────────────────┤
  │ PFC-Function.md v1.2             │ 24 functions, 5 categories.          │
  ...
  │                                  │ "Hardware online from birth."        │
  ├──────────────────────────────────┼──────────────────────────────────────┤
  │ Drill-Simulation-Engine v1.0     │ 1 engine, 3 components, 3 axes.     │
  │                                  │ SOURCE cho §9 Simulation Engine.     │
  ├──────────────────────────────────┼──────────────────────────────────────┤
  │ Logic-Feeling.md v2.1            │ Observer labels. Compiled/Fresh      │
  │                                  │ distinction. SOURCE cho §12.         │
  ├──────────────────────────────────┼──────────────────────────────────────┤
  │ Logic-Feeling-Balance.md         │ PFC = Lawyer. Domain = final arbiter.│
...
**END OF DRILL — v1.0 (24 citations, 16 sections)**
