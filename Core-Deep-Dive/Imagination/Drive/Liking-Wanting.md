...
🟡 CƠ CHẾ:
  ...
  H10 — Body Signal Precondition Hypothesis (03-Reward.md §3):
    ...
    ④ Goldilocks zone (40-70% match)
       → Match ratio không quá cao (boring) cũng không quá thấp (confusion)
       → No: 100% match = "biết rồi" = no delta → no liking
       → No: 5% match = "không hiểu gì" = confusion → cortisol → no liking
       → Yes: 50% match = "quen nhưng mới!" = sweet spot → opioid
...
```
  ...
  → Goldilocks 40-70%: inverted-U supported (Berlyne 🟢), exact range = estimate
```
...
DEPENDENCY FILES (file NÀY phụ thuộc):
  [Dopamine-Reward-Rejection.md](Dopamine-Reward-Rejection.md) — 7-step + 7 evidence + framework position
