...
dependencies: |
  ...
  Somatic-Articulation-Loop.md (body → explicit knowledge, 7-stage SAL)
...
### §1.1 — What feeling literacy IS (mechanism-grounded)
...
🟡 MỖI LITERACY COMPONENT → VỊ TRÍ TRONG 8-STEP FLOW:
  ...
  → LITERACY TRAINING = improve SPECIFIC STEPS in 8-step flow
  → NOT vague "be more aware" → PRECISE "train step ④ threshold via X"
  → = §3 5-stage training maps to this table
```
...
🟡 TẠI SAO AI ERA LÀM FEELING LITERACY QUAN TRỌNG HƠN:
  ...
  ① DECISION MAKING:
     Before: Think + Feel → Decide
     Now: AI Think → Human Feel → Decide together
     → If feeling inaccurate → decision WORSE (no logic compensation)

  ② EVALUATION:
     Before: Analyze + "feels right" → Evaluate
     Now: AI Analyze → Human "feels right/wrong" → Evaluate
     → Body vote becomes PRIMARY quality check on AI output
     → Illiterate person → cannot evaluate AI → accepts wrong output
  ...
  ④ MEANING:
     Before: Experience + Reflect → Meaning
     Now: AI can summarize/organize → Human still MUST feel meaning
     → Meaning is body-generated (schema + chunk + feeling system)
     → Cannot outsource to AI → must have own literacy

  ⑤ RELATIONSHIP:
     Before + Now: SAME → Human-only territory
     → No AI substitution for feeling-in-relationship
     → Literacy = capacity for authentic connection
     → AI era: more screen time → LESS natural feeling practice
  ...
  ⭐ PARADOX:
    ...
    AI era also makes feeling HARDER to develop naturally
      (more screen time, less body engagement, fewer face-to-face)
    → DELIBERATE literacy training becomes NECESSARY, not optional

  → = Framework justification for THIS FILE:
    Feeling literacy training = most important skill development
    in AI era. Not optional. Not luxury. Survival skill.
```

### §2.3 — Literacy as human competitive advantage

```
🟡 FEELING LITERACY = HUMAN COMPETITIVE ADVANTAGE:

  WHAT AI CANNOT DO (no body):
    → Detect convergence zones (requires body-chunk interaction)
  ...
  WHAT ILLITERATE HUMAN + AI CANNOT DO:
    → Human: "AI, fix everything" (no feeling input)
    → AI: produces technically correct but meaningless output
    → Human: cannot evaluate → accepts → builds on wrong foundation
    → = FAILURE: AI amplifies wrong direction because no feeling check

  → Literacy = ability to DIRECT AI meaningfully
  → Illiteracy = dependency on AI without quality control
  → = Feeling literacy training IS preparation for AI collaboration
```
...
⭐ 5-STAGE FEELING LITERACY TRAINING:
  ...
  │    "This feeling = this situation" — build body-situation links    │
  │                              ↓                                     │
  │  Stage 4: LABEL ATTACHMENT                                         │
  │    "Name the feeling" — verbal handle → logic-planning access     │
  │                              ↓                                     │
  │  Stage 5: META-FEELING                                             │
  │    "Feeling about feelings" — recursive awareness + calibration    │
  │                                                                    │
  │  EACH STAGE = CHUNK COMPILE PROCESS                                │
  │  → Stage builds ON TOP of previous                                 │
  │  → Cannot skip stages (each = prerequisite for next)               │
...
⭐ STAGE 1 — BODY AWARENESS:
  ...
  CHUNK MECHANISM — HOW training works:
    ...
    MECHANISM A — INTEROCEPTIVE PREDICTION REFINEMENT:
      ...
      → Updated model → smaller prediction errors detectable
    ...
    MECHANISM B — ATTENTION PATHWAY STRENGTHENING:
      ...
      → 🟢 Meditation neuroscience: insula cortex thickens with practice
  ...
  TRAINING METHODS (mechanism-grounded):

    ① BODY SCAN (Vipassana / MBSR tradition):
       → Systematic attention: head → feet (or reverse)
       → Detect whatever IS present (not seeking specific feeling)
       → Duration: 10-20 min daily
       → Mechanism: A + B + C all active
       → 🟢 MBSR: 8-week programs well-validated (Kabat-Zinn)
    ...
    ③ BREATH AWARENESS:
       → Attend to breathing without controlling
       → Breath = ALWAYS PRESENT → natural body signal to practice detecting
       → Duration: 5-10 min daily
       → Mechanism: primarily B (attention pathway strengthening)
  ...
  CHUNK SYSTEM CHANGE:
    → NEW feeling-detection chunks compiled (via repetition: 01b §2.2 ①)
    → "Body-attention" chunks strengthened (pathway grows)
    → PFC-to-insula pathway = higher probability (01b §2 distribution shift)
...
⭐ STAGE 2 — SIGNAL DISCRIMINATION:
  ...
  TARGET IN 8-STEP FLOW:
    ...
    → Step ⑤ (chunk match): different signals → different chunk candidates
  ...
  CHUNK MECHANISM — HOW training works:
    ...
    MECHANISM D — FEELING CHUNK LIBRARY EXPANSION:
      ...
      → = Barrett 2017 emotional granularity:
        more fine-grained categories → better health outcomes 🟢

    MECHANISM E — CROSS-CHANNEL COMPARISON:
    ...
    MECHANISM F — INTENSITY CALIBRATION:
      → Repeated exposure → scale develops
...
### §3.3 — Stage 3: Pattern Recognition (this feeling = this situation)

```
⭐ STAGE 3 — PATTERN RECOGNITION:

  GOAL: Link FEELING PATTERNS to SITUATION PATTERNS
        → "When this body state occurs, it usually means X context."

  TARGET IN 8-STEP FLOW:
    → Step ⑤ (chunk match): BETTER matches because situation-feeling
      associations are compiled → fewer false positives/negatives
  ...
  CHUNK MECHANISM — HOW training works:
    ...
    MECHANISM H — MULTI-SITUATION COMPARISON:
      ...
      → = This is HOW body wisdom accumulates


  HOW PATTERN RECOGNITION BUILDS:

    STEP 1 — Notice feeling (Stage 1 output)
    STEP 2 — Discriminate feeling (Stage 2 output)
    STEP 3 — Note situation context
    STEP 4 — Over time: same feeling + similar situations → LINK forms
    STEP 5 — Link strengthens → AUTOMATIC recognition
    STEP 6 — Pattern available for prediction (Imagine-Final preview)

  → = 01c Discovery Cycle applied to feeling domain:
    Accumulate → Vague → Clarify → Label → Logic-Planning access


  TRAINING METHODS:

    ① FEELING-SITUATION TRACKING:
       → After each significant feeling episode, note:
         a) What feeling (Stage 2 output)
         b) What situation (context, people, activity)
         c) What happened next (outcome)
       → Reviewing over weeks → patterns visible
       → = Deliberate compilation via attention (not just experience)

    ② RETROACTIVE PATTERN SEARCH:
       → When feeling arises: "have I felt this BEFORE?"
       → Search memory for similar body state → what context was it?
       → = PFC deliberate linking (01 Type 4)
       → Connects current episode to compiled patterns
       → = Enriches pattern library with each connection
  ...
  EXPECTED OUTCOMES:
    → After ~6-12 months:
      → "Lờ mờ" feelings now have recognizable PATTERNS
  ...
  CHUNK SYSTEM CHANGE:
    ...
    → Pattern-level chunks: "anticipation pattern", "overwhelm pattern"
    → Prediction chunks: "situation X → expect body state Y"
    → Cross-situational links: similar patterns across different contexts
    → = Convergence zones now RESOLVE quickly in familiar territory
```

### §3.4 — Stage 4: Label Attachment (name the feeling)
...
⭐ STAGE 4 — LABEL ATTACHMENT:
  ...
  CHUNK MECHANISM — HOW training works:
    ...
    MECHANISM K — LABEL VERIFICATION VIA BODY VOTE:
      ...
      → = SAL Stage 5 (body-confirm filter)
  ...
  TRAINING METHODS:
    ...
    ① VOCABULARY EXPANSION:
       ...
       → Cross-language: German "Schadenfreude", Japanese "mono no aware",
         Portuguese "saudade" — each = chunk category no English equivalent
       → = More labels = more retrieval paths = more granularity
       → 🟢 Barrett 2017: vocabulary predicts emotional granularity
  ...
  EXPECTED OUTCOMES:
    → After ~6-12 months:
      → Active feeling vocabulary: from ~20 → ~100+ words
      → Label-body match improves → fewer misattributions
      → Can COMMUNICATE feelings to others with precision
      → Feelings enter logic-planning → more strategic responses
      → = Step ⑥ success rate dramatically increased
  ...
  CHUNK SYSTEM CHANGE:
    ...
    → = Logic-Planning access: feelings that were stuck at body level
      now AVAILABLE for deliberate reasoning
```
...
⭐ STAGE 5 �� META-FEELING:
  ...
  TARGET IN 8-STEP FLOW:
    → Step ⑦→⑧ (body vote → action): META-LEVEL CALIBRATION
    → = Knowing WHEN to trust body vote and WHEN to be skeptical
    → = §1 component E (calibration) as chunk-compiled skill
  ...
  CHUNK MECHANISM — HOW training works:
    ...
    MECHANISM L — META-CHUNK COMPILATION:
      ...
      → = F4 05 (Insight-Tacit-Upgrade §2): self-model as chunk system

    MECHANISM M — DOMAIN-SPECIFIC CALIBRATION:
      → Expert in domain X: meta-chunk says "trust gut in X"
      → Beginner in domain Y: meta-chunk says "don't trust gut in Y"
      → = Mechanism-Deep §6.2: expert vs beginner feeling reliability
      → Meta-feeling = KNOWING which domain your feeling is expert vs beginner
      → = Calibrated confidence per domain — NOT blanket "trust/distrust"
  ...
  TRAINING METHODS:
    ...
    ① ACCURACY TRACKING:
       → Record: feeling → prediction → actual outcome
       → Over time: WHERE are my feelings accurate? Where not?
       → = Building domain-specific meta-calibration chunks
       → After 50+ episodes in same domain → calibration chunk STABLE
    ...
    ④ CROSS-MODAL VERIFICATION:
       → Feeling from body → verify with logic
       → Analysis from logic → verify with body vote
       → = Logic-Feeling.md parallel processing
       → = Neither alone sufficient — both needed for quality
       → = Meta-feeling includes awareness of WHEN to use which
  ...
  EXPECTED OUTCOMES:
    → After ~1-2 years sustained practice:
      ...
      → = Mechanism-Deep §6.1: expert feeling = large library + meta-awareness
...
```
🟡 5 STAGES — HOW THEY INTEGRATE:

  ⭐ NOT STRICTLY SEQUENTIAL — OVERLAPPING:

    Stage 1 (body awareness) starts FIRST → foundation required
    Stage 2 (discrimination) starts SHORTLY AFTER → builds on awareness
    Stage 3 (pattern recognition) starts within MONTHS → needs data
    Stage 4 (label attachment) starts PARALLEL with 3 → feed each other
    Stage 5 (meta-feeling) starts LATER → needs stages 1-4 substrate

    ┌──────────────────────────────────────────────────────────────┐
    │ Month:  1    2    3    4    5    6    9   12   18   24+     │
    │                                                              │
    │ Stage 1 ████████████████████████████████████████ (ongoing)   │
    │ Stage 2    ████████████████████████████████████ (ongoing)    │
    │ Stage 3         ████████████████████████████ (ongoing)       │
    ...
    │   Month 3-6:  All first 4 stages active, basic competency   │
    │   Month 6-12: Pattern library growing, labels expanding     │
    │   Month 12+:  Meta-feeling developing, calibration starts   │
    │   Month 24+:  Integrated system, wisdom accumulating        │
    └──────────────────────────────────────────────────────────────┘


  ⭐ CHUNK ACCUMULATION CURVE:

    → Stage 1 chunks: grow rapidly at first, plateau
    ...
    → Cannot train just 1 stage and call it "literacy"
    → All 5 needed — but proportional time/effort varies by person


  ⭐ ENTRY POINTS (where to start given different backgrounds):

    IF never practiced body awareness:
      → Start Stage 1 (body scan, 8 weeks minimum)
      → Then add Stage 2 gradually

    IF meditator (Stage 1 already developed):
      → Focus on Stage 2 + 3 (discrimination + pattern)
      → Many meditators have Stage 1 but WEAK Stage 2-4
    ...
    IF trauma history:
      ...
      → Stage 2 + competitive re-linking (01b §3)
      → May need therapy support for Stage 1-3 before self-directed
      → See §5.3 for specific failure modes
```
...
⭐ GENDLIN FOCUSING — 6 STEPS × CHUNK MECHANISM:
  ...
  🟢 Gendlin 1978 + 50 years clinical validation.
  Below: each step → chunk mechanism explanation.


  STEP ① CLEARING A SPACE:
    Gendlin: set aside current issues, create mental space.

    Chunk mechanism:
      → PFC deliberately RELEASES held chunks
      → Working memory freed (PFC ~4-7 items capacity)
      → = Create bandwidth for body signals to cross threshold
      → Without clearing: PFC occupied → body signals BLOCKED
        (step ④ threshold raised by PFC load)
      → = Practical: imagine placing each worry on a shelf
      → = Mechanism: working memory management → threshold optimization


  STEP ② FELT SENSE:
    Gendlin: let body sensation form around an issue.
    ...
    Chunk mechanism:
      ...
      → = Mechanism-Deep §5.4: convergence zone = what "lờ mờ" IS
      → Felt sense properties match convergence zone properties:
        → Vague (no single source)
        → Bodied (insula reports body state)
        → Directional (VMPFC adds valence)
        → Resistant to simple labels (no dedicated chunk yet)


  STEP ③ HANDLE:
    Gendlin: find word, phrase, or image that fits the felt sense.

    Chunk mechanism:
      → PFC attempts to LABEL convergence zone
      → = Step ⑥ in 8-step flow (label search)
  ...
  STEP ④ RESONATING:
    ...
    Chunk mechanism:
      ...
      → 3 outcomes:
        → Smooth: "yes, that captures it" → handle ACCEPTED
        → Resist: "close but not right" → body tension → try another
        → Empty: "doesn't connect at all" → discard, start over
      → = SAL Stage 5: body-confirm filter
      → = Each resonance check = competitive re-linking (01b §3):
        → Accepted handle → link STRENGTHENED (Hebbian)
        → Rejected handle → link NOT strengthened → fades
      → = Natural selection of ACCURATE labels through body vote


  STEP ⑤ ASKING:
    Gendlin: ask the felt sense "what is it about this?"

    Chunk mechanism:
      → PFC HOLDS convergence zone (now partially labeled)
      → Asks Type 4 deliberate question: "what makes this feel like X?"
      → = Step ⑤ in 8-step flow (chunk match) — DEEPER iteration
      → Spreading activation from convergence zone → reaches FURTHER chunks
      → = Discovery cycle step ④ CLARIFY (01c §2.5):
        → Strategy A: deliberate search
        → Strategy B: describe boundaries (triangulation)
      → New chunks activate → convergence zone SHIFTS slightly
      → = Zone RESOLVES toward more specific pattern
      → = Each "asking" iteration → narrower, clearer


  STEP ⑥ RECEIVING:
    Gendlin: welcome whatever comes, even if surprising.
    ...
    Chunk mechanism:
      ...
      → = SAL Stage 7: felt shift = pattern becomes explicit


  ⭐ BODY SHIFT = CONVERGENCE ZONE RESOLUTION:

    The "felt shift" that Gendlin describes = convergence zone
    resolving into a clearer pattern:

    BEFORE shift:
      → Many chunks converge weakly → vague, no label
    ...
    AFTER shift:
      → Clear understanding: "ah, THAT'S what it was about"
      → Label available → logic-planning enabled
      → Body: settled, lighter, "right" feeling
      → = 01c step ⑤ LABEL/ANCHOR achieved

    🟢 Gendlin: body shift measurable via galvanic skin response
    🟢 Felt shift predicts therapy outcome across modalities
    🟡 "Convergence zone resolution" = framework interpretation
```

### §4.2 — Focusing × 5-stage training

```
🟡 FOCUSING TRAINS ALL 5 STAGES SIMULTANEOUSLY:

  ┌─────────────────────┬───────────────────────────────────────────┐
  │ Training stage       │ How Focusing trains it                   │
  ├─────────────────────┼───────────────────────────────────────────┤
  │ 1. Body awareness   │ Step ② felt sense = attend to body       │
  │                     │ Step ① clearing = lower threshold         │
  ├─────────────────────┼───────────────────────────────────────────┤
  ...
  │                     │ Step ④ resonating = verify label via body │
  ├─────────────────────┼───────────────────────────────────────────┤
  │ 5. Meta-feeling     │ Step ⑥ receiving = awareness of process   │
  │                     │ Overall practice = meta-chunks about      │
  │                     │ own feeling access process                │
  └─────────────────────┴───────────────────────────────────────────┘
...
### §4.3 — SAL connection: Focusing = Somatic-Articulation-Loop applied to feeling

```
🟡 FOCUSING = SAL APPLIED TO FEELING SPECIFICALLY:

  SAL 7-stage loop (Somatic-Articulation-Loop.md §2):
  ...
  = SAL is GENERAL mechanism (body-knowledge → explicit-knowledge)
  = Focusing is SAL APPLIED TO FEELING DOMAIN
  = 5-stage training is SYSTEMATIC version of what SAL does naturally

  WHERE FOCUSING ADDS VALUE beyond natural SAL:
    → Natural SAL: happens spontaneously (shower thoughts, aha moments)
    → Focusing: DELIBERATE → can trigger SAL on-demand
  ...
  ⭐ 3 SAL CATALYSTS × FEELING TRAINING:

    SAL Catalyst Type 1 — Human partner (Focusing partner, therapist):
      → Partner suggests handles → body vote → refine
      → BEST for: deep emotional patterns, trauma-adjacent content
      → = Therapy modalities (EFT, Hakomi, SE) all use this

    SAL Catalyst Type 2 — Self (solo Focusing):
      → Self-generated handles → body vote → refine
      → BEST for: daily practice, building skill, non-threatening content
      → = Accessible without external help

    SAL Catalyst Type 3 — AI (AI-assisted articulation):
...
⭐ COGNITIVE OVERRIDE — PFC SuppRESS body signals:
  ...
  CHUNK MECHANISM:
    ...
    → Body → PFC pathway: WEAK (underused → Hebbian: weakens)
    → = 01b §2.2 factor ①: low repetition → weak link
    → PFC OVERRIDES body signal because PFC pathway STRONGER
  ...
  INTERVENTION:
    ...
    → De-emphasize verbal processing initially
    → Focus on SENSATION before LABELING
    → = Build body → PFC pathway FIRST (Stage 1 Mechanism B)
    → = Then add labeling (Stage 4) AFTER body access established
    → Estimated timeline: 2-4 months of body-first practice
      before verbal integration
```

### §5.2 — Failure Mode 2: Alexithymia Pattern

```
⭐ ALEXITHYMIA — cannot identify/describe feelings:
  ...
  CHUNK MECHANISM:
    ...
    → Body signals exist BUT:
      ...
      → Labels ABSENT (step ⑥: no verbal handles compiled)
    → = Not that body is quiet — body is LOUD but PFC can't READ
  ...
  INTERVENTION:
    ...
    → Build library from scratch: one feeling at a time
    → Estimated timeline: 6-18 months with consistent support
    → 🟢 Research: alexithymia DOES improve with targeted intervention
      (Grabe et al. 2008, limited but promising)
```

### §5.3 — Failure Mode 3: Trauma Contamination

```
⭐ TRAUMA — distorted signals block accurate training:
  ...
  WHY TRAINING FAILS:
    ...
    → Body awareness → accesses traumatic body memory
    → Without SAFE CONTAINER → retraumatization risk
    → = Chunk mechanism: training strengthens body-attention pathway
      → body-attention pathway fires → trauma chunks fire
      → trauma response → person STOPS training → avoidance compiled

  INTERVENTION:
    → TITRATED exposure (not flood):
      → Small doses of body awareness → return to safety
      → = Somatic Experiencing (Levine): "pendulation"
        = alternate between activation and resource
      → 🟢 Levine 2010: titrated exposure clinically validated
    → SAFE CONTEXT first:
      → Trained therapist, safe environment
      → Build safety chunks FIRST (01b §3: competitive re-linking)
      → New "body awareness = safe" chunks COMPETE with
        old "body awareness = danger" chunks
    → COMPETITIVE RE-LINKING as core strategy:
      → New positive body experiences → new links form
      → Over time: new links take probability share from trauma links
      → = 01b §2.3: probability distribution shift
      → Old trauma links never deleted — but RELATIVELY weaker
    → Stage progression: 1→2 SLOWLY with support
      → Stage 3-4 may take years to develop
      → Stage 5 meta-awareness = eventual goal for self-management
    → Estimated timeline: 1-5 years depending on severity + support
```
...
### §6.1 — Cultural variation in feeling literacy
...
🟡 CULTURE SHAPES FEELING LITERACY VIA CHUNK COMPILATION:
  ...
  ② EXPRESSION RULES:
     → Which feelings are "appropriate" to show → culturally installed
     → East Asian: emotional restraint valued → less expression ≠ less feeling
     → Western: emotional expression valued → more expression ≠ more literacy
     → = Cultural chunks act as FILTER on step ⑧ (action/expression)
  ...
  ④ COLLECTIVE vs INDIVIDUAL FEELING:
     → Collectivist: "we feel" → group feeling compiled as primary
     → Individualist: "I feel" → individual feeling compiled as primary
     → Neither wrong — different literacy emphasis
     → = F3 social referencing: culture shapes WHOSE feelings matter


  ⭐ FRAMEWORK POSITION:
    → No "correct" feeling culture
    → All cultures train SOME literacy, suppress SOME
    → Awareness of own cultural bias = meta-feeling (Stage 5)
    → Cross-cultural exposure → expanded library → richer literacy
    → = Chunk library diversity benefits feeling accuracy
```

### §6.2 — Developmental perspective: when training matters most

```
🟡 CRITICAL WINDOWS FOR FEELING LITERACY (mapped to F1 timeline):
  ...
  ② TODDLER (18m-3y):
     → Basic label compilation begins (Stage 4 early)
     → "Angry", "sad", "happy" = first feeling handles
     → Parent labeling helps → F3 Direction B for feelings
     → Tantrums = Stage 2 discrimination failure (flood, can't sort)

  ③ CHILDHOOD (3-12y):
     → Vocabulary expansion + pattern library growth
     → Social feelings emerge (embarrassment, pride, jealousy)
     → Cultural scripts installed (what's OK to feel/show)
     → = Critical for Stage 3 (pattern recognition begins)
     → = Skill-Intro.md: skill acquisition includes feeling skills

  ④ ADOLESCENCE (12-18y):
     → Hormonal changes → NEW body signals → library needs updating
     → Identity formation → meta-feelings begin (Stage 5)
     → Peer influence > parental → cultural chunks reconfigure
     → = Vulnerable window: literacy can surge OR stall

  ⑤ YOUNG ADULT (18-30y):
     → Domain expertise building → domain-specific feeling literacy
     → Therapy often begins → explicit literacy training
  ...
  ⭐ KEY INSIGHT:
    → Literacy can be trained at ANY AGE (chunks always compilable)
...
🟢 HIGH CONFIDENCE — research-established:
  ...
  ✓ Interoceptive accuracy trainable (Farb 2013, Lazar 2005)
  ✓ Meditation → improved insula function (Lazar 2005, Farb 2013)
  ✓ Emotional granularity → better outcomes (Barrett 2017)
  ✓ Gendlin Focusing → clinical effectiveness (50+ years, meta-analyses)
  ...
  ✓ Titrated exposure effective for trauma (Levine 2010, SE validation)
...
🟡 MEDIUM CONFIDENCE — framework synthesis:
  ...
  ⚠ 5-stage training framework (each stage grounded individually,
    integrated model = framework synthesis)
  ⚠ Stage → 8-step flow mapping (logical but not empirically tested as unit)
  ⚠ 14 mechanisms (A-N) as discrete training targets (each plausible,
    taxonomy = framework organizing)
  ⚠ Focusing 6 steps × chunk mechanism interpretation (consistent with
    Gendlin phenomenology + chunk model, but not directly tested)
  ⚠ Convergence zone resolution = felt shift (framework interpretation
...
🔴 LOW CONFIDENCE — hypotheses:
  ...
  ⚠ Specific timelines (8 weeks, 3-6 months, etc.) — approximate
    heuristics from clinical literature, massive individual variation
  ⚠ Chunk library size estimates (50→200+ categories) — illustrative,
    not measured
  ⚠ Meta-chunk recursive structure (chunks about chunks) — conceptually
    sound, neural implementation = frontier
  ⚠ AI-as-catalyst for Focusing (SAL Type 3) — emerging practice,
    not yet validated
```
...
⭐ PREDICTIONS FROM THIS FILE:
  ...
  P-LT2: Focusing trains all 5 stages simultaneously
  ...
  P-LT4: Cultural vocabulary → literacy
    ...
    → Status: 🟡 Barrett granularity research supports indirectly;
      cross-language training = new

  P-LT5: Trauma pattern requires titrated approach
    → Prediction: Standard meditation training for trauma population →
      higher dropout + worse outcomes than titrated somatic approach.
    → Test: Randomize trauma patients: MBSR vs Somatic Experiencing.
      Compare outcomes + dropout rates.
    → Status: 🟢 Partially supported (meditation contraindication for
      some trauma patients known — Treleaven 2018)

  P-LT6: Meta-feeling as wisdom predictor
    → Prediction: Stage 5 (meta-feeling) scores → predict decision quality
      across domains (not just emotional health).
...
⚠️ WHAT THIS FILE DOES NOT CLAIM:
  ...
  ✗ "AI era makes feeling training urgent for everyone"
    ...
    → Depends on how AI integration unfolds.
  ...
  ✗ "This replaces professional clinical guidance"
    → Framework is theoretical. Trauma, alexithymia,
      clinical conditions → professional support needed.
```
...
📚 KEY RESEARCH REFERENCES:
  ...
  Training methods:
    Gendlin E. (1978) "Focusing" — felt sense, 6 steps, body shift
    Farb N. (2013) — meditation → interoceptive accuracy
    Lazar S. (2005) — meditation → insula thickening
    Kabat-Zinn J. — MBSR program validation
    Levine P. (2010) — Somatic Experiencing for trauma
    Treleaven D. (2018) — trauma-sensitive mindfulness

  Feeling granularity + vocabulary:
    Barrett L.F. (2017) — emotional granularity → health outcomes
    Taylor G.J. (2000) — TAS-20 alexithymia measurement

  Neuroscience:
    Seth A. (2013) — interoceptive inference (prediction model)
    Craig A.D. (2002, 2009) — interoception, insula integration
    Collins A. & Loftus E. (1975) — spreading activation
    Hebb D. (1949) — Hebbian learning principle

  Expertise + implicit knowledge:
    Polanyi M. (1958) — tacit knowledge ("we know more than we can tell")
...
>   §4: Gendlin Focusing = SAL applied to feeling, trains all 5 stages
