...
═══════════════════════════════════════════════════════
  ...
  SYSTEMS ACTIVATED FROM 1 MELODIC STREAM:
    ...
    ④ Prediction system (PFC/inferior frontal gyrus — hierarchical prediction)
...
═══════════════════════════════════════════════════════
  ...
  🟡 "Tonic/Cyclic" labeling = framework terms (Gap-Body-Need.md §3).
    Components (MMN, cocktail party) = 🟢 established.
    Labeling overlay = 🟡 framework synthesis.
...
═══════════════════════════════════════════════════════
§2.4 — Sound → Prediction
═══════════════════════════════════════════════════════

  STIMULUS: Footsteps through wall, engine sound, approaching vehicle,
    door opening, rain on roof.

  PROPERTIES ENABLE:
    #2 Cross-barrier → prediction WITHOUT visual confirmation.
...
═══════════════════════════════════════════════════════
§2.5 — Sound → By-Product-Gap-Resonance
═══════════════════════════════════════════════════════

  STIMULUS: Conversation, singing together, shared music experience.
...
═══════════════════════════════════════════════════════
§2.6 — Sound → Background-Pattern
═══════════════════════════════════════════════════════

  STIMULUS: Genre exposure (years), ambient sound environment,
...
```
⭐ WHY AUDITORY DEVELOPED 6 UNIQUE PROPERTIES:

  4 evolutionary pressures → 6 properties:
  ...
  ④ VISUAL COMPLEMENT:
    Property #4 Temporal > Spatial → covers what visual CANNOT.
    Visual = spatial precision, stops at walls, needs light.
    Auditory = temporal precision, crosses barriers, always-on.
    → Together = COMPLETE prediction system:
      Spatial (WHERE) + Temporal (WHEN/WHAT NEXT) + 24/7 coverage.
    → = WHY animals with BOTH modalities → survival advantage.

  🟢 Evolutionary arguments: well-supported (comparative biology, primatology).
  🟡 "Complete prediction system" framing = framework synthesis.
  🟡 Integration of 4 pressures into 6 properties = framework synthesis.
```
...
═══════════════════════════════════════════════════════
  ...
  HARDWARE PROPERTIES:
    No ear closure mechanism: basic anatomy
    ABR during sleep: clinical neuroscience (established)
    Sound cross-barrier: physics (diffraction + transmission)
    Binaural localization: interaural time/level difference (established)
    Cochlea functional ~25 weeks: developmental biology
    Newborn voice preference: DeCasper & Fifer 1980 (Science)
    Prenatal language rhythm: Moon, Cooper & Fifer 1993
    Auditory temporal resolution ~2-5ms: gap detection research
    Visual spatial > auditory spatial: established psychophysics
    Modality appropriateness: Welch & Warren 1980

  ACTIVATION + PROCESSING:
    Motor activation passive listening: Grahn & Brett 2007 (J Cogn Neurosci)
    Reward system music: Salimpoor et al. 2011 (Nature Neuroscience)
    Emotion system music: Blood & Zatorre 2001 (PNAS)
    Multi-system activation: Koelsch 2014 (review)
    Mismatch negativity: Näätänen 1995 (Int J Neuroscience)
    Cocktail party effect: Cherry 1953

  CROSS-PARAMETER (individual connections):
    Amygdala subcortical pathway ~12ms: LeDoux 1996
    Language = auditory temporal sequence: established linguistics
    Voice as entity identifier: established psychology
...
═══════════════════════════════════════════════════════
  ...
  "Sentinel function" framing
  "Activation efficiency" as comparative metric across modalities
  "Tonic/Cyclic" dual function labeling (components 🟢, labels = framework)
  Cross-parameter mapping as UNIFIED analysis
  "Complete prediction system" (auditory + visual complement)
  Evolutionary integration (4 pressures → 6 properties)
  Sound = "hardware channel, not observation parameter" distinction
...
═══════════════════════════════════════════════════════
  ...
  Exact activation efficiency COMPARISON across modalities
    (nobody has measured "systems per passive input" across all modalities)
  "Always-on → stronger Background-Pattern" (logical but not measured)
  Noise pollution → Tonic overload → cognitive impact VIA this specific mechanism
    (noise-cognition link established, specific mechanism = framework prediction)
...
═══════════════════════════════════════════════════════
  ...
  ④ CULTURAL VARIATION:
    Some properties (pre-birth, always-on) = universal.
...
→ Modality.md v1.0 ⭐
   §1.1 ② Auditory (brief → this file = deep dive hardware)
   §2 Hardware basis (sensor → cortex, many-to-many)
...
→ 04-Auditory-System-Arc.md ⭐
   (911L) Prenatal → 24 months auditory development
   Phoneme narrowing, word segmentation, DeCasper evidence
   = WHEN file (development). This = WHY file (architecture).


═══════════════════════════════════════════════════════
DRILL (domain-specific evidence)
═══════════════════════════════════════════════════════

→ Drill-Sound-Brain/ ⭐
   12 files, ~6,600L, 44 insights, 150+ citations
   = WHAT file (music × brain). This = WHY + HOW file (all sound).
   Key files:
     01 Evidence base (120+ citations)
     02 Background-Pattern × music (8 insights)
     03 Reward pipeline (7-step validation)
     04 Social resonance (5-step chain)
     05 Multi-modal compound (3-tier hierarchy)
     08 Voice 3-channel, melody efficiency
     10 Synthesis (49% 🟢, 43% 🟡, 7% 🔴, 0% wrong)


═══════════════════════════════════════════════════════
PARAMETER FILES (§2 cross-ref targets)
═══════════════════════════════════════════════════════

→ Prediction-Error-Is-Not-Reward.md v2.1 — PE + coherence (§2.2, §2.4)
→ Dopamine-Is-Not-Reward.md v1.3 — 7-step, musical anhedonia (§2.2)
→ Background-Pattern.md — formation, 2D model (§2.6)
→ By-Product-Gap-Resonance.md — resonance conditions (§2.5)
→ Entity-Compiled.md v1.0 — entity formation (§2.7)
...
→ PFC-Development.md v1.1
   §3b PFC Architecture Ceiling: conduction delay + metabolic = WHY PFC limited
   Drill-Sound-Brain referenced in §8 Cross-References
```

---

> *Auditory-Hardware v1.0 — "Sound = T1 hardware channel, NOT observation parameter.
> 6 unique properties: Always-on + Cross-barrier + Pre-birth + Temporal>Spatial
> + Activation Efficiency + Dual Tonic/Cyclic.
> Cross-parameter mapping: 8 parameters affected via 6 properties.
> 4🟢 + 2🟡 properties. Individual connections 🟢. Unified analysis 🟡.
> Evolutionary logic: nocturnal + social + bond + visual complement.
> Core insight: sound affects ALL parameters because HARDWARE architecture unique,
> not because sound 'more important' than other modalities."*
