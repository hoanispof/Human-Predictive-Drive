---
title: 00 — Chunk System Sketch — Light Overview + 3-Layer Architecture
file: 00-Chunk-System-Sketch.md
phase: F1 foundation (first of 3 foundation files, before 01 PFC reframe + 02 Womb-Birth baseline)
parent: plan.md (F1 v3)
session: N+4a
scope: |
  Light orientation file. No new deep analysis. Synthesizes 3-layer architecture + 4-folder mechanism split + drilled sub-folders contribution (N+1 H8, N+2 H9, N+3 Body-Feedback-Precondition). Reader orientation before entering 01 + 02 + body-input arcs.
...
> **This file is an ORIENTATION**, not a deep drill. Purpose: to bring the reader into F1 with a complete mental model of the 3-layer architecture (Body-Feedback L1 / Chunk System L2 / Feeling L3) + 4-folder mechanism split + N+1/N+2/N+3 drilled contributions + F1 v3 mission. Deep content begins at [01-PFC-Hardware-Reframe.md](01-PFC-Hardware-Reframe.md) and [02-Womb-to-Birth-Baseline.md](02-Womb-to-Birth-Baseline.md).
>
> **Style**: English primary + framework technical terms. 🟢 Research-backed / 🟡 Framework inference / 🔴 Hypothesis. "Imagine-Final" written in full, no abbreviation.
>
> **Drill meta-principle**: "Slow and steady, no steps skipped."

---

## §0 — Abstract

F1 Child-Chunk-Development drill aims to formalize the **substrate chunk formation mechanism** for 0-24 months, through 6 body-input arcs (visual / auditory / motor / interoceptive / social / verbal) and event-inference methodology (observe events → infer chunks state, without overclaiming PFC developmental timeline). This file is the first foundation file in the 12-file structure — introducing the 3-layer architecture (Body-Feedback ✅ drilled / Chunk System 🎯 current / Feeling ⏳ future), the 4-folder mechanism split of Chunk-Analysis (Child F1 / External F3 / Internal F4 / Master 99), the 3 drilled sub-folders that provided substrate evidence (Learning-Cycle H8, Agent H9, Body-Feedback-Precondition), and the F1 v3 mission with PFC hardware reframe + PFC-Inference Ladder framework contribution. No new analysis in this file — pure synthesis pointing the reader to deep content in 01-08.

---

## §1 — 3-Layer Architecture Map

### §1.1 — User's layered vision (committed)
...
LAYER 3: FEELING SYSTEM                                         ⏳ Future
         ...
         folder complete at Body-Base/Feeling/.

         NEEDS: developmental mechanism for Feel-Consciousification–Feel-Explanation
         emergence. Waiting for Chunk System to be drilled.
         ───────────────────────────────────────────
LAYER 2: CHUNK SYSTEM                                           🎯 Current
         ...
         ───────────────────────────────────────────
LAYER 1: BODY-FEEDBACK                                          ✅ Done N+3
         ───────────────────────────────────────────
         Raw body signals. Sensors + insula + ACC upstream
         integration. Feel-RawSignals–Feel-Integration of 7-layer model. Invariant
         across developmental stages (raw signal doesn't
         change — same ghrelin, nociceptor, cortisol).
         5 preconditions (Body-Feedback-Precondition) for signal → conscious feeling.
         ───────────────────────────────────────────
```
...
**F1 drill mission**: formalize Layer 2 emergence from Layer 1 substrate, via developmental arc 0-24mo. This completes the substrate bridge the Feeling folder needs.
...
ALREADY compiled: 1 wired unit → fires once = knows everything
...
- **Schema** = molecule (chunk network linked → has purpose/function)

**Chunk multi-modal** — spans multiple cortices: e.g. chunk "mother" = face (visual) + voice (auditory) + embrace (somatic) + warmth (emotional). Fire 1 part (hear voice) → pulls whole chunk to fire (recall face, recall embrace). 🟢 Distributed representations (Rumelhart & McClelland 1986).
...
| ② | **Emotional peak** | Once sufficient | Amygdala + NE → wire extremely fast | 🟢 Brown & Kulik 1977 flashbulb |
| ③ | **Multi-modal binding** | Strong per exposure | Visual + auditory + somatic + motor co-fire | 🟡 Cross-cortex binding |
| ④ | **Sleep consolidation** | Offline | Hippocampus replay → strengthen or prune | 🟢 Walker 2017 |

**F1 drill expected finding**: Infant chunks use all 4 mechanisms in parallel, with different distribution across developmental arc:
...
1. **Genesis** — first compile from 1 of 4 mechanisms
...
5. **Reconsolidation** — when chunk recalled, temporarily unstable → modify window → re-compile (Nader 2000)
6. **Decay** — not used → retrieval path weakens → "forgotten" (Ebbinghaus 1885, savings in relearning)
7. **⚠️ No deletion** — no unwiring mechanism. Chunks only strengthen/weaken/modify, never delete. "Breaking old habits" = compile a new chunk strong enough to override the old one.

**F1 focus**: Stages 1-3 (genesis + consolidation + connection) during 0-24mo developmental arc. Stages 4-7 mostly cross-cutting or post-F1 window.
...
> **Chunk = DATA. 2 operators working with the same database**:
> - **Unconscious = PRIMARY OPERATOR (~95%)** — compile, run schemas automatically, evaluate via body signals, process in background
...
**Unconscious CANNOT**: check domain accuracy, simulate future, compare with memory. Unconscious **only measures body-base** → body pleasant → unconscious "OK" → even if domain may be wrong (addiction, limerence, comfort zone).
...
**Implication for F1**: Infant has both PFC hardware + unconscious operational from prenatal (per reframe §1 below). What is missing = **compiled chunks (content)**. Chunks accumulate through experience → unconscious database grows → PFC has material to search/hold/direct.

---

## §3 — Chunk-Analysis 4-Folder Mechanism Architecture

### §3.1 — Split axis: mechanism type (NOT age or topic)

Chunk-Analysis master drill split into **4 folders** by mechanism type:
...
│   ├── Mechanism: external-shared chunks (language, grammar, abstract concepts)
│   ├── Test case: language acquisition + cognition upgrade
│   └── Hypotheses: H3 + H4 + H7 + H12 tentative
│
├── F4 Chunk-Internal-Processing/       ⏳ N+8-N+9
│   ├── 7 files
│   ├── Mechanism: internal novel chunk formation (logical, intuition, insight)
...
└── 99-Master-Synthesis.md              ⏳ N+10+
    ...
    └── Framework commit recommendations
```

### §3.2 — Why split by mechanism, not by age

**Alternative rejected**: Age-based split (0-2yr / 3-6yr / 7-12yr / adult). Problem: mechanisms overlap across ages. Verbal label attachment mechanism operates the same way in a 2-year-old saying "hungry" as in a 40-year-old learning Spanish vocabulary. Age-based split would duplicate mechanism analysis.
...
### §3.3 — Folders that will answer which hypotheses

| # | Hypothesis | Primary home | Support from |
|---|---|---|---|
| H1 | Chunk substrate cross-cutting | 99-Master-Synthesis | F1 + F3 + F4 (all provide evidence) |
| H2 | Thread 2 = 4th connection type | F4 | — |
| H3 | Grammar-Scaffolding partial-embedded | F3/Language | F1 pre-verbal bridge |
| H4 | Abstract vs metaphysical = modality encoding | F3 | — |
| H5 | Vague = multi-weak-signal convergence | F4 | — |
| H6 | Chunk decay Ebbinghaus | F4 | Learning-Cycle (H8) |
| H7 | Valence = chunk × schema interaction | F3 | — |
| **H8** | **Learning Dissonance Cycle** | **Learning-Cycle ✅** | **— (standalone N+1 drilled)** |
| **H9** | **Agent = Self-Pattern-Modeling unified** | **Agent ✅** | **— (standalone N+2 drilled)** |
| **H10** | **Body Signal Precondition (5 preconditions)** | **Body-Feedback-Draft ✅** | **— (standalone N+3 drilled)** |
...
**F1 directly addresses**: H1 (support via substrate evidence) + H11 (formalization in 08) + PFC-Inference Ladder (novel framework contribution).
...
The mechanism F1 formalizes (substrate chunk compile from body signals) operates the same way in adult learning — F1 just uses child window as the cleanest test bed.

---

## §4 — 3 Drilled Sub-Folders Contribution

### §4.1 — Learning-Cycle (H8, N+1, 2026-04-14)

**Scope**: Learning Dissonance Cycle — formalization of how a single chunk or chunk-chunk linkage is compiled, refined, and integrated over multiple sleep cycles.
...
**File**: [../Learning-Cycle/Learning-Cycle.md](../Learning-Cycle/Learning-Cycle.md) (1556 lines).

### §4.2 — Agent (H9, N+2, 2026-04-15)

**Scope**: Agent-reading mechanism unified. Resolves 3 tensions (binary vs spectrum / mirror confusion / Agent function vs category).
...
**Files**: [../Agent/Agent.md](../Agent/Agent.md), [../Agent/Self-Pattern-Modeling.md](../Agent/Self-Pattern-Modeling.md), [../Agent/By-Product-Gap-Resonance.md](../Agent/By-Product-Gap-Resonance.md) (5754 lines across 4 files).
...
- **Body-Feedback-Precondition — Body Signal Precondition Hypothesis**: Body signal fires when ALL 5 preconditions met:
  1. **Precondition-1 Directed-Gap** (Schema pending status) — schema waiting to be filled/resolved/upgraded
  2. **Precondition-2 Chunk-Substrate** ⭐ — sufficient chunks substrate to decode pattern
  3. **Precondition-3 Delta-Gate** (prediction-delta threshold) — variation large enough for VTA to detect (habituation-based)
  4. **Precondition-4 Match-Range** (Goldilocks zone) — dynamic: novel enough + right direction (neither too familiar nor too alien)
  5. **Precondition-5 Compile-Gate** ⭐ (Chunk association tag) — chunks fire has association pattern determined by the body state fingerprint during compile: novelty-path body state (novelty cortisol + dopamine/opioid) → chunks tagged opioid → "preferred to use"; threat-path body state (threat cortisol + NE/adrenaline) → chunks tagged cortisol-avoidance → "avoided". See [06a §7.0.1](06a-Interoceptive-Bladder-Keystone.md#§7.0.1) for the refined mechanism (same molecule, different body state, opposite outcomes).
...
- **Developmental gradient** (examples mapped): hunger / pain / discomfort / sadness all show Feel-RawSignals–Feel-Integration invariant but Feel-Consciousification–Feel-Explanation compiles progressively via chunks accumulation + caregiver mirroring + language emergence
...
- **Bladder gradient** (§5.5.1 of 01-Foundation): newborn Feel-RawSignals–Feel-Integration only → slightly older child post-hoc Feel-Consciousification → child who can call out Feel-Observation–Feel-Labeling → adult full Feel-Explanation. This IS the full developmental arc that F1 [06a-Interoceptive-Bladder-Keystone.md](06a-Interoceptive-Bladder-Keystone.md) will deep-drill as "freeze-face" exposition.
...
### §4.4 — Triangulation of 3 drilled hypotheses

The 3 drilled hypotheses triangulate on chunk substrate architecture:

```
   H8 (Learning-Cycle)          H9 (Agent)              Body-Feedback-Precondition
   ──────────────────          ────────────            ──────────────────────────
   HOW chunks consolidate       WHAT chunks DO          WHEN chunks fire
   (offline mechanism)          in social cognition     into conscious feeling
                               
   8-stage cycle                Self-Pattern-Modeling      5 preconditions
   6+1 sleep mechanisms         as solo mechanism        Precondition-2 Chunk-Substrate
   Baseline shift               Resonance        = keystone for F1
                                emergent mutual         
                               
                               ↓  ↓  ↓
                               
                      All 3 point to H1 (chunk substrate
                      cross-cutting) — F1 provides the
                      missing "HOW chunks FIRST compile"
                      foundation.
```

**F1 position**: F1 sits **upstream** of all 3 drilled hypotheses. F1 asks "**how do chunks compile in the first place, at substrate level, before they can cycle (H8), before they can model agents (H9), before they can precondition to feeling (Body-Feedback-Precondition)**". Developmental window 0-24mo is chosen because substrate compile is most visible there.

---

## §5 — F1 v3 Mission Preview
...
2. **Body-Input Axis** — 6 systems (visual / auditory / motor output / interoceptive / social / verbal) each with Connect → Read → Control arc. Measurable via observable events per developmental stage. Replaces v1 abstract mechanism categories. Key insight (plan §0.5 Q7): "just focus on the direction of body-input the child needs to connect and read-understand, control, and you can measure cognitive development quite accurately".

3. **Event-Inference Methodology** — F1 claims observable events + chunks state inference, NOT PFC developmental timeline. Every PFC-involvement claim carries 🟡 plausibility qualifier. Key insight (plan §0.5 Q10): "the question is not at what stage PFC appears — we only need to list events that likely involve PFC, because we cannot actually measure PFC". Epistemologically weaker but scientifically stronger.

4. **PFC-Inference Ladder** ⭐ **novel framework contribution** — 5-level event classifier formalizing the "freeze-face" hypothesis:
   - **L0** Reflex-only (no chunks required)
   - **L1** Post-hoc response (raw chunks compiling, response after event)
   - **L2** ⭐ **Pattern-match activation** ("freeze-face") — chunks fire on antecedent, no action intention
...
1. **Proto-chunk verdict**: gradient vs discrete → drill home [04-Auditory-System-Arc.md](04-Auditory-System-Arc.md) §6 (phoneme gradient)
2. **Precondition-2 Chunk-Substrate mechanism** ⭐ **KEYSTONE** → drill home [06a-Interoceptive-Bladder-Keystone.md](06a-Interoceptive-Bladder-Keystone.md) §6
3. **Multi-modal binding at birth** → drill home [07-Social-Recognition-Arc.md](07-Social-Recognition-Arc.md) §6 (social smile E12)
4. **PFC hardware × chunks missing reconciliation** → drill home [01-PFC-Hardware-Reframe.md](01-PFC-Hardware-Reframe.md) + every arc file
5. **Receptive-Productive gap (H11)** → drill home [08-Verbal-Production-Arc.md](08-Verbal-Production-Arc.md) §6 + [06a §8](06a-Interoceptive-Bladder-Keystone.md) anchor
...
Each convergence point verdict committed in [10-F1-Synthesis.md](10-F1-Synthesis.md).
...
**Session split** (per "slow and steady"):
...
## §6 — F1 Pending Work + Other Folders Preview

### §6.1 — What F1 will deliver

By end of F1 drill (N+4a + N+4b + N+4c), deliverables:

- ✅ Full PFC hardware reframe exposition with 5 evidence pillars + Hodel 2018 anchor
...
- ✅ Framework update recommendations R-F1-* for 99-Master-Synthesis

### §6.2 — What F1 will NOT cover

- **Adult cognition + expertise** — belongs to F4 Chunk-Internal-Processing
- **Language grammar deep drill** — belongs to F3/Language sub-folder
- **Abstract vs metaphysical concepts** — belongs to F3 (H4)
- **Direct PFC neural measurement claims** — methodologically out of scope (event-inference only)
- **Clinical application** (autism screening, developmental delay diagnosis) — referenced briefly, not primary
- **Cross-cultural variation deep drill** — general principles only

### §6.3 — F3 preview (N+5-N+7)
...
### §6.4 — F4 preview (N+8-N+9)

**F4 Chunk-Internal-Processing** handles internal novel chunk formation — logical connection, intuition, insight, chain + anchor decay. 7 files. Addresses H2 (4th connection type), H5 (multi-weak-signal convergence = "vague"), H6 (chunk decay Ebbinghaus). Also formalizes H12 tentative (language evolution driver via post-verbal bypass).

### §6.5 — 99-Master-Synthesis (N+10+)

Final integration of all 4 sub-folders + drilled triples (H8/H9/Body-Feedback-Precondition). Delivers:
- All 12 hypotheses final verdicts
- Architectural commitments (H1 substrate, feeling system dependencies)
- Framework commit recommendations (including R-F1-* from F1)
- Bridge to future Feeling L3 drill (deferred)
...
- Read plan.md §0 North Star + §0.5 insights + §0.6 Convergence Points + §0.7 PFC Ladder first
...
**If you are reviewing F1 methodology**:
- plan.md §0.3 event-inference methodology
- plan.md §0.7 PFC-Inference Ladder
- plan.md §4 design decisions
- This file §5 F1 v3 mission preview

**If you are integrating F1 into 99-Master-Synthesis**:
- 10-F1-Synthesis.md (verdicts + R-F1-* recommendations)
...
### §7.2 — Dependency ordering within F1

```
00-Chunk-System-Sketch (orientation)
       ↓
01-PFC-Hardware-Reframe (foundation for every arc's PFC inference)
       ↓
02-Womb-to-Birth-Baseline (prenatal state before arcs begin)
       ↓
03 Visual  ──┐
04 Auditory ─┤  parallel, order flexible
05 Motor    ─┤
06a Bladder ─┤  (keystone, do carefully)
06b Other  ──┤
07 Social  ──┤
08 Verbal ───┘
       ↓
09-Event-Matrix (cross-cutting synthesis, aggregates 03-08)
       ↓
10-F1-Synthesis (verdicts + recommendations)
```
