...
status: v1.2 REFINED
scope: |
  Analysis session: body-base boundary is DEFINED by compilation,
  ...
  Viscera REVEAL: proximity ≠ access. Rubber hand: 120s = sufficient for body-schema update.
  Even the "real hand" only goes through 5-7 relays — NO PFC neuron reaches all the way to the hand.
  CHAIN MECHANISM: all control is indirect — adding a tool/prosthetic = adding a link.
  Fingernail = NATURAL BRIDGE (body part, 0 neural connection, compiled from birth).
  LOSS MECHANISM: Chunk-Miss × unfillability — unified from chopstick to child.
  "Entity's state = my state" = compressed prediction, NOT state-sharing.
  "Away from mother" = proof: mother is fine, I am in pain → baseline violation = mechanism.
...
language: English
source_version: Vietnamese v1.2
...
>
> **Conclusion:** SAME UNDERLYING PRINCIPLE, DIFFERENT LEVELS.
> Principle: **Compilation, not connection, defines body-base boundary.**
> Neural connection = only 1 input pathway for compilation.
...
> Viscera = proof: physically closer than the hand, but PFC access LESS than the hand.
...
- §6 — POS 3: Basic Prosthetic (Chronic Compilation)
...
```
⭐ UNDERLYING PRINCIPLE (Body-Base.md §2):

  Body responds to COMPILED PATTERN FIRE, not to INPUT CAUSE.

  6 SUPPORTING CASES (Body-Base.md §2):
    ① Phantom limb pain: NO limb → pattern STILL fires → real pain
    ② Placebo nausea: sugar → installed pattern fires → real vomiting
    ③ Moral disgust: NO toxin → same insula fire → real "nausea"
    ④ Anxiety nausea: NO toxin → fear pattern → real vomiting
    ⑤ Vicarious nausea: empathy pattern → real nausea when WATCHING others vomit
    ⑥ Morning sickness: PREDICTIVE pattern fires BEFORE actual toxin

  IMPLICATIONS FOR BODY-BASE BOUNDARY:
  ...
  THIS IS THE CORE INSIGHT OF THIS DRILL:
    ...
    → "My hand" = compiled representation, NOT "neuron reaching all the way to the hand"
    → "My mother" = compiled representation, NOT physical connection
    → "My gut" = physically INSIDE but limited PFC access
    → "My fingernail" = real body part, 0 neurons in nail plate, compiled via nail bed
    → "Chopstick in hand" = 0 neural, temporarily compiled while in use, retracted when set down
    → = SAME principle, DIFFERENT parameters
...
```
⭐⭐ 9 POSITIONS — ORDERED BY COMPILATION TYPE + CONNECTION:
  ...
  │     │                          │ (object)  │ (TEMPORARY)      │ Bayesian inference.     │
  ├─────┼──────────────────────────┼───────────┼──────────────────┼────────────────────────┤
  │  2  │ Tool-in-Hand             │ 0         │ Visual + motor   │ REVERSIBLE: use→extend, │
  │     │                          │ (object)  │ (DYNAMIC)        │ stop→retract.           │
  ├─────┼──────────────────────────┼───────────┼──────────────────┼────────────────────────┤
  │  3  │ Basic Prosthetic         │ 0         │ Visual + motor   │ Chronic compilation     │
  │     │                          │ (object)  │ (YEARS)          │ > temporary.            │
  ├─────┼──────────────────────────┼───────────┼──────────────────┼────────────────────────┤
  │  4  │ Prosthetic + Phantom     │ 0 +       │ Visual + motor   │ 2 representations       │
  │     │                          │ residual  │ + phantom-somato.│ MERGE.                  │
  ├─────┼──────────────────────────┼───────────┼──────────────────┼────────────────────────┤
  │  5  │ Neural Interface Prosth. │ Artificial│ Visual + motor   │ Artificial channels     │
  │     │                          │ partial   │ + artificial sens.│ restore full access.   │
  ├─────┼──────────────────────────┼───────────┼──────────────────┼────────────────────────┤
  │  6  │ Real Hand + Fake Threat  │ Indirect  │ ALL (maximum)    │ 5-7 relays, NOT direct. │
  │     │                          │ (5-7 rel.)│                  │ Prediction > reality.   │
  ├─────┼──────────────────────────┼───────────┼──────────────────┼────────────────────────┤
  │  7  │ Viscera / Internal Organs│ Indirect  │ Interoceptive    │ ⭐ CLOSER physically    │
  │     │                          │ (2+ rel.) │ (AGGREGATE only) │ but LESS PFC access.    │
  ├─────┼──────────────────────────┼───────────┼──────────────────┼────────────────────────┤
  │ 8-9 │ Near-Cortical + PFC Dir. │ Direct /  │ Cortical / WM    │ Max access but STILL    │
  │     │                          │ SELF      │                  │ loses to compiled.      │
  └─────┴──────────────────────────┴───────────┴──────────────────┴────────────────────────┘

  ⭐ Position 7 (Viscera) = BREAKS the naive model.
    Physically closer than the hand, but PFC access LESS than the hand.
    → Spectrum is NOT 1D (distance → access).
    → Spectrum = 2D (compilation channels × connectivity architecture).

  ⭐ CHAIN MECHANISM (§9): ALL control goes via chain, NEVER direct.
    → PFC → [5-7 relays] → hand. Adding a chopstick/prosthetic = adding 1 link.
    → Fingernail = REAL body part, 0 neurons → body HAS ALWAYS included non-neural parts.
    → = NO qualitative jump between "body part" and "tool extension."

  ⭐⭐ LOSS MECHANISM (§12): WHY DIFFERENT POSITIONS = DIFFERENT PAIN LEVELS?
    Loss pain = Chunk-Miss on compiled baseline × predicted unfillability
    → Chopstick: shallow baseline, instant replacement → pain ≈ 0
    → Child lost: deepest baseline, permanently unfillable → max grief
    → SAME mechanism, different parameters.
    → "Entity's state = my state" = compressed prediction, NOT state-sharing.
    → "Away from mother" = proof: mother is fine, I am in pain → baseline violated.
```
...
⭐ BODY-BASE EXTENSION VIA AGENT COMPILATION (0 neural connection):
  ...
  NEURAL CONNECTION: 0 — two nervous systems entirely separate.
  COMPILATION INPUT: Self-Pattern-Modeling interactions, multi-gap fill episodes, valence compilation
  CHANNELS COMPILED: Agent-specific (Self-Pattern-Modeling, valence, multi-gap) — no somatosensory

  MECHANISM (Entity-Valence-Dynamics §2):
    Per-agent valence compile DEEP ENOUGH:
    → Body shifts from "agent X affects me" → "agent X's state = MY state"
    → Agent BECOMES body-base extension
  ...
  EVIDENCE THAT PAIN IS SHARED (no neural connection):
    🟢 Singer et al. 2004 (Science):
       ...
       = SAME pain matrix components fire in OBSERVER
       = Body responds to PREDICTED entity-pain, not actual own-pain
    🟢 Eisenberger 2003:
       Social pain (rejection) activates SAME neural pathways (dACC)
       = Social pain = physical pain at substrate level
    🟢 Mitchell 2006, Tamir & Mitchell 2010:
       Close others processed in SAME vmPFC circuits as SELF
       = Brain represents close entity SIMILAR TO representing self
  ...
  ENTITY-OWNED UNIQUE PROPERTIES (Drill-Entity-Owned §2):
    ...
    → REQUIRES agent (Self-Pattern-Modeling predicts internal STATE, not just function)
    ...
    → "Losing a house = financial stress. Losing a child = losing a body part." (Entity-Valence-Dynamics §2)

  THREAT TO ENTITY = THREAT TO SELF:
    → Anyone threatening one's child → body response: cortisol ↑, sympathetic ↑, protective drive
    → = Compiled pattern: "entity's threat = MY threat"
    → Body fires defensive response for entity OUTSIDE physical body-base
    → HOWEVER: no pain in MY OWN HAND when the child breaks theirs
    → = EMOTIONAL channel, does not map onto somatosensory topography

  UNDERLYING MECHANISM (→ §12 Loss Mechanism):
    ...
    Self-Pattern-Modeling predicts entity state → predicts gap-fillability change → body responds
    Key factor: total gap body-need being fed, NOT state-sharing
    "Away from mother" = proof: mother is fine, I AM in pain → baseline violated, NOT state transfer

  🟢 Singer 2004, Eisenberger 2003, Mitchell 2006
  🟡 Entity-Owned 5-factor, body-base extension = framework synthesis
  🟡 Gap-fillability as mechanism under "entity's state = my state" = drill reframe
```
...
⭐⭐ BODY-SCHEMA EXTENSION IN ~120 SECONDS (0 neural connection):
  ...
  COMPILATION INPUT: Visual (seeing rubber hand) + Tactile (stroking real hand + rubber hand in sync)
  ...
  CLASSIC EXPERIMENT:
    🟢 Botvinick & Cohen 1998 (Nature):
      Setup: real hand hidden behind a screen. Rubber hand placed in view.
      Manipulation: stroke real hand + rubber hand SIMULTANEOUSLY (~2 minutes)
      Result ①: Proprioceptive drift — sensation of real hand "shifts" toward rubber hand
        = Brain HAS UPDATED real hand position in body-schema
      Result ②: Ownership illusion — "rubber hand = my hand"

  REVEALING THE MECHANISM: BAYESIAN INFERENCE
  ...
  THREAT RESPONSE (key evidence):
    🟢 Armel & Ramachandran 2003:
      After illusion: threatening rubber hand with a needle
      → Skin Conductance Response (SCR) SPIKES STRONGLY
      → = Body mounts defensive response for something that is NOT its hand
      → = Brain HAS UPDATED body-schema → rubber hand = "my hand" → threat = real

    🟢 Ehrsson et al. 2004 (Science):
      fMRI during rubber hand illusion:
      → Premotor cortex + intraparietal sulcus ACTIVATE
      → = SAME areas processing body ownership for the real hand
      → = Brain processes rubber hand VIA THE SAME CIRCUITS as real hand

  WHY THIS POSITION MATTERS:
    → Basic prosthetic: requires MONTHS/YEARS of use → deep compilation
    → Rubber hand: requires ~120 SECONDS → SHALLOW but SUFFICIENT for defensive response
    → = Shows MINIMUM CONDITIONS for body-schema extension:
      ① Visual input (seeing it)
      ② Temporal correlation (synchronous visual + tactile)
  ...
  FRAMEWORK FIT:
    ...
    → Reality: a rubber hand, not my hand
...
⭐ BODY-SCHEMA EXTENSION — DYNAMIC AND REVERSIBLE:
  ...
  COMPILATION INPUT: Visual + motor feedback (holding tool, using tool)
  DURATION: within minutes of use
  KEY DIFFERENCE: put tool down → extension RETRACTS

  🟢 Iriki, Tanaka & Iwamura 1996 (NeuroReport):
    Macaques using a rake to retrieve food
    → Bimodal neurons (parietal intraparietal sulcus):
      BEFORE use: receptive field = hand area only
      DURING use: receptive field EXTENDS → encompasses the entire rake
      AFTER putting down: receptive field RETRACTS → returns to original size
    → = Body-schema extension DYNAMIC — tied to ACTIVE USE

  🟢 Cardinali et al. 2009 (Current Biology):
    After tool use:
    → Arm kinematics CHANGE (reaches longer, grasp adjusted)
    → = Body-schema update is NOT JUST "representation" — changes MOTOR PLANNING
    → Body GENUINELY treats tool-extended arm AS LONGER ARM

  🟢 Farnè & Làdavas 2000:
    Tool-use → peripersonal space EXTENDS to tool tip
    → Objects near tool tip trigger SAME defensive neurons as objects near hand
    → = The "safety zone" extends along with the tool
  ...
  🟢 Maravita & Iriki 2004 (review, Trends in Cognitive Sciences):
    ...
    → = Brain does NOT distinguish "biological vs tool" — only checks sensory correlation
  ...
  SPECTRUM POSITION — WHY SEPARATE FROM RUBBER HAND:
    ...
    → HOWEVER: still REVERSIBLE (put down → retract)
  ...
  CHAIN INSIGHT (→ §9 Chain Mechanism):
    PFC → [5-7 relays] → hand → [grasps] → chopstick
    → PFC does NOT directly control the hand (5-7 relays)
    → Adding a chopstick = adding 1 link to an ALREADY indirect chain
    → = NO qualitative jump between "using the hand" and "using chopsticks"
    → Likewise: prying with a fingernail (0 neurons in the nail) = same pattern
...
⭐ BODY-SCHEMA EXTENSION — CHRONIC, PERSISTENT:
  ...
  COMPILATION INPUT: Visual (seeing it daily) + Motor (controlling even mechanically)
  DURATION: months-years of daily use
  KEY DIFFERENCE from tool: CHRONIC use → DEEPER compilation → more persistent

  CHANNELS COMPILED:
    ✓ Visual (seeing prosthetic daily — STRONG)
    ✓ Motor-feedback (movement intention → prosthetic moves — MODERATE)
    ✓ Proprioceptive-proxy (prosthetic position → visual estimation — PARTIAL)
    ✗ Somatosensory (cannot feel touch — MISSING)
    ✗ Thermal (cannot feel temperature — MISSING)
    ✗ Nociceptive (cannot feel pain directly — MISSING)
  ...
  🟢 De Preester & Tsakiris 2009:
    ...
    → Visual + motor feedback = minimum requirement
    → More sensory channels → deeper embodiment
    → = Compilation depth = f(number of channels × use duration)

  SUDDEN PROSTHETIC BREAK — PREDICTION:
    → Visual: "MY ARM broke" → body-image violation → startle + distress ✓
    → Functional loss: "cannot grip anymore" → gap-miss → frustration ✓
    → Identity disruption: "a part of my body is broken" → distress ✓
    → Somatosensory pain: ✗ UNLIKELY (channel not compiled)

    TOTAL: DISTRESS (real, body-level) but NOT somatic pain.
    LEVEL: > losing an ordinary object (because body-schema compiled)
           < losing real limb (fewer channels)

    ⭐ COMPARISON: losing prosthetic vs ordinary object vs real limb:

    ┌──────────────────────┬─────────────────────┬────────────────────────────────┐
    │ Losing an object     │ Losing prosthetic   │ Losing real limb               │
    │ (cup, table)         │ (10 years of use)   │                                │
    ├──────────────────────┼─────────────────────┼────────────────────────────────┤
    │ Visual only           │ Visual + motor      │ ALL channels                  │
    │ NOT body-schema       │ IN body-schema      │ DEEP body-schema              │
    │ "Regret" (schema)     │ "Lost a body part"  │ "Body-base amputation"        │
    │ Replace: easy         │ Replace: hard       │ Replace: impossible           │
    └──────────────────────┴─────────────────────┴────────────────────────────────┘

  CHAIN INSIGHT (→ §9 Chain Mechanism):
    PFC → [5-7 relays] → stump → [socket] → prosthetic
    → Same pattern as chopstick: PFC → relays → body part → extension
    → Differs from chopstick ONLY IN: duration (chronic vs temporary), attachment (fixed vs held)
    → Real hand already 5-7 relays — prosthetic just adds 1 link
...
⭐⭐ MOST INTERESTING CASE: 2 REPRESENTATIONS MERGE:
  ...
  MECHANISM — 2 REPRESENTATIONS:
    ① PHANTOM: body-schema map compiled BEFORE amputation → PERSISTS after amputation
       → Penfield homunculus: S1 area for the hand STILL EXISTS even after the hand is lost
    ...
    ② PROSTHETIC: visual + motor compile through years of use
  ...
  EVIDENCE — MIRROR BOX:
    🟢 Ramachandran & Rogers-Ramachandran 1996:
      Mirror reflects intact hand → brain "sees" phantom hand moving
      → Phantom pain DECREASES
      → WHY: visual input CORRECTS phantom mismatch
      → Phantom pattern: "hand stuck, not moving" → pain
      → Mirror visual: "hand moving normally" → pain resolves
      → = Visual CAN OVERRIDE somatosensory compiled map

    🟢 Flor et al. 2006:
      Phantom pain correlates with cortical reorganization in S1
      → Adjacent cortical areas INVADE abandoned hand territory
      → More reorganization → more phantom pain
      → Prosthetic use → REDUCES reorganization → less pain

    IMPLICATION FOR PROSTHETIC USE:
  ...
  SUDDEN PROSTHETIC BREAK (WITH PHANTOM):
    → All responses of Pos 3 (distress, functional loss, identity) +
    → ADDITIONALLY: phantom map loses visual anchor → phantom pain may FLARE
    → Visual "my arm broke" fires THROUGH phantom circuit → MAY produce referred somatic sensation
    → = Body-schema violation MORE INTENSE (because phantom + visual merge runs deep)

  🟢 Ramachandran 1996, Flor et al. 2006
  🟡 Prosthetic-phantom merge + visual break → phantom flare = hypothesis
     (logic consistent, specific experiment not found in published literature)
```
...
⭐ ARTIFICIAL NEURAL CONNECTION — RESTORING MISSING CHANNELS:
  ...
  NEURAL CONNECTION: ARTIFICIAL — electrodes attached to peripheral nerves or cortex
  COMPILATION INPUT: Visual + motor + ARTIFICIAL somatosensory feedback
  KEY DIFFERENCE: ADDS channels that basic prosthetic lacks

  🟢 Marasco et al. 2011 (Science Translational Medicine):
    Targeted Sensory Reinnervation (TSR):
    → Peripheral nerves from amputated limb → redirected to chest skin
  ...
  🟢 Raspopovic et al. 2014 (Science Translational Medicine):
    ...
    → Electrodes ON peripheral nerves
    → Prosthetic sensors → electrical signal → nerve → brain
    → Brain → electrical signal → prosthetic motors
    → Result: users discriminate object shape + stiffness
    → = SENSORY channel partially RESTORED

  🟢 Tan et al. 2014:
    Peripheral nerve interfaces → users feel pressure gradients
  ...
  POSITION IN SPECTRUM:
    ...
    → HOWEVER: compilation engine does NOT CARE "biological or artificial"
    → Compilation engine only cares: "correlated sensory input? → yes → compile"

  SUDDEN NEURAL INTERFACE PROSTHETIC BREAK:
  ...
  🟢 Marasco 2011, Raspopovic 2014, Tan 2014
  🟡 Compilation engine "doesn't care biological vs artificial" = framework synthesis
```
...
⭐⭐⭐ KEY INSIGHT: EVEN THE REAL HAND GOES THROUGH 5-7 RELAYS:
  ...
  CONFIRMED: NO PFC NEURON EXTENDS ALL THE WAY TO THE HAND.
  ...
  MOTOR pathway (PFC → hand):
    ...
    (Neural-Architecture §3: "PFC holds intention → premotor plans → M1 executes → spinal")
  ...
  INSIGHT:
    → From PFC's perspective: real hand = COMPILED REPRESENTATION
    → PFC does NOT "touch" the hand — PFC accesses the hand via COMPILED BODY-SCHEMA
    → How does it differ from prosthetic? IN CHANNEL COUNT + DEPTH. Not in "type."
    → Real hand: 7+ channels compiled from birth (somatosensory, motor, proprioceptive,
      thermal, nociceptive, visual, interoceptive)
    → = QUANTITATIVE difference, NOT qualitative


  ⭐⭐ CHAIN MECHANISM — PFC ALWAYS CONTROLS VIA CHAIN:

    3 examples, SAME PATTERN:
    ┌─────────────┬──────────────────────────────────────────────────────────┐
    │ Prosthetic  │ PFC → [5-7 relays] → stump → [socket] → prosthetic     │
    │ Chopstick   │ PFC → [5-7 relays] → hand  → [grasps] → chopstick      │
    │ Fingernail  │ PFC → [5-7 relays] → finger → [attached] → nail        │
    └─────────────┴──────────────────────────────────────────────────────────┘

    OBSERVATION:
    → PFC NEVER directly controls anything outside PFC.
    ...
    → = QUANTITATIVE extension (adding a link), NOT qualitative change (different type).

    PARAMETER COMPARISON:
    ┌──────────────────┬────────────────┬──────────────┬──────────────────┐
    │                  │ Fingernail     │ Chopstick    │ Prosthetic       │
    ├──────────────────┼────────────────┼──────────────┼──────────────────┤
    │ Neural within    │ 0 (keratin,    │ 0 (wood/     │ 0 (synthetic     │
    │ extension        │ dead cells)    │ metal)       │ material)        │
    ├──────────────────┼────────────────┼──────────────┼──────────────────┤
    │ Attachment       │ Always         │ While held   │ Daily wear       │
    │ (duration)       │ (from birth)   │ (minutes)    │ (years)          │
    ├──────────────────┼────────────────┼──────────────┼──────────────────┤
    │ Feedback         │ Pressure via   │ Pressure +   │ Pressure via     │
    │ mechanism        │ nail → nail    │ vibration    │ socket → stump   │
    │                  │ bed (nerves)   │ through hand │ (nerves)         │
    │                  │                │ (nerves)     │                  │
    ├──────────────────┼────────────────┼──────────────┼──────────────────┤
  ...
  ⭐ FINGERNAIL = NATURAL BRIDGE BETWEEN "BODY" AND "TOOL":
    ...
    Fingernail (nail plate):
    → Keratin = dead cells. NO neurons in the nail plate.
    → Prying with a fingernail → clearly feels pressure.
    → WHY: force transmits mechanically through nail plate → nail bed
      (HAS Meissner's corpuscles + free nerve endings) → peripheral nerve → brain.
    → = SAME MECHANISM as prosthetic: force through prosthetic → socket → stump nerves → brain.
    → = SAME MECHANISM as chopstick: force through chopstick → hand (mechanoreceptors) → brain.

    Hair = parallel case:
    → Hair strand = keratin (dead cells), NO neurons in the hair strand.
    → HOWEVER: wind blows hair → hair follicle mechanoreceptors FIRE → "feel" the wind.
    → Hair follicle = among most sensitive mechanoreceptors in body.
    → Dead extension + living base with nerve endings → "MINE."

    Outer skin layer (stratum corneum) = another parallel:
    → Stratum corneum = dead keratinized cells (outermost skin layer).
    → "Touching skin" = pressing on dead cells → force transmits → live receptors in dermis.
    → = Even "touching skin" is NOT touching live cells directly.

    IMPLICATION:
    → "My body" HAS ALWAYS INCLUDED parts WITHOUT neural connection.
    → Fingernails, hair, stratum corneum = dead keratin = 0 neurons.
    → Brain treats them AS "body parts" because: always attached + compiled from birth.
    → = Body-base membership was NEVER about "having neurons or not."
    → Prosthetic, tool = continue the SAME pattern body ALREADY has.
  ...
  FAKE THREAT — "PERSON WITH FAKE SWORD SWINGS AT HAND, WITHOUT CONTACT":

  ANALOGOUS EXPERIMENTS EXIST — MANY OF THEM:
  ...
  PARADIGM 1 — Peripersonal Space Defense:
    🟢 Graziano, Reiss & Gross 1999 (Journal of Neurophysiology):
       ...
       APPROACHES hand — BEFORE CONTACT.
       Firing rate INCREASES as object gets CLOSER.
       = Brain responds to PROXIMITY, not contact.
       = Compiled prediction: "object in danger zone" → fire.

    🟢 Graziano & Cooke 2006 (Annual Review of Neuroscience):
       Stimulate polysensory zone → AUTOMATIC DEFENSIVE MOVEMENTS
  ...
  PARADIGM 2 — Anticipatory Pain:
    🟢 Ploghaus et al. 1999 (Science):
       Visual cue signaling "pain coming" → fMRI:
       ACC + anterior insula + cerebellum ACTIVATE
       = SAME pain matrix fires EVEN BEFORE pain stimulus
       = Brain fires pain PREDICTION, not pain reality.

    🟢 Koyama et al. 2005 (PNAS):
       Told "will hurt a lot" vs "will hurt a little" → SAME stimulus
  ...
  PARADIGM 3 — Threat to Body Part WITHOUT Contact:
    🟢 Sambo & Iannetti 2013 (PNAS):
       ...
       = Body-schema spatial representation of hand
         → modulates defensive reflex in EYES (distant body part)
    ...
    🟢 Slater et al. 2008, 2010:
       ...
       (HR↑, SCR↑) — even when person KNOWS it's virtual.

  EXACT "FAKE SWORD" SCENARIO:
    Fake sword → fake swing at real hand → NO SKIN CONTACT:
    ...
    → = "Body evaluates PATTERNS, not reality" — IN DIRECT ACTION
    → Reality: no contact. Pattern: "about to be hit." Body follows: PATTERN.
...
⭐⭐⭐ MOST IMPORTANT POSITION — BREAKS THE NAIVE MODEL:
  ...
  PHYSICAL LOCATION: INSIDE the body — CLOSER than the hand
  ...
  ENS (Enteric Nervous System — "Second Brain"):
    🟢 Gershon 1998 ("The Second Brain"):
      ...
      CAN operate INDEPENDENTLY: cut vagus → gut STILL digests
      = Local processing DOES NOT REQUIRE brain permission
    ...
    🟢 Neural-Architecture §5 (framework established):
      ...
      = Gut SENDS information TO brain more than brain SENDS TO gut
  ...
  INTEROCEPTION — PFC'S FILTERED WINDOW:
    🟢 Craig 2002, 2009 (Nature Reviews Neuroscience):
      ...
      = PFC accesses interoception via INSULAR CORTEX filter
    ...
    🟢 Critchley et al. 2004:
      ...
      Most people: DO NOT know heart rate precisely
      = PFC access to OWN BODY varies per-person
      = Physical proximity GUARANTEED — PFC access NOT guaranteed


  BREATHING — UNIQUE EXCEPTION:
    🟢 Neural-Architecture §5:
      Breathing = UNIQUE: voluntary + involuntary bridge
      PFC CAN directly control breathing rate (voluntary)
      HOWEVER: stop paying attention → brainstem resumes automatic control
      = Uniquely 1 function with a VOLUNTARY BRIDGE between PFC and ANS
      = Evidence: everything ELSE in ANS → PFC can barely reach


  WHY THIS POSITION IS CRITICAL FOR THE SPECTRUM:

    NAIVE INTUITION:
      "Closer to PFC = more PFC access"
      "Hand = far → less access. Gut = close → more access."
    ...
    → SAME PRINCIPLE:
      ...
      Gut (internal, physically inside) MAY NOT be compiled sufficiently for PFC access
...
⭐ 2 POSITIONS CLOSEST TO PFC:
  ...
  POS 8 — NEAR-CORTICAL (Zone B: Cortical Modality Areas):
    ...
    PFC CANNOT:
      → Turn off hearing (hearing hated music → PFC shifts attention, does NOT turn off auditory)
    ...
    🟢 Fedorenko et al. 2024: language network = dissociable from PFC reasoning
       → Language areas = Zone B AUTONOMOUS from PFC (drunk person still speaks)


  POS 9 — PFC DIRECT (Zone A: 0 relay):
    NEURAL CONNECTION: SELF — PFC IS the substrate
    E.g.: Working memory, inner speech, deliberate simulation
    ...
    PFC STILL CANNOT:
      → Stop intrusive thoughts: compiled pattern fires THROUGH INTO PFC workspace
      → Break rumination loop: compiled loop PFC CANNOT suppress
      → Override body signal too strong: "knows it's wrong but drive is too strong"
        (PFC ACCESS spectrum Pos "Hijack" — Bilateral Drill §5)
      → = Compiled patterns CAN INVADE even PFC's own territory

    IMPLICATION:
      Even at the CLOSEST Level (0 relay, PFC = substrate):
      → Compiled patterns CAN STILL override PFC
...
```
⭐⭐⭐ SPECTRUM IS NOT 1D BUT 2D:

  NAIVE ASSUMPTION (1D):
    "Further from PFC physically → less access"
  ...
  REALITY (2D):
    ...
    AXIS 1 — PHYSICAL CONNECTION:
      Has neural wiring or not? How many relays?
      0 (entity/prosthetic) → indirect (hand) → direct (cortical) → self (PFC)

    AXIS 2 — COMPILATION RICHNESS:
      How many channels compiled? How deep?
      Few (rubber hand: 2 channels, 120s) → many (real hand: 7+ channels, lifetime)

    PFC ACCESS = f(Axis 1 × Axis 2):
      Access does NOT depend solely on connection
      Access does NOT depend solely on compilation
      Access = BOTH
  ...
  CHAIN MECHANISM IS PROOF #2 (§9):
    → All PFC control = indirect chain (5-7+ relays to the hand)
    → Fingernail = REAL body part with 0 neural connection in the nail
    → Hair, stratum corneum = same pattern (dead keratin, 0 neurons)
    → = Body-base HAS ALWAYS included non-neural parts
    → Adding tool/prosthetic = adding a link to an ALREADY indirect chain
    → NO qualitative boundary between "body" and "non-body"
    → = 2D model confirmed: membership = f(compilation), NOT f(neural wiring)

  3 CONVERGING PIECES OF EVIDENCE:
    ① Viscera: closer → LESS access → proximity ≠ access
    ② Chain: body ALREADY includes non-neural → neural ≠ membership
    ③ Rubber hand: 120s = sufficient for ownership update → compilation = fast + automatic
  ...
  NOTES:
    Entity-Owned (⓪) = low physical connection + agent-specific channels
      → PFC access: MODERATE (PFC CAN articulate "mother is here" — Drill-Entity-Owned §5)
    ...
    Real hand (⑥) = indirect (5-7 relays) but MAX compiled channels
      → HIGH PFC access because of COMPILATION RICHNESS, not because of "directness"
  ...
  UNIFIED PRINCIPLE:
    ...
    HOWEVER: compilation CAN happen WITHOUT neural connection (entity, prosthetic)
    AND: neural connection DOES NOT GUARANTEE compilation richness (viscera)


  ⭐⭐⭐ LOSS MECHANISM — WHY DIFFERENT POSITIONS = DIFFERENT PAIN LEVELS?

    UNIFIED: Loss pain = Chunk-Miss on compiled baseline × predicted unfillability.

    "Belonging to the body" = compiled into baseline (Body-Base §2: body evaluates patterns)
    "Sudden loss" = baseline violated → Chunk-Miss (Body-Feedback-Mechanism §3)
    "Irreplaceable" = predicted unfillability (Gap-Body-Need §11: entity-gap matching)

    = SAME mechanism for ALL positions in spectrum. Different parameters → different pain levels.

    Pain = Σ (baseline_depth × gap_breadth × Δfillability × predicted_duration)


  PROOF 1 — "YOU ONLY KNOW HOW MUCH YOU LOVE YOUR MOTHER WHEN YOU'RE AWAY FROM HER"
  (Entity-Valence-Dynamics §8):

    Living with mother for 20 years:
    ...
    Away from mother at university:
      SUDDENLY gaps unfilled → Chunk-Miss ACROSS MULTIPLE compiled channels
      → Body: "baseline VIOLATED" → pain → "missing mother"
      → PFC (observing FOR THE FIRST TIME): "so mother was important all along"

    ⭐ KEY: Mother's state UNCHANGED (mother is still at home, healthy).
      What changed = MY gap-fillability baseline was violated.
      → If "entity's state = my state" were the mechanism:
        mother is fine → I should be fine → BUT I AM NOT FINE → ???
  ...
  PROOF 2 — BOSS LOSES A TALENTED COLLEAGUE (NOT entity-owned):

    Boss: Level 1-2 relationship with colleague (professional, NOT "entity's state = my state")
    HOWEVER: colleague is currently filling important gaps:
    ...
    → EVEN WITHOUT an entity-owned relationship
    → = PROOF: mechanism = gap-fillability change, entity-owned NOT REQUIRED.
  ...
  APPLIED ACROSS THE ENTIRE SPECTRUM:
    ...
    ├──────────────────────┼──────────────────┼──────────────────┼───────────────┤
    │ Chopstick breaks     │ Shallow (minutes)│ Tiny             │ Instant       │
    │                      │                  │                  │ → pain ≈ 0    │
    ├──────────────────────┼──────────────────┼──────────────────┼───────────────┤
    │ PC lost (can afford) │ Moderate (daily) │ Workflow         │ Days          │
    │                      │                  │ disrupted        │ → annoyance   │
    ├──────────────────────┼──────────────────┼──────────────────┼───────────────┤
    │ PC lost (no money+   │ Moderate (daily) │ Workflow +       │ INDEFINITE    │
    │ data)                │                  │ data LOST        │ → real distress│
    ├──────────────────────┼──────────────────┼──────────────────┼───────────────┤
    │ Prosthetic arm breaks│ Deep (years)     │ Body-schema      │ Weeks-months  │
    │                      │ body-schema      │ VIOLATION        │ → distress    │
    ├──────────────────────┼──────────────────┼──────────────────┼───────────────┤
    │ Boss loses colleague │ Project baseline │ Expertise gap    │ Hard          │
    │ (irreplaceable)      │ (months)         │ UNFILLABLE       │ → stress      │
    ├──────────────────────┼──────────────────┼──────────────────┼───────────────┤
    │ Away from mother     │ 20yr baseline    │ MULTIPLE channels│ Partial       │
    │ (university)         │ INVISIBLE        │ simultaneously   │ → "missing    │
    │                      │                  │                  │   mother"     │
    ├──────────────────────┼──────────────────┼──────────────────┼───────────────┤
    │ Close friend         │ Very deep        │ MANY gaps        │ PERMANENT     │
    │ passes away          │ (years, Self-    │ unique filler    │ → grief       │
    │                      │ Pattern-Modeling)│                  │               │
    ├──────────────────────┼──────────────────┼──────────────────┼───────────────┤
    │ Child passes away    │ DEEPEST          │ Existence-based  │ PERMANENT     │
    │                      │ (lifetime+HW)    │ PHANTOM GAP      │ UNFILLABLE    │
    │                      │                  │ (Gap-Body-Need)  │ → max grief   │
    └──────────────────────┴──────────────────┴──────────────────┴───────────────┘

  ⭐ SAME OBJECT, DIFFERENT CONTEXT → DIFFERENT PAIN LEVEL:
    PC lost (can afford) vs PC lost (no money + data)
    = Compilation depth THE SAME. Fillability DIFFERENT.
    → PROOF: pain is NOT SOLELY due to compilation depth.
    → Fillability (replaceability × dependency) = the decisive factor.

  "ENTITY'S STATE = MY STATE" — REFRAME:

    DESCRIPTION (what it looks like): entity is sad → I am sad
    MECHANISM (what actually happens):
      ① Self-Pattern-Modeling predicts entity state (agent sad, happy, angry...)
      ② Self-Pattern-Modeling predicts: this state → HOW will entity fill gaps?
         (mother sad → less available → gaps filled more poorly)
      ③ Gap-fillability prediction CHANGES → body responds

    = Self-Pattern-Modeling predicts agent state = 1 INPUT for gap-fillability prediction
    = NOT direct "state transfer"
    ...
    ⭐ ENTITY-OWNED REFRAME:
      ...
      → = Compressed prediction, NOT state-sharing mechanism
      → = EMERGES FROM compilation depth + gap-fillability dependency

  🟢 "Away from mother" proof: Entity-Valence-Dynamics §8 (3 cumulative mechanisms)
  ...
  🟡 Loss mechanism unified across spectrum = framework synthesis (drill discovery)
  🟡 "Entity's state = my state" reframe = compressed prediction (drill discovery)
  🟡 2D model (compilation × connectivity) = core synthesis from drill analysis
```
...
⭐ CONNECT TO BILATERAL DRILL:
  ...
  2 DIMENSIONS ORTHOGONAL (perpendicular — INDEPENDENT):
    ...
    ├────────────────────────────────────┼───────────────────────────────┤
    │ WHAT belongs to body-base?         │ Can PFC SEE the source?       │
    │ → Defined by COMPILATION           │ → Defined by PFC VISIBILITY   │
  ...
  ① Entity-Owned + PFC full access (secure):
     → "Mother is always here" — body-base extension + PFC articulates clearly

  ② Entity-Owned + PFC no access (anxious background-pattern):
     → Body clings but PFC does not know why

  ③ Prosthetic + PFC access:
     → "My prosthetic arm" — body-schema extension + PFC aware

  ④ Viscera + PFC no access:
     → Gut distress → PFC: "don't know why I feel uncomfortable" (interoception poor)

  ⑤ Real hand + PFC full access:
     → "My hand hurts from touching fire" — full channels + PFC accesses source

  ⑥ Real hand + PFC no access (background-pattern):
     → Flinch when parent raises hand (compiled PTSD) — PFC does not know source

  → 2 dimensions combine freely — INDEPENDENT of each other.
...
⭐ ENTITY-VALENCE-DYNAMICS §2 STATES:
  ...
  "Body-base extension = EMERGENT property ONLY present in agent valence"
  ...
  ASSESSMENT: CORRECT at ENTITY-LEVEL. Does not yet address BODY-SCHEMA level.
  ...
  │                                                                     │
  │   WHAT: Parietal/premotor extend body representation                │
  ...
  │   RESEARCH: 🟢 Iriki 1996, Botvinick 1998, Maravita 2004           │
  │   FRAMEWORK: currently NOT EXPLICITLY addressed                     │
  ├─────────────────────────────────────────────────────────────────────┤
  ...
  Entity-Valence-Dynamics §2 CORRECT: "losing a house ≠ losing a child"
    → House: NO body-schema extension (not used like a hand)
    → Child: entity body-base extension (agent, Self-Pattern-Modeling, multi-gap)
    → = Claim holds

  HOWEVER: "losing a prosthetic used for 20 years" ≠ "losing a house"
    → Prosthetic: HAS body-schema extension (Level 1)
    → Losing prosthetic > losing ordinary object because body-schema compiled
    → Losing prosthetic < losing a child because only Level 1 (schema), no Level 2 (entity)
    → = Entity-Valence §2 needs 1 NOTE of clarification:
      "Object cannot extend Level 2 (entity body-base).
       Object CAN extend Level 1 (body-schema) through motor-visual compilation."

  SCOPE OF GAP: SMALL. Original claim CORRECT. Needs only a 2-level clarification.

  ADDITIONAL FROM §12 LOSS MECHANISM:
    ...
    → "Entity's state = my state" = compressed prediction:
      entity state → Self-Pattern-Modeling predicts → gap-fillability change → body responds
    → "Away from mother" = proof: mother is fine, I AM in pain → baseline violation, NOT state transfer
    ...
    LEVEL 1 + LEVEL 2 BOTH use the SAME loss mechanism:
      ...
      = SAME formula: Chunk-Miss × predicted unfillability
  ...
  🟢 "Away from mother" as proof: Entity-Valence-Dynamics §8
```
...
═══════════════════════════════════════
  ...
  fMRI rubber hand: premotor + parietal = same as real hand
    → Ehrsson et al. 2004
  Synchrony condition for illusion:
    → Tsakiris & Haggard 2005
  Tool-use body-schema extension: bimodal neurons extend/retract
    → Iriki et al. 1996, Maravita & Iriki 2004
  Tool-use changes motor planning:
    → Cardinali et al. 2009
  Mirror box therapy for phantom:
    → Ramachandran 1996
  Cortical reorganization × phantom pain:
    → Flor et al. 2006
  Neural interface restoration:
    → Marasco 2011, Raspopovic 2014, Tan 2014
  Peripersonal space defense (no contact):
    → Graziano 1999, Graziano & Cooke 2006
  Anticipatory pain = same pain matrix:
    → Ploghaus 1999, Koyama 2005
  Threat to body part → cross-body reflex:
    → Sambo & Iannetti 2013
  VR body ownership → real physiology:
    → Slater 2008, 2010
  ENS = independent neural network (~500M neurons):
    → Gershon 1998
  Interoception pathway (posterior → anterior insula):
    → Craig 2002, 2009
  Interoceptive accuracy varies widely:
    → Critchley et al. 2004
  Social pain = physical pain pathways:
    → Eisenberger 2003
  Empathy pain shared matrix:
    → Singer et al. 2004
  Close others = self circuits in vmPFC:
    → Mitchell 2006, Tamir & Mitchell 2010
  PFC → hand: minimum 5-6 relay steps:
    → Neural-Architecture §3, §5, §6 (anatomy established)
  PFC → ENS: minimum 2 relay steps:
    → Neural-Architecture §5
  Fingernail = keratin, 0 neural connection in nail plate:
    → Textbook anatomy (nail plate = dead keratinized cells)
  Nail bed innervation (Meissner's corpuscles + free nerve endings):
    → Johnson 2001 (Current Opinion in Neurobiology)
  Hair follicle mechanoreceptors = among most sensitive in body:
    → Abraira & Ginty 2013 (Neuron)
  Stratum corneum = dead keratinized cells (outermost skin layer):
    → Textbook anatomy (touch = force through dead cells → live receptors)

═══════════════════════════════════════
🟡 FRAMEWORK SYNTHESIS
═══════════════════════════════════════

  "Compilation defines body-base boundary, not neural connection"
  ...
  Body-base boundary × PFC ACCESS = 2 orthogonal dimensions
    = Novel connection between 2 drills
  Compilation engine "doesn't care biological vs artificial"
    = Novel implication from neural interface data
  ...
  Loss mechanism unified: Chunk-Miss × unfillability across ALL positions
    = Novel unification of loss-pain from chopstick to child under 1 formula
  ...
  "Away from mother" as proof: baseline violation, NOT state-sharing
...
═══════════════════════════════════════
🔴 HYPOTHESIS (testable, not yet confirmed)
═══════════════════════════════════════

  Prosthetic + phantom merge → breaking prosthetic triggers phantom flare
    Logic consistent, specific experiment not found in literature
  Neural interface prosthetic break → pain-LIKE response
    Logical from compilation theory, not specifically tested
  Compilation depth measurable for body-schema objects
    What metric? fMRI activation? Proprioceptive drift magnitude?
  Viscera compilation CAN improve with interoceptive training
    Consistent with Critchley 2004 variability data, mechanism unclear
```
...
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R1 │ Ramachandran & Rogers-Ramachandran 1996           │ §1, §7  │ 🟢      │
│    │ Phantom limb, mirror box therapy                  │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R2 │ Botvinick & Cohen 1998 (Nature)                   │ §4      │ 🟢      │
│    │ Rubber hand illusion — original demonstration     │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R3 │ Armel & Ramachandran 2003                         │ §4      │ 🟢      │
│    │ SCR to rubber hand threat                         │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R4 │ Ehrsson et al. 2004 (Science)                     │ §4      │ 🟢      │
│    │ fMRI: rubber hand → premotor + parietal           │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R5 │ Tsakiris & Haggard 2005                           │ §4      │ 🟢      │
│    │ Synchrony condition for body ownership             │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R6 │ Iriki, Tanaka & Iwamura 1996 (NeuroReport)       │ §5      │ 🟢      │
│    │ Tool-use: bimodal neurons extend/retract          │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R7 │ Cardinali et al. 2009 (Current Biology)           │ §5      │ 🟢      │
│    │ Tool-use changes arm kinematics                   │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R8 │ Farnè & Làdavas 2000                              │ §5      │ 🟢      │
│    │ Tool-use extends peripersonal space               │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R9 │ Maravita & Iriki 2004 (Trends in Cog. Sci.)      │ §5      │ 🟢      │
│    │ Cross-modal body-schema integration review        │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R10│ Murray 2004                                       │ §6      │ 🟢      │
│    │ Prosthetic users body-schema integration          │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R11│ De Preester & Tsakiris 2009                       │ §6      │ 🟢      │
│    │ Prosthetic embodiment + sensory feedback loop     │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R12│ Flor et al. 2006                                  │ §7      │ 🟢      │
│    │ Cortical reorganization × phantom pain            │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R13│ Marasco et al. 2011 (Sci. Transl. Med.)           │ §8      │ 🟢      │
│    │ Targeted Sensory Reinnervation                    │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R14│ Raspopovic et al. 2014 (Sci. Transl. Med.)        │ §8      │ 🟢      │
│    │ Bidirectional neural interface prosthetic         │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R15│ Tan et al. 2014                                   │ §8      │ 🟢      │
│    │ Peripheral nerve interface sensory feedback       │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R16│ Graziano, Reiss & Gross 1999 (J. Neurophysiol.)  │ §9      │ 🟢      │
...
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R19│ Koyama et al. 2005 (PNAS)                         │ §9      │ 🟢      │
│    │ Expected pain modulates actual pain               │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R20│ Sambo & Iannetti 2013 (PNAS)                      │ §9      │ 🟢      │
│    │ Hand-Blink Reflex: body-schema spatial defense    │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R21│ Slater et al. 2008, 2010                          │ §9      │ 🟢      │
│    │ VR body ownership → real physiology               │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R22│ Gershon 1998                                      │ §10     │ 🟢      │
│    │ ENS "Second Brain" (~500M neurons)                │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R23│ Craig 2002, 2009 (Nature Reviews Neuroscience)    │ §10     │ 🟢      │
│    │ Interoception: posterior → anterior insula        │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R24│ Critchley et al. 2004                             │ §10     │ 🟢      │
│    │ Interoceptive accuracy varies widely              │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R25│ Eisenberger 2003                                  │ §3      │ 🟢      │
│    │ Social pain = physical pain (dACC)                │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R26│ Singer et al. 2004 (Science)                      │ §3      │ 🟢      │
│    │ Empathy pain: ACC + anterior insula fire          │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R27│ Mitchell 2006, Tamir & Mitchell 2010              │ §3      │ 🟢      │
│    │ Close others = self circuits in vmPFC             │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R28│ Penfield & Boldrey 1937                           │ §7      │ 🟢      │
│    │ Somatotopic mapping (homunculus)                  │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R29│ Cowan 2001                                        │ §11     │ 🟢      │
│    │ Working memory capacity ~4±1                      │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R30│ Aron et al. 2004                                  │ §11     │ 🟢      │
│    │ Response inhibition: vlPFC                        │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R31│ Fedorenko, Ivanova & Regev 2024 (Nat. Rev. Neuro.)│ §11    │ 🟢      │
│    │ Language network dissociable from PFC reasoning   │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R32│ Porges 2011                                       │ §10     │ 🟢      │
│    │ Polyvagal theory — vagal tone regulation          │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R33│ Johnson 2001 (Cur. Opin. Neurobiol.)              │ §9      │ 🟢      │
│    │ Cutaneous mechanoreceptors: nail bed, fingertip   │         │          │
├────┼──────────────────────────────────────────────────┼─────────┼──────────┤
│ R34│ Abraira & Ginty 2013 (Neuron)                     │ §9      │ 🟢      │
│    │ Sensory neurons of touch: hair follicle receptors │         │          │
└────┴──────────────────────────────────────────────────┴─────────┴──────────┘
```
...
> Viscera = proof: physically closer than the hand but LESS PFC access → spectrum = 2D.
> Chain mechanism: all control via indirect chain — adding a tool = adding a link.
> Fingernail bridge: real body part, 0 neurons — body already included non-neural elements from the start.
> Loss mechanism: Chunk-Miss × unfillability — unified from chopstick to child.
> "Entity's state = my state" reframe: compressed prediction over gap-fillability.
> "Away from mother" = proof: mother is fine, I am in pain → baseline violation = mechanism.
