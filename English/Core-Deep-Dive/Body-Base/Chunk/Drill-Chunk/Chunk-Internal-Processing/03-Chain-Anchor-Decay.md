...
dependencies:
  ...
  - F1 Convergence Point 1 gradient (chunks = strength-weighted)
test_hypothesis: H6
language: English
...
> What does the decay mechanism look like? Thread 8 from the Feeling Deep Analysis.
> H6 test: does decay follow Ebbinghaus without anchor? Anchor type ranking?
...
```
WHY DO CHUNKS NEED ANCHORS? HOW DOES DECAY OCCUR WITHOUT AN ANCHOR?
...
User's formulation (verbatim):
  ...
  [Translation: "Why must there be a verbal anchor, or an image, or an object,
   or context, or ritual, to keep a chunk in the chain?"]
  "Nếu chunk không thể gán anchor thì sẽ duy trì được dài không hay
   sẽ dễ suy thoái và quên"
  [Translation: "If a chunk cannot be assigned an anchor, will it persist for long
   or will it easily decay and be forgotten?"]

Specific questions:
  ① CHAIN: How do chunks link into sequences?
  ② ANCHOR: What is the actual role of anchor in a chain?
  ③ DECAY: Without anchor → does chunk decay faster?
  ④ H6: Does decay follow Ebbinghaus? Anchor type ranking?
  ⑤ "Knowing but unable to recall" = what, at the mechanism level?
```
...
CHAIN = a sequence of chunks with SEQUENTIAL LINKS (A→B→C→D):
  → Chunk A fires → activates B → activates C → activates D
  → The sequence is DIRECTIONAL (A leads to B, not necessarily B leads to A)
  → = "thought chains", "memory chains", "action sequences"

CHAIN ≠ SINGLE CONNECTION:
  → Connection (01-Chunk-Connection): A↔B (1 link, 2 chunks)
  → Chain: A→B→C→D→E (many links, many chunks, with sequence)
  → Chain = MANY connections joined into a sequence

EXAMPLES:

  ① NARRATIVE CHAIN:
     [old friend] → [old school] → [teacher] → [opening ceremony] → [mom walked me there]
     → Remember old friend → automatically recall school → recall teacher → recall ceremony...
     → = Chain of associated memories

  ② REASONING CHAIN:
     [oil price rises] → [transport costs rise] → [goods prices rise] → [inflation]
     → = Causal chain (Type 4 connections chained)

  ③ SKILL CHAIN:
     [hold knife] → [chop vegetables] → [add to pot] → [start stove] → [stir]
     → = Motor sequence (meta-chunk = compiled chain)

  ④ EMOTIONAL CHAIN:
     [bad news] → [shock] → [sadness] → [longing for someone] → [guilt] → [paralysis]
...
CHAIN FORMATION = TEMPORAL CO-OCCURRENCE + REPETITION + EMOTIONAL WEIGHT:
  ...
  ① TEMPORAL CO-OCCURRENCE:
     A fires → B fires right after → Hebbian: A→B link strengthens
     → 🟢 Hebbian learning: "neurons that fire together wire together"
     → 🟢 Temporal contiguity in associative learning (Rescorla & Wagner 1972)
     → = A and B occur close together in time → link forms

  ② REPETITION:
     A→B occurs MANY TIMES → link strengthens progressively
     → = 4-channel compile mechanism (F1 Convergence Point 2): repetition = channel 1
     → Chain link strength = f(number of co-occurrences)
     → More repetitions → stronger chain → more automatic

  ③ EMOTIONAL WEIGHT:
     A→B occurs in EMOTIONAL PEAK → link compiles QUICKLY
     → = 4-channel compile: emotional peak = channel 2
     → 1 trauma event CAN create an extremely strong chain
     → = "Flashbulb memory" chains (🟢 Brown & Kulik 1977)

  ④ MULTI-MODAL ENRICHMENT:
     A→B occurs across many modalities → link goes DEEPER
...
```
⭐ CRITICAL DISTINCTION (from F1 Convergence Point 6):
...
ANCHOR ≠ CHUNK CONTENT:
  ...
  F1 Convergence Point 6 committed: labels are enhancement layer, not constitutive
...
TYPE A — VERBAL LABEL:
  ...
  → = Convergence Point 6 (F1): verbal-as-handle hybrid
...
TYPE C — CONTEXT/ENVIRONMENT:
  ...
  → "Church" context → anchor for spiritual chunks
...
TIER 2 — GROUP ANCHOR:
  ...
  → "Bốn phương" = Vietnamese speakers know, English speakers don't
...
FRAMEWORK INTERPRETATION:
  ...
  → NOT chunk deletion (F1 Convergence Point 1: chunks don't get deleted, only weaken)
...
SCENARIO: Chunk compiled but NEVER given anchor (no label, no context, no ritual)
  ...
  → = "That which I sense but cannot name" = unanchored chunk influencing body
...
"I know it... almost there... CAN'T recall it!"
...
FRAMEWORK INTERPRETATION:
  ...
  → PFC detects: "something there, not quite reached"
  ...
  ⑤ Eventually: additional cue → fires → "Ah! TOM HANKS!"
...
### §6.2 — "Forgetting" vs "Loss"

```
⭐ FRAMEWORK DISTINCTION:

"FORGETTING" (retrieval loss):
...
"NEVER COMPILED":
  ...
  → "Cannot recall" ≠ "forgot" → "never compiled"
...
AI ERA IMPLICATIONS:
  ...
  ④ RE-ANCHORING:
     ...
     → "I've forgotten concept X" → AI provides context + examples + connections
...
═══════════════════════════════════════
  ...
  Anchor = retrieval path (not content):   Extends Convergence Point 6, consistent
  ...
  "Forgetting" vs "Loss" vs "Never Compiled": Framework distinction
...
WITHIN CHUNK-ANALYSIS:
  → F1 10-F1-Synthesis.md (Convergence Point 1 gradient, Convergence Point 6 verbal-as-handle,
                             Convergence Point 2 compile mechanism)
...
✅ English primary throughout
  ...
  FRAMEWORK CONTRIBUTIONS:
    ...
    ⑤ "Forgetting" vs "Loss" vs "Never Compiled" distinction (§6.2)
...
> Anchor = retrieval path, not content. "Forgetting" ≠ "Loss".
