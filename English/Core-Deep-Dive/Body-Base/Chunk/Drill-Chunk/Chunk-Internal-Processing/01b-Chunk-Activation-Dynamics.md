...
dependencies:
  ...
  - F1 Convergence Point 1 (gradient, strength-weighted)
  - F1 Convergence Point 7 (body-state-at-compile, 3 update paths)
  - Neural-Processing-Flow.md (hardware substrate)
plan_note: |
  File added to F4 due to plan adjustment (reality exceeded the plan).
  User insight identified gap: probability mechanism not yet formalized.
  Content = CHUNK CORE, cross-cutting. Target for Chunk.md v2.0 rewrite
  after 99-Master-Synthesis.
language: English
...
> They belong to no single thread — cross-cutting across the entire framework.
> Emerged from user insight: "chunks are never deleted — they can only decay,
> be overridden, or be re-triggered to create new pattern links."
...
WHAT THE FRAMEWORK ALREADY HAS:
  ✅ Chunks are not deleted (Chunk.md §2, F1 Convergence Point 1)
  ✅ Reconsolidation window (Chunk.md §2, Nader 2000)
  ✅ 3 update paths (F1 Convergence Point 7: re-associate / novelty hijack / therapy)
  ✅ 4 compile mechanisms (F1 Convergence Point 2)
  ✅ Spreading activation (Collins & Loftus 1975)
  ✅ 4 connection types (F4 01)
  ✅ Anchor = retrieval path (F4 03)

WHAT THE FRAMEWORK HAS NOT YET FORMALIZED:
  ...
  → Without this = missing the most fundamental OPERATING mechanism
  → = We know chunks connect (01), know chunks decay (03)
  → = But NOT YET known: when triggered → where activation GOES, how MUCH, WHY
```
...
WHEN CHUNK X IS TRIGGERED (by PFC hold or sensory input):
  ...
  ③ Each link has DIFFERENT STRENGTH (Hebbian: used more = stronger)
  ...
  → Chunks receiving SUFFICIENT activation (exceeding threshold) → fire
  → Chunks receiving INSUFFICIENT activation → partial activation
    (can contribute to H5 multi-weak-signal convergence but do not fire independently)

  → = "When thinking about X, I mostly think of A"
  → = It is not that X→A is the ONLY link
  → = Rather, X→A is the STRONGEST link → most probable activation
...
LINK STRENGTH = f(7 factors):
  ...
  ③ EMOTIONAL WEIGHT AT COMPILE:
     ...
     → = F1 Convergence Point 2 channel 2: emotional peak compile
     → 1 trauma event CAN create a link stronger than 100 neutral repetitions

  ④ MULTI-MODAL RICHNESS:
     → A link spanning many modalities → more neurons involved → stronger
     → Visual + auditory + somatic + emotional → maximally strong
     → = F1 Convergence Point 2 channel 3: multi-modal compile
...
⭐ FACTORS INTERACT MULTIPLICATIVELY (not additively):
  ...
  → Low repetition + no emotion + single modal + no anchor
    ...
    = Example: a phone number heard once and forgotten
...
PROBABILITY DISTRIBUTION CHANGES CONTINUOUSLY:
  ...
  → NEW LINKS GROW → take up progressively more probability share
...
NOT "DELETE OLD LINK, REPLACE WITH NEW":
  → The neural substrate has NO "unwire on demand" mechanism
  → Chunk.md §2: "No one can delete a chunk"
  → F1 Convergence Point 1: chunks only strengthen/weaken, never delete
...
INSTEAD: "NEW LINK COMPETES WITH OLD LINK":
  ...
  ④ PROBABILITY CROSSOVER:
     ...
     → PFC experience: "normal / positive" (dominant) alongside "slightly afraid" (minority)
     → = "Still slightly afraid of dogs but no longer panicking"
...
### §3.3 — 3 re-linking strategies (F1 Convergence Point 7, extended)

```
F1 Convergence Point 7 committed 3 update paths. §3 extends with probability mechanism:
...
STRATEGY 1 — RE-ASSOCIATE (gradual probability shift):
  ...
  → = "Living positively in a new context" (user's formulation)
...
STRATEGY 2 — NOVELTY HIJACK (context-driven probability jump):
  ...
  → Example: "hating math" chunk → game development interest activates
...
STRATEGY 3 — THERAPY-ASSISTED (reconsolidation-based):
  ...
  Best for: Level 3-4 trauma (F1 Convergence Point 7), deep pathological associations
...
DEFINITION:
  ...
  → = "How many entrances does this chunk have?"
...
TRIGGER SURFACE SIZE = f(4 factors):
  ...
  ① NUMBER OF MODALITIES in chunk:
     → Visual + auditory + somatic + motor + emotional = 5 "entrances"
...
TRAUMA COMPILE CREATES MAXIMUM TRIGGER SURFACE:
  ...
  ① MULTI-MODAL: visual + auditory + somatic + olfactory + emotional
     → Being bitten by a dog: seeing the dog (visual), hearing barking (auditory),
       feeling pain (somatic), smell (olfactory), fear (emotional)
  ...
  ④ MANY ASSOCIATED CHUNKS compiled simultaneously:
     ...
     → = "Smelling a similar scent → the entire trauma scene fires"

  RESULT:
    Trauma multi-chunk = DOZENS of entry points
    → Random daily input → HIGH probability of matching at least 1 entry point
    → = "A vague fear of something without knowing what" (user's formulation)
...
⭐ CRITICAL PARALLEL — EXPERTISE HAS SAME MECHANISM:
  ...
  → TRAUMA AND EXPERTISE = SAME MECHANISM, DIFFERENT DIRECTION:
    ...
    → = F1 Convergence Point 7 confirmed: novelty vs threat DIRECTION matters, not mechanism
```
...
TRIGGER SURFACE CAN SHRINK OVER TIME:
  ...
  NATURAL REDUCTION ("living positively in a new context"):
    ...
    → = "Gradually diminishes over time when living positively" (user's formulation, confirmed)
  ...
  ⚠️ TRIGGER SURFACE NEVER REACHES ZERO:
    ...
    → = "Forgetting completely" = unrealistic expectation
```
...
EXAMPLE — Full lifecycle:
  ...
  COMPILE:
    ...
    → Strong links: [dog] → [pain, fear, run] (dominant pathway)
  ...
  YEARS 5-10 (avoidance):
    ...
    → Random triggers: frequent → "dog fear" identity consolidated
  ...
  YEAR 20 (resolved):
    ...
    → PFC: "ah, I used to be afraid of dogs as a child — I'm fine now, just staying careful"
...
═══════════════════════════════════════
  ...
  Trauma = expertise same mechanism diff direction: Novel insight, consistent with Convergence Point 7
  7-factor link strength model:                    Framework formalization
  3 strategies as speed variants of same mechanism: Extends F1 Convergence Point 7 with mechanism
...
WITHIN F4:
  ...
  → 04-Right-Wrong-Vague.md (vague detection = competing signals, probability undetermined)
  → 05-Insight-Tacit-Upgrade.md (insight = low-probability path fires, §5.2 creativity)

WITHIN CHUNK-ANALYSIS:
  → F1 Convergence Point 1: gradient (strength-weighted) = §2 probability model substrate
  → F1 Convergence Point 2: 4 compile mechanisms = §2.2 link strength factors ①-④
  → F1 Convergence Point 7: 3 update paths = §3.3 strategies (extended with mechanism)
...
> "Reality does not follow our plans" — plan adjusted to capture core insight.
>
