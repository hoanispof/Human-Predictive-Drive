...
scope: |
  ...
  Brain compiles an agent into body-base at the STRUCTURAL level.
  ...
  natural fit → state/trait convergence → grief → love↔hate → decay →
...
purpose: |
  ...
  = "Valence-Propagation §3 expanded" — Valence-Propagation covers 2-3 pages; this file = full deep-dive.
...
dependencies:
  ...
  - By-Product-Gap-Resonance.md v1.0 — mutual phenomenon, 5 axes
  ...
  - PFC-Operations.md v1.0 — Hold/Suppress, Fresh Type A/B
...
> **Entity-Compiled = the brain compiles an agent into body-base at the STRUCTURAL level.**
>
> **NOT "knowing about that person" (schema). NOT "having a momentary emotion" (valence event).**
> **The brain INTEGRATES the agent into its OPERATIONAL FOUNDATION:**
> **Agent's state = MY state. Agent's loss = body-base amputation.**
>
> **8 independent research programs all found the SAME phenomenon.**
> **Hub-and-Spoke neural architecture. My body = simulator for a close person.**
...
- §0 — POSITION + THESIS
- §1 — DEFINITION + 8 CONVERGING RESEARCH STREAMS
...
- §5 — NATURAL FIT: 2D MATRIX + META-MATCH
...
THIS FILE ADDS:
  ① 8 research streams PROVING Entity-Compiled = real
  ...
  ⑤ Natural fit = 2D matrix + meta-match
...
READING FLOW:
  Agent-Mechanism.md (integration hub)
    → Self-Pattern-Modeling.md (HOW 1 body simulates another entity)
...
COMPANION FILES:
  ...
  Connection.md = connection observation parameter (3 Generative Primitives)
  Simulation-Engine.md = 1 engine, 3 components (self/other model gradient)
```
...
= NOT "having a momentary emotion" (valence event)
= The brain INTEGRATES the agent into the OPERATIONAL FOUNDATION:
  → Agent's state = MY state (Valence-Propagation §3.1)
  → Agent's loss = body-base amputation (Valence-Propagation §3.4)

3 SUBTYPES (spectrum, not binary — Valence-Propagation §3.2):
...
### §1.2 — 8 converging research streams
...
|---|---|---|---|---|
| 1 | **Shared pain circuits** | Singer et al. 2004 (R1) | Wife watching husband in pain → wife's anterior insula + ACC activate. Same regions as when wife herself feels pain | 🟢 fMRI |
| 2 | **Social Baseline Theory** | Coan & Beckes 2006, 2011 (R2) | Brain DEFAULTS to treating close others as extensions. Holding spouse's hand → threat circuits REDUCE | 🟢 fMRI |
| 3 | **Self-Other overlap** | Courtney & Meyer 2020 (R3) | vMPFC (the "self" region) uses SHARED neural substrate for "I" and "close person" | 🟢 fMRI |
| 4 | **Biobehavioral synchrony** | Feldman 2012 (R4) | Mother-infant synchronize across 4 systems: behavior + heart rate + oxytocin + brain. Measurable | 🟢 Physiological |
| 5 | **Cortisol synchrony** | Saxbe & Repetti 2010 (R5) | Couples synchronize cortisol slope. One person stressed → other person's cortisol SHIFTS | 🟢 Endocrine |
| 6 | **Interbrain synchrony** | Nguyen et al. 2020 (R6) | fNIRS hyperscanning: mother-child cooperating → PFC in both brains oscillate TOGETHER | 🟢 fNIRS |
| 7 | **IOS (Inclusion of Other in Self)** | Aron et al. 1992 (R7) | Close relationship → incorporate partner into self-concept. Loss = "self-concept confusion" | 🟢 Behavioral + longitudinal |
| 8 | **Grief = phantom limb** | Ratcliffe 2018 (R8) | Neural networks built over years CONTINUE FIRING after loss | 🟡 Phenomenological + neuro |
...
ENTITY-COMPILED (Valence-Propagation v2.0 §3) = CONFIRMED:
  ...
  → Agent's state = MY state (not merely momentary empathy) ✅ (Singer)
...
ENRICHED BY:
  → Social Baseline Theory: brain DEFAULTS to social → Entity-Compiled
    is NOT "adding an agent" but "DEEPENING what is already the default"
...
MECHANISM NOTE (→ §7, Entity-Valence-Dynamics §2.4):
  ...
  The underlying MECHANISM = gap-fillability baseline:
    → Entity compiled deep → fills MANY gaps → part of BASELINE
    → Entity state change → Self-Pattern-Modeling predicts gap-fillability change → body responds
    → Entity lost → baseline VIOLATED → Chunk-Miss → pain (§7: A+B+C)
  = "State sharing" = compressed prediction over gap-fillability.
  = The primary factor: total gap body-need being fed.
...
```
ENTITY-COMPILED HAS NO PRECISE WORDS IN NATURAL LANGUAGE.
Natural language captures individual ASPECTS:

  "My flesh and blood"          → body-base extension (structural)
  "Walking in my gut"           → compiled Self-Pattern-Modeling + interoceptive + entity inside body-base
  "Understanding without words" → Compiled predicts accurately
  "We" replacing "I"            → merged identity (Gallotti & Frith 2013: We-Mode)

Vietnamese "bụng" = gut as seat of emotion (🟢 Dang 2017).
"Đi guốc trong bụng" (walking in clogs inside someone's gut) = 4 layers:
  Self-Pattern-Modeling accuracy + interoception + entity-compiled + transactive.
...
```
⭐ PERSON-MODEL DOES NOT LIVE IN ONE PLACE.
   Architecture = HUB-AND-SPOKE (Patterson & Lambon Ralph 2007, 2017).

WHEN THINKING OF "MOTHER":

SPOKES (modality-specific, distributed across cortex):
  ├── Visual cortex (fusiform) ── mother's face, gait, home
  ├── Auditory cortex ─────────── mother's voice, her calling your name
  ├── Somatosensory (SI/SII) ─── sensation of her hands, her embrace
  ├── Olfactory cortex ────────── her scent, smell of her kitchen
  ├── Motor cortex ────────────── how you move when around her
  ├── Insula ──────────────────── the feeling of "warmth" when near her
  ├── Interoceptive (visceral) ── gut sense of her state
  └── mPFC ────────────────────── trait knowledge ("what kind of person she is")

HUB (binding center):
  └── Anterior Temporal Lobe (ATL) ── gathers ALL spokes = "MOTHER"
      ↕ Amygdala (emotional evaluation)
      ↕ OFC (decisions related to her)
      ↕ Hippocampus (episodic memories with her)
...
FRAMEWORK: Person-model = CHUNK ASSEMBLY (Schema.md v2.0)
  ...
  Entity-Compiled = when assembly is DEEP + STRUCTURAL enough:
    → Body treats person-schema channels AS its own channels (Valence-Propagation §3)

🟡 Mapping Hub-and-Spoke → chunk assembly = framework synthesis.
   Consistent: both describe the same phenomenon at two levels of description.
```
...
🟢 Semantic dementia (ATL atrophy):
  → Gradually LOSES ALL knowledge about close people
  → Not just forgetting the name — forgets faces, voice, even emotion
  → Spokes remain but hub is lost → CANNOT bind into "this person"
...
🟢 PNAS 2024 — temporal pole:
  → Dedicated region processes FAMILIAR persons (distinct from strangers)
...
### §2.3 — Body simulation: my body = simulator for a close person

```
⭐ KEY FINDING:

🟢 Beckes & Coan 2012 (fMRI):
  Threat to a CLOSE FRIEND: anterior insula + putamen + supramarginal
  = IDENTICAL to threat to SELF. Different from STRANGER.
  → Close person → simulated USING MY OWN BODY

🟢 Keysers & Gazzola 2010 (Nat Rev Neurosci):
  Watching someone being touched → MY OWN SI/SII activates, SOMATOTOPIC
  → Their hand touched → my HAND region fires

🟢 Gordon et al. 2013 (PNAS):
  SI discriminates AFFECTIVE SIGNIFICANCE of social touch
  = Same stroking — sensation DIFFERS DEPENDING ON WHO touches
  → SI has compiled PER-AGENT VALENCE into somatosensory processing

🟢 Mirror neuron familiarity gradient (EEG mu-suppression):
  Self → fires STRONGEST
  Close person → fires MODERATELY
  Stranger → fires WEAKEST
  → Higher Entity-Compiled level → deeper body simulation

FRAMEWORK:
  Self-Pattern-Modeling Compiled = embodied simulation (body fires a weak copy)
  Self-Pattern-Modeling Fresh = cognitive simulation (PFC drafts)
  Entity-Compiled DEEP → Compiled dominates → MORE body involvement
  → Explains: close person's pain = "MY pain"
    (brain uses MY OWN pain circuits to simulate)

🟢 Simulation-Engine.md v1.0: Self/Other Model component —
   mPFC gradient ventral (self+close) → dorsal (far/stranger).
   Entity-Compiled DEEP = ventral processing (near self).
```
...
WHERE IN THE BODY sensations are felt = 3 ADDITIVE SOURCES:
  ① Bottom-up physiological (actual physical signals)
  ② Top-down behavioral (motor preparation)
  ③ Conceptual/cultural schemas (Vietnamese "bụng"/gut, English "heart")
  → Each person = unique combination → no right or wrong
...
⭐ ALEXITHYMIA (~10% of population) — PROOF that body signals are NECESSARY:
  ...
  → Lost input → lost ability to read own simulation → social deficit
  → Entity-Compiled NEEDS body signals to MAINTAIN awareness
  → Simulation-Engine.md §7: alexithymia degrades ALL applications

🟢 Decisive proof: if body signals were a luxury → alexithymia would
   affect only self. But it affects SOCIAL cognition too
...
FRAMEWORK: "Hours" = proxy for CHUNK COMPILE CYCLES
  ...
  → Not literally "hours" per se — it is the NUMBER OF TIMES
    Self-Pattern-Modeling successfully PREDICTS the entity
```
...
🟢 Tse et al. 2007 (Science):
  ...
  Condition: a pre-existing SCHEMA must already exist

FRAMEWORK: NON-LINEAR acceleration curve

  Compile speed
  ↑
  │              ┌──── Entity-Compiled deep (Compiled dominant)
...
③ Schema deep (~200+h): new data assimilates NEARLY INSTANTLY
   → "Understanding without having to say it" = schema predicts BEFORE input
...
FRAMEWORK:
  ...
  Incompatible: low overlap → Compiled misses → cost → compile slower or fails
```
...
🟢 2025 research: CA2 pyramidal neurons reactivation during
  ...
  Disruption → SUPPRESSES social memory. Enhancement → PROLONGS it.
...
FRAMEWORK:
  ...
  Sleep deprivation (elevated cortisol, chronic stress) →
    social knowledge compiles MORE SLOWLY →
    Entity-Compiled formation SLOWS → relationships STAY SHALLOWER
...
```
⭐ FRESH PROCESSING HAS 2 VERY DIFFERENT TYPES (PFC-Operations.md v1.0):

Fresh TYPE A — "LEARNING NEW" (no existing compiled to fight):
  = No compiled chunks yet for this target
  = PFC drafts from scratch → Cost: PFC draft ONLY (suppress = 0)
  = Body OPEN (curiosity, novelty) → Hebbian compile NATURAL
  Example: genuinely exploring a new person, mother modeling a newborn

Fresh TYPE B — "OVERWRITING" (suppress existing compiled):
  = Existing compiled chunks PRESENT → PFC must SUPPRESS → then draft new
  = Cost: PFC draft + SUPPRESS (body RESISTS)
  = Compile potential LOWER (body in conflict)
  Example: trying to move past a bad first impression, "giving a second chance"

KEY DIFFERENCE:
  Type A: body + PFC = SAME DIRECTION → compile fast
  Type B: body vs. PFC = OPPOSING DIRECTIONS → compile hard
  "Forcing yourself to open up" = suppress ≠ "truly opening up" = recompile

AUTHENTICITY GATE:
  Body DETECTS fake vs. genuine (efference copy — Autonomy-Hardware):
  PFC overrides "pretending to care" → Type B → suppress accumulates → abandons
  PFC overrides "genuinely curious" → Type A → body engages → compiles
...
🟢 Aron 1997 (Fast Friends): 45 minutes of reciprocal disclosure → IOS increases
...
🟢 Feldman 2012: mother-infant interaction → oxytocin SURGE in both
  Oxytocin + prolactin → increase peak_valence → increase compile_rate
...
🟢 Biobehavioral synchrony well-established (Feldman 2012)
🟡 Compile rate formula = framework synthesis
```
...
🟢 Zhou et al. 2005 (Proc. Royal Society B — empirical):
  ...
  │ S4     │ ~150  │ Self-Pattern-Modeling Fresh only, no body-base ext │
...
```
⭐ CEILING EXISTS BECAUSE OF 3 RESOURCE LIMITS:
...
STRATEGY A — FEW AGENTS, DEEP (introvert pattern):
  ...
  Risk: HIGH if 1 is lost (small network = fragile)

STRATEGY B — MANY SIMILAR, MEDIUM DEPTH (selective extrovert):
...
→ Each person = MIX, tends toward 1
→ Hardware + Background-Pattern BIAS which feels "natural"
→ "Close friends tend to resemble each other" = optimization, not coincidence
...
② NO CHOICE REQUIRED (given → no selection cost)
...
BUT: kin = GIVEN → may NOT match
  → Extreme mismatch → Mixed Entity-Compiled (Valence-Propagation §3.2 ③)
  → = "Both love and anger at once" = both positive + negative channels
...
```
⭐ WHY DO SOME PEOPLE NATURALLY FIT?
→ Self-Pattern-Modeling Compiled uses MY OWN BODY as simulator.
...
③ BOTH experience "this person understands me" (mutual accuracy)
④ Accurate prediction → reward → more interaction → more overlap
   → POSITIVE SPIRAL (Hebbian)

OPPOSITE (negative spiral):
① No shared chunks → Compiled uses own body → MISSES
② Cost INCREASES (switch to Fresh) → less interaction → no new overlap
③ Self-selection apart (no one is wrong — templates simply don't match)
...
```
⭐ SHARED CHUNKS ≠ AUTOMATIC RESONANCE. DIRECTION ALSO REQUIRED:

                   Direction: COMPATIBLE       Direction: COMPETING
              ┌──────────────────────────┬──────────────────────────┐
 HIGH Overlap │ ⭐ STRONGEST RESONANCE   │ ⚡ RIVALRY / CONFLICT    │
              │ Compiled accurate + match │ Compiled accurate + ANTI │
              ├──────────────────────────┼──────────────────────────┤
 LOW Overlap  │ 💫 COMPLEMENTARY         │ 😐 INDIFFERENCE          │
              │ By-product match via      │ No match, no conflict    │
              │ META-GOAL                 │ → self-selection apart   │
              └──────────────────────────┴──────────────────────────┘

→ High overlap + compatible = strongest bond potential
→ High overlap + competing = intense rivalry (two goalkeepers on the same team)
...
```
NATURAL FIT HAS 2 LAYERS:

LAYER 1 — SURFACE: shared interests (math, football, K-pop)
  = Domain chunks overlap directly → Compiled accurate at content level
  BUT: can still be COMPETING (two goalkeepers)

LAYER 2 — META: shared WAY of engaging interests (excitement pattern, intensity)
  = Hardware + meta-chunk overlap → Compiled accurate at BODY-FEEDBACK PATTERN level
  No shared content needed → shared WAY OF EXPERIENCING

META > SURFACE because:
  Surface match + meta mismatch = "like the same things but don't click"
  Surface mismatch + meta match = "different interests but really click"

🟢 Parkinson 2025: neural similarity at OFC (subjective value) predicts friendship
  = Evidence for meta-level match (similar body-feedback patterns)

🟡 Meta > surface distinction = framework synthesis
```

### §5.4 — "Earned fit" = possible

```
"NATURAL FIT" = Compiled default match. Cost ≈ 0. Felt as "chemistry."
"EARNED FIT" = Fresh Type A (genuine) → compiles → new Compiled
  → Cost HIGH initially → DECREASES gradually → eventually ≈ 0
  → "Before I had to work to understand them; now it's automatic"

CONDITIONS:
  ① Genuine engagement (NOT fake → Type B = suppress = fails)
  ② Sufficient frequency (Hebbian needs repetition)
  ③ Reward confirmed (predict → verify → match → strengthen)
  ④ No hardware floor (extreme hardware difference = ceiling)

🟡 "Earned fit" = framework synthesis from Fresh A/B model
```
...
NO CONVERGENCE (trait-level):             CONVERGENCE OCCURS (state-level):
┌────────────────────────────┐  ┌──────────────────────────────────┐
...
FRAMEWORK:
  ...
  → By-Product-Gap-Resonance match occurs at STATE layer, trait match not required
  → "Same interest, different reasons, still click" = state match ≠ trait match
```
...
STATE CONVERGENCE operates WITHIN a trait tolerance band:
  ...
  Within band (POSSIBLE): emotional sync natural → compiles
  Outside band (NOT POSSIBLE): forcing it → co-dysregulation
...
FRAMEWORK:
  ...
  Large overlap → state convergence WIDE → "compatible" across many domains
  Small overlap → state convergence NARROW → "compatible" in few domains
```
...
② Trait variation = maintained FOR THE POPULATION (independent):
...
🟡 Independence of 2 mechanisms = framework synthesis
```
...
```
⭐ LOSING A DEEPLY ENTITY-COMPILED AGENT → NOT JUST SADNESS. LOSING A PROCESSING NODE.

TP1: Routine fires → target absent → STRANDED (repeat firing, no response)
TP2: Chunk-Miss → compiled chunks trigger → no response → body-feedback = pain
TP3: Valence-Momentary stream stops → Self-Pattern-Modeling emotional feed CUT → input source lost
TP4: Valence-Structural collapses gradually → body still treats agent AS extension → PHANTOM
TP5: Identity vacuum → self-schema linked to role ("wife/mother/friend") → lost → "who am I?"
...
🟢 O'Connor 2023: basal ganglia STILL fires "partner will be here"
   WHILE medial temporal lobe knows "they are gone" → 2 systems CONFLICT
🟢 Biddle et al. 2020 (JAMA): widowhood → cognitive decline ACCELERATES
...
COMPONENT A — GAP-FEED LOSS (primary):
  = Amount of gap body-need the agent WAS filling
  ...
  Agent fills 4 domains (sensory + emotional + cognitive + identity) = A very high
  Agent fills 1 domain (work colleague) = A low

COMPONENT B — FRESH-REBUILD COST (secondary):
  = Number of routines requiring PFC Fresh rebuild WITHOUT the agent
...
COMPONENT C — PHANTOM FIRING (longest lasting):
  = Entity-Compiled chunks STILL fire → target absent → continuous Chunk-Miss
...
WEIGHTS:
  Losing a SPOUSE:       A(very high) + B(very high) + C(very high) = maximum
  Losing a PARENT:       A(very high) + B(moderate) + C(very high) = devastating
  Losing a CLOSE FRIEND: A(high) + B(low) + C(high) = significant
  Losing a COLLEAGUE:    A(low) + B(low) + C(low) = minor
...
UNIFIED LOSS MECHANISM (Drill-Body-Base-Boundary-Spectrum v1.2 §12):
  ...
  = Same mechanism applies from object loss (chopsticks, PC) to entity loss (child)
...
🟢 Robinaugh et al. 2021:
  ...
  = Like addiction withdrawal: psychological + physiological disorganization
...
FRAMEWORK:
  ...
  Agent AS PART OF baseline → losing agent = baseline drops DOWN
...
🟢 Bonanno et al. 2002, 2019, 2020:
  ...
  ③ CHRONIC GRIEF (10-15%): persistent long-term
    → A very high + C very high + extinction learning fails
    → 🟢 O'Connor: nucleus accumbens CONTINUES firing (craving)

  ④ DELAYED (5-10%): normal initially → erupts after latency
...
🟢 Fernandez-Alcantara 2024: attachment anxiety → more severe PGD
🟢 Djelantik 2022: sudden loss → higher PGD caseness
  (brain has not pre-compiled → prediction errors SIMULTANEOUS)
```

### §7.5 — Per-Entity Guilt = Consequence of Entity-Compiled (v1.1)

```
⭐ PER-ENTITY GUILT = WHEN I HURT AN ENTITY I HAVE COMPILED:

  When I hurt an entity → Entity-Compiled fires PAIN signal.
  Singer 2004: "shared pain circuits" = same neural pathway.
  Guilt layer ① = ongoing pain + inability to resolve WITHOUT entity's signal.

  Entity FORGIVES → signal matches → guilt layer ① resolves.
  Entity NOT YET FORGIVEN → guilt keeps firing (no resolution signal).
  Entity GONE (death/lost contact) → resolution source GONE →
    guilt fires PERMANENTLY = moral injury component (Litz 2009).

  Guilt when entity is dead ≠ grief alone:
    Grief = loss of gap-fill source (§7 model A+B+C).
    Guilt = ADDITION on top of grief = COMPOUND.
    → Why "regret after someone has died" = grief + guilt layer ① compound.

  Guilt INTENSITY = f(Entity-Compiled depth):
    Entity-Compiled deep (parent, spouse) → guilt EXTREMELY STRONG.
    Entity-Compiled shallow (colleague) → guilt lighter.
...
🟢 Zeki & Romaya 2008 (PLoS ONE — fMRI):
  ...
  → Hate and love are CLOSER to each other than hate is to INDIFFERENCE

FRAMEWORK:
  Self-Pattern-Modeling fires DEEPLY regardless of whether valence is positive or negative
...
### §8.2 — 3 shift pathways

```
DEEP Self-Pattern-Modeling ON A RIVAL = NECESSARY BUT NOT SUFFICIENT. A CATALYST IS NEEDED:

① SHARED EXTREME EXPERIENCE:
  🟢 Sherif 1961 (Robbers Cave): 2 hostile groups → forced to cooperate → friendship
...
③ COGNITIVE DISSONANCE:
  ...
  Doing a favor for someone you dislike → dissonance → resolve by increasing positive regard
```

### §8.3 — Fundamental asymmetry

```
⭐ DESTRUCTION IS EASIER THAN CONSTRUCTION. ALWAYS.

LOVE → HATE: 1 event (betrayal), INSTANT
  → Exploits existing coupling depth → flips direction

HATE → LOVE: sustained effort, YEARS
  → Fights: negativity bias (🟢 Baumeister 2001: 1 bad > 5 good)
  → + confirmation bias ("if good → probably has an ulterior motive")
  → + no equivalent accelerator to betrayal
  BUT: Self-Pattern-Modeling model ALREADY EXISTS → if context shifts → faster than with a stranger

"WORTHY OPPONENT" = INTERMEDIATE STATE:
  🟢 Kilduff 2010: rivalry = relational, per-pair
  → Deep model + respect + negative valence on "the threat"
...
```
⭐ ENTITY-COMPILED DECAY = NON-UNIFORM:
...
① PERSON-SCHEMA (neocortical, ATL hub):
  ...
  = "Recognized instantly upon reunion"

② AFFECTIVE/MOTIVATIONAL (reward circuits, opioid):
  = Warmth, craving, approach motivation
  = Requires ACTIVE MAINTENANCE (interaction → opioid → reinforce)
  = Decays without contact: months-years
  = "Familiar upon reunion but no longer intimate"
...
→ "Remember them but the pain is gone" (schema persists + affect fades)
→ "Unexpected reunion brings pain back" (schema triggers → affect reactivates)
→ "When will I stop remembering?" → NEVER completely
   Only fires LESS OFTEN, WEAKER (Hebbian: less fire → weaker, never zero)
...
🟢 Roberts & Dunbar 2011 (longitudinal, 18 months):
  48.6% inner circle friends CHANGED when context changed (school→university)
  Contact frequency drops → Hebbian weakens → layer decays

🟢 Bhattacharya, Dunbar & Kaski 2022 (cellphone data):
  Inner circle: 1-4% turnover per year (stable)
  Ages 17-21: turnover HIGHER (life transitions = gap distribution shifts)

KIN vs. FRIEND DECAY:
  🟢 Kin: absence → MORE resilient (hardware scaffold maintains activation)
  🟢 Friends: absence → DECLINE (no hardware scaffold → voluntary maintenance only)

🟢 Roberts & Dunbar 2015: Gender difference in maintenance:
  Women maintain via: TALKING (verbal Self-Pattern-Modeling channel primary)
...
```
IF ENGINEERING A MACHINE — Entity-Compiled = VULNERABLE:
...
③ UNRECOVERABLE INVESTMENT:
  ...
  Sunk cost is REAL (compiled chunks = resource spent)
```

### §10.2 — Why evolution still chose this architecture

```
BENEFIT >> VULNERABILITY:

① ROI EXTREMELY HIGH when agent is present:
...
② VULNERABILITY = RARE EVENT (ancestral environment):
  ...
  Expected ROI = decades of benefit vs. small risk of loss
  → Positive expected value → evolution selects FOR

③ PHANTOM = BONDING REINFORCER (not just a bug):
  "Missing them" when apart → MOTIVATES RETURN (find agent, restore connection)
  Ancestral context: absent ≠ dead — usually = temporarily away
...
🟡 FRAMEWORK SYNTHESIS (logical, consistent, not yet directly tested):
  ...
  "Natural fit" = shared body template → mutual accurate Compiled
  2D matrix (Overlap × Direction) → 4 quadrants
  Meta-level match > surface match
  Fresh Type A (learning new) vs. Type B (overriding old) in formation
...
🔴 HYPOTHESIS (reasonable but no direct evidence yet):
...
⭐ KEY RISK:
  ...
  Convergent evidence (🟢) vs. analogical reasoning (🟡): must be distinguished.
```
...
| Autonomy-Hardware v1.1 | vmPFC/DRN, efference copy | Authenticity gate |
| [PLANNED] Entity-Access.md | — | Gradient model (how access changes) |
...
|---|----------|---------|----------|
| R1 | Singer et al. 2004 (Science) — Empathy for Pain | §1 | 🟢 fMRI |
| R2 | Coan & Beckes 2006, 2011 — Social Baseline Theory | §1, §2 | 🟢 fMRI |
| R3 | Courtney & Meyer 2020 — Self-Other overlap in mPFC | §1 | 🟢 fMRI |
| R4 | Feldman 2012 — Biobehavioral Synchrony | §1, §3 | 🟢 Physiological |
| R5 | Saxbe & Repetti 2010 — Cortisol Synchrony | §1 | 🟢 Endocrine |
| R6 | Nguyen et al. 2020 — Interbrain Synchrony (fNIRS) | §1 | 🟢 fNIRS |
| R7 | Aron et al. 1992 — IOS Scale | §1, §3 | 🟢 Behavioral |
| R8 | Ratcliffe 2018 — Grief = Phantom Limb | §1, §7 | 🟡 Phenomenological |
| R9 | Patterson & Lambon Ralph 2007, 2017 — Hub-and-Spoke | §2 | 🟢 Multi-method |
| R10 | Fernandino et al. 2022 (J. Neuroscience) — Distributed Concepts | §2 | 🟢 fMRI |
| R11 | PNAS 2024 — Temporal pole familiar persons | §2 | 🟢 fMRI |
| R12 | Arsalidou et al. 2010 — Person-specific neural signatures | §2 | 🟢 fMRI |
| R13 | Beckes & Coan 2012 — Self-Friend overlap in threat | §2, §3 | 🟢 fMRI |
| R14 | Keysers & Gazzola 2010 (Nat Rev Neurosci) — Somatosensory mirror | §2 | 🟢 fMRI + TMS |
| R15 | Gordon et al. 2013 (PNAS) — SI affective touch | §2 | 🟢 PNAS |
...
| R23 | Hall 2018 (J. Social and Personal Relationships) — Hours→Friendship | §3 | 🟢 N=355 |
| R24 | Tse et al. 2007 (Science) — Schema-accelerated consolidation | §3 | 🟢 Experimental |
| R25 | Parkinson et al. 2018 (Nature Communications) — Neural similarity | §3, §5 | 🟢 fMRI |
| R26 | Parkinson et al. 2025 (Nature Human Behaviour) — Predict friendship | §3, §5 | 🟢 Longitudinal |
| R27 | Diekelmann & Born 2010 — Sleep and memory consolidation | §3 | 🟢 Review |
| R28 | Bhatt et al. 2025 — Oxytocin + REM social memory | §3 | 🟢 |
| R29 | Aron et al. 1997 — Fast Friends Procedure | §3 | 🟢 Experimental |
| R30 | Roberts & Dunbar 2015 — Ties gradual change | §3, §9 | 🟢 |
| R31 | Dunbar 1992, 2024 — Social Brain Hypothesis | §4 | 🟢 Cross-primate |
| R32 | Zhou et al. 2005 (Proc. Royal Society B) — Hierarchical layers | §4 | 🟢 Empirical |
| R33 | Roberts et al. 2009 (Social Networks) — Size ↔ Closeness | §4 | 🟢 |
| R34 | Hill & Dunbar 2003 (Human Nature) — Christmas card study | §4 | 🟢 |
| R35 | Sutcliffe et al. 2012 (JASSS) — Computational validation | §4 | 🟢 |
| R36 | Byrne 1971 — Similarity-Attraction | §4 | 🟢 |
| R37 | Roberts & Dunbar 2011 — Kin vs. Friend cost | §4, §9 | 🟢 |
| R38 | Hamilton 1964 — Inclusive Fitness / Kin Selection | §4 | 🟢 |
| R39 | Gonzaga 2010, Humbad 2010, Tea-Makorn 2020 — Trait no convergence | §6 | 🟢 |
| R40 | Anderson 2003, Laurent 2015, Roels 2016 — State convergence | §6 | 🟢 |
| R41 | Butler & Randall 2013 — Co-regulation ↔ co-dysregulation | §6 | 🟢 |
| R42 | Eysenck — Cortical arousal theory | §6 | 🟢 |
| R43 | Wilson 2018 — HRV synchrony + inflammation | §6 | 🟢 |
| R44 | Nettle 2006 — Personality trade-offs | §6 | 🟢 |
| R45 | PNAS 2015 — OXT-OXTR positive selection | §6 | 🟢 |
| R46 | Vukasovic 2015 — Big Five heritability | §6 | 🟢 |
| R47 | O'Connor 2022, 2023 — Grief as learning, habit vs. declarative | §7 | 🟢 |
| R48 | Harrison et al. 2022 — IOS predicts PGD severity | §7 | 🟢 |
| R49 | Boerner et al. 2011 — Adaptive/maladaptive dependency | §7 | 🟢 |
| R50 | Robinaugh et al. 2021 — Opioid withdrawal model of grief | §7 | 🟢 |
| R51 | Pope & O'Brien 2021 — Identity disruption in grief | §7 | 🟢 |
| R52 | Bonanno et al. 2002, 2019, 2020 — 4 grief trajectories | §7 | 🟢 |
| R53 | Fernandez-Alcantara 2024 — Attachment anxiety → PGD | §7 | 🟢 |
| R54 | Djelantik et al. 2022 — Sudden loss → PGD | §7 | 🟢 |
| R55 | Arizmendi et al. 2020 — Reward system in PGD | §7 | 🟢 |
| R56 | Eisenberger 2003 — Social pain = physical pain circuits | §7 | 🟢 |
| R57 | Biddle et al. 2020 (JAMA) — Widowhood cognitive decline | §7 | 🟢 |
| R58 | Wegner 1985, 1991 — Transactive Memory Systems | §7, §10 | 🟢 |
| R59 | Harris et al. 2014 — Couples distributed cognition | §7, §10 | 🟢 |
| R60 | Barnier et al. 2018 — Groups > sum of parts | §10 | 🟢 |
| R61 | Zeki & Romaya 2008 (PLoS ONE) — Neural Correlates of Hate | §8 | 🟢 fMRI |
| R62 | Sherif 1961 — Robbers Cave Experiment | §8 | 🟢 |
| R63 | Zillmann 1971, Dutton & Aron 1974 — Excitation Transfer | §8 | 🟢 |
| R64 | Kilduff et al. 2010 — Rivalry relational | §8 | 🟢 |
| R65 | Baumeister 2001 — Bad stronger than good | §8 | 🟢 |
| R66 | Jecker & Landy 1969 — Ben Franklin Effect | §8 | 🟢 |
| R67 | Bhattacharya, Dunbar & Kaski 2022 — Inner circle turnover | §9 | 🟢 |
