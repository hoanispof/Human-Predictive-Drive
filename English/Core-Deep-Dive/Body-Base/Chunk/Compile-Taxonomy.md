...
scope: |
  ...
  Multi-stream compile: Content/Value/Entity/Behavior running SIMULTANEOUSLY.
  Feedback loop: bidirectional, PFC→Entity-Valence slow/costly, Entity-Valence→PFC fast/free.
  3 Hardware Constraints + 6 Trade-offs + 4 Pathways (retained from v2.0).
  Sleep = offline maintenance system (detail → Compile-Sleep.md v1.0).
...
purpose: |
  ...
  v2.0 NOT WRONG — patterns CORRECT. v3.0 = DEEPER layer: why 3 types exist.
...
related: |
  Compile-Sleep.md v1.0 — Sleep Maintenance detail (6 mechanisms)
  Body-Base.md v3.3 §4 — summary 3 Compile Types (references this file)
...
>
> The difference = which MODULATOR is dominant, NOT which engine runs.
...
>
> This file: ARCHITECTURE underneath the taxonomy. Why 3 types exist,
...
- §6 — 4 Compile Pathways: Same Output, Different Mechanism
- §7 — Feedback Loop: Bidirectional + Asymmetry
- §8 — 6 Trade-offs of Short Compilation
...
```
⭐ THIS FILE = ARCHITECTURE UNDERNEATH COMPILE TAXONOMY:
  ...
  v3.0 (this file):
    ...
    DEEPER layer: why 3 types exist, interact, overlap.

  Content IDENTICAL. Organizing principle DIFFERENT.
  v2.0 NOT WRONG — v3.0 = deeper understanding.
  ...
  FILE HIERARCHY:
    ...
    Chunk.md v3.0 §2 (4 compile mechanisms = foundation)
      ...
      → Compile-Sleep.md v1.0 (Sleep Maintenance detail)

    Body-Base.md v3.3 §4 (summary) → [this file] (detail)
    Collective-Body.md v2.1 §2 (4 pathways × 3 levels) → [this file] (pathway detail)
  ...
  WHAT'S NEW IN v3.0:
    ...
    ③ Multi-Stream Compile (§5) — Content/Value/Entity/Behavior simultaneously
...
⭐⭐⭐ CORE THESIS:
  ...
  ① compile_rate formula (Chunk.md v3.0 §2.2):
     ...
     Exposure = FOUNDATIONAL parameter. 4 remaining = QUALITY of exposure.
  ...
  ② 4 compile mechanisms (Chunk.md v3.0 §2.1) = 4 FORMS of exposure:
     ...
     Multi-modal = exposure across many channels. Sleep = replay exposure offline.
     NO mechanism exists OUTSIDE of exposure.

  ③ "PFC does NOT compile" (PFC-Operations.md v1.3):
     PFC directs attention, holds, imagines, domain-checks, changes environment.
     ALL = creating/directing exposure. NO role = compile.

  ④ "Imagination = internal body experience":
     Imagine a lemon → real saliva. Imagine a dog bite → real heartbeat.
     Same pathways, same molecules, different intensity.

  ⑤ Child forced to study (no trust) → STILL compiles knowledge:
     Trust "closed" but body EXPOSED to content → content compiles.
  ...
  ⑥ Hebbian = conserved across ALL species:
     ...
     Engine THE SAME. Only modulators differ.
...
```
🟢🟡 COMPILE ENGINE = THE ONLY COMPILE MECHANISM:
  ...
  │       ↓              ↓                  ↓          │
  │  Pattern contacts  Neurons fire      LTP/LTD       │
  ...
  Compile Engine always operates THE SAME WAY regardless of:
    Who provides exposure (mother, teacher, self, reality)
    Where exposure comes from (external, PFC imagination, spontaneous)
    Whether trust exists or not (trust only AMPLIFIES, doesn't change mechanism)
    Whether PFC is involved or not (PFC only DIRECTS, body still compiles)
  ...
  NO SOURCE TAG = EVIDENCE FOR 1 ENGINE (Chunk.md v3.0 §1.1):
    ...
    Chunks from experience = chunks from trust = chunks from PFC-directed.
    = EQUAL in the body.
    WHY: because all go through the SAME 1 engine (Hebbian LTP).
    Product IDENTICAL → mechanism IDENTICAL.
...
🟡🟢 ENTITY-VALENCE BIAS:
  ...
  2 WAYS TO MODULATE:
    ...
    ① AMPLIFY COMPILE RATE (quality):
       ...
       Mother says "fire is hot" = auditory + emotional contagion + coupling = RICH.
       Stranger says "fire is hot" = auditory only = thin.
       Trust adds nothing magical — trust makes THE SAME external event
       create RICHER exposure inside the body.
  ...
  Trust = VALENCE META-DIMENSION (Valence-Propagation.md v4.1 §2):
    ...
    Trust = 1 dimension WITHIN the valence profile per-entity.
    Trust = MULTIPLIER for the entire profile.

  → Detail Trust Reframe: §3.
...
```
🟡🟢 EXPOSURE COMES FROM 3 PARALLEL CHANNELS:
  ...
  EXPOSURE-EXTERNAL (body-input from reality):
    ...
    FEATURES: multi-modal RICHEST, domain-checked, compiles DEEPEST.
  ...
  EXPOSURE-DELIBERATE (PFC imagination/simulate):
    PFC actively creates internal exposure: imagine scenario, mentally rehearse,
    mental rehearsal, thought experiment, planning, deliberate recall.
    = Simulation-Engine.md v1.1: Constructive Simulation.
    Body RESPONDS to imagination FOR REAL (saliva, heartbeat, mild opioid).
    FEATURES: flexible, PFC-controlled, abstract domains (math, physics, philosophy).
    LIMITS: poorer multi-modal than Exposure-External, self-referencing risk, PFC budget.
    Human-dominant (PFC developed).

  EXPOSURE-SPONTANEOUS (automatic chunk fire):
    Compiled chunks fire automatically (NO PFC direction):
    ...
    FEATURES: cost ≈ 0, self-reinforcing (strong → fire → stronger).
    REQUIRES compiled chunks (Exposure-Spontaneous does NOT operate in newborns).
  ...
  ├─────────────────────┼──────────┼───────────┼─────────────┤
  │ 6-month infant      │ 90%      │  2%       │  8%         │
  │ playing             │          │           │             │
  │ Child forced to     │ 60%      │  5%       │ 35%         │
  │ study               │          │           │             │
  │ Student reading     │ 40%      │ 40%       │ 20%         │
  │ Expert thinking     │ 10%      │ 60%       │ 30%         │
  │ Dreaming (REM)      │  0%      │  0%       │100%         │
  └─────────────────────┴──────────┴───────────┴─────────────┘
  (% = ILLUSTRATIVE ESTIMATES, NOT precise measurements.)


  PFC × EXPOSURE CHANNELS:
    PFC CAN amplify External (listening attentively, practicing) + Deliberate (rehearsing, imagining).
    PFC CANNOT directly control Spontaneous (Background-Pattern fires automatically).
    PFC suppresses Spontaneous = PFC Modulation cost, CANNOT turn off Spontaneous source.
...
🟡🟢 PFC MODULATION = PFC OPERATIONS (PFC-Operations.md v1.3):
  ...
  SUPPRESS = PFC Inhibit
    ...
    REDUCES exposure for specific pattern.
  ...
  PFC = DIRECTOR, BODY = COMPILER:
    ...
    ❌ "PFC compile" = WRONG (PFC does not compile)
    ✅ "PFC-directed body compile" = ACCURATE

    PFC 5 roles (PFC-Operations.md v1.3):
      ① Direct attention    ② Hold in working memory
      ③ Imagine/simulate    ④ Domain-check
      ⑤ Change environment
    EVERY role = modulate exposure. NO role = compile.


  IMPORTANT — PFC MODULATION = ONLY MODULATOR WITH COST:
...
### §1.5 — Sleep Maintenance: Offline Maintenance System
...
🟡🟢 SLEEP MAINTENANCE = SLEEP OFFLINE MAINTENANCE SYSTEM:
  ...
  → DETAIL: Compile-Sleep.md v1.0 (6 mechanisms, architecture interaction,
...
```
🟢🟡 3 INDEPENDENT CONSTRAINTS ALL POINT TO SHORT COMPILATION:
  ...
  ① PFC ~4±1 SLOTS = PHYSICS LIMIT:
     ...
     PFC-Hold-Dimensions.md §2: 6 INDEPENDENT forces converge at ~4:
       ...
       False positive: >4 dimensions → signal-to-noise COLLAPSES
       Interference: >4 → oscillations collide
       Energy: maintaining each dimension = COSTS processing load
       Evolution: insufficient selection pressure for >4
  ...
  ② PROCESSING SPEED: PFC ~200ms+, AMYGDALA ~12ms:
     ...
     Evolution prioritizes FAST for survival.
     Compile short = faster = survival advantage.
  ...
  ③ CORTISOL COST: LONG CHAIN = ACTIVE LOCK → SUSTAINED CORTISOL:
     ...
     PFC holds chunks → Active Lock → cortisol.
     Cortisol-Baseline.md v2.2: sustained cortisol → neural wear.
     Long chain → PFC holds LONG → cortisol LONG → HIGH cost.
     Compile short → PFC holds LESS → cortisol LESS → biological economy.

     🟢 Cortisol + sustained cognitive load: Lupien et al. 2009


  ⭐ CONVERGENCE — 3 CONSTRAINTS POINTING THE SAME WAY:

    PFC capacity:    long chain = EXCEEDS limit
    Processing speed: long chain = TOO SLOW
    Energy cost:      long chain = TOO COSTLY
    → Brain IS FORCED to compile short = NOT a choice

    Analogy: human eyes have only 3 types of cones (RGB).
    The light spectrum = continuous. But survival only NEEDS 3 axes.
    → PFC ~4 slots: survival causation only needs ~4 dimensions → no more.
...
🟡 CONTRADICTION IN FRAMEWORK:
  ...
  SOURCE 1 — Chunk.md v3.0 §2.3 (OLD text, now resolved → AMPLIFIER):
    "TRUST = GATE FOR EXTERNAL INSTALL"
    → Language "gate" = BINARY (open/closed).

  SOURCE 2 — Entity-Access.md v1.2 §2:
    "gradient Level 0-5 per-entity"
    → Language "gradient" = CONTINUOUS (0→1→2→3→4→5).

  SOURCE 3 — Child forced to study:
    Trust "closed" (child doesn't believe studying is good).
    BUT content still compiles through forced exposure.
    → "Gate" = wrong for content compile.

  → "GATE" and "GRADIENT" contradict.
  → "GATE" and "content compile without trust" contradict.
```
...
🟡 TRUST = AMPLIFIER (GRADIENT), NOT A GATE (BINARY):
  ...
  │ (acquaintance) │ compensating exposure                 │
  ├────────────────┼──────────────────────────────────────┤
  │ Level 2-3      │ Multiplier moderate → normal         │
  │ (friend)       │ compile rate                          │
  ├────────────────┼──────────────────────────────────────┤
  │ Level 4        │ Multiplier high → compiles with      │
  │ (partner)      │ fewer exposures                       │
  ├────────────────┼──────────────────────────────────────┤
  │ Level 5        │ Multiplier MAX → 1-2 exposure         │
  │ (self/child)   │ sessions = deep compile               │
  └────────────────┴──────────────────────────────────────┘

  "Gate" = limit case OF amplifier when multiplier → 0.
  Gate behavior CORRECT at Level 0. But NOT correct for the full spectrum.
  "Amplifier" encompasses both gate behavior + gradient behavior.

  VALIDATION EXAMPLES:
    Mother says "go study" → trust Level 4-5 → few repetitions already compile [study → good]
    Stranger says "should study field A" → trust Level 0-1 → need to hear MANY people say it
    Close friend says "try jazz music" → trust Level 2-3 → a few times + experience
...
⭐ IMPORTANT DISTINCTION:
  ...
  ├────────────────────┼────────────────────┼──────────────────────┤
  │ CONTENT compile    │ Does NOT amplify   │ Child forced → still  │
  │ (knowledge, skill) │ (exposure alone    │ compiles math.        │
  │                    │ = sufficient for   │ Trust in mother       │
  │                    │ content)           │ not needed.           │
  ├────────────────────┼────────────────────┼──────────────────────┤
  │ VALUE compile      │ AMPLIFIES          │ Mother says "studying │
  │ ([X → good/bad])   │ (trust = multiplier│ is good" → trust →   │
  │                    │ for value install) │ compile [study →      │
  │                    │                    │ good] FAST.           │
  ├────────────────────┼────────────────────┼──────────────────────┤
  │ ENTITY compile     │ IS the trust weight│ Trust weight itself   │
  │ (trust itself)     │ being compiled     │ = product of compile  │
  │                    │                    │ from direct           │
  │                    │                    │ experience.           │
  └────────────────────┴────────────────────┴──────────────────────┘

  ⭐ "GOOD AT IT BUT HATES STUDYING" = ARCHITECTURE PREDICTION:
    Content stream: compile successful (exposure → knowledge) ✓
...
🟡🟢 EXPERIENCE COMPILE — DIRECT EXPOSURE → HEBBIAN → COMPILE:
  ...
  MODULATOR CONFIGURATION:
    ...
    PFC Modulation: Minimal (body direct, PFC not needed)
  ...
  EXAMPLES:
    [giving → recipient happy → warmth]  → compiled [giving → warmth]
    [touch fire → hot → pain]            → compiled [fire → avoid]
    [eat well → opioid]                  → compiled [this restaurant → good]

  FEATURES:
    ✅ STRONGEST: multi-modal (see + hear + touch + emotion → wire deep)
    ✅ FASTEST: no PFC deliberation needed, amygdala ~12ms
...
🟡 TRUST COMPILE — ENTITY-VALENCE AMPLIFIES VALUE INSTALL:
  ...
  MODULATOR CONFIGURATION:
    ...
    PFC Modulation: Low (choosing whom to trust + post-hoc confabulation)

  = "Engine + auto-turbo from Entity-Valence."

  MECHANISM:
    Trust source → body receives input → compile [action→result] SHORT.
    Long chain RESIDES IN collective (Level 2), NOT in individual (Level 1).

  EXAMPLES:
    Child: trusts mother → compile [study → good]
      Collective holds: [study → university → job → salary → body-base feed]
    Language: trusts environment → compile [word → meaning]
      Collective holds: [grammar system + vocabulary + pragmatics]

  5 FORMS OF TRUST SOURCE:
    ① Mother / caregiver: FOUNDATIONAL trust (Level 4-5)
    ② Teacher: DELEGATED trust (trust inherited through mother)
  ...
  FEATURES:
    ✅ FAST: bypasses chain verification through trust
    ✅ WIDE: access collective knowledge without needing personal experience
    ❌ COLLECTIVE-DEPENDENT: chain breaks → individual bears the cost
    ❌ EASILY EXPLOITED: trust wrong source → compile wrong

  ⭐ TRUST COMPILE = MOST SOCIAL BEHAVIOR:
    Language, ethics, social skills, world knowledge, religious beliefs,
    political views, career expectations = ALL Trust Compile installed.
...
🟡🟢 EXPERTISE COMPILE — PFC-DIRECTED SUSTAINED EXPOSURE:
  ...
  MECHANISM:
    Experience MANY TIMES → chunks → meta-chunks → pyramidal compression.
    PFC DIRECTS attention over many years → unconscious compiles DEEP.

  EXAMPLES:
    Expert chess: 50,000+ patterns → "feels" the position
    Physician 20 years: "sees the patient and knows immediately"
    Einstein: decades → E=mc² = 1 meta-chunk (inside = decades compiled)

  PFC DIRECTS through 5 ways (NONE of them is compile):
  ...
  External tools EXTEND (Collective-Body.md v2.1 §3.3):
    PFC hold 4 → write down → PFC freed → hold 4 more → STACK
    = "Individual Level 2": paper, computers, databases, AI

  FEATURES:
    ✅ MOST ACCURATE: calibrated through domain feedback
    ❌ SLOWEST: needs YEARS in 1 domain (Chase & Simon 1973: ~10 years)
    ❌ DOMAIN-SPECIFIC: Einstein = expert physicist + novice cook

  ~5% of behavior = when individual operates WITHIN expert domain.
...
🟡 COMPARISON TABLE — 3 CONFIGURATIONS OF THE SAME 1 ENGINE:
  ...
  │ (overlap)        │          │ (amplify │ trusted      │ (select    │
  │                  │          │ value)   │ source       │ trust)     │
  ├──────────────────┼──────────┼──────────┼──────────────┼────────────┤
  ...
  ⚠️ IMPORTANT NOTE:
    ~90% + ~5% + Trust = DO NOT add to 100% due to overlapping.
    Experience/Trust boundary is NOT binary → SPECTRUM.
    Example: [eat well → warmth] = Experience (direct) + Trust (cultural "delicious").
    Most real-world compile = MIX of multiple modulators.

  WHY KEEPING 3 LABELS IS STILL USEFUL:
    "Experience" = shorthand: engine dominant, direct exposure.
    "Trust" = shorthand: Entity-Valence dominant.
    "Expertise" = shorthand: PFC dominant.
    = Labels for DOMINANT configuration, not separate mechanisms.
    = Like "walking / running / mountain climbing" = useful labels despite same mechanism.
...
⭐⭐ EVERY SITUATION = MULTIPLE COMPILE STREAMS RUNNING SIMULTANEOUSLY:
  ...
  │               │ Motor-association   │ (Chunk.md §2.4)    │
...
🟡 TEST CASE — CHILD FORCED BY MOTHER TO STUDY MATH:
  ...
  ① Content:  [2+2=4], [multiplication], [formulas]
     → Compile Engine through Exposure-External (sitting reading) → compile ✓
     → Trust does NOT affect (content = direct exposure)

  ② Value:    [study → fear of scolding → avoid scolding → "must do"]
     → Compile Engine + context: threat direction → compile ✓ but AVOIDANCE
     → Trust for "study = good" does NOT compile ✗ (amplifier ≈ 0)

  ③ Entity:   [mother + forcing] → Entity-Valence update
     → [mother] structural valence: mixed (love + coercion)
     → compile ✓ (direct experience)

  ④ Behavior: [follow mother → sit down → open book]
     → Motor-association compile: avoidance-driven habit → compile ✓

  = 4 INDEPENDENT streams. 3/4 compile. 1/4 does not (value).
  = "Good at it but hates studying" = ①✓ ②✗.
  = "Fears mother but doesn't love her" = ④✓ ③mixed.
```

### §5.3 — Test case: Student who likes professor, likes the subject (4 streams positive)

```
🟡 TEST CASE — STUDENT WHO LIKES PROFESSOR, LIKES THE SUBJECT:

  ① Content:  [theories], [methods], [terminology]
     → Compile Engine through External (attending lectures) + Deliberate (thinking through assignments) → compile ✓

  ② Value:    [this subject = interesting → approach]
     → Trust amplifies: Professor says "this part is important" → body amplifies → compile ✓

  ③ Entity:   [professor] → Entity-Valence STRENGTHENS positive
     → Trust grows → future value amplification GROWS → compile ✓

  ④ Behavior: [arrive on time → sit in front → ask questions]
...
### §5.4 — Implication: Educational intervention needs to know which stream is failing

```
🟡 IMPLICATION:

  Educational intervention NEEDS to know which stream is failing:
    Content ✗: need more exposure (time, practice)
    Value ✗: need to build trust FIRST (entity-valence toward teacher/subject)
    Entity ✗: need to fix relationship (teacher-student dynamic)
    Behavior ✗: need to change direction (approach, not avoidance)

  Wrong stream → wrong intervention → "try harder" without effectiveness.
...
```
🟡 4 PATHWAYS — TEST CASE: "ALL ARE STUDENTS GOING TO STUDY MATH":

  IN REALITY — 4 children, 4 DIFFERENT compile pathways:


  ① HARDWARE FIT (Experience Compile — direct short):

     [math → brain fire → Goldilocks zone → VTA → opioid]
     = "Finds it intrinsically interesting." Level 1 direct.
     ...
     → VTA fire → dopamine → body pursues → opioid
     → = Hardware fit, NOT "innate talent"


  ② TRUST + MODERATE FIT (Trust + Experience — compound):

     Parents say "studying is important" → trust → compile [study → good]
     + Self-study → fits hardware → moderate cost → status
     = Level 1: [study → mother praises + grades + status → body warmth] (2 nodes)
     = Level 2: parents hold long chain [study → future → body-base]
     ...
     → = Most common? Many students = MIX pathway ①②


  ③ SOCIAL DEFAULT (Trust Compile — installed pure):

     "Sees everyone going to school → of course I go too"
     ...
     → "Compile" does not even NEED strong body experience
     → Only needs social environment → body auto-conforms


  ④ THREAT AVOIDANCE (Experience Compile — direct short):

     [not studying → mother scolds / gets punished → threat → study to avoid]
  ...
  ⭐ CONVERGENCE — 4 PATHWAYS, 1 OUTPUT:
     ...
     4 DIFFERENT pathways → same output: "going to school"
     ALL compile SHORT at Level 1 (1-2 nodes)
     Long chain resides at Level 2 (collective)
     → Same engine. Different modulator configurations. Different direction.


  PRACTICAL VALUE OF 4 PATHWAYS:

    ① Understanding why SAME behavior = DIFFERENT mechanism
       → Educational intervention NEEDS to know which pathway the student follows
    ② Pathway ④ (threat) = compiles well but HIGH cortisol cost
    ③ Pathway ①② = more sustainable (approach direction)
    ④ GOOD education = creates conditions for pathway ①② instead of ④

  🟡 REWARD IMPLICATION (Reward-Signal-Architecture.md v2.1 §8.4):
    4 pathways → different Precondition-5 tags → different reward capacity in adults:
...
```
⭐⭐ COMPONENTS ARE NOT INDEPENDENT — BIDIRECTIONAL INTERACTION:
  ...
  │                           │ Example: hates teacher → body auto-rejects →         │
  │                           │ PFC must suppress → PFC exhausted → body wins        │
  ├───────────────────────────┼──────────────────────────────────────────────────────┤
  ...
  │                           │ Example: stomach ache → PFC wakes → engage           │
  │                           │ problem-solving                                      │
  ├───────────────────────────┼──────────────────────────────────────────────────────┤
  ...
  ├───────────────────────────┼──────────────────────────────────────────────────────┤
  │ Entity-Valence → Engine   │ Entity-Valence amplifies compile RATE from that      │
  │                           │ entity. Trust = multiplier. Same exposure → compile  │
  │                           │ STRONGER.                                            │
  ├───────────────────────────┼──────────────────────────────────────────────────────┤
  │ Engine → Entity-Valence   │ Compiled chunks update Entity-Valence profile.       │
  │                           │ Example: newly learns teacher is skilled →           │
  │                           │ [teacher → positive] grows                           │
  ├───────────────────────────┼──────────────────────────────────────────────────────┤
...
⭐⭐ ASYMMETRY EXPLAINS MANY PHENOMENA:
  ...
  │ Cost     │ HIGH (continuous PFC hold │ LOW (compiled =           │
  │          │ + create exposure)        │ cost ≈ 0)                 │
  │ Path     │ INDIRECT (PFC→Exposure→   │ DIRECT (Entity-Valence    │
  │          │ Engine→Entity-Valence)    │ → PFC)                    │
  │ Sustain  │ Durable IF compile        │ Very durable (compiled)   │
  │          │ completes                 │                           │
  └──────────┴───────────────────────────┴───────────────────────────┘

  ANALOGY:
    Entity-Valence = "gravity" — always pulls toward compiled direction.
    PFC = "rocket" — CAN escape, but USES FUEL.
    Fuel (PFC budget) = FINITE → must reach orbit (compile) before fuel runs out.
    If compile completes → flies free. If fuel runs out first → falls back.

  IMPLICATION:
    → "Knowing but not doing" = PFC rocket out of fuel, Entity-Valence pulls back.
    → Therapy takes months = PFC→Entity-Valence indirect path.
    → Forced learning unsustainable = PFC fights Entity-Valence continuously, PFC exhausted.
...
🟡 REVERSE — "HATE THE TEACHER → AUTOMATIC REJECTION":
  ...
  Step 2: Teacher speaks → Entity-Valence ATTENUATES exposure
          Body auto-filters: reduce attention, reduce receptivity.
  ...
  Step 4: PFC fatigue → releases → Entity-Valence reasserts
  ...
  Step 6: BREAK THE LOOP — MUST inject positive Exposure-External:
          ...
          → BUT: old Entity-Valence NEVER deleted, only competitive re-linking (Chunk.md §2.5)
...
```
🟡 SHORT COMPILATION = DOMINANT MODE WITH 6 CLEAR TRADE-OFFS:


  T1 — FAST BUT EASILY DECEIVED (trust wrong source):

    Short compilation + trust = bypasses verification.
    HIGH trust amplifier → body acts on trust without verifying.
    → If source WRONG → compile WRONG → body acts on wrong pattern.

    Example:
      Parents: "women don't need higher education" → trust → compile DEEP
        → Age 30: body compiled [study = unnecessary] → HARD to change
      Cult: installs chain through trust → compile DEEP → weaponized trust

    Root cause: HIGH trust amplifier = bypasses verification.
    Trust CORRECT → compile correct → economical. Trust WRONG → costly.
  ...
  T2 — PFC CANNOT SELF-CORRECT (confabulation):
    ...
    PFC explains post-hoc → label ≠ mechanism → fixes WRONG PLACE → loop extends.

    Example:
      "Why did I marry this person?" (marriage stress)
        PFC: "was deceived" / REAL: body compiled [partner → warmth] from experience
        Context changes → pattern DOES NOT MATCH → confused

    ⭐ Trade-off IS ONLY REVEALED when context changes or chain breaks.
    When everything is smooth → this trade-off is INVISIBLE.
  ...
  T3 — COMPILED PATTERN MORE DURABLE THAN CONTEXT (lag):
    ...
    Example:
      HTML code compiled DEEP → HTML obsolete → body STILL fires "I'm good at HTML"
      Break-up → body STILL fires [evening → call partner] → Chunk-Miss

    Root cause: chunks have NO active "unwire" mechanism.
    "Breaking old habits" = NEW chunk compiled STRONG ENOUGH → SUPPRESSES old chunk.


  T4 — "WHY DID I DO THAT?" = NO SINGLE CORRECT ANSWER:

    Body compiles compound (many sources), PFC does NOT tag source → confabulation.

    Example:
      "Why do I want to have children?"
        PFC: "instinct" / REAL: hormone + collective + Valence-Structural
        + identity + meaning = COMPOUND → PFC selects 1 → MISSES compound
  ...
  T5 — COLLECTIVE-DEPENDENT (collective breaks → individual bears the cost):
    ...
    Example:
      AI disruption: [field X → job X] BREAKS → "why am I studying?"
        Chain breaks at collective, NOT the individual's fault.
        PFC usually blames SELF (because PFC only sees Level 1).

    Root cause: Trust Compile = individual does NOT hold the chain → does NOT know where it breaks.
  ...
  T6 — "KNOWING BUT NOT DOING" (PFC logic ≠ compiled pattern):
    ...
    Example:
      "Knows should exercise" → body: compiled [morning → sleep → comfortable]
        PFC chain = TRUST (installed). Body = EXPERIENCE (direct).
        EXPERIENCE wins (multi-modal, direct > verbal install).

    Root cause: 2 Compile Types compete. Experience > Trust in most cases.
    ...
    → Example: want to exercise → ACTUALLY EXERCISE → body compiles [exercise → warmth] → override.


  ⭐ 6 TRADE-OFFS = NOT A BUG, BUT A FEATURE:

    Short compilation + trust = OPTIMAL for survival environment:
      Fast (ms, not days). Wide (access collective). Cheap (low cortisol).

    Trade-offs = PRICE OF OPTIMIZATION:
      T1: fast → easily deceived
      T2: auto → hard to self-correct
      T3: durable → lag when context changes
      T4: compound → PFC confused
      T5: dependent → collective breaks = burden
...
🟡 EXPERIENCE × TRUST INTERACTIONS:
  ...
  ⭐ SYNERGY — when Experience confirms Trust:
    ...
    Trust installs [study → good] + Experience confirms [study → mother praises → warmth]
    = COMPOUND STRONG. Both configurations point same direction → pattern extremely durable.
    = "Educated + enjoy" case. Ideal.
  ...
  ⭐ TRUST OVERRIDE EXPERIENCE — rare but occurs:

    Condition: Trust installs EXTREMELY DEEP + Experience compiles SHALLOW.
    Example: Strong religious prohibition (trust deep + community reinforce + repeated)
      → Body compiled [alcohol → sin] EXTREMELY DEEP over MANY YEARS
      → Direct experience [alcohol → fun] = 1 time → SHALLOW → Trust wins.

    → Trust overriding Experience REQUIRES: trust deep + repetition + emotional reinforcement.
    → = Why education + culture CAN work, but needs TIME + INTENSITY.


  ⭐ OVERLAPPING — blurred boundary:

    [eat well → warmth] = Experience (direct) + Trust (cultural "delicious").
...
🟡 WHEN COLLECTIVE CHAIN BREAKS — BODY DETECTS BEFORE PFC:
  ...
  ② LINK BREAKS:
     ...
     Trust collapses → entire chain AFFECTED
     "Studying is useless" = chain broken → valence of "study" FLIPS negative.

  ③ COMPOUND:
     Cost RISING + link BROKEN + trust COLLAPSE
     = Body-Feedback-Mechanism.md v2.1 §4 COMPOUND
     → PFC wakes STRONGLY → "why study" = PFC RE-EVALUATES

  ⭐ KEY INSIGHT:
    "Why study" is NOT PFC spontaneously thinking philosophically.
    = Collective chain BREAKS → body DETECTS → PFC engages.
    Individual does NOT know WHERE it broke (because does NOT hold the chain).
...
### §9.3 — Expertise × External Tools: Individual Level 2
...
🟡 EXPERTISE COMPILE EXTENDS THROUGH EXTERNAL TOOLS:
  ...
  EINSTEIN MODEL:
    ...
    ① WRITE DOWN = external memory
       PFC hold 4 → write down → PFC freed → hold 4 more
    ② TRUST PAST SELF
       "Formula X I have verified" → trust tag → build on top
    ③ ITERATIVE RECURSIVE
       Verify A → trust A → build B → verify B → trust B → stack
    ④ SLEEP CONSOLIDATE between steps

  HIERARCHY:
    PFC 4±1 (hardware limit)
    → ×4 pyramidal compression (unconscious — Valence-Propagation.md v4.1 §5b)
    → ×∞ external tools (paper, computers, database, AI)

  → External tools = "Individual Level 2" (Collective-Body.md v2.1 §3.3)
  → EXTEND individual compile BEYOND hardware limit

  🟢 Extended mind thesis: Clark & Chalmers 1998
  🟢 Epistemic actions: Kirsh & Maglio 1994
  🟡 "Individual Level 2": framework synthesis
```
...
```
⭐ MODULATORS ADDED THROUGH EVOLUTION — ENGINE CONSERVED:
  ...
  PATTERN:
    Compile Engine (Hebbian) = CONSERVED from Aplysia → human.
    Evolution ADDS modulators + Exposure Channels, does NOT change the engine.
    EACH STEP = ADD MODULATOR, does NOT change ENGINE.
  ...
  IMPLICATIONS:
    ...
    ③ Compilable Architecture (Body-Base.md v3.3 §2):
       ...
       BUT: no pre-wired content → needs 15-20 years to calibrate.
...
⭐ 8 EDGE CASES — ALL CONSISTENT WITH ARCHITECTURE:
  ...
  ① TRAUMA (emotional peak):
     Compile Engine: 1 exposure × peak_valence EXTREMELY HIGH → compile immediate.
     Exposure: External. Entity-Valence: [dog] flips negative immediately. PFC: BYPASSED (~12ms).
  ...
  ④ CHILD FORCED TO STUDY (multi-stream):
     ...
     → Multi-stream: content STILL compiles despite low trust.
  ...
  ⑦ EXPERT BLIND SPOT:
     Expert: decades PFC-compiled → pyramidal compression → "sees 4."
  ...
  Architecture explains EVERYTHING that 3-type taxonomy explains,
  PLUS additional cases that taxonomy does NOT explain clearly:
    → "Good at it but hates studying" (multi-stream: content ✓ value ✗)
...
🟡 FRAMEWORK SYNTHESIS (logic consistent, built on established):
  ...
  Multi-stream compile (4 streams):
    ...
    TESTABLE: "good at it but hates studying" = content ✓ value ✗.
...
BODY-BASE + BODY-FEEDBACK:
  ...
  Cortisol-Baseline.md v2.2 — sustained cortisol → neural wear
  Why-Body-Knows.md v1.2 — 2-layer calibration
