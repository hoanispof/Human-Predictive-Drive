...
scope: |
  ...
  v1.0: Refined from drill H10 — adds direction component, genesis/decode split,
...
purpose: |
  ...
  This file adds a new axis: WHEN — 5 preconditions determine WHETHER signal fires.
  Completes the 4-sibling set: Mechanism → Label → Precondition → Signal Architecture.
dependencies:
  ...
  - Gap-Direction.md v2.0 — §3 "not knowing = no gap", §6.3 hierarchy
...
source_version: Vietnamese v1.0 (REFERENCE v1.0)
...
> **Same beautiful piece of music.**
> Person who has never heard anything like it → "too strange" (insufficient chunks to decode).
> Person hearing it for the first time → "this is great!" (new + just enough match).
> Person hearing it for the 1,000th time → "boring" (no delta remaining).
> Person forced to listen as a child → "dislike it even though I know it's good" (avoidance tag).
> Person already saturated with music → "feel nothing" (no active gap).
>
> Same stimulus. 5 people. 5 DIFFERENT responses.
> Each response = 1 DIFFERENT precondition failing.
>
...
│                 5 preconditions: Precondition-1 through Precondition-5 │
...
**This file DEFINES**:
- 5 preconditions for body-feedback signal to fire (Precondition-1 through Precondition-5)
...
🟡 This file formalizes "H10" — the hypothesis name from drill 03-Reward.md §3.
...
FRAMEWORK EVOLUTION:
  ...
  ④ Reward-Calibration.md → Goldilocks zone is dynamic (not fixed 40-70%)
...
THIS FILE: Refines + formalizes all above insights into the official reference.
```
...
> |---|---|---|
> | Precondition-1 | **Directed-Gap** | Is there an active gap? What direction does the gap point? |
> | Precondition-2 | **Chunk-Substrate** | Are there sufficient chunks to decode the stimulus? |
> | Precondition-3 | **Delta-Gate** | Is there sufficient change for VTA to detect? |
> | Precondition-4 | **Match-Range** | Does the match fall within a suitable range? |
> | Precondition-5 | **Compile-Gate** | Does the chunk tag permit the signal to fire in the right direction? |
>
...
but the conjunction claim "ALL 5 required" is a framework-level claim without direct empirical test.
...
EVALUATIVE (compiled patterns, learning-dependent):
  → Precondition-1 through Precondition-5 ALL required
  → Examples: appreciating music, solving a problem, appreciating Van Gogh
...
DIRECT-STATE (hardware signals, body-need-driven):
  ...
  → Precondition-2 minimal: hardware paths from birth (no compiled chunks needed)
  ...
  → Details on type-split per precondition: Reward-Signal-Architecture §1.3
...
CLINICAL IMPLICATION:
  ...
  Losing BOTH = rare + devastating (advanced depression, severe trauma)
```
...
| Violated | Prediction | Observable |
|---|---|---|
| Precondition-1 | Full/satiated person offered food → no reward | Alliesthesia research (Cabanac 1971) 🟢 |
| Precondition-2 | Van Gogh shown to untrained viewer → confusion not reward | Art appreciation studies 🟢 |
...
Cases of "shouldn't reward but does" = ALL 5 actually met but observer hasn't recognized it.
...
```
"Not knowing" → Precondition-2 → Precondition-1 → Precondition-3 → Precondition-4 → Precondition-5 → signal fires

Layer 0 (GENESIS):   "Not knowing = no gap" — prerequisite for Precondition-2
Layer 1 (SUBSTRATE):  Precondition-2 Chunk-Substrate — Are there chunks to form a gap?
Layer 2 (GAP):        Precondition-1 Directed-Gap — Is the gap active + directional?
Layer 3 (DETECTION):  Precondition-3 Delta-Gate — Does VTA detect the delta?
Layer 4 (MATCH):      Precondition-4 Match-Range — Is the match in the suitable range?
Layer 5 (DIRECTION):  Precondition-5 Compile-Gate — Does the tag permit the right-direction signal?

⭐ Hierarchy does NOT change the "ALL 5 required" claim.
  → But it SHOWS which failure mode occurs FIRST.
...
> Body must have an **active gap** — an unresolved body-need OR a schema seeking
> upgrade — AND that gap must have **direction** (toward what kind of fill).
>
> Gap = an opening in the schema landscape where new information can fit.
> Direction = constraints on valid fill, determined by surrounding chunks.

**Former name**: "Schema Pending Status" (drill 03-Reward.md §3.2)
**Renamed because**: Former name missed (a) hardware-driven gaps and (b) the direction component.
...
- ALWAYS exist at some level (homeostatic regulation)
- Present from birth — no learning required
- Direction is inherent: hunger → food direction, thirst → water direction

```
Precondition-1a characteristics:
  ① Always available (homeostatic cycle)
  ② Direction = hardware-defined (specific body-need category)
  ③ Does not need Precondition-2 (chunks) — hardware path from birth
  ④ Intensity varies (satiated ↔ deprived)
  ⑤ → Primarily for DIRECT-STATE reward/dissonance
```
...
- Skill gap: knowing one could do better but hasn't yet
- Social gap: disconnection from expected social state

```
Precondition-1b characteristics:
  ① Requires Precondition-2 first (must have chunks before a gap can form — §2.5)
  ...
  ⑤ → Primarily for EVALUATIVE reward/dissonance
```
...
- A child who doesn't know guitar exists → gap CANNOT FORM (→ §2.5)

### §2.4 — Direction is implicit
...
> Surrounding chunks DEFINE direction.

**Why direction matters for Precondition-1**:
- Gap without direction = not really a gap (vague unease, not actionable)
- Direction specifies WHAT TYPE of stimulus will fill the gap
- Direction determines what Precondition-4 (Match-Range) evaluates match AGAINST
- Without direction → Precondition-4 has no reference point → signal doesn't fire
...
Gap-Direction 4 properties (Gap-Direction.md §4):
  ...
  ③ Depth — signal strength (how much body "wants" the fill)
...
- If Precondition-2 fails at the genesis level → Precondition-1b CANNOT form
- Hardware Precondition-1a does NOT need Precondition-2 (body-needs exist from birth)
- This is "Layer 0" — prior to the entire precondition model
...
5 consequences (Gap-Direction.md §3.4):
  → Desire is CONSTRUCTED (not innate except for hardware Tier 1)
  → Education must BUILD chunks before presenting answers
  → Marketing = gap installation (create chunks → create gap → sell fill)
  → Therapy must MAP gap landscape (what gaps currently exist?)
...
|---|---|---|
| Satiated | Body-need filled → gap closed | "Don't want it" (no desire) |
| No active goal | Schema landscape stable | "Don't need anything" (contentment) |
| Empty scroll | Stimuli arrive, no gap matches | "Scrolling but nothing sticks" |
...
> Body must have **sufficient chunks** to (a) form a gap about the domain (genesis)
> AND (b) decode incoming stimulus for evaluation (decode).
>
> Chunks = compiled patterns from past experience (Chunk.md §2).
> Without chunks, the body cannot even RECOGNIZE a stimulus as relevant.

**Former name**: "Chunks Base Adequacy" (drill 03-Reward.md §3.3)
**Renamed because**: "Adequacy" is vague. "Substrate" = necessary foundation, clearer meaning.

### §3.2 — Precondition-2a: Genesis failure ("not knowing = no gap")

🟡 From Gap-Direction.md §3 — LAYER 0, before the precondition model itself:

> **Precondition-2a Genesis**: Chunks_related_to_X = ∅ → Gap CANNOT EXIST
>
> This is NOT "stimulus arrives and cannot be decoded" (= Precondition-2b).
> This is "a gap about X has NEVER formed" — prior even to Precondition-1.

**Precondition-2a examples**:
- A 3-year-old who doesn't know guitars exist → CANNOT have a gap "want to play guitar"
- A person who has never heard jazz → CANNOT have a gap "want to hear jazz"
- A community with no concept of "examinations" → CANNOT have a gap "want to pass an exam"

```
Precondition-2a vs Precondition-2b — key distinction:
  Precondition-2a = BEFORE stimulus (gap can't form → desire doesn't exist)
...
- Van Gogh shown to untrained viewer → chunks insufficient for art evaluation
  → "Don't understand" (confusion, not reward) — even though the painting is objectively great
- Atonal music to a pop music listener → chunks insufficient for harmonic evaluation
  → "Sounds chaotic" (confusion) — same music = masterpiece to trained listener
- Advanced mathematics to a beginner → chunks insufficient for pattern recognition
  → "Can't follow this" — same proof = "beautiful" to a mathematician
...
```
EVALUATIVE:
  Precondition-2 = REQUIRED — compiled chunks to evaluate pattern
  → Without chunks = confusion → no reward
...
DIRECT-STATE:
  ...
  → No compiled chunks needed — hardware decoding is sufficient
  → Infants reward from touch, sweetness from birth
```
...
⚠️ Precondition-2 and Precondition-5 DIFFER — both involve chunks but at DIFFERENT layers:
  ...
  Precondition-2 Chunk-Substrate:
    Question: Can you DECODE the stimulus? (existence of chunks)
    Layer: SUBSTRATE — the foundation
    Failure: confusion ("I don't understand")
    → Chunks EXIST or DON'T

  Precondition-5 Compile-Gate:
    Question: Do decoded chunks' tags PERMIT signal in the right direction?
    Layer: DIRECTION — which way the signal flows
    Failure: signal goes wrong direction ("understand but dislike")
    → Chunks exist BUT tagged approach/avoidance

  Example: Mathematics
    Precondition-2 fail: "I don't understand this math" (insufficient chunks to decode)
    Precondition-5 fail: "I understand the math but hate math" (chunks exist, tagged avoidance)
```
...
| Failure type | Mechanism | Subjective experience |
|---|---|---|
| Genesis (Precondition-2a) | No chunks about domain → gap impossible | "Didn't know that existed" |
| Decode (Precondition-2b) | Stimulus arrives, insufficient chunks to parse | "I don't understand" / confusion |
...
- Infant: minimal chunks → most stimuli = Precondition-2b fail → exploration is critical
- Child: chunks accumulating → Precondition-2b failures narrow → domains opening up
...
🟢 Research: Expertise studies (Chi et al. 1981) — experts chunk patterns novices cannot see.
...
> to exceed VTA habituation threshold, triggering a dopamine alert.
>
> Precondition-3 answers: "Is there anything NEW?" — detection mechanism, not value judgment.
> VTA dopamine = doorbell. Precondition-3 = whether the bell RINGS.

**Former name**: "Prediction-Delta Threshold" (drill 03-Reward.md §3.4)
**Renamed because**: "Delta-Gate" is more concise and emphasizes the gate property (open/close).
...
Threshold affected by:
  ...
  → Recent history (many recent deltas → threshold rises)
  → DRD4 variant (§4.5)
...
DIRECT-STATE:
  ...
  → Example: a hungry person eats → reward fires WITHOUT needing "surprise"
...
DRD4-7R ("novelty-seeking" variant):
  → Lower sensitivity → needs MORE delta for the same response
...
⚠️ DRD4 does NOT change the Precondition-3 gate itself.
...
Precondition-3 pass → Precondition-4 CHECK:
  → Delta detected → attention → NOW evaluate: where does match fall?
...
| Failure type | Mechanism | Subjective experience |
|---|---|---|
| Habituation | Same pattern repeated → VTA adapts | "Bored" (hedonic treadmill) |
...
**Former name**: "Goldilocks Zone Position" (drill 03-Reward.md §3.5)
**Renamed because**: (a) "Zone" missed the direction-match component. (b) "40-70%" is not fixed → "Range" = dynamic.
**Informal shorthand**: "Goldilocks zone" — used across the framework as an intuitive label for this precondition.

### §5.2 — Direction-match (not just magnitude)

🟡 From Gap-Direction.md v2.0 §6.3:

> Match WITH WHAT? → Match with **gap direction**.
>
> Gap has direction = f(surrounding chunks).
> Precondition-4 evaluates: does the stimulus match the DIRECTION + MAGNITUDE of the gap?
...
Examples:
  A composer searching for a melancholic melody (gap direction = melancholy)
  → Receives a moderately new melancholic melody → reward ✅ (direction + range match)
  → Receives an upbeat melody → no reward ❌ (direction mismatch)
  → Receives an intensely complex melancholic piece → confusion ❌ (range miss)
```
...
> Zone changes across 4 axes:

```
① PER-PERSON:
  COMT Val/Val → needs many shallow rewards → zone wide
  COMT Met/Met → needs fewer deep rewards → zone narrow
  DRD4-7R → higher threshold → zone shifted toward MORE novel
  (PFC-Hardware §3.4)

② PER-GAP-TYPE:
  Chunk-Shift → small delta is sufficient → zone wide
...
④ PER-TIME:
  Baseline shift from repeated reward → zone drifts
...
E₁ (Beginner):
  → Few chunks → zone WIDE (many things feel "new + just enough match")
  → Easy to reward, but low quality
  → Example: learning guitar, played first chord → reward!

E₂ (Intermediate):
  → Many chunks → zone NARROWS (needs to be more refined to feel "new enough")
  → Reward harder to trigger, but higher quality
  → Example: 5 years of guitar, needs new technique to find it "interesting"
...
E₃ (Expert):
  ...
  → Example: a 30-year musician, only fine nuances produce "wow"

Pattern: expertise NARROWS zone + SHIFTS toward subtlety
  → SAME stimulus: beginner rewards, expert is bored (or vice versa)
  → Van Gogh: beginner = Precondition-2 fail, art student = Precondition-4 zone match,
    expert = Precondition-4 too familiar
```
...
| Failure type | Mechanism | Subjective experience |
|---|---|---|
| Too alien | Match < lower bound → cannot evaluate | "Too strange, can't follow it" |
| Too familiar | Match > upper bound → habituated | "Too familiar, nothing new" |
| Direction mismatch | Stimulus matches wrong gap direction | "Not what I was looking for" |
| Zone shifted | Expertise/context changed zone → stimulus no longer fits | "Used to love this, now bored" |
...
> Chunks being activated must have an **appropriate association tag** that permits
...
>
> Precondition-5 answers: "Does the tag PERMIT this signal?" — direction gate, not substrate check.

**Former name**: "Chunk Association Tag" (drill 03-Reward.md §3.6)
**Renamed because**: (a) "Association Tag" describes the mechanism, not the function.
(b) "Compile-Gate" emphasizes: tag is compiled into the chunk → it gates signal direction.
...
⚠️ Precondition-5 = N/A is the ONLY precondition completely absent in Direct-State.
  (Precondition-3 is "uncertain", others are "simplified" — Precondition-5 is a clean N/A.)
```
...
│    "everyone does    │                      │ PFC confabulation ~30%      │
│    it")              │                      │                             │
├──────────────────────┼──────────────────────┼────────────────────────────┤
...
⭐ Precondition-5 tag at childhood compile → PERSISTS → constrains ADULT reward profile.
  "Competent but aversive" = Precondition-5 fail (understand but avoidance-tagged).
  Pathway ④ = explains why many people are "successful but unhappy."
```

### §6.5 — Re-association 3 paths

🟡 Precondition-5 tag CAN change, but costs resources (Cortisol-Baseline.md §7.6):
...
3 Re-association Paths:
  ...
  Path 1: NEW POSITIVE CONTEXT (months → years)
    ...
    → Example: math-phobic adult finds a great teacher → years → "math is ok"

  Path 2: NOVELTY HIJACK (weeks → months)
    → Completely new angle/context for the same domain
    → Bypasses old chunks → builds new approach-tagged chunks
...
⭐ Key: re-association = OVERLAY, not deletion.
  ...
  New approach-tagged chunks are added.
...
Summary distinction:
  ...
  Precondition-5 fail: "I understand the problem but hate this subject" (chunks exist, tagged avoid)
...
| Failure type | Mechanism | Subjective experience |
|---|---|---|
| Threat-tagged | Chunks compiled under fear → avoidance direction | "Dislike it even knowing it's good" |
| Social-default | Chunks compiled via consensus → relief only | "Relieved when done, not engaged" |
| Mixed-tag | Some chunks approach + some avoidance → ambivalent | "Both like and afraid" |
...
> Preconditions operate as an AND-gate — if ANY 1 is missing → signal doesn't fire at full magnitude.
...
⚠️ "Attenuated" ≠ "absent":
  → Near-miss cases produce MILD signal ("almost good", "mildly interesting")
...
DEPENDENCY 1: Precondition-2 → Precondition-1 (schema-level)
  ...
  "Not knowing = no gap" → Precondition-2a fail = Precondition-1b impossible.
...
DEPENDENCY 2: Precondition-3 → Precondition-4 (sequential)
  ...
  Precondition-3 fail → PFC doesn't attend → Precondition-4 is not checked → Precondition-4 irrelevant.
  ⚠️ Stimulus may be a PERFECT match (Precondition-4 pass) but Precondition-3 blocks detection.
...
|---|---|---|---|
| Precondition-1 | Precondition-2–Precondition-5 ✅ | No desire → signal irrelevant | "Don't want it," "Already have enough" |
| Precondition-2 | Precondition-1, Precondition-3–Precondition-5 ✅ | Confusion → cannot evaluate | "Don't understand," "Too strange" |
| Precondition-3 | Precondition-1, Precondition-2, Precondition-4, Precondition-5 ✅ | Unnoticed → attention not triggered | "Normal," "Used to it" |
| Precondition-4 | Precondition-1–Precondition-3, Precondition-5 ✅ | Wrong range → too alien or too familiar | "Too hard" / "Too easy" |
| Precondition-5 | Precondition-1–Precondition-4 ✅ | Wrong direction → aversion despite understanding | "Understand it but hate it" |
...
⚠️ Compound dynamics = Precondition-1–Precondition-5 checked MULTIPLE TIMES per event.
  Example: scammed by a close friend → Shift (betrayal) + Miss (trust lost) + Gap (how to repair?)
...
Precondition-3 + Precondition-5 co-fail:
  ...
  → Example: forced daily work by someone who has avoidance-tagged the work
...
Precondition-1 + Precondition-2 + Precondition-3 co-fail:
  ...
  → Nothing wanted, nothing understood, nothing new → existential emptiness
```
...
Precondition-4: ⚠️ NARROW — few existing chunks = most inputs outside match range
    ...
    → Complex stimuli → Precondition-2 fails before Precondition-4 is reached
...
Precondition-5: ⚠️ ENTIRELY DEPENDENT ON CAREGIVING
    ...
    → ⭐ Precondition-5 in infancy = CAREGIVER'S responsibility, not the child's
```
...
Precondition-1: Precondition-1b DEVELOPING — schema gaps form as chunk network grows
    → "Why?" curiosity gaps emerge ~age 3-4
...
Precondition-5: ACCUMULATED BIOGRAPHICAL TAGS
    → Childhood compilations persist: "competent but aversive" patterns established
...
Precondition-4: CAN RE-WIDEN — if person enters a new domain
...
BURNOUT FORMULA (compound):
  ...
  → "Don't want to, don't enjoy it, don't know what I want" = triple precondition failure
```
...
### §9.1 — Precondition-1 through Precondition-5 apply to dissonance, not just reward

🟡 The precondition model is a **body-feedback** model, not only a reward model:

> Body-feedback signal = reward OR dissonance.
> Precondition-1 through Precondition-5 determine WHETHER signal fires — for BOTH directions.
...
Examples:
  Musician (approach-tagged) makes a mistake → mild dissonance, quickly resolved
  Math-phobic adult (avoidance-tagged) makes a math error → intense dissonance, slow to resolve
...
DISSONANCE → REWARD: SLOW
  ...
  → Opioid RE-activation requires FULL precondition re-check (all 5 preconditions)
...
Evolutionary logic: missing a threat = death (max cost), false alarm = low cost.
...
Evaluative gates can:
  ...
  ④ CONFLICT — "pain from someone loved" (mismatch between signal directions)
...
→ Details of mechanism: Dissonance-Signal-Architecture.md §3
```
...
| ALL | Conjunction | 🟡 MODERATE | Each Precondition individually 🟢, but "ALL 5 required" = framework-level claim. |
...
Q1: Precondition-3 × Precondition-4 interaction boundary
  ...
  → Currently: framework says NO (Precondition-4 still required)
...
Q2: Precondition-5 re-association speed
  ...
  → If AI CAN accelerate → educational implications massive

Q3: Direct-State Precondition-3 mechanism
  Does Direct-State bypass VTA entirely or use a different threshold?
...
Q4: Dissonance precondition thresholds
  Are dissonance Precondition-1–Precondition-5 thresholds LOWER than reward thresholds?
...
Q5: Compound dynamics × precondition cycling
  When a compound event triggers Shift + Miss + Gap simultaneously,
...
🟡 If the Body-Feedback Precondition model is correct:

```
PREDICTION 1: "Should reward but doesn't" cases
  → ALWAYS traceable to a specific Precondition-failure
  → If ANY case found where ALL 5 are met but NO reward → model falsified
...
| File | Relevant section | What was refined |
|---|---|---|
| Gap-Direction.md v2.0 | §3, §6.3 | Precondition-1 direction + "not knowing = no gap" + hierarchy |
...
| File | Relevant section | What it informed |
|---|---|---|
