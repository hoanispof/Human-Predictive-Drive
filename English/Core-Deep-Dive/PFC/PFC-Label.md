...
source_version: Vietnamese v1.1 (2026-05-25)
...
scope: |
  ...
  Does NOT explain mechanism (source files do that).
  ONLY formalizes: WHICH LABEL, WHAT IT MEANS, WHEN TO USE IT.
purpose: |
  PFC vocabulary appears in 78+ files but has NEVER been formalized.
  ...
  Overlap at Body-Feedback-Label §8/§9C = bridge zone (cross-reference, no duplication).
dependencies:
...
> **But the same concept → 3-5 different names → confusion.**
>
> **"Fresh" / "PFC-Fresh" / "draft" / "operation" = same concept area, DIFFERENT level.**
> **"PFC" = Observer? Lawyer? Director? Universal Resource? = context-dependent.**
>
> **This file: VOCABULARY REFERENCE for all PFC-related labels.**
> **Does NOT explain mechanism (source files do that).**
> **ONLY formalizes: WHICH LABEL, WHAT IT MEANS, WHEN TO USE IT.**
...
- §2 — PFC ROLE LABELS (5 roles by context)
...
⭐ PFC VOCABULARY = SCATTERED + INCONSISTENT:
  ...
  PROBLEM 1 — SAME CONCEPT, MULTIPLE NAMES:
    ...
    for the concept of PROCESSING THAT HAS NOT YET BEEN COMPILED.
    → Which file uses which name? Nobody knows.

  PROBLEM 2 — SAME TERM, DIFFERENT LEVEL:
    "Fresh" = STATE on the spectrum (position)
    "PFC operations" = ACTION (HOLD + SUPPRESS)
    "draft" = SPECIFIC action (one type of HOLD)
    → 3 different levels used interchangeably → confusion.
  ...
  PROBLEM 4 — PFC FILES ARE SCATTERED:
    ...
    → 9+ separate files about PFC → which vocabulary belongs where?
...
```
⭐ TIER SELECTION RULES (parallel to Body-Feedback-Label §1):

  Tier 3 (MOST SPECIFIC) > Tier 2 (DIRECTION) > Tier 1 (MOST GENERAL)

  → When you know SPECIFICALLY what PFC is doing → use Tier 3:
    "dlPFC HOLD new pattern → ① PFC draft cost"

  → When you know the DIRECTION but not the specific → use Tier 2:
    "Fresh processing" / "HOLD" / "SUPPRESS"

  → When you ONLY know PFC is involved → use Tier 1:
  ...
  AUDIENCE-APPROPRIATE:
    ...
    Learners / Research files:
      → Tier 2 primary: "Fresh processing", "HOLD", "suppress cost"
      → Precise enough, sufficient for grasping basic mechanism.

    General / overview:
      → Tier 1 when needed: "PFC is processing" / "PFC is consuming resources"
  ...
  4 LEVELS OF PFC VOCABULARY:
    ...
    │   Quality Controller                               │
    │                                                    │
    │ LEVEL 2 — STATES (position on spectrum):           │
    │   Compiled ←→ Fresh                                │
    │                                                    │
  ...
  ⚠️ DO NOT mix levels:
    ...
    ✓ "Fresh processing requires HOLD operation" (clearly distinguished)
...
```
⭐ PFC HAS 5 ROLES DEPENDING ON CONTEXT — USE THE CORRECT ROLE:

  ┌─────────────────────────┬──────────────────────────────────────────┐
  │ Role Label              │ Definition + when to use                 │
  ├─────────────────────────┼──────────────────────────────────────────┤
  │                         │                                          │
  │ ① PFC = Observer        │ PFC OBSERVES body-feedback output.       │
  │                         │ Reactive, NOT generative.                │
  │                         │ ~5% decisions, 95% processed             │
  │                         │ automatically by body.                   │
  │                         │ ⚠️ % = calibration anchor.               │
  │                         │ "Reads output, doesn't generate it."     │
  │                         │ Use: when emphasizing PFC does NOT       │
  │                         │ control — only observes.                 │
  ...
  │                         │                                          │
  │ ③ PFC = Director        │ PFC directs, does NOT compute directly.  │
  │   (Orchestrator)        │ Biases spreading activation. Selects     │
  │                         │ direction.                               │
  │                         │ "Director points the way, actors         │
  │                         │ perform on their own."                   │
  │                         │ Use: when emphasizing PFC only BIASES,   │
  │                         │ does not perform computation.            │
  │                         │ (Neural-Processing-Flow)                 │
  │                         │                                          │
  │ ④ PFC = Universal       │ PFC budget = FINITE, SHARED for          │
  │   Resource              │ EVERYTHING.                              │
  │                         │ Learning, Self-Pattern-Modeling,         │
  │                         │ decisions, suppress, social,             │
  │                         │ self-monitor, Imagine-Final.             │
  │                         │ Exhausted from work → Self-Pattern-      │
  │                         │ Modeling for child WEAKENED.             │
  │                         │ Use: when analyzing PFC budget           │
  ...
  │                         │ reality.                                 │
  │                         │ THE ONLY system that can check against   │
  │                         │ domain.                                  │
  │                         │ Use: when emphasizing the PFC            │
  │                         │ verification function.                   │
  │                         │ (Ask-AI v3.1 §6.1)                      │
  │                         │                                          │
  └─────────────────────────┴──────────────────────────────────────────┘

  ⚠️ 5 roles NOT CONTRADICTORY — same PFC, DIFFERENT PERSPECTIVE:
     Observer = perspective (what PFC reads)
     Lawyer = output quality (PFC justifies for whom)
     Director = mechanism (how PFC acts)
  ...
  ⚠️ DO NOT use "PFC = controller" or "PFC = boss":
     PFC does NOT control body. Body processes 95% automatically.
...
> Body-Feedback-Label v2.0 §8 = same labels on the body side.
> This section = BRIEF label reference.

```
⭐ THE TRUE AXIS OF PROCESSING = COMPILATION LEVEL:

  COMPILED ─────────────────────────────────────── FRESH
  (automatic)                                    (PFC-mediated)
  body-direct                                    deliberate
  cost ≈ 0                                      cost > 0
  "feels known"                                 "must think through"
  ...
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │ TIER 2  │ Fresh → Compiled        │ LEARNING: repetition + verified  │
  │ (trans.)│ (learning)              │ OK. = "Logic → feeling"          │
  │         │                         │ (observer).                      │
  ...
  ⚠️ "Compiled/Fresh" = MECHANISM AXIS. "Logic/Feeling" = OBSERVER LABELS.
     When analyzing mechanism: REQUIRED to use Compiled/Fresh.
     When describing to outsiders: "Logic/Feeling" OK (but know it is approximate).

  ⚠️ A spectrum, NOT binary:
     Each chunk sits SOMEWHERE on the spectrum.
     "Expert partially stuck" = compiled on familiar parts, fresh on new parts.
...
⭐ PFC HAS 2 OPERATIONS ON THE SPECTRUM:
  ...
  │ Tier    │ Label                   │ Meaning                          │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │ TIER 1  │ PFC operation           │ Most general. PFC is taking      │
  │         │                         │ action.                          │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │ TIER 2  │ HOLD (= PFC Amplify)    │ Amplifies new/weak pattern.      │
  │         │                         │ CAN compile → sustainable.       │
  │         │                         │ Cost: ① PFC draft.               │
  │         │                         │ Region: dlPFC, FEF.              │
  │         ├─────────────────────────┼──────────────────────────────────┤
  │         │ SUPPRESS (= PFC Inhibit)│ Blocks an already-compiled       │
  │         │                         │ pattern.                         │
  ...
  4 COMBINATIONS:
    ...
    ① Hold only       — Learning something new, no conflict. Cost: ①.
                         EASIEST.
    ② Hold + Suppress — Learning new WHILE blocking old. Cost: ① + ②.
                         DOUBLE.
                         3 outcomes: A (genuine shift), B (compiled
                         suppress), C (failure/burst).
                         (PFC-Operations.md §4)
    ③ Suppress only   — Blocking without replacing. Cost: ②. WORST CASE.
                         Wegner ironic process: rebound.
    ④ Neither         — Compiled runs automatically. Cost: ≈ 0.
                         No PFC involved.
  ...
  PFC OPERATIONS × DISSONANCE TYPE (Dissonance-Signal-Architecture.md v1.0 §7.2):
    ...
    │                  │                      │ Direct-State (proof)    │
    └──────────────────┴──────────────────────┴─────────────────────────┘
    → PFC is STRONGER with Evaluative Dissonance, WEAKER with Direct-State.
    → Parallel: PFC Hold/Suppress × Reward (Dissonance-Signal-Architecture §7.2 = full table).

  (PFC-Operations.md §2-§4 = SOURCE for full mechanism detail.)
```
...
> Body-Feedback-Label v2.0 §9C = same labels on the body side.
> This section = LABEL reference only.
...
⭐ 3 INDEPENDENT COST SOURCES + BUDGET:
  ...
  │ Tier    │ Label                   │ Meaning                          │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │ TIER 1  │ PFC cost                │ Most general. PFC is consuming   │
  │         │                         │ resources.                       │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │ TIER 2  │ ① PFC draft cost        │ Processing load from HOLD.       │
  │         │                         │ f(chain_length × novelty).       │
  │         │                         │ DECREASES as compiled (→ ≈ 0).   │
  │         │                         │ Region: dlPFC, FEF.              │
  │         ├─────────────────────────┼──────────────────────────────────┤
  │         │ ② Suppress cost         │ Efference mismatch from          │
  │         │                         │ SUPPRESS.                        │
  │         │                         │ f(intensity × duration).         │
  │         │                         │ DOES NOT DECREASE (pattern       │
  │         │                         │ continues to fire).              │
  ...
  │         │                         │ DECREASES upon commitment        │
  │         │                         │ (choose 1 → hold).               │
  │         │                         │ Region: ACC (conflict).           │
  ├─────────┼─────────────────────────┼──────────────────────────────────┤
  │ TIER 1  │ PFC budget              │ TOTAL resource PFC can use.      │
  │ (system)│ (= Universal Resource)  │ FINITE. SHARED for ALL           │
  │         │                         │ activities: learning,            │
  │         │                         │ Self-Pattern-Modeling, decisions, │
  ...
  ⚠️ 3 cost sources INDEPENDENT (can appear separately or simultaneously):
     ...
     ① + ② + ③ = changing careers without knowing the destination
               (TRIPLE — maximum load)

  ⚠️ PFC budget REDUCED by:
     Fatigue (end of day), Cortisol (stress), Sleep deprivation,
     Illness, Chronic suppress (accumulated ② cost)
     → "Exhausted from work → Self-Pattern-Modeling for child WEAKENED"
       = PFC budget DEPLETED, not lazy.
...
```
⭐ COMPILED CHUNKS HAVE A QUALITY DIMENSION — 3 TYPES TO DISTINGUISH:

  ┌─────────────────────────┬──────────────────────────────────────────┐
  │ Label                   │ Definition + when to use                 │
  ├─────────────────────────┼──────────────────────────────────────────┤
  │                         │                                          │
  │ Genuine-compiled        │ Genuine body reward confirmed →          │
  │                         │ compiled.                                │
  │                         │ → approach tag.                          │
  │                         │ → Self-Pattern-Modeling EXPANSIVE        │
  │                         │   (rich body data).                      │
  │                         │ e.g., A parent who genuinely loves       │
  │                         │ cooking → compiled understanding of      │
  │                         │ "what delicious means" → expansive       │
  │                         │ Self-Pattern-Modeling for a child.       │
  │                         │ Use: when compiled through genuine        │
  ...
  │                         │   (narrow, rule-based).                  │
  │                         │ e.g., "Everyone studies" → compiles      │
  │                         │ HOW TO do it but does NOT compile        │
  │                         │ WHY the body likes it.                   │
  │                         │ Use: when compiled through rule/schema,  │
  │                         │ not body engagement.                     │
  │                         │                                          │
  │ Threat-compiled         │ Forced / threat / coercion → compiled.  │
  │                         │ → avoidance tag.                         │
  │                         │ → Self-Pattern-Modeling BIASED           │
  │                         │   (fear-based patterns).                 │
  │                         │ e.g., Experienced punishment → compiled  │
  │                         │ "don't speak up" → Self-Pattern-Modeling:│
  │                         │ predicts others will punish.             │
  │                         │ Use: when compiled under pressure or     │
  │                         │ threat.                                  │
  │                         │                                          │
  └─────────────────────────┴──────────────────────────────────────────┘
  ...
  ⭐ QUALITY = COMPILE-TIME LOCK:
    ...
    Same knowledge, same compilation level — DIFFERENT quality →
    DIFFERENT for life.

  ⚠️ SAME OUTPUT, DIFFERENT QUALITY:
    Genuine-compiled "knowing how to make soup" ≠
    Schema-compiled "knowing how to make soup"
    Genuine: body data rich → CAN generalize → creative
    Schema: rule data only → CANNOT generalize → repeat same

  ⚠️ 3 quality types CANNOT be changed → must BUILD NEW (genuine) alongside.
    "Fixing" threat-compiled ≠ erasing → compile ADDITIONAL genuine
    patterns alongside.

  (PFC-Operations.md §5 = SOURCE for full detail.)
```
...
⭐ PFC REGIONS AND THEIR FRAMEWORK MAPPING:
  ...
  │                         │ Use: when a precise anatomy label is     │
  │                         │ needed.                                  │
  │                         │ (Aron 2007)                              │
  │                         │                                          │
  ...
  ⚠️ GAP FROM REGION → FRAMEWORK CONCEPT:
    ...
    MAPPING may NOT be 1:1 (1 region → many functions,
    1 function → many regions).
    Use region labels when PRECISION is needed.
    Use framework labels when ANALYZING.

  ⚠️ Use TIER 2 (HOLD/SUPPRESS) instead of Tier 3 (dlPFC/rIFG)
    in MOST cases. Region labels only needed when:
    → Research citation (anatomy must be specified)
...
```
⭐ INDIVIDUAL DIFFERENCES — WHY SAME 24 FUNCTIONS YET DIFFERENT OUTPUT:
  ...
  │                         │ connection density.                      │
  │                         │ = WHAT DOESN'T CHANGE (or changes        │
  │                         │ extremely slowly).                       │
  ...
  │  dimensions)            │ Slot COUNT is THE SAME for everyone      │
  │                         │ (hardware).                              │
  │                         │ QUALITY per-slot differs (PFC quality).  │
  │                         │ Compiled chunks: pyramidal stacking →    │
  │                         │ experts fit MORE within the same slots.  │
  ...
  │ (= mood stability)      │ PFC-only).                               │
  │                         │ Serotonin + dopamine + NE metabolism.    │
  │                         │ Use: when analyzing emotional stability. │
  │                         │ (PFC-Hardware.md §5)                     │
  │                         │                                          │
  └─────────────────────────┴──────────────────────────────────────────┘

  ⚠️ "Improviser" vs "Specialist" = NOT GOOD/BAD — just DIFFERENT profiles.
  ⚠️ PFC capacity REDUCED by: age, cortisol, fatigue, sleep deprivation.
     Observed capacity = hardware ceiling × current state.
  ⚠️ "IQ" NOT USED in the framework — too crude.
     Use instead: PFC quality + clear speed + DRD4 + compiled chunks
     = richer model.
```
...
⭐ 1 ENGINE, 3 COMPONENTS, N APPLICATIONS:
  ...
  │ Self/Other Model        │ Dorsal=distant.                          │
  │                         │ "CONTROL PANEL" — selects simulation     │
  │                         │ target.                                  │
  ...
  3 AXES (coordinates):
    ...
    Axis B — Time:      Past ← Present → Future (+ Counterfact.)
  ...
  ⚠️ Damage to 1 component = degrades ALL applications (alexithymia = proof).
  ⚠️ Named applications = POINTS — unnamed apps = equally real.

  (Simulation-Engine.md v1.0 = SOURCE for full detail.)


  SELF-OBSERVATION GRADIENT (Self-Observation.md v1.0 §4):

    Level 0: Body-React          [NO PFC]          Hardware reflex
    Level 1: Body-Detect         [MINIMAL PFC]     "Something's there"
    ─── BOUNDARY: GAP between signal and response ───
    Level 2: Body-Recognize      [LOW-MID PFC]     "I know I am X"
    Level 3: Body-Predict        [MIDDLE PFC]      "I will soon be Y"
    Level 4: Pattern-Observe     [MID-HIGH PFC]    "I tend to X when Y"
    Level 5: Meta-Observe        [HIGH PFC]        "I am observing my
                                                    observation"
    Level 6: Calibrated-Observe  [HIGH+COMPILED]   "Can this observation
                                                    be trusted?"

    The Level 1→2 boundary = Self-Observation begins when a GAP exists
    between signal and response (PFC encodes state SEPARATELY from response).
    Level 2 = the most basic genuine Self-Observation = Body-Knowing Inward.
```
...
⭐ VOCABULARY FOR SPECIFIC PFC COGNITIVE OPERATIONS:
  ...
  │                         │ "Increases volume, does not issue        │
  │                         │ computational commands."                 │
  │                         │ Use: when describing what PFC chooses    │
  │                         │ to attend to.                            │
  ...
  │                         │                                          │
  │ Labeling                │ PFC assigns a verbal chunk (word) to a  │
  │ (= verbal coding)       │ body pattern. Fidelity DROPS 40-80%.    │
  ...
  │                         │ Use: when describing the verification    │
  │                         │ process.                                 │
  │                         │ (Ask-AI v3.1)                            │
  │                         │                                          │
  ...
  │                         │ Use: when describing "knowing but unable │
  │                         │ to articulate" → the articulation        │
  │                         │ process.                                 │
  ...
  ⚠️ PFC spotlight ≠ attention (general):
     ...
     Attention (general) = umbrella term covering both bottom-up
     + top-down.
  ...
  ⚠️ PFC narrative ≠ PFC check:
     ...
     Explaining ≠ Verifying. "Why" ≠ "Is it correct?"
```
...
```
⭐ 5 FAILURE PATTERNS — LABELS FOR IDENTIFICATION:

  ┌─────────────────────────┬──────────────────────────────────────────┐
  │ Failure Label           │ Definition + when to use                 │
  ├─────────────────────────┼──────────────────────────────────────────┤
  │                         │                                          │
  │ ① Lawyer failure        │ PFC narrative ≠ body truth.              │
  │   (confabulation)       │ PFC justifies body-need with INCORRECT   │
  │                         │ logic.                                   │
  │                         │ "I'm sad because XYZ" → in reality body  │
  │                         │ is tired + hungry + stressed → any       │
  │                         │ trigger → PFC attaches wrong label.      │
  ...
  ⚠️ Failure modes = for IDENTIFICATION, not BLAME:
...
> Body-Feedback-Label v2.0 §10 = same distinction on the body side.
> This section = BRIEF reference.

```
⭐ "LOGIC/FEELING" vs "COMPILED/FRESH" — 2 DIFFERENT LAYERS:

  ┌──────────────────┬───────────────────────────────────────────────┐
  │ Layer            │ Labels + when to use                          │
  ├──────────────────┼───────────────────────────────────────────────┤
  │ MECHANISM        │ Compiled / Fresh.                             │
  │ (the true axis)  │ = Processing level INSIDE the body.          │
  ...
  │ (observer labels)│ = OBSERVER perspective — the view from        │
  │                  │ outside.                                      │
  │                  │ "Logic" = compiled chunks SHAREABLE           │
  │                  │   (deterministic: math, physics).             │
  │                  │ "Intuition"/"Feeling" = compiled chunks       │
  │                  │   NOT EASILY SHAREABLE                        │
  │                  │   (probabilistic: psychology, inter-personal).│
  │                  │ INTERNALLY: the mechanism is IDENTICAL.       │
  │                  │ The difference: SHAREABILITY, not quality.    │
  │                  │ Use: describing to outsiders. Overview files. │
  └──────────────────┴───────────────────────────────────────────────┘
  ...
  │ audience:                  │                                       │
  │ Observation parameter file:│ Logic-Feeling (file name, convention) │
  │ General "human being":     │ PFC = Lawyer + Learning Trajectory    │
  │                            │ = PFC serves body-base, learning →    │
  │                            │   Body-Knowing (compiled body-direct)  │
  └────────────────────────────┴───────────────────────────────────────┘

  ⚠️ DO NOT SAY "feeling opposes logic":
    → SAY "compiled pattern conflicts with fresh processing"
    → Because INTERNALLY there is no "feeling" vs "logic" —
      only compilation levels.

  ⚠️ "Intuition" = compiled processing (math AND emotion alike):
    A chef knowing the dish needs salt = intuition = COMPILED
    (not "feeling only")
    Einstein "seeing" the solution = intuition = COMPILED
    (not "logic only")

  ⚠️ RETIRED: "Humans are 100% feeling at the base layer"
    — terminology collision
    (Logic-Feeling.md v4.0 §4.3: RETIRED. PFC = Lawyer + Learning
     Trajectory = same insight, mechanism-level, avoiding the
     collision-prone term "feeling")
...
⭐⭐ TERMS TO DEPRECATE + REPLACEMENTS:
  ...
  │                     │  "PFC HOLD"         │ Use "draft" ONLY as      │
  │                     │ SPECIFIC: "PFC draft│ VERB + object:           │
  │                     │  novel path"        │ "PFC draft novel path."  │
  │                     │                     │                          │
  │ "thinking" (as      │ "Fresh processing"  │ "Thinking" is ambiguous  │
  │  mechanism label)   │                     │ — even compiled          │
  ...
  │  body"              │                     │ body. PFC biases         │
  │                     │                     │ direction. ~95%          │
  │                     │                     │ automatic.               │
  │                     │                     │                          │
  │ "conscious          │ "Deliberate" or     │ "Conscious" misleading — │
...
⭐ COMPREHENSIVE USAGE RULES:
  ...
  ① USE THE CORRECT LEVEL:
     ...
     Sub-operation → "PFC draft novel path" (verb + specific object)
     Role → "Observer" / "Lawyer" / "Director" / etc. (context-dependent)
  ...
  ② COMPILED/FRESH FOR MECHANISM, LOGIC/FEELING FOR OBSERVATION:
     ...
     ✗ "feeling opposes logic" (INCORRECT at mechanism level)
     Overview/describing: "Logic vs Feeling" (OK, approximate)

  ③ SPECIFY COST SOURCE:
     ✗ "PFC is tired" (too vague — tired from what?)
     ✓ "① PFC draft cost high because of new learning"
     ✓ "② Suppress cost because of impulse suppression"
     ✓ "① + ② double cost because of habit change"

  ④ SPECIFY SUPPRESS TYPE:
     ✗ "suppressing" (active or compiled?)
     ✓ "Active suppress — PFC is actively blocking"
     ✓ "Compiled suppress — already compiled into meta-pattern"

  ⑤ PFC BUDGET TRADE-OFF:
     When analyzing "why performance drops":
     → Check: what was PFC budget spent on before?
     → "Exhausted from work → Self-Pattern-Modeling for child weakened"
       = ④ Universal Resource

  ⑥ DON'T BLAME — EXPLAIN THE MECHANISM:
     ✗ "Lacks willpower" (moral judgment)
  ...
  ⑧ ROLE LABEL IS CONTEXT-DEPENDENT:
     ...
     DO NOT use multiple roles simultaneously (confusing) —
     choose the 1 most appropriate.

  ⑨ RELATIONSHIP WITH BODY-FEEDBACK-LABEL:
     Body-Feedback-Label §8 already covers Compiled/Fresh processing labels.
     Body-Feedback-Label §9C already covers 3-cost labels.
     → This file ADDS: operations, roles, regions, hardware, failure modes.
     → DO NOT duplicate content — cross-reference.
     → Using 2 files together → consistent vocabulary.
```
...
WHAT THIS FILE DOES WELL:
  ...
  ✓ Formalizes 78+ files' PFC vocabulary into 1 reference
  ✓ 3-tier system consistent with Body-Feedback-Label v2.0
  ✓ 4 vocabulary levels (Role/State/Operation/Sub-operation)
    clearly distinguished
  ✓ 5 PFC roles with context rules
  ✓ Deprecated terms with specific replacements
  ✓ Cross-reference strategy: SUPPLEMENTS, does not duplicate
    PFC-Operations / Simulation-Engine / Body-Feedback-Label


WHAT REMAINS UNCERTAIN:

  ? Region-to-function mapping may NOT be 1:1 — the framework simplifies.
    Neuroscience is still refining these mappings.

  ? "~4±1 slots" — exact number is still debated.
    The framework uses this as an approximate guide, not absolute.

  ❌ Ego depletion glucose model — FALSIFIED (Hagger 2016: d=0.04,
    Dang 2021).
    The framework uses "processing load" (serial bottleneck +
    catecholamine + allocation).
    Details: PFC-Operations.md v1.3 §8.3.

  ? PFC narrative confabulation — extent varies by individual.
    The framework uses "PFC = Lawyer" as a tendency, not 100%
    confabulation.


WHAT THIS FILE DOES NOT COVER:

  ✗ Detailed mechanism (→ PFC-Operations.md v1.0)
  ...
  ✗ PFC hardware individual differences mechanism
    (→ PFC-Hardware.md v1.1)
```
...
```
  ...
  ├──────────────────────────────────┼──────────────────────────────────────┤
  │ Somatic-Articulation-Loop.md     │ Somatic-Articulation-Loop: body →   │
  │                                  │ articulate → verify.                │
...
**END OF FILE — PFC-Label v1.1 (24 citations, 17 sections)**

> *PFC-Label Convention v1.1 — Vocabulary Reference.*
> *Companion to Body-Feedback-Label v2.0.*
>
> *§2 Roles: Observer / Lawyer / Director / Universal Resource / Quality Controller.*
> *§3 Spectrum: Compiled ←→ Fresh (TRUE AXIS, not Logic/Feeling).*
