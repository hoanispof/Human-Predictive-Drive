...
version: 1.2
created: 2026-05-22
updated: 2026-06-02 (v1.2 — Self-Observation.md v1.0 cross-refs: APPLICATION-3 "no dedicated file yet" → Self-Observation.md v1.0)
...
scope: |
  ARCHITECTURE file: The brain has 1 general-purpose Simulation-Engine, NOT N separate modules.
  ...
  Application = coordinate in 3D space. Self-Pattern-Modeling, Self-Observation, Imagine-Final = 3 named points.
...
purpose: |
  Self-Pattern-Modeling.md = APPLICATION-1 (dedicated mechanism for agent prediction).
  Imagine-Final.md = APPLICATION-2 (dedicated mechanism for future simulation).
  Self-Observation = APPLICATION-3 (PFC observes body state; Self-Observation.md v1.0).
  THIS file = SHARED ENGINE underneath all of them — UNIFIED ARCHITECTURE.
  Explains WHY: training 1 → improves all. Breaking 1 → degrades all.
  = The "missing architecture file" — each application file describes 1 app; this file describes the ENGINE.
position: |
  Core-Deep-Dive/PFC/ — at the same level as PFC-Operations.md.
  Engine comprises DMN + mPFC + insula + hippocampus (not purely PFC),
...
> **The brain has NO separate module for each "mental function."**
...
>
> **Training 1 component = training ALL applications.**
> **Breaking 1 component = breaking ALL applications.**
...
⭐⭐⭐ SIMULATION-ENGINE:
  ...
  ① The brain has 1 general-purpose SIMULATION-ENGINE (not N separate modules)
  ② Engine = 3 COMPONENTS: Interoception × Constructive Simulation × Self/Other Model
  ③ 3 AXES defining application: Target × Time × Operation
  ④ All "mental functions" = specific coordinates in 3D space
  ⑤ Self-Pattern-Modeling, Self-Observation, Imagine-Final = 3 NAMED POINTS, not separate systems
  ⑥ Training 1 component = improve ALL applications (shared substrate)
  ⑦ Breaking 1 component = degrade ALL applications (alexithymia proof)
...
```
  ...
  ★ This file = ENGINE ARCHITECTURE (unification)
    ...
    │   = PFC observes body-state, the self-awareness mechanism
    │
...
⭐⭐⭐ COMPILED/FRESH = GENERAL-PURPOSE HARDWARE (Inter-Body-Mechanism §3).
  ...
  3 COMPONENTS OPERATING SIMULTANEOUSLY:
    Component 2 (DMN) = engine running simulation
  ...
  ANALOGY:
    ...
    = Self-Observation = app aimed at ONESELF
    = Imagine-Final = app aimed at the FUTURE
    = ALL use the same CPU, same RAM, same display
...
⭐⭐ WHO is being simulated?
  ...
  Close Other: simulate using SELF-TEMPLATE (Entity-Access high)
    ...
    → = "You are a part of my life" LITERALLY true at neural level
  ...
  ⭐ CONTINUOUS, not binary:
    Self → spouse → child → mother → close friend → colleague → stranger
...
```
  ...
  Counterfactual: RE-CONSTRUCT with altered parameters
    → "If only I had done it differently..." = past + altered element + re-simulate
...
⭐⭐ APPLICATION = (TARGET, TIME, OPERATION):
  ...
  ├───┼──────────────────┼──────────────┼───────────┼──────────┼──────────────────────┤
  │ ⑥ │ Recall Other's   │ Other        │ Past      │ Simulate │ "you back then..."   │
  │   │ Past             │              │           │          │                      │
  ├───┼──────────────────┼──────────────┼───────────┼──────────┼──────────────────────┤
  │ ⑦ │ Counterfactual    │ Self/Other   │ Alt-Past  │ Simulate │ "If only I had done  │
...
```
  THE APPLICATIONS BLEND INTO EACH OTHER:
    "I remember you being sad back then" = ⑥ (recall other) + ① (Self-Pattern-Modeling past-state)
    "If mother forces it, the child will be sad" = ④ (predict other future) + ⑧ (moral evaluate)
    "I want to become a better person" = ⑨ (narrative) + ③ (imagine future)
    → NO CLEAR BOUNDARY → continuous space

  THE 3 NAMED APPLICATIONS (①②③) ARE NOT MORE SPECIAL THAN THE OTHERS:
    The framework names Self-Pattern-Modeling, Self-Observation, and Imagine-Final because they are IMPORTANT
  ...
  COMPOUND APPLICATIONS = MULTI-STEP on the engine:
  ...
  DREAM + MIND WANDERING = ENGINE in AUTO MODE:
    No PFC active control → engine "free runs"
...
🟡 SIMULATION-ENGINE = EVALUATIVE DISSONANCE GENERATOR (Dissonance-Signal-Architecture.md v1.0 §7.1):
  ...
  Simulation-Engine CAN generate Evaluative Dissonance WITHOUT external input:
    "Imagine-Final: company goes bankrupt" → body pre-feel → Evaluative Dissonance
    "Preview: getting fired" → body simulate → Evaluative Dissonance
  ...
  BUT: Simulation-Engine CAN CASCADE Evaluative → Direct-State body symptoms:
    "Imagine-Final: tiger attacking" → cortisol spike → heart rate up → body symptoms
...
⭐⭐ INTEROCEPTION = "DISPLAY" — READOUT DEVICE:
  ...
  MECHANISM (anterior insula hierarchy):
    ...
    Anterior insula: EMOTIONAL representation ("I am worried," "I am happy")
  ...
  UNIQUE TO SELF-TARGET:
    ...
    → Self-Observation has 1 EXTRA input channel that Self-Pattern-Modeling→other lacks
...
```
  PARADOX — BUT REALITY MAY BE THE OPPOSITE:
    ...
    → = WHY "a mother knows her child is sad" (novel external cue)
      but "doesn't know she herself is worried" (habituated internal signal)
...
```
  MEMORY = RECONSTRUCTION (not replay):
    ...
    The brain does NOT store video → stores CHUNKS
...
```
  ⭐ ENTITY-ACCESS = NEURAL MIGRATION:
    ...
    → = Phantom resonance = ventral mPFC firing for absent entity
    → = "I lost a part of my life" LITERALLY TRUE at neural level: entity WAS in self-circuits
...
```
  ...
  ⭐ IMPLICATION FOR SELF-CALIBRATION:
    ...
    → "They are DIFFERENT from me in this way" → discover own bias → self-model REFINED
...
⭐⭐ DECISIVE PROOF FOR SHARED SUBSTRATE:
  ...
  🟢 Bird, Silani et al. 2010 (Brain):
    ...
    → SAME substrate broken → BOTH directions lost simultaneously
  ...
  Compiled STILL FIRES but READOUT BROKEN:
    ...
    → = Like having a camera but a broken screen: still recording but cannot view
...
```
⭐⭐ SAME ENGINE, DIFFERENT DEGREE OF PFC INVOLVEMENT:
  ...
  ├──────────────┼────────────────────────────────────────────────────┤
  │ ⑫ Mind wander│ LOW PFC: engine drifts, attention unfocused       │
  │              │ "Suddenly remembering something old" = engine      │
  │              │ auto-associating                                   │
  ├──────────────┼────────────────────────────────────────────────────┤
  │ ① Self-Pattern-Modeling Compiled     │ MINIMAL: compiled, automatic, invisible           │
  │ (compiled)   │ "Feeling drawn to someone without knowing why"    │
  │              │ PFC = PASSIVE observer (if even aware)             │
  ├──────────────┼────────────────────────────────────────────────────┤
  │ ② Self-Observation │ MIDDLE: PFC observes body-feedback output         │
  │              │ PFC = READER (can label but not manipulate input)  │
  │              │ "I am sad" = readout, not active simulation        │
  ├──────────────┼────────────────────────────────────────────────────┤
  │ ① Self-Pattern-Modeling Fresh     │ MODERATE: PFC actively drafts model for           │
  │ (fresh)      │ unfamiliar other                                   │
  │              │ "What is this person thinking?" = deliberate       │
  │              │ simulation                                         │
  ├──────────────┼────────────────────────────────────────────────────┤
  │ ③ Imagine-   │ HIGH: PFC actively SETS scenario, READS output,   │
  │ Final        │ ITERATES ("what if I change X?")                  │
  ...
  ENTITY-ACCESS CONNECTION:
    PFC Resource (Entity-Access §12) DETERMINES spectrum position:
      Full resources → Imagine-Final mode ("how would the child feel if...")
      Depleted → Self-Pattern-Modeling Compiled default ("did you eat yet?" = compiled, automatic)
...
⭐⭐ SAME ENGINE, DIFFERENT USE QUALITY:
  ...
  ⭐ APPLIES TO ALL APPLICATIONS (universal modifier):
    ...
    Self-Pattern-Modeling with threat: "you must hate me" → PROJECT fear onto other
    Imagine-Final with curiosity: "the future will be interesting" → EXPLORE options
    Imagine-Final with threat: "the future will be terrible" → CATASTROPHIZE
...
⭐⭐ SHARED SUBSTRATE → 1 TRAINING POINT → N IMPROVEMENTS:
  ...
  INTEROCEPTION TRAINING (Component 1 — highest leverage):
    ...
    → Better readout → improve:
      Self-Observation (②): "knowing what one is currently feeling" — clearer
      Self-Pattern-Modeling (①): "knowing what others are currently feeling" — more accurate
  ...
  REFLECTION PRACTICE (engine use quality):
    ...
    → Replace "I must be wrong" with "why do I feel this way?"
...
> Train 1 → improve all. Break 1 → degrade all.
> Alexithymia = decisive proof. 3 axes = coordinates, not categories.
>
