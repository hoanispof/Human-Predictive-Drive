...
> But between behavior and physical neurons = MANY UNKNOWN LAYERS.
>
> Like a computer: you know the USB mouse and the cursor on screen.
> But the driver code in between — you didn't write it, can't read it, can't control it.
> The body-brain system is the same.
>
> This file maps EXACTLY where the framework "jumps over" —
> not to say the "framework is weak," but to point to:
> ① A clear boundary between KNOWN and UNKNOWN
...
> If the framework is wrong somewhere → finding that is equally valuable.

---

> **Status:** v1.0
> **Date:** 2026-05-12
> **Position:** Core-Deep-Dive/ (fundamental analysis — on par with Neural-Architecture, Neural-Processing-Flow)
...
>   integrated into §4 + §7 of this file. No information lost.
> **Dual purpose:**
>   ① Honest assessment — what the framework knows, what it does not
>   ② Research roadmap — each gap = testable research direction
> **Confidence:** 🟢 Research support | 🟡 Framework synthesis | 🔴 Hypothesis
> **Source version:** Vietnamese v1.0 (2026-05-12)
...
```
The framework describes the body-brain system at the BEHAVIORAL level:
  → "Chunk compiles through repetition + emotional peak"
...
But at EACH claim, the framework JUMPS OVER a gap:
  ...
  ──────────────────              ──────────────────────────
  "chunk compiles"           →    which neurons fire? which synapses strengthen?
  "cortisol amplifier"       →    which GR/MR receptor? which gene expression?
  "VTA fires salience"       →    which pathway does the signal take to which area?
  "body-feedback reward"     →    which opioid receptor subtype? in which neurons?

This JUMP gap = does NOT mean the framework is wrong.
= The framework operates at a DIFFERENT LEVEL — behavioral, not molecular.
= Like a software engineer who knows "mouse click → menu appears"
  but did not write the driver code between USB signal → OS interrupt → render.

THIS FILE MAPS EXACTLY those 5 jump gaps:
  → Each gap = 1 GAP between framework and biological reality
  → Each GAP = specific research direction
  → Each GAP has a different severity level
  → Researchers can choose the GAP matching their expertise → drill

DUAL PURPOSE:
  ① For readers: know how far the framework can be trusted
  ② For researchers: know where drilling will yield the most value
```
...
🟡 COMPUTER ANALOGY — where the framework operates in the stack:
  ...
  Layer 5: USER BEHAVIOR            Layer 5: HUMAN BEHAVIOR
    (click, type, drag-and-drop)      (actions, decisions, reactions)
    = OBSERVABLE                      = framework CAN DESCRIBE ⭐

  Layer 4: USER INTERFACE           Layer 4: FEELING + PFC OBSERVATION
    (cursor, windows, menus)          (consciousness, feeling, observation)
  ...
  Layer 3: SOFTWARE                 Layer 3: CHUNK PROCESSING
    ...
    = CAN write, CAN debug            = framework describes FUNCTION ⚠️
                                        but does NOT KNOW implementation

  Layer 2: DRIVERS                  Layer 2: SIGNAL TRANSDUCTION
    (USB driver, GPU driver)          (neurotransmitter, receptor binding,
    = few read it, few understand it   ion channel, gene expression)
                                      = framework JUMPS OVER ⚠️⚠️

  Layer 1: HARDWARE                 Layer 1: PHYSICAL BODY
    (sensor, chip, circuit board)     (receptor, neuron, organ, hormone)
    = KNOWN to exist, function        = neuroscience knows MOST
      known but not everything          but NOT EVERYTHING
...
🟡 FRAMEWORK COVERAGE MAP:
  ...
  Layer 4 (Feeling + PFC):
    ...
    → PATTERN can be described, HOW integration = partial

  Layer 3 (Chunk Processing):
    ████████░░░░░░░░░░░░ ~40% coverage
    → FUNCTION can be described (compile, fire, link, install)
    → IMPLEMENTATION = BLACKBOX (which neurons? which circuits?)

  Layer 2 (Signal Transduction):
    ██░░░░░░░░░░░░░░░░░░ ~10% coverage
    → Framework uses RESULTS (cortisol = amplifier)
  ...
  ⭐ Clear pattern:
    → Framework coverage DECREASES from Layer 5 → Layer 2
    → Layer 1 coverage depends on neuroscience (not framework)
    → LARGEST GAP = Layer 2→3 ("Driver Layer")
```

### §1.3 Why the gaps are NOT weaknesses

```
Framework operates at behavioral level = BY DESIGN:

  ① Describing PATTERNS requires no knowledge of implementation
     → Software engineer writes code WITHOUT NEEDING to know transistors
     → Framework describes behavior WITHOUT NEEDING to know each synapse
     → Each layer has its own LIMITS + VALUE

  ② Behavioral description = TESTABLE without a lab
     → "Chunk compiles through repetition" — observe behavior = verify
  ...
  ③ Gaps = RESEARCH OPPORTUNITY, not weakness
     → If framework is correct at behavioral level
     → Then biological level MUST have a matching mechanism
     → Finding that mechanism = validates OR falsifies the framework
     → Both outcomes are valuable

  ⚠️ GAP BECOMES DANGEROUS when:
     → Someone claims the framework explains at molecular level — OVERCLAIM
     → Someone uses the framework to replace clinical protocol — OVERAPPLICATION
     → Framework = a compass — shows direction, does NOT provide the specific path
```
...
```
The framework lists 17 body-input categories (Core-Software.md §3.2,
...
```
⚠️ 17 CATEGORIES = BEST AVAILABLE LIST, NOT A FINAL LIST.

Body-input at the detailed level = NEARLY INFINITE.
The framework does not attempt to enumerate everything — it lists CATEGORIES only.
But the categories themselves may be incomplete.
...
CANDIDATES NOT YET IN THE LIST:
  ...
  ① Magnetoreception (🟡 Open question)
     ...
     → Birds, sea turtles, salmon = CLEARLY USE magnetoreception
     → Humans = unclear whether it influences behavior
     → If so → adds 1 exteroceptive channel

  ② Microbiome → Brain neural signals (🟢 Active research, incomplete)
     → Gut-brain axis exists (🟢 — ⑬ Visceral partially covers this)
     → ~95% serotonin produced in the gut (🟢 Yano et al. 2015)
     → BUT: microbiome composition → specific neural signals
       → specific behavior changes = actively researched
     → ⑬ Visceral may NOT COVER the full scope of microbiome influence

  ③ Pheromone processing (🟡 Debated)
     → Vomeronasal organ exists in humans (vestigial?)
     → Androstadienone, estratetraenol = candidates
     → May influence social behavior the framework has not tracked

  ④ Infrasound sensing (🟡 Some evidence)
     → Humans may sense < 20Hz
     → Induces unease, "feeling of presence" (Vic Tandy 1998)
     → If real → 1 hidden exteroceptive channel

  ⑤ Barometric pressure sensing (🟡 Anecdotal + some research)
     → "Joint pain before rain" = commonly reported
     → Nociceptors may respond to pressure changes
     → Mechanism not fully clear

  ⑥ Immune system → Brain signaling (🟢 Established, framework not yet integrated)
     → Cytokines cross BBB → sickness behavior (🟢 Dantzer 2008)
     → ⑮ Hormonal-sensed covers this PARTIALLY
     → BUT: immune signaling is more complex than "hormonal-sensed"
     → "Sick → sad, tired, withdrawal" = immune signal, not just hormone

⭐ PRINCIPLE: Body-input list = OPEN-ENDED.
   Framework acknowledges: additional channels may be discovered.
   Mechanism (delta rule, baseline adaptation) = UNIVERSAL —
   applies to ANY body-input, including unknown ones.
   Discovering a new input → add it → framework still operates.
```

### §2.3 Depth within EACH category

```
Each category the framework lists = an AGGREGATION of many sub-systems.

EXAMPLE — ① VISION:
  Framework states: "Vision: retina, V1→IT pathway, FFA"
  The reality inside:
    ...
    → LGN → V1 → V2, V4, MT, IT — each area with sub-specializations
    → Koniocellular, Magnocellular, Parvocellular pathways
    → = THOUSANDS of details the framework does not (and does not need to) describe

EXAMPLE — ⑬ VISCERAL:
  Framework states: "ENS ~100-500M neurons, gut-brain axis"
  The reality inside:
    → ENS has 2 plexuses (Auerbach + Meissner)
    → ~30 neurotransmitters (serotonin, dopamine, acetylcholine,...)
    → Vagus nerve: ~80% AFFERENT (gut → brain, NOT only brain → gut)
...
⭐ PRINCIPLE:
   Framework describes at CATEGORY level. Internal detail = infinite.
...
RESEARCH DIRECTIONS FROM GAP 1:
  ...
  RD-1.1: Magnetoreception in humans — does it influence behavior?
    → Method: fMRI in controlled alternating magnetic fields
    → Predict (if framework is correct): if magnetoreception is active
  ...
  RD-1.4: Discovery of NEW body-input channels
    → Any channel not yet known
    → Predict: if it is a real body-input → MUST have baseline,
...
But at EACH arrow (→) = AN ENTIRE molecular biology layer:
  → Ion channels opening/closing
  → Action potential propagating down the axon
  → Neurotransmitter release at synapse
  → Receptor binding at the receiving neuron
  → Second messenger cascade within the neuron
...
COMPUTER ANALOGY:
  ...
  = The entire "driver layer" the user never sees, never needs to know.

The body-brain system is the same:
  Framework describes: "experience → chunk compiles"
  Between "experience" and "chunk compiles":
...
🟡 6 SPECIFIC SUB-GAPS:
  ...
  SG-2.1: RECEPTOR → NEURAL SIGNAL
    ...
    Does not know details: which mechanoreceptor (Meissner? Pacinian?
  ...
  SG-2.2: SIGNAL → THALAMUS → CORTEX routing
    ...
    Does not know details: at which nucleus (LGN/MGN/VPL/...)?
      → which neurons receive? → inhibitory/excitatory balance?
      → what is the specific TRN gating pattern?
  ...
  SG-2.3: CORTICAL PROCESSING → "PATTERN" FORMATION ⭐⭐
    ...
    Does not know details: WHICH neurons fire together?
      → Within 6 cortical layers (Mountcastle), at which layer?
      → By which mechanism does gamma binding (Singer) synchronize?
      → When does a "pattern" become a "chunk"? Where is the threshold?
    = Neuroscience is ACTIVELY RESEARCHING this (🟡). This is the frontier.
    = THIS GAP CONNECTS DIRECTLY TO §4 (Chunk Substrate Blackbox).
  ...
  SG-2.4: NEUROTRANSMITTER → BEHAVIORAL FUNCTION mapping
    ...
    Does not know details:
      ...
      → Cortisol → GR (glucocorticoid) vs MR (mineralocorticoid)
        → gene expression cascade → which proteins → how does neuron behavior change?
      → μ-opioid vs κ-opioid vs δ-opioid → how do they differ at behavior level?
    = Pharmacology is ACTIVELY RESEARCHING the details (🟢🟡). Framework uses synthesized results.
  ...
  SG-2.5: SYNAPTIC PLASTICITY → "COMPILE" mechanism ⭐
    ...
    Does not know details:
      ...
      → Consolidation: hippocampus replay → cortical storage (🟢 Walker 2017)
        → which specific replay pattern? → cortical storage in which neurons?
      → Emotional peak compile: NE + cortisol → amygdala tag
        → by which specific mechanism is the tag created? (🟢 McGaugh 2004: partial)
    = Neuroscience KNOWS each step individually (🟢).
      BUT: connecting the full chain into "1 experience → 1 chunk" = NOT YET DONE.
  ...
  SG-2.6: SLEEP CONSOLIDATION mechanism in detail
    ...
    Does not know details:
      → Slow-wave sleep: hippocampal sharp-wave ripples → cortical replay
        → does replay reproduce the FULL pattern or is it SELECTIVE?
      → REM: emotional memory processing — what is the specific mechanism?
      → Gist extraction: HOW does the brain "distill" experience during sleep?
...
⭐ DRIVER LAYER = WHERE FRAMEWORK AND NEUROSCIENCE MEET:
  ...
  SPECIFIC EXAMPLE:
    ...
    → If matches amplifier → framework validated at molecular level
    → If matches stress only → framework claim needs refinement

⚠️ CURRENT STATE:
   The framework has NOT yet explicitly named this gap.
   Neural-Processing-Flow.md describes PATHWAY (hardware flow) —
   but has not yet explicitly stated: "between pathway and chunk = AN ENTIRE
   molecular biology layer the framework does not describe."
...
RESEARCH DIRECTIONS FROM GAP 2:
  ...
  RD-2.1: "Experience → Chunk" pathway tracing
    ...
    → Framework predict: compile is FASTER with multi-modal + emotional peak

  RD-2.2: Cortisol "amplifier" molecular verification
    → Cortisol + novel context → measure cortical excitability
    → Cortisol + threat context → measure the SAME cortical regions
...
### §4.1 The Sole Foundational Blackbox

```
⭐⭐⭐ THIS IS THE FRAMEWORK'S MOST FUNDAMENTAL BLACKBOX.

  The framework uses "chunk" as the basic unit.
  Everything — feeling, schema, logic, connection — builds on chunks.

  The framework describes chunks at the BEHAVIORAL level:
    → Compiles through repetition + emotional peak + multi-modal + sleep
    → Fires via probability-weighted spreading activation
  ...
  The framework does NOT KNOW chunks at the BIOLOGICAL level:
    → Which neural structure does a chunk correspond to?
      (cell assembly? neural ensemble? engram? distributed pattern?)
    → Stored in WHICH brain area? Distributed how?
      (hippocampal index? cortical storage? distributed?)
    → Fires by WHICH physical mechanism?
      (synchronous firing? oscillation coupling? phase-locking?)
    → Compiles specifically into WHAT at the synapse level?
      (Hebbian assembly? synaptic tag-and-capture? structural change?)

  The framework only observes chunks at the behavioral level
  (what people do, feel, and how they react)
...
⭐ FINDING (Drill §26 — Framework-Boundaries.md v2.0):
  ...
  Before (v1.0): the framework called these "2 blackboxes":
  ...
  "Valence complexity" (old Blackbox-2) = WHY?
    Because chunk substrate (Blackbox-1) operates at LARGER SCALE:
    → LONG chain + distributed + many links + trust × many sources
    → = Blackbox-2 "complexity" is MOSTLY at Level 2 (collective), NOT Level 1
...
```
🟢🟡 Neuroscience has MANY candidates — none conclusively established:

  ① Cell Assembly (Hebb 1949)
     → Group of neurons firing in synchrony = 1 unit
     → 🟢 Foundational concept. Closest match to "chunk."
     → Limitation: cannot yet measure 1 specific assembly end-to-end
  ...
  ⭐ FRAMEWORK POSITION:
    ...
    When that happens, the framework will be verified more clearly
    — or falsified if the actual substrate does not match.
    Both outcomes are valuable.
```
...
RESEARCH DIRECTIONS FROM GAP 3:
  ...
  RD-3.1: "Chunk" = engram?
    → Framework predict: chunk = multi-modal engram
      (not just 1 brain area, but distributed across cortical areas)
  ...
  RD-3.3: "Never delete, only probability shift"
    ...
    → Consistent with reconsolidation (🟢 Nader 2000) but not proven UNIVERSAL
...
```
The framework describes feeling via 7-layer fidelity gradient
...
```
⚠️ FRAMEWORK DESCRIBES PATTERN, NOT MECHANISM:
  ...
  SUB-GAP 1: Integration — HOW?
    ...
    Missing: insula RECEIVES signals from vagus, spinal, hormonal...
      ...
      → What is the integration mechanism? Summation? Weighted average?
  ...
  SUB-GAP 2: "Consciousification" — HOW?
    ...
    Missing: EXACTLY how neural activity BECOMES
...
RESEARCH DIRECTIONS FROM GAP 4:
  ...
  RD-4.2: 7-layer fidelity at neural level
    → Framework: fidelity DECREASES from body → PFC
...
Each hormone has a dedicated file or section. Evidence strong (🟢🟡).
```

### §6.2 Gap: combinatorial interaction

```
⚠️⚠️ FRAMEWORK DESCRIBES EACH HORMONE SEPARATELY. SIMULTANEOUS INTERACTION = NOT COVERED.
...
REALITY:
  ...
  N hormones × M interactions × T timepoints = nearly INFINITE combinations.

CURRENT PHARMACOLOGY:
  → Studying 1-2 hormones simultaneously = feasible
  → Studying 3+ hormone interactions = very difficult
  → Studying the FULL hormone system simultaneously = not yet possible
  → Drug side effects = partly because combinatorial interactions cannot yet be predicted

FRAMEWORK GAP:
  Framework states: "cortisol + NE threat → PFC offline"
  Framework does NOT state: "cortisol + NE + LOW serotonin + HIGH estrogen
    + MODERATE testosterone + insulin resistance → then WHAT?"
  = Framework describes EACH ONE. Does NOT describe COMBINATIONS.
```

### §6.3 Why this gap is IRREDUCIBLE (at current levels)

```
This is NOT a framework flaw. This is the limit of THE ENTIRE DOMAIN:

  ① Pharmacology also cannot yet predict combinatorial effects
     → Polypharmacy (many drugs) = one of the largest challenges in medicine
  ...
  ② Hormone system = nonlinear dynamic system
     ...
     → Sensitive to initial conditions (analogous to chaos theory)
     → Individual variation (gene + receptor + history) multiplies the effect

  ③ Framework acknowledges (Framework-Boundaries.md v2.0):
     "Hormones change continuously (cortisol, estrogen, thyroid,...)
      — each biases valence in 1 direction, and they interact
      with each other in ways I have not yet captured"
     = Hardware Variance complexity dimension

  ⭐ FRAMEWORK POSITION:
    Framework describes the FUNCTION of each hormone (useful, testable).
    Combinatorial interaction = beyond framework scope.
    Anyone wanting to predict their exact body state at a given moment
    → needs measurement (hormone panel, HRV, cortisol test,...).
    The framework provides a MAP to INTERPRET measurements — not a replacement for measurements.
```
...
RESEARCH DIRECTIONS FROM GAP 5:
  ...
  RD-5.1: Hormone interaction profiles
    → Framework predict: cortisol AMPLIFIES any direction
...
that affect the ability to PREDICT:
...
Short valence chains at Level 1 (individual) → easy to observe:
  → See friend → joy → approach
  → Thorn → pain → withdraw hand
...
but CANNOT predict specific instances — too many variables
interacting at each link in the chain.

Like weather forecasting: knows the mechanism but still cannot predict
30-day weather — because too many variables interact simultaneously.
```

### §7.2 Hardware Variance

```
Same event, different people, completely different outcomes:

  → Trauma compiled since childhood — even the person themselves cannot verbalize it,
    so it is impossible to know which chunks are firing

  → Hormones change continuously (→ §6 Hormone Combinatorics)
    — each biasing valence in 1 direction, interacting complexly

  → Hardware (gene, receptor variants) creates thousands of subtle variations
    — same event, different person, completely different valence chain

Hardware variance = IRREDUCIBLE — not a question of tools or skill level.
...
5 Gaps (§2-§6) + 2 Complexity Dimensions (§7.1-§7.2) mean:
  ...
  (THIS person, at THIS moment, will feel WHAT, do WHAT).

  If anyone uses the framework to claim "I know exactly what you are thinking"
  — that is overclaim, not what the framework says.

  The framework is like a compass — shows direction, does not provide the specific path.
  Useful for orientation, but does not replace
  the act of walking and sensing it yourself.
```
...
IF THE FRAMEWORK IS CORRECT, neuroscience MUST FIND:
  ...
  ⑦ 3 Compile Types = DIFFER at molecular level
...
If any prediction is WRONG → the framework needs to revise or drop that claim.
BOTH outcomes (confirm or reject) are valuable.
```
...
AI can serve as a DYNAMIC DRILL TOOL:
  ...
  ③ User asks: "Is there a body-input from magnetic fields?"
  ...
  = The Blackbox-Map file does NOT NEED to contain all this detail.
  = The Blackbox-Map file only needs to MAP where the gaps are.
  = AI fills detail ON DEMAND — by expertise and user's specific question.

  ⚠️ AI also has limits:
     → AI knowledge = training data (has a cutoff date)
     → Neuroscience frontier = changing continuously
     → AI may be wrong on molecular detail → user verifies with original papers
...
Each gap = an unknown region + specific research directions.
...
If the framework is wrong → this is where it will be found out.
Both → progress.
```
