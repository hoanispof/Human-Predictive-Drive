...
>
> **Refinement v1.3 (2026-05-25) — 5 new concept integrations:**
>   → §1 +Gap-Distribution-Profile (credential homogenization)
>   → §3 +Dissonance-Signal-Architecture (Evaluative + Direct-State compound)
>   → §5.4 +Hardware-Subsidy (parent subsidy INVERTED to threat)
...
🟢 CREDENTIAL INFLATION — RED QUEEN EFFECT:
  ...
  ⭐ v1.1 FRAMEWORK CONNECTION:
    ...
    → Credential inflation = Gap-Distribution-Profile ĐỒNG NHẤT HOÁ:
      Tất cả graduates cùng profile: narrow abstract center, installed-dominant, rigid
    → Arms race FORCE 1 gap landscape cho TOÀN XÃ HỘI → diversity collapse
    → Millions có cùng profile → value of profile → 0
    → = Credential inflation MECHANISM qua Gap-Distribution-Profile lens
...
⭐ CHI TIẾT 3 CASE QUAN TRỌNG NHẤT:
  ...
  ② HÀN QUỐC — CỰC ĐOAN NHẤT:
    ...
    🟡 Framework: Hàn = Nash equilibrium LOCKED:
      ...
      → Bỏ hagwon = "giết tương lai con" → body detect substrate/evaluative THREAT
...
🟢 ② MENTAL HEALTH — DATA NGHIÊM TRỌNG:
  ...
  🟡 DISSONANCE SIGNAL ARCHITECTURE LENS (Dissonance-Signal-Architecture.md v1.0):
    → Arms race tạo CHRONIC Evaluative Dissonance (E₂-E₃ compiled):
      "tôi không đủ giỏi" = compiled VÀO identity, không chỉ stress tạm thời
    → Ranking hàng ngày = Evaluative Dissonance injection LIÊN TỤC
    → Direct-State (thiếu ngủ, cortisol) + Evaluative (ranking) = COMPOUND
    → Harder: cần address CẢ HAI loại cùng lúc (REST + REFRAME)
    → Thế hệ trước: chủ yếu Evaluative bounded → nay: Evaluative + Direct-State
...
🟡 FRAMEWORK — TẠI SAO KHÔNG DỪNG ĐƯỢC:
  ...
  ① Unilateral disarmament = suicide:
    ...
    → Body detect: "con tôi sẽ THẤT BẠI" = substrate/evaluative THREAT
...
🟡 EDUCATION ARMS RACE QUA LENS FRAMEWORK:
  ...
  ⭐ TẠI SAO BODY KHÔNG THỂ DỪNG:
    ...
    → "Con tụt hậu" = L1 threat (substrate, kinh tế tương lai) + status threat (evaluative)
...
═══════════════════════════════════════════════════════
  ...
  🟡 HARDWARE-SUBSIDY LENS (Valence-Propagation.md v3.0 §7):
    ...
    → Arms race INVERTS subsidy direction:
      Parent ĐÁNG LẼ = subsidy (an toàn, khám phá, approach tag)
      → BỊ BIẾN THÀNH threat transmitter (Level 1 imposed, avoidance tag)
    → = Hardware-Subsidy bị CORRUPTED: cùng entity, ĐỔI CHIỀU
    → Teacher: Moderate subsidy → cũng bị biến thành ranking tool
    → = Arms race = ESCALATING demand cho subsidy mà KHÔNG AI provide được
...
═══════════════════════════════════════════════════════
  ...
  🟡 RESONANCE DECLINE CREDENTIAL DECAY (Bond-Architecture v2.0 §4):
    → Credential = initially Generative reward (mỗi bằng mới = thành tựu)
    → Reward-Habituated: bằng cấp thứ 2, 3... → prediction-delta GIẢM (Weber-Fechner)
    → Novelty dưới ngưỡng: hệ thống thi cử predictable → no surprise → no gap
    → Compiled-Suppress (★ LEVERAGE): học sinh suppress sở thích riêng để theo track
      → SOURCE of by-product bị TẮT → accelerates Reward-Habituated + drains novelty
    → = Cần THÊM credentials để maintain visible reward → INFLATION spiral

  🟡 4-LAYER SUSTAINABILITY (Resonance-Sustainability.md v1.0):
    → Arms race = UNSUSTAINABLE ở MỌI tầng:
    → Tầng 1: Proximity forced (14h/ngày) nhưng Agent-mode = avoidance → anti-resonance
    → Tầng 2: Verbal = performance only, Body-sync = cortisol (không phải oxytocin)
    → Tầng 3: No Active-Constructive responding (chỉ rank + compare)
    → Tầng 4: Maintenance DECLINE — motivation erode faster than competence grows
    → = Càng LÂU trong race → càng burned out → pattern TỰ HỦY

  🟡 Cả hai lens (Resonance Decline + 4-Layer) giải thích: arms race = self-defeating pattern
  🟡 Credential inflation = không chỉ economic → mà NEUROLOGICAL necessity
```
...
```
  ...
  FRAMEWORK ANALYSIS (🟡):
    ...
    🟡 "Body không thể dừng vì substrate/evaluative threat" — logical từ framework
    ...
    🟡 "Arms race = Hardware-Subsidy inversion" — logical from Valence-Propagation v3.0 §7
