...
updated: 2026-05-19 (v4.0 — Architecture B, 2-Stream Resonance, 3-cost, 5-Channel, PFC=Lawyer; rename Pattern-Resonance → By-Product-Gap-Resonance)
previous_updates:
  - 2026-05-16 (v3.3 — §4.2 F1/F2 resonance quality forward-ref Resonance §7.10)
...
scope: |
  ...
  v4.0 KEY CHANGES (Inter-Body drill integration):
    + Architecture B: Social = Architecture REQUIREMENT (4 reasons, NOT luxury)
    + Resonance 2-Stream Architecture: Stream 1 (hardware, habituates) + Stream 2 (SPM mutual, deepens)
    + By-product match: foundation of inter-body reward
    + 3 Independent Cost Sources (PFC draft + Suppress + Uncertainty)
    + Compiled/Fresh = real processing axis (NOT Feeling/Logic content)
    + 5-Channel Input Vector framing
    + PFC = Lawyer not Judge (Gazzaniga, Haidt 2001, Nisbett & Wilson 1977)
...
dependencies:
  ...
  - Self-Pattern-Match.md v3.0 — Compiled/Fresh, PFC=Lawyer, per-domain, F1/F2
  - By-Product-Gap-Resonance.md v1.0 — 2-Stream Architecture, by-product match, SPM NOT prerequisite
  - Empathy.md v2.2 — SPM function applied to other agents, 3 mechanisms, double empathy
...
sources_backup: |
  v3.3: Connection.md (2,004L, 2026-05-16) → backup/Connection-v3.3-backup.md
...
⭐ TẠI SAO CONNECTION DRIVE CỰC MẠNH — ARCHITECTURE B GIẢI THÍCH:
  ...
  REASON 4 — SOCIAL BASELINE THEORY:
    ...
    With others = body RELAXES (less vigilance, less self-regulation).
    Alone = body must WORK EXTRA (self-regulate, self-protect).
...
⭐ SPM = CƠ CHẾ CHÍNH TẠO CONNECTION:
  ...
  (Self-Pattern-Match.md v3.0 — Compiled/Fresh, PFC=Lawyer, per-domain)
  ...
  2 FUNCTIONS SONG SONG:
    ...
    F2 — Fresh prediction: PFC chain predict hành vi target tiếp theo
      → Deliberate, consume PFC resource (3-cost model: §4.5)
...
⭐ BY-PRODUCT-GAP-RESONANCE v1.0 — 2-STREAM ARCHITECTURE:
  ...
  (By-Product-Gap-Resonance.md v1.0: 2-Stream, by-product match, SPM NOT prerequisite)
  ...
  🟡 2-Stream Architecture = framework synthesis (By-Product-Gap-Resonance.md v1.0)
  🟡 Anti-match concept = framework synthesis, logically derived
...
🔴 CONNECTION QUALITY TỈ LỆ THUẬN VỚI SỐ CHANNELS HOẠT ĐỘNG:
  ...
  ⭐ 5-CHANNEL INPUT VECTOR × CONNECTION (Inter-Body-Mechanism.md §6):
    ...
    Mỗi episode connection = 5-dimensional input vector:
      ...
      Ch4 — ENTITY ACTIONS: what agent DOES/SAYS right now
      Ch5 — PFC ACTIVE CHAIN: ongoing reasoning process

    Channel DOMINANT → determines CONNECTION EXPERIENCE at that moment:
      Face-to-face: Ch1+Ch4 STRONG → grounded, multi-channel → DEEPEST
      Video call: Ch1 PARTIAL + Ch4 → reduced but functional
      Text: Ch4 ONLY → PFC must IMAGINE tone (Ch5 fills gaps) → prone to error
      Memory: Ch3 ONLY → open-loop, no real-time calibration

    VULNERABILITY: When Ch4 dominant WITHOUT Ch1:
...
```
  ESTABLISHED (🟢):
    ...
    🟢 PFC = interpreter/lawyer (Gazzaniga split-brain, Haidt 2001)
    🟢 People wrong about own behavior causes (Nisbett & Wilson 1977)
    🟢 Reciprocal altruism (Trivers 1971) — supports by-product match
  ...
  FRAMEWORK SỰ LUẬN (🟡):
    ...
    🟡 2-tầng mechanism (❶ body-level pre-SPM + ❷ SPM enables Resonance) — framework
    ...
    🟡 F1/F2 resonance quality spectrum: F1+F1 deepest, F1+F2 functional,
       F2+F2 transactional — builds on SPM Compiled/Fresh + Resonance 2-Stream
    🟡 Architecture B: Social = Requirement (4 reasons) — framework synthesis,
       each reason individually 🟢 supported
    🟡 2-Stream Architecture (§4.2): Stream 1 habituates, Stream 2 deepens —
       By-Product-Gap-Resonance.md v1.0, logically derived, explains temporal dynamics
    🟡 By-product match as Resonance foundation — framework synthesis (Inter-Body §5.4)
    🟡 Anti-match: by-products can CONFLICT gap direction — logical extension
    🟡 Per-pair topology: Resonance has "shape" per pair, per domain — logical, not tested
    🟡 3-cost model applied to connection (§4.3, §10.2) — framework synthesis
    🟡 5-Channel Input Vector × connection quality (§11) — framework synthesis
    🟡 PFC=Lawyer in connection domain: mask/amplify/mislabel (§16.5) — logical
    🟡 Domain = Final Arbiter for connection quality — framework principle
    🟡 2-source social need (§3.1): hardware + compiled baseline — framework synthesis
    🟡 Compiled/Fresh as real processing axis for ❷ SPM — Inter-Body drill concept
  ...
  HYPOTHESIS (🔴):
    ...
    🔴 5-Channel intensity quantification — conceptual, not measured
```
...
```
  MECHANISM FILES:
    → Inter-Body-Mechanism.md v1.0 — Architecture B, 4 reasons, by-product match,
      3-cost, 5-Channel, PFC=Lawyer (source-of-truth cho drill insights)
    → Agent-Mechanism.md v2.0 — integration hub, §12 body-need feeder
    → Self-Pattern-Match.md v3.0 — Compiled/Fresh, PFC=Lawyer, per-domain, F1/F2
    → By-Product-Gap-Resonance.md v1.0 — 2-Stream Architecture, by-product match,
      SPM NOT prerequisite, anti-match, per-pair topology
    → Body-Coupling.md v2.0 — coupling mechanism (extends §3.3 ❸ + 2-luồng):
      2D model, 3 Phase, extension/entanglement/neutral, system compilation
    → Empathy.md v2.2 — SPM function, 3 mechanisms, double empathy universal
    → Reward-Signal-Architecture.md v2.0 — reward Type A/B, Compiled/Fresh mapping
  ...
  OBSERVATION FILES (cùng folder):
    ...
    → Boredom.md — Dissonance + Imagine-Final unclear (phân biệt "chán" vs "cô đơn")
    → Autonomy-Hardware.md — efference copy (skip pattern = 3-cost)
  ...
  v3.0 — 2026-04-24 (REWRITE):
    → 3 Generative Primitives, §16 (Cô đơn deep analysis)
    → Connection = ❶ hardware + ❷ SPM (2-tầng)
    → 2-luồng reward, 8 pathways fire NGƯỢC
    → Backup: backup/Connection-v2.0-backup.md

  v3.1 — 2026-04-28:
    → Body-Coupling.md reference note (mechanism deep-dive)

  v3.2 — 2026-05-15:
    → Alzheimer caregiver = one-sided body-coupling

  v3.3 — 2026-05-16:
    → §4.2 F1/F2 resonance quality: F1+F1 deepest, F1+F2 functional, F2+F2 transactional
    → Double empathy (Milton 2012) = universal mechanism

  v4.0 — 2026-05-17 (Inter-Body drill integration):
    → §0.0b NEW: Architecture B — Social = Requirement (4 reasons)
    → §3.1 ENRICHED: 2-source social need (hardware + compiled baseline)
    → §3.2 UPDATED: Compiled/Fresh terminology (F1=Compiled, F2=Fresh)
    → §4.2 ★ REWRITTEN: 2-Stream Architecture, by-product match, anti-match,
      per-pair topology, conditions broadened, PFC=Lawyer in detection
    → §4.3 ENRICHED: "skip pattern" → 3-cost model (3 independent sources)
    → §10.2 ENRICHED: 3-cost explains trục ③ (SPM library depth)
    → §11 ENRICHED: 5-Channel Input Vector × connection quality
    → §16.5 NEW: PFC=Lawyer in "cô đơn" domain (mask/amplify/mislabel)
      + Domain = Final Arbiter for connection quality
    → §17 UPDATED: +6🟢 (Panksepp, Gazzaniga, Haidt, Nisbett&Wilson, Trivers,
      Schultz, Kahneman/Klein) + 13🟡 new items + 1🔴 new
    → §18 UPDATED: ALL version numbers, +Inter-Body ref, +Logic-Feeling,
      +NPF, +Gap-Direction, +BF-Label, restructured changelog
    → Backup: backup/Connection-v3.3-backup.md
    → ALL 22+ original citations PRESERVED
```
...
>
> *Mẹ cho ăn. Bạn tạo Resonance — chunks match, mới emerge. Đồng nghiệp build cùng.*
