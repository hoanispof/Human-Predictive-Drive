...
session: N+4a
scope: File định hướng nhẹ. Không phân tích mới. Tổng hợp kiến trúc 3 lớp + cách chia 4-folder mechanism + đóng góp từ các sub-folder đã drilled (N+1 H8, N+2 H9, N+3 Body-Feedback-Precondition). Định hướng reader trước khi vào 01 + 02 + body-input arcs.
...
LỚP 1: BODY-FEEDBACK                                             ✅ Xong N+3
         ...
         5 điều kiện tiên quyết (Body-Feedback-Precondition) cho signal → feeling
         có ý thức.
         ───────────────────────────────────────────
```
...
| **H10** | **Body-Feedback-Precondition — Body Signal Precondition (5 điều kiện)** | **Body-Feedback-Draft ✅** | **— (standalone N+3 drilled)** |
...
4. **Bottleneck Precondition-2 Chunk-Substrate rõ ràng nhất** — Body-Feedback-Draft N+3 xác định "Chunks Base Adequacy" là keystone; cửa sổ 0-24 tháng cho thấy bottleneck này trực tiếp nhất
...
**Các claim chính**:
- **Body-Feedback-Precondition — Body Signal Precondition Hypothesis**: Body signal fires khi TẤT CẢ 5 điều kiện tiên quyết được thỏa mãn:
  ...
  3. **Precondition-3 Delta-Gate** (prediction-delta threshold) — biến động đủ lớn cho VTA detect (dựa trên habituation)
  4. **Precondition-4 Match-Range** (Goldilocks zone) — dynamic: vừa đủ mới + đúng hướng (không quá quen thuộc cũng không quá xa lạ)
  5. **Precondition-5 Compile-Gate ⭐** (Chunk association tag) — chunks fire có pattern liên kết được xác định bởi body state fingerprint trong thời gian compile: novelty-path body state (novelty cortisol + dopamine/opioid) → chunks gắn opioid → "thích dùng"; threat-path body state (threat cortisol + NE/adrenaline) → chunks gắn cortisol-avoidance → "tránh dùng". Xem [06a §7.0.1](06a-Interoceptive-Bladder-Keystone.md#§7.0.1) cho cơ chế được tinh chỉnh (cùng phân tử, body state khác nhau, kết quả ngược chiều).
- **Lớp body-feedback (Feel-RawSignals–Feel-Integration)** ≠ **Lớp Feeling (Feel-Consciousification–Feel-Explanation)**:
...
- **Gradient phát triển** (các ví dụ của user được ánh xạ): đái / đau / discomfort / buồn đều cho thấy Feel-RawSignals–Feel-Integration bất biến nhưng Feel-Consciousification–Feel-Explanation compile dần dần qua tích lũy chunks + caregiver mirroring + language emergence
...
- **Precondition-2 Chunk-Substrate = câu hỏi cơ chế trung tâm của F1**. N+3 xác định Precondition-2 là keystone developmental bottleneck nhưng chưa drill HOW chunks tích lũy. F1 trả lời điều này qua 12-file body-input axis drill.
- **Bladder gradient** (§5.5.1 của 01-Foundation): trẻ sơ sinh Feel-RawSignals–Feel-Integration only → trẻ lớn chút post-hoc Feel-Consciousification → trẻ biết gọi Feel-Observation–Feel-Labeling → người lớn full Feel-Explanation. ĐÂY chính là developmental arc đầy đủ mà F1 [06a-Interoceptive-Bladder-Keystone.md](06a-Interoceptive-Bladder-Keystone.md) sẽ deep-drill như phần trình bày "đơ mặt".
...
- **Precondition-5 Compile-Gate — Body state at compile** (được tinh chỉnh N+4c2-REV1, trước đây là "Cortisol tagging"): F1 cung cấp cơ chế critical period + sự phân biệt novelty vs threat cortisol + neural wear dimension + 4-threshold gradient + phân loại 3 cơ chế × 3 nguồn gốc (Nút Thắt 7 trong plan §0.6). Drill home [06a §7](06a-Interoceptive-Bladder-Keystone.md#§7) + các cam kết framework upstream [06a §7.0](06a-Interoceptive-Bladder-Keystone.md#§7.0).
...
```
   H8 (Learning-Cycle)          H9 (Agent)              Body-Feedback-Precondition (Body-Feedback)
   ──────────────────          ────────────            ───────────────────
   ...
   6+1 cơ chế ngủ              như cơ chế solo          Precondition-2 Chunk-Substrate
...
**Vị trí của F1**: F1 nằm ở **upstream** của cả 3 hypotheses đã drilled. F1 hỏi "**chunks compile như thế nào ngay từ đầu, ở cấp độ substrate, trước khi chúng có thể cycle (H8), trước khi chúng có thể mô hình hóa agents (H9), trước khi chúng có điều kiện tiên quyết cho feeling (Body-Feedback-Precondition)**". Cửa sổ phát triển 0-24 tháng được chọn vì substrate compile là rõ ràng nhất ở đó.
...
2. **Cơ chế Precondition-2 Chunk-Substrate** ⭐ **KEYSTONE** → drill home [06a-Interoceptive-Bladder-Keystone.md](06a-Interoceptive-Bladder-Keystone.md) §6
...
- ✅ Tổng hợp bằng chứng substrate chunk H1 (cross-folder triangulation với H8/H9/Body-Feedback-Precondition)
...
Tích hợp cuối cùng của tất cả 4 sub-folder + bộ ba đã drilled (H8/H9/Body-Feedback-Precondition). Cung cấp:
...
- [../Body-Feedback-Draft/02-Dissonance.md](../Body-Feedback-Draft/02-Dissonance.md) — Body-Feedback-Precondition các trường hợp dissonance
- [../Body-Feedback-Draft/03-Reward.md](../Body-Feedback-Draft/03-Reward.md) — Body-Feedback-Precondition các trường hợp reward
- [../Body-Feedback-Draft/04-Integration.md](../Body-Feedback-Draft/04-Integration.md) — Body-Feedback-Precondition tích hợp + Precondition-5 finalization
...
3. **Tại sao interoception là keystone body-input?** → [06a-Interoceptive-Bladder-Keystone.md](06a-Interoceptive-Bladder-Keystone.md) §6 (liên kết Precondition-2)
...
- Đóng góp từ 3 sub-folder đã drilled (triangulation H8/H9/Body-Feedback-Precondition)
