...
scope: |
  ...
  +Compilable Architecture framing (WHY body-feedback system this complex).
...
dependencies: |
  ...
  Reward-Signal-Architecture.md v2.0 (Evaluative/Direct-State, 5 Profiles, Evaluative Gates Direct-State, Compilable Architecture)
...
> 5. Compilable Architecture (general-purpose reward) = WHY this complexity exists
...
│   └── Reward-Signal-Architecture.md (1,986L) — ⭐ Reward-Signal-Architecture v2.0
│         Evaluative/Direct-State distinction, E₀→E₃ gradient, Evaluative Gates Direct-State,
│         5 Reward Profiles, Temporal Stack, Compilable Architecture connection
│
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ④ Gap-Direction.md v2.0 — WHY signals are personal
     ...
     → +v2.0: By-product match × gap direction, Compiled/Fresh ×
       evaluation, Compilable Architecture connection, 5-Channel × install
  ...
  ⑤ Reward-Signal-Architecture.md v2.0 — WHAT kinds of reward
     ...
     → +v2.0: Compilable Architecture = WHY Evaluative possible,
       Compiled/Fresh = E₀→E₃ mechanism, domain verification
...
```
🟡 COMPILABLE ARCHITECTURE = TẠI SAO FOLDER NÀY CẦN 16,500 LINES:
  ...
  Humans (Compilable Architecture — Inter-Body-Mechanism.md §1):
  ...
  CONSEQUENCE cho folder này:
    Body-Feedback-Mechanism: 2 sources × 3 dynamics = WHY (general-purpose creates variety)
    Gap-Direction: gap = personal = WHY (compiled differently per person)
    Reward-Signal-Architecture: Evaluative/Direct-State + 5 Profiles = WHAT (evaluation diversity)
  ...
  WITH Compilable Architecture: folder = 10 files, 16,500L (complex adaptive system).
...
⭐ 4TH CLASSIFICATION AXIS — HOW CHUNKS FIRE TẠO RA SIGNAL:
  ...
  2 NGUỒN ĐẦU VÀO (Body-Feedback-Mechanism v2.0 §2):
  ...
  BODY-NEED = 2-SOURCE AGGREGATE (Body-Feedback-Mechanism v2.0 §1):
...
⭐ H10 v3 — BODY SIGNAL PRECONDITION HYPOTHESIS:
  ...
  ⭐ H10 × EVALUATIVE/DIRECT-STATE (Reward-Signal-Architecture v2.0 §1.3):
    Evaluative: ALL 5 preconditions required (full evaluation)
    Direct-State: P1 simplified, P2-P5 reduced/N/A (hardware pathways)
    → Direct-State = RELIABLE always-available (not gated by chunks)
...
🟡 TRAINING GRADIENT — chunks base determines appreciation:
  ...
  → Reward-Signal-Architecture v2.0: E₀→E₃ gradient = WHY expert sees MORE (deeper compiled evaluation)
```
...
🟡 "CÀNG CỐ GẮNG LÂU THÌ CÀNG SƯỚNG" — mechanism:
  ...
  Reward-Signal-Architecture v2.0 connection:
  ...
  → "Einstein days of ecstasy" = E₃ coherence + massive ④ relief compound
```
...
🟡 HEDONIC TRAP — WHY "GIÀU CHÁN":
  ...
  HEDONIC ADAPTATION (🟢 Brickman 1978):
    ...
    → Reward-Signal-Architecture v2.0: = primarily Evaluative phenomenon (VTA habituation)
    → Direct-State RESISTANT — "hedonic floor" persists
...
CONTENT ABSORBED BY FEELING DEEP (authoritative versions elsewhere):
  ...
  ├──────────────────────────────┼──────────────────────────────────────┤
  │ ⭐ Body-Need 2-source model  │ Body-Feedback-Mechanism v2.0 §1 (Body-Need aggregate)   │
  │ ⭐ Chunk dynamics (4th axis) │ Body-Feedback-Mechanism v2.0 §2-§3 (Shift/Miss/Gap)    │
  ...
  │ ⭐ 5 Reward Profiles         │ Reward-Signal-Architecture v2.0 §4 (5 "hương vị")         │
  │ ⭐ Reward calibration        │ Reward-Calibration v1.1 (Goldilocks)│
  │ 2 input sources              │ Body-Feedback-Mechanism v2.0 §2 (Sensory/Pattern-Driven)│
  │ Compound mechanism           │ Body-Feedback-Mechanism v2.0 §4                         │
  │ Quality Baseline Shift       │ Body-Feedback-Mechanism v2.0 §5 (SNC research)         │
  │ Conditional Interaction Model│ Reward-Signal-Architecture v2.0 §6 (4 variables)           │
  │ 3 types State Dependence     │ Reward-Signal-Architecture v2.0 §7 (Cabanac refined)      │
  │ 5 Forces Holding Model       │ Reward-Signal-Architecture v2.0 §10 (sustained creative)  │
...
📚 INTER-BODY DRILL (v2.0 connections):
  ...
  Inter-Body-Mechanism.md v1.0   → Compilable Architecture, 3 foundations,
                                    Compiled/Fresh, 3-cost, domain arbiter
  Body-Base.md v3.1              → entry point, L0-L1, Compilable Architecture
  Body-Coupling.md v2.0          → coupling mechanism, Entity-Compiled
  Self-Pattern-Modeling.md v3.0                    → Compiled/Fresh, per-domain
...
>   §6: H10 5-precondition hypothesis + Evaluative/Direct-State × H10 connection
...
>
> **v1.1 → v2.0 changes:**
>   +§1.1 complete folder listing (3 files added: Gap-Direction, Reward-Signal-Architecture, Calibration)
>   +§1.2 3-tier reading guide (restructured for 10 files)
>   +§1.3 Compilable Architecture framing (WHY complexity exists)
>   +§2 Compiled/Fresh × Dual-Pull connection
>   +§3 Domain=arbiter note
>   +§4 Body-Feedback-Mechanism v2.0 refs + Body-Need summary
>   +§6 H10 × Evaluative/Direct-State connection (Reward-Signal-Architecture v2.0)
>   +§7 Gap-Direction + Reward-Signal-Architecture connections in cases
>   +§8.1 PFC=Lawyer note, §8.2 Evaluative habituation + Calibration ref
>   +§9 expanded unique contributions table (6 new ⭐ entries)
>   +§10.1 new 🟢 + 🟡 items (Evaluative/Direct-State, insula, SNC)
