...
scope: |
  ...
  v2.1: +ENGINE/ROAD/VEHICLE alignment (era = ROAD CONDITIONS thay đổi),
  +Simulation-Engine lens, +Gap-Distribution-Profile era impact, +Hardware-Subsidy shift,
  +Phantom 4-factor (AI relationships), +Connection-Education v1.0.
purpose: |
  ...
  Qua ENGINE/ROAD/VEHICLE: file này = ROAD CONDITIONS analysis
...
dependencies:
  ...
  core-mechanism:
    ...
    - Imagine-Final.md v3.0 — constructive simulation
  pfc-simulation:
    - PFC-Operations.md v1.1 — PFC Budget
    - Simulation-Engine.md v1.0 — 1 engine, 3 components (AI = computational analogue)
  agent-entity:
    - Self-Pattern-Modeling.md v3.1 — 1 mechanism × 3 dimensions
    - Entity-Access.md v1.2 — gradient Mức 0-5 (AI = Mức 0 Tool)
    - Connection.md v5.0 — Phantom 4-factor
  body-feedback:
...
changes_v2:
  ...
  - §5 Era-specific skills: aligned Domain-Knowledge-Map §2 (6 domains including Basic Science)
...
changes_v2.1:
  - Quote block: ENGINE/ROAD/VEHICLE alignment (era = ROAD CONDITIONS thay đổi)
  - §1 +Simulation-Engine note (AI = computational Simulation-Engine analogue)
  - §2 +Body-Base v3.1, Self-Pattern-Modeling v3.1 rename
  - §3 +Connection-Education v1.0 (replaces empathy-only ref), Self-Pattern-Modeling rename
  - §4 +⑦ Phantom AI relationships (Connection v5.0 Phantom 4-factor)
  - §5 +Gap-Distribution-Profile (era changing gap landscape)
...
>
> **Hardware = ENGINE — luôn chạy, source of ALL drives. SAME mọi era.**
> **Collective-Arc = ROAD — infrastructure thay đổi per era.**
> **Compilation = VEHICLE — cá nhân build 15-20 năm, CẦN ADAPT per era.**
>
> **File này = ROAD CONDITIONS analysis — era NÀY road thế nào?**
> **ENGINE (não) không đổi. ROAD (collective) THAY ĐỔI. VEHICLE cần adapt.**
>
...
TẠI SAO CẦN FILE NÀY:
  ...
  Tức là:
    ...
    → Domain-Knowledge-Map nói: "meta-learning = foundation domain" (§1.6)
...
🟡 TỐC ĐỘ THAY ĐỔI — PATTERN ĐẶC BIỆT:
  ...
  🟡 SIMULATION ENGINE LENS (Simulation-Engine.md v1.0):
    → AI = computational Simulation-Engine analogue
    → Não người có 1 Simulation-Engine (3 components, 3 axes)
    → AI = external simulation engine — nhanh hơn, rộng hơn, nhưng KHÔNG có body
    → = AI bổ trợ simulation → con người vẫn cần NỘI tại Simulation-Engine
    → = Dạy CÁCH DÙNG cả hai (internal + external simulation) = era-skill
```
...
🟡 JOB MARKET THAY ĐỔI:
  ...
  HUMAN-VALUE skills tăng giá trị:
    ...
    → Empathy: human connection, care, emotional support
      (Self-Pattern-Modeling v3.1 — social processing = irreplaceable)
    → Somatic skills: surgery, craft, physical care, sports, performance
      (Body-Base.md v3.1 — body = processing channel AI không có)
...
🟢 SOCIAL NEEDS = SAME:
  ...
  → ⚠️ AI CÓ THỂ simulate connection (chatbots, AI friends)
    ...
    → = KHÔNG có Hardware-Subsidy, KHÔNG compile entity
...
6 UNCERTAINTIES LỚN:
  ...
  ⑦ AI RELATIONSHIPS — PHANTOM TYPE (Connection.md v5.0 Phantom 4-factor)
     → AI companions, chatbots, AI friends = CÓ THẬT và GROWING
     → Phantom 4-factor: AI relationship = phantom type — tưởng connection, thiếu reciprocity
     → Entity-Access Mức 0 Tool: AI KHÔNG compile entity, KHÔNG có Hardware-Subsidy
     → Trẻ growing up với AI companion → TƯỞNG có connection → body-base BIẾT thiếu
...
④ INFORMATION OVERLOAD = NEW THREAT SOURCE
  ...
  → Chronic low-grade cortisol amplification
    (Cortisol-Baseline.md v2.1 — most common modern cortisol pattern)
...
⑥ ERA-SPECIFIC SKILLS — THÊM, KHÔNG THAY THẾ
  ...
  │                                                              │
  │ ERA-SPECIFIC (2025+, THÊM ON TOP — Domain-Knowledge-Map §2):                  │
...
⭐ ERA IMPACT ON GAP LANDSCAPE (Gap-Distribution-Profile v1.1):
  → AI era THAY ĐỔI gap landscape ở population level:
    → Access democratized → gap CÓ THỂ hình thành ở domains TRƯỚC ĐÂY locked
    → Nhưng: attention fragmentation → gap landscape SHALLOW (nhiều domain, không sâu)
    → Credential inflation → triệu người cùng gap profile → oversupply
  → Healthy: AI era mở NHIỀU ROAD → gap landscape CAN BE more diverse
  → Unhealthy: nếu mọi người dùng AI CÙNG CÁCH → gap HOMOGENIZATION tăng
  → = Curriculum design cần TẠO gap diversity, không để AI đồng hóa

⭐ ERA IMPACT ON HARDWARE-SUBSIDY (Entity-Valence-Dynamics.md v1.0 §5):
  → Era thay đổi SUBSIDY SOURCES:
    → Parent = MAX subsidy — KHÔNG ĐỔI (biology)
    → Teacher = MODERATE subsidy — CÓ THỂ BỊ GIẢM nếu class size tăng
    → AI = NONE (Mức 0 Tool — Entity-Access v1.2)
  → ⚠️ Era trend: tăng AI, giảm teacher time → tổng subsidy GIẢM
  → = Nếu AI THAY THẾ teacher → MẤT Hardware-Subsidy MODERATE
  → = AI phải SUPPLEMENT teacher, KHÔNG THAY THẾ
  → = Parent role CÒN QUAN TRỌNG HƠN trong AI era (MAX subsidy = irreplaceable)
...
⭐ CÁI FILE NÀY CÓ THỂ LÀM:
  ...
  ✅ Identify era-specific skill domains
     → Aligned with Domain-Knowledge-Map §2 (6 domains)
...
→ Entity-Valence-Dynamics.md v1.0 — Structural/current, Hardware-Subsidy
  File NÀY §5: Hardware-Subsidy spectrum — AI = NONE.

→ Imagine-Final.md v3.0 — Constructive simulation
  Era: Imagine-Final range EXPANDS (more possible futures).

→ Simulation-Engine.md v1.0 — 1 engine, 3 components
  File NÀY §1: AI = computational Simulation-Engine analogue.


═══════════════════════════════════════════════════════
AGENT-ENTITY + BODY-FEEDBACK (Tầng 1)
═══════════════════════════════════════════════════════
...
→ Entity-Access.md v1.2 — Gradient Mức 0-5
  AI = Mức 0 Tool. KHÔNG Hardware-Subsidy.

→ Connection.md v5.0 — Phantom 4-factor
  File NÀY §4 ⑦: AI relationships = phantom type.

→ Reward-Signal-Architecture.md v2.1 — Evaluative/Direct-State Reward
  Assessment challenge: AI answer = surface → miss Evaluative depth.

→ Dissonance-Signal-Architecture.md v1.0 — Evaluative/Direct-State Dissonance
  Information overload = compound dissonance.

→ Gap-Distribution-Profile.md v1.1 — Per-person gap landscape
  File NÀY §5: era changing gap landscape.
