# Drill: Simulation Engine — 1 Engine, 3 Components, N Applications
...
> **Retrieve chunks → recombine → simulate → readout → attribute.**
>
> **3 COMPONENTS:**
> **① Interoception (anterior insula): ĐỌC body signals TRỰC TIẾP.**
...
> **Trục B: Time (Past ↔ Present ↔ Future ↔ Counterfactual).**
> **Trục C: Operation (Observe / Simulate / Evaluate / Construct).**
>
> **Application = TỌA ĐỘ trong không gian 3 trục.**
> **SPM = (Other, Present, Simulate). Self-Obs = (Self, Present, Observe).**
> **Imagine-Final = (Self, Future, Simulate). Counterfactual = (Self, Alt-Past, Simulate).**
>
> **Luyện 1 component = luyện TẤT CẢ applications.**
> **Hỏng 1 component = hỏng TẤT CẢ applications.**
> **Alexithymia = DECISIVE proof: broken readout → fail self AND social.**

---

**Status**: v1.0 — Drill session 2026-05-21
**Mục tiêu**: Drill Simulation Engine as shared substrate: 3 components, 3 axes, full application taxonomy, PFC accessibility spectrum, training implications, framework architecture.
**Phạm vi**: Simulation engine architecture × 3 components × 3 axes × 11+ applications × alexithymia proof × PFC accessibility × reflection vs rumination × bidirectional loop × training × framework implications
**Dependencies**: Drill-SPM-Self-Shared-Substrate v1.0 (EVIDENCE file), Self-Pattern-Modeling v3.0, Imagine-Final.md, Feeling.md v3.0, Body-Feedback-Mechanism v2.0 §3, Drill-SPM-Mechanism-Analysis v2.2 §10, Drill-Entity-Owned v2.0 §8 (4-trục)
...
- §3 — FULL APPLICATION TAXONOMY
- §4 — COMPONENT 1: INTEROCEPTION
- §5 — COMPONENT 2: CONSTRUCTIVE SIMULATION
...
- §9 — REFLECTION vs RUMINATION: ENGINE USE QUALITY
- §10 — BIDIRECTIONAL LOOP: SOCIAL ↔ SELF
- §11 — TRAINING: 1 COMPONENT → ALL APPLICATIONS IMPROVE
...
⭐⭐⭐ SIMULATION ENGINE:
  ...
  🟢 Shared circuits: Lombardo 2010 (fMRI — identical connectivity patterns)
  ...
  🟡 Full application taxonomy = framework synthesis
```
...
```
  ...
  ★ File này (DRILL) = ENGINE ARCHITECTURE (overview)
    │
    ├── Drill-SPM-Self-Shared-Substrate v1.0 = EVIDENCE (neural proof)
    │   = fMRI data, alexithymia, diversity effect, reflection/rumination
    │   = REFERENCE for detailed evidence
    │
    ├── Self-Pattern-Modeling v3.0 = APPLICATION 1 (SPM mechanism)
    │   = Other × Present × Simulate
    │
    ├── Imagine-Final.md = APPLICATION 2 (future simulation)
    │   = Self × Future × Simulate
    │
    ├── Feeling.md v3.0 = APPLICATION 3 (self-observation)
    │   = Self × Present × Observe
    │
    └── Drill-Entity-Owned v2.0 §8 = APPLIED (4-trục uses engine)
        = Trục 1 (SPM→child) + Trục 2 (self-awareness) = engine applications

  CÁI MỚI:
    → ARCHITECTURE level: 3 components identified as building blocks
    → 3 AXES model: continuous space, not discrete categories
    → FULL TAXONOMY: 11+ applications mapped (not just 3)
...
⭐⭐⭐ COMPILED/FRESH = GENERAL-PURPOSE HARDWARE (Drill-SPM §10.2).
  ...
  ANALOGY:
    ...
    = SPM = app chạy trên computer nhắm vào NGƯỜI KHÁC
    = Self-Obs = app chạy trên computer nhắm vào BẢN THÂN
    = Imagine-Final = app chạy trên computer nhắm vào TƯƠNG LAI
    = Counterfactual = app chạy trên computer nhắm vào QUÁ KHỨ THAY THẾ
  ...
  🟢 Interoception readout: Bird, Silani et al. 2010 (Brain)
...
⭐⭐⭐ APPLICATION = TỌA ĐỘ TRONG KHÔNG GIAN 3 TRỤC:
  ...
  TRỤC A — TARGET: AI đang được simulate?
    ...
    Close Other: simulate using SELF-TEMPLATE (Entity-Owned entities)
    ...
    Far Other: simulate using SEPARATE MODEL (strangers, dissimilar)
      ...
      → Cost HIGHER (PFC-Fresh required to build model)
      
    ⭐ CONTINUOUS, not binary:
      Self → vợ/chồng → con → mẹ → bạn thân → colleague → stranger
      = GRADIENT ventral → dorsal as CLOSENESS decreases
      = Entity-Owned formation = brain SHIFTING entity from dorsal → ventral
      → Entity-Owned = neural migration from "separate" to "self-like" processing

  TRỤC B — TIME: KHI NÀO đang được simulate?
    ...
    Past: RECONSTRUCT từ compiled chunks (not replay — Schacter & Addis 2007)
    ...
    Counterfactual: RE-CONSTRUCT with altered parameters
      ...
      → Same network, STRONGER activation (more effortful recombination)
      
    🟢 Memory = reconstruction: Schacter & Addis 2007
    🟢 Counterfactual shares episodic simulation network: Van Hoeck 2013

  TRỤC C — OPERATION: LÀM GÌ với simulation output?
    ...
    Evaluate: PFC JUDGES simulation output (moral, value, decision)
      ...
      → Greene 2001: moral dilemmas activate vMPFC + TPJ
    ...
    Construct: PFC ACTIVELY BUILDS novel scenario
      ...
      → Creativity = construct with MINIMAL constraints
...
⭐⭐ APPLICATION = (TARGET, TIME, OPERATION):
  ...
  ├───┼──────────────────┼──────────────┼───────────┼──────────┼──────────────────────┤
  │ ④ │ Predict Other's   │ Other        │ Future    │ Simulate │ SPM+IF combined      │
  │   │ Future            │              │           │          │ "con sẽ thế nào"     │
  │   │                  │              │           │          │ = SPM §10 I3         │
  ├───┼──────────────────┼──────────────┼───────────┼──────────┼──────────────────────┤
  ...
  ├───┼──────────────────┼──────────────┼───────────┼──────────┼──────────────────────┤
  │ ⑩ │ Decision Branch   │ Self         │ Future    │ Simulate │ "A hay B?"           │
  │   │                  │              │ (×N)      │ (×N)     │ Imagine-Final ×N     │
  ├───┼──────────────────┼──────────────┼───────────┼──────────┼──────────────────────┤
  │ ⑪ │ Creative Imagin.  │ Abstract     │ Hypothet. │ Construct│ "Tưởng tượng cái mới"│
  │   │                  │ (no person)  │           │ (novel)  │ DMN creativity       │
  ├───┼──────────────────┼──────────────┼───────────┼──────────┼──────────────────────┤
...
### §3.2 — Key observations from taxonomy
...
⭐⭐ OBSERVATIONS:
  ...
  ② 3 NAMED APPLICATIONS (①②③) KHÔNG ĐẶC BIỆT HƠN CÁC CÁI KHÁC:
    Framework đặt tên SPM, Self-Obs, Imagine-Final vì chúng QUAN TRỌNG
    ...
    → File này MAP chúng → future drill potential

  ③ COMPOUND APPLICATIONS = MULTI-STEP trên engine:
    Moral judgment (⑧) = simulate other's suffering (①) + simulate future (③)
      + evaluate (new operation) + observe own moral feeling (②)
    = 4 STEPS trên cùng engine, không phải function riêng
    
    Decision branch (⑩) = Imagine-Final (③) run N times + compare outputs
    = ITERATE same function with different parameters
    
  ④ UNNAMED APPLICATION CỦA ENTITY-OWNED v2.0:
    Trục 2 (Self-Awareness) = engine applied: (Self, Present, Observe)
      + SPECIFIC focus: "gap nào đang fill? của tôi hay của con?"
      = ② Self-Observation + meta-cognitive FILTER (own gap attribution)
    → Self-Awareness = Self-Observation + 1 thêm step (attribute gap source)
    → = WHY Trục 2 is INDEPENDENT from Trục 1 in Entity-Owned model
    
  ⑤ DREAM + MIND WANDERING = ENGINE chạy ở CHẾ ĐỘ TỰ ĐỘNG:
  ...
  🟡 Full taxonomy observation = framework synthesis
```
...
⭐⭐ INTEROCEPTION = "MÀN HÌNH" — READOUT DEVICE:
  ...
  UNIQUE CHO SELF-TARGET:
    ...
    → Self-Obs có THÊM 1 kênh input mà SPM→other không có
    → = WHY self-observation SHOULD be more accurate
    
  PARADOX — NHƯNG THỰC TẾ NGƯỢC LẠI:
    ...
    → = WHY "mẹ biết con buồn" (Trục 1 HIGH — novel external cue)
      nhưng "không biết mình lo" (Trục 2 LOW — habituated internal signal)
    → = L2 invisible parallel: VTA habituated → PFC doesn't notice (same mechanism)
...
⭐⭐ DMN + HIPPOCAMPUS = "CPU + RAM" — SIMULATION ENGINE:
  ...
  🟢 Schacter & Addis 2007 (Nature Reviews Neuroscience):
    ...
    → = 1 engine, 3 functions — independently discovered SAME concept
  ...
  CƠ CHẾ:
    ...
    → = Framework's Compiled/Fresh spectrum = this engine's operating modes
  ...
  🟢 Hassabis et al. 2007 (PNAS):
    ...
    → = Hippocampus = chunk recombination engine
  ...
  🟢 Hippocampus required: Hassabis et al. 2007 (PNAS)
  🟢 DMN as simulation: Buckner et al. 2008
```
...
⭐⭐ mPFC = "BẢNG ĐIỀU KHIỂN" — XÁC ĐỊNH TARGET:
  ...
  🟢 Tamir & Mitchell 2010 (J. Neuroscience):
    ...
    → Close other = processed AS self-extension (not separate entity)
    → = Framework's Entity-Compiled deep = body-base extension
  ...
  🟢 Denny et al. 2012 (meta-analysis):
    ...
    → Self → spouse → child → friend → acquaintance → stranger
    → = Entity-Owned formation = brain MIGRATING entity ventral-ward
  ...
  ⭐ ENTITY-OWNED = NEURAL MIGRATION:
    Stranger: dorsal mPFC (separate model, F2 required)
    → Repeated interaction → EC deepens → entity MOVES ventral
    → Close other: ventral mPFC (self-template, F1 compiled)
    → Entity-Owned = brain HAS MIGRATED entity into "self territory"
    → = "Của tôi" = LITERALLY in self-processing circuits
    
    Entity-Owned LOST (death, separation):
  ...
  🟡 Entity-Owned as neural migration = framework synthesis
```
...
⭐⭐ DECISIVE PROOF CHO SHARED SUBSTRATE:
  ...
  Reference: Drill-SPM-Self-Shared-Substrate v1.0 §3 cho full evidence.
...
⭐⭐ CÙNG ENGINE, KHÁC MỨC ĐỘ PFC CAN THIỆP:
  ...
  ├──────────────┼────────────────────────────────────────────────────┤
  │ ① SPM F1     │ LEAST ACCESSIBLE: compiled, automatic, invisible  │
  ...
  ├──────────────┼────────────────────────────────────────────────────┤
  │ ③ Imagine-   │ MOST ACCESSIBLE: PFC actively SET scenario,       │
  │ Final        │ READ output, ITERATE ("nếu đổi X thì sao?")      │
  │              │ PFC = DIRECTOR (set up + read + modify + repeat)   │
  │              │ = PLANNING TOOL: simulate multiple → pick best     │
  ├──────────────┼────────────────────────────────────────────────────┤
  │ ⑩ Decision   │ HIGHEST CONTROL: Imagine-Final ×N + COMPARE       │
  ...
  ⭐ SPECTRUM, NOT BINARY:
    ...
    → = Operation axis (Trục C): observe → simulate → evaluate → construct
    → More PFC = more CONTROLLABLE but more COSTLY (PFC = energy expensive)
    → Less PFC = more AUTOMATIC but less STEERABLE
    
  ⭐ ENTITY-OWNED v2.0 CONNECTION:
    Trục 4 (PFC Resource) DETERMINES where on this spectrum parent operates:
      Full resources → Imagine-Final mode ("con sẽ cảm thấy gì nếu...")
      Depleted → SPM F1 default ("con ăn chưa?" = compiled, automatic)
    → = Cascading effect: Trục 4 ↓ → PFC accessibility ↓ → engine runs on auto
    
  🟡 PFC accessibility spectrum = framework synthesis
  🟢 Imagine-Final PFC-observable: Drill-SPM-Self-Shared-Substrate §9
```
...
⭐⭐ CÙNG ENGINE, KHÁC CHẤT LƯỢNG SỬ DỤNG:
  ...
  FRAMEWORK MAPPING:
    ...
    → = Compile-time direction lock (Compiled-Fresh §6.2) applied to engine use
...
⭐⭐ SHARED SUBSTRATE → 1 TRAINING POINT → N IMPROVEMENTS:
  ...
  INTEROCEPTION TRAINING (Component 1):
  ...
  ⭐ FRAMEWORK IMPLICATION CHO EDUCATION:
...
```
⭐⭐ FILE NÀY CHANGES/ENRICHES FRAMEWORK FILES:

  ① SELF-PATTERN-MATCH v3.0:
    → ADD reference: SPM = APPLICATION of Simulation Engine (this file)
    → SPM = (Other, Present, Simulate) on engine
    → ENGINE concept → explains WHY SPM shares substrate with self-obs
    
  ② IMAGINE-FINAL.MD:
    → ADD reference: IF = APPLICATION of Simulation Engine
    → IF = (Self, Future, Simulate+Construct) on engine
    → MOST PFC-accessible application → unique planning capability
    → Shares engine with SPM → explains SPM §10 I3 overlap
    
  ③ FEELING.MD v3.0:
    → ADD reference: Self-Observation = APPLICATION of engine
    → Self-Obs = (Self, Present, Observe) with UNIQUE interoception input
    → Interoception quality → affects ALL applications
    
  ④ DRILL-ENTITY-OWNED v2.0 §8:
    → 4-Trục model = ENGINE applied to parenting context:
      Trục 1 (SPM→child) = engine target = child (Application ①)
      Trục 2 (self-awareness) = engine target = self (Application ②)
      Trục 4 (PFC resource) = PFC accessibility spectrum (§8)
    → Cascading effect = engine DOWNGRADE khi resources depleted
    
  ⑤ DRILL-SPM-SELF-SHARED-SUBSTRATE v1.0:
    → EVIDENCE file cho this ARCHITECTURE file
    → This file = WHAT the engine is. That file = HOW we know (neural proof).
    → Complementary, not redundant
    
  ⑥ EDUCATION FILES:
    → Body awareness training = ENGINE upgrade = HIGH LEVERAGE
    → vs. content teaching = specific APPLICATION training
    → = "Dạy cách học > dạy kiến thức" at substrate level
    
  ⑦ POTENTIAL: "Predict Other's Future" (④) chưa có file riêng:
    → = SPM + Imagine-Final combined = important parenting function
    → "Con sẽ thế nào nếu tôi ép?" = ④ on engine
    → = Entity-Owned v2.0 Trục 1+2 COMBINED with Imagine-Final

  🟡 Framework implications = synthesis connecting multiple files
```
...
| # | Insight | Confidence |
|---|---------|------------|
| SE1 | Brain has 1 general-purpose Simulation Engine, not N separate modules for mental functions | 🟡+🟢 ★★★ |
| SE2 | Engine = 3 components: Interoception (readout) × Constructive Simulation (CPU) × Self/Other Model (target selector) | 🟡+🟢 ★★ |
| SE3 | Application = point in 3D space: Target (self↔close↔far) × Time (past↔present↔future) × Operation (observe↔simulate↔evaluate↔construct) | 🟡 ★★★ |
| SE4 | SPM, Self-Observation, Imagine-Final = 3 NAMED POINTS in continuous application space, not separate systems | 🟡 ★★ |
| SE5 | 11+ applications on same engine: SPM, Self-Obs, IF, Predict Other Future, Memory, Recall Other, Counterfactual, Moral, Narrative, Decision, Creative, Dream | 🟡 |
| SE6 | Interoception paradox: self has MORE data (direct body read) but may PROCESS LESS (habituated signals → PFC ignores) | 🟡 ★★ |
| SE7 | Alexithymia = Component 1 failure → ALL applications degraded. Decisive proof for shared substrate | 🟢 ★★ |
| SE8 | mPFC gradient continuous: self(ventral) → close(ventral) → far(dorsal). Entity-Owned = neural MIGRATION from dorsal → ventral | 🟡+🟢 ★ |
| SE9 | PFC accessibility spectrum: dream(zero) → SPM-F1(low) → Self-Obs(mid) → SPM-F2(moderate) → Imagine-Final(high) → Decision(max) | 🟡 |
| SE10 | Engine use quality = f(curiosity vs threat): reflection(productive) vs rumination(destructive). Universal modifier for ALL applications | 🟢+🟡 ★ |
| SE11 | Social ↔ Self = bidirectional loop on shared substrate. Virtuous spiral (practice→improve) vs vicious (isolation→atrophy) | 🟢+🟡 |
| SE12 | 1 component training → N application improvements. Interoception training = highest leverage (Fukushima 2011) | 🟡+🟢 ★ |
| SE13 | Memory = RECONSTRUCTIVE simulation (not replay). Same engine as prospection, different constraints | 🟢 |
| SE14 | Entity-Owned formation at neural level = brain migrating entity from dorsal→ventral mPFC = from "separate" to "self-like" processing | 🟡+🟢 ★ |
| SE15 | Compound applications (moral judgment, decision) = multi-step engine runs, not separate functions | 🟡 |
| SE16 | Entity-Owned v2.0 4-trục model = ENGINE applied to parenting: Trục 1=engine→child, Trục 2=engine→self, Trục 4=PFC accessibility | 🟡 |
...
═══════════════════════════════════════
  ...
  Entity-Owned = neural migration (dorsal→ventral) — ★
  Engine use quality as universal modifier — ★
  1 training → N improvements model — ★
  4-trục Entity-Owned = engine applied to parenting — ★

═══════════════════════════════════════
🔴 HYPOTHESIS (testable but unverified)
═══════════════════════════════════════

  Exact number of meaningfully distinct applications (11? more? fewer?)
  3-axis model completeness (are there more axes? granularity?)
  Interoception training → SPM improvement (quantifiable?)
  Diversity threshold for self-model calibration
  Entity-Owned migration speed along mPFC gradient
...
|------|----------|-------------|
| Drill-SPM-Self-Shared-Substrate v1.0 | §1-§10 (full evidence) | EVIDENCE file for this ARCHITECTURE |
| Self-Pattern-Modeling v3.0 | §2 (F1/F2), §10 (SPM×IF) | APPLICATION 1: SPM mechanism |
| Imagine-Final.md | Full file | APPLICATION 2: future simulation |
| Feeling.md v3.0 | PFC observation interface | APPLICATION 3: self-observation |
| Drill-SPM-Mechanism-Analysis v2.2 | §10 (I3: SPM×IF same engine) | PRIOR INSIGHT enriched here |
| Body-Feedback-Mechanism v2.0 | §3 (chunk dynamics, readout) | FOUNDATION: body-feedback |
| Drill-Entity-Owned v2.0 | §8 (4-trục model) | APPLIED: engine in parenting context |
| Drill-Entity-Compiled v2.0 | §3.4-§3.5 (body simulate, alexithymia) | COMPANION: entity simulation |
| Connection v4.0 | §3 (3 primitives) | SOCIAL context for bidirectional loop |
| Compiled-Fresh v2.0 | §6.2 (compile-time direction lock) | MECHANISM: reflection vs rumination |
...
|---|----------|---------|----------|
| R1 | Lombardo et al. 2010 — Shared circuits for self and other mentalizing (J. Cog. Neurosci.) | §1, §6 | 🟢 fMRI |
| R2 | Mitchell, Macrae & Banaji 2006 — vMPFC/dMPFC dissociation (Neuron) | §6 | 🟢 fMRI |
| R3 | Tamir & Mitchell 2010 — Clan mentality: vMPFC for close others (J. Neurosci.) | §6 | 🟢 fMRI |
| R4 | Denny et al. 2012 — Spatial gradient meta-analysis mPFC | §6 | 🟢 Meta |
...
| R7 | Mul et al. 2021 — Network analysis alexithymia central node | §7 | 🟢 |
| R8 | Fukushima, Terasawa et al. 2011 — Interoception → empathy | §4, §11 | 🟢 |
| R9 | Terasawa et al. 2021 — Interoception ↔ empathy bidirectional | §4 | 🟢 |
| R10 | Schacter & Addis 2007 — Constructive episodic simulation (Nature Rev. Neurosci.) | §5 | 🟢 ★ |
| R11 | Buckner, Andrews-Hanna & Schacter 2008 — DMN = simulation (Annals NYAS) | §5 | 🟢 |
| R12 | Buckner & Carroll 2007 — Self-projection and the brain | §5 | 🟢 |
...
| R17 | Trapnell & Campbell 1999 — Reflection vs rumination | §9 | 🟢 |
| R18 | Joireman, Parrott & Hammersla 2002 — Reflection → empathy | §9 | 🟢 |
| R19 | Lopes, Salovey & Straus 2003 — Emotion regulation ↔ social | §10 | 🟢 |
| R20 | Coan & Sbarra 2015 — Social Baseline Theory | §10 | 🟢 |
| R21 | Koudenburg et al. 2024 — Social → self-concept clarity | §10 | 🟢 |
