...
> Chunk.md, Modality-Analysis.md, Attention-Spectrum.md
...
⭐ IMAGINE-FINAL = REFERENCE PATTERN mà toàn bộ hệ thống dùng để navigate:
  ...
  → (Khi compass ĐƯỢC CHỌN + COMMITTED → thành Anchor-Schema: Anchor-Schema.md)
  →   Anchor-Schema = compass ACTIVE — vô thức sync theo
  →   Trust = binding strength — giữ compass qua dissonance (Anchor-Schema.md §2)
  →   "Compass SAI" = Anchor-Schema ở MISMATCH hoặc DELUSION (Evaluation.md §4)
  →   "Compass ĐÚNG" = Anchor-Schema ở SWEET SPOT (Domain ✓ + Hardware ✓)
  → (Đánh giá CHẤT LƯỢNG hướng đi: Imagine-Final-Evaluation.md)
...
🟡 "Hiểu ai đó" = SIMULATE melody CỦA HỌ bằng melody CỦA MÌNH:
  ...
  → Mechanism behind "simulate melody người khác" = Empathy-Mirror:
    Tầng 1 vô thức: body mirror state HỌ → cảm nhận melody HỌ ở body level
    Tầng 2 PFC: mPFC simulate SÂU (dùng schema MÌNH → approximate melody HỌ)
    → 3 layers overlap (species/culture/personal) = mirror accuracy
    → Chi tiết mechanism: Empathy-Mirror.md §2-3
```
...
→ Chunk.md: chunk mechanism chi tiết
...
→ Imagine-Final.md: 14 ngưỡng imagine final — CLARITY (chi tiết §6)
→ Anchor-Schema.md: khi compass COMMITTED → sync mechanism + Trust + 4 nguồn
→ Imagine-Final-Evaluation.md: 2 trục đánh giá — QUALITY (compass đúng/sai)
→ Personal-Melody-Example.md: 3 profiles ví dụ
