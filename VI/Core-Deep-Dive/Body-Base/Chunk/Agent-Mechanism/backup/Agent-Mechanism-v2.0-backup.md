...
version: 2.0
created: 2026-04-15
rewritten: 2026-05-17 (v2.0 — ALL REWRITE: Architecture B, Compiled/Fresh, 2-Stream Resonance, By-product match, PFC=Lawyer, 3-cost, 5-Channel, Domain=Arbiter)
previous: v1.0 DRAFT → backup/Agent-Mechanism-v1.0-backup.md
status: v2.0 INTEGRATION HUB
scope: |
  INTEGRATION HUB cho Agent Mechanism trong framework.
  v2.0 KEY CHANGES (Phase 16 ALL REWRITE, Inter-Body drill 8 principles):
    ① Architecture B: WHY agent mechanism exists (general-purpose → 15-20yr compile → need others)
    ② SPM preview REWRITE: F1=Compiled, F2=Fresh (not Feeling/Logic labels)
    ③ Resonance preview REWRITE: 2-Stream Architecture, by-product match (SPM NOT required)
    ④ 3-cost model integrated (PFC draft + Suppress + Uncertainty)
    ⑤ PFC=Lawyer (narrative bias, not neutral judge)
    ⑥ 5-Channel Input Vector context
  ...
  Integration hub: connect SPM v3.0 + By-Product-Gap-Resonance v1.0 + body-need + schema + development.
purpose: |
  File NỀN TẢNG integration cho toàn bộ Agent/ folder:
  ① Định nghĩa Agent concept (reject binary, accept unified)
  ② Architecture B context: WHY agent mechanism exists
  ③ 3-concept split (Self-Pattern / SPM / Resonance)
  ④ Preview SPM v3.0 + By-Product-Gap-Resonance v1.0 ở high level (accurate, current)
...
previous_version: backup/Agent-Mechanism-v1.0-backup.md
parent: Core-Deep-Dive/Body-Base/Chunk/ (foundation file)
supporting_files:
  - Self-Pattern-Match.md v3.0 (solo forward mechanism deep drill)
  - By-Product-Gap-Resonance.md v1.0 (emergent mutual phenomenon deep drill)
dependencies:
  - Self-Pattern-Match.md v3.0 — Compiled/Fresh, PFC=Lawyer, 3-cost, per-domain
  ...
  - Body-Coupling.md v2.0 — coupling mechanism, extension/entanglement/mixed
  ...
  - Connection.md v4.0 — 3 Generative Primitives, agents=external tools
  - Empathy.md v2.2 — SPM function applied, 2-luồng organic
...
> → `Self-Pattern-Match.md v3.0` (solo mechanism) → `By-Product-Gap-Resonance.md v1.0` (mutual phenomenon)
...
- §2 — Reject binary Object-Agent (VTC = trigger, not category)
- §3 — Architecture (chunk substrate + function overlay + Compiled/Fresh)
- §4 — 3-concept split (Self-Pattern / Self-Pattern-Match / Resonance)
- §5 — Self-Pattern-Match preview (v3.0: Compiled/Fresh, PFC=Lawyer, 3-cost)
- §6 — Resonance preview (v3.1: 2-Stream, by-product match)
- §7 — Quality axes (4 axes + Compiled/Fresh dimension)
- §8 — Pattern-Type modalities (5 types)
- §9 — Gradient validation (17+ cases, 2-stream annotated)
...
- §14 — Failure modes (9 modes + PFC=Lawyer + Domain=Arbiter)
- §15 — Individual variation
- §16 — H9 falsifiable predictions (12 predictions)
- §17 — Framework status (Phase 1-16 updates)
- §18 — Open questions
...
> **Agent-reading = Self-Pattern-Match mechanism**: PFC retrieve self chunks matching
...
**Vai trò**:
1. Định nghĩa Agent concept (reject binary, accept unified)
2. Architecture B context: WHY agent mechanism exists
3. 3-concept split + accurate previews (SPM v3.0, By-Product-Gap-Resonance v1.0)
4. Gradient validation + schema override + collective + body-need feeder
5. Developmental timeline + failure modes + predictions
6. Entry point cho readers

**Không phải**:
- Không phải deep mechanism (→ `Self-Pattern-Match.md v3.0`)
- Không phải mutual phenomenon deep-dive (→ `By-Product-Gap-Resonance.md v1.0`)
- Không phải inter-body mechanism (→ `Inter-Body-Mechanism.md v1.0`)

### §1.2 — Reading flow

**Standalone entry** (hiểu Agent concept):
`Agent-Mechanism.md` full → optionally `Self-Pattern-Match.md` + `By-Product-Gap-Resonance.md`

**Mechanism reference** (cite mechanism):
Direct to `Self-Pattern-Match.md v3.0` (solo) hoặc `By-Product-Gap-Resonance.md v1.0` (mutual)

**Architecture context** (WHY mechanism exists):
`Inter-Body-Mechanism.md v1.0` §1 Architecture B → `Agent-Mechanism.md` §0.2

**Body-need feeder**:
`Agent-Mechanism.md` §12 → `Connection.md v4.0` → `Body-Coupling.md v2.0`
...
| `Body-Base/Feeling/backup/Self-Pattern-Match.md` | Inward self-labeling | Conflicts với forward simulation |
...
**IN scope**: Agent concept, 3-concept split, previews (SPM v3.0 + By-Product-Gap-Resonance v1.0), Architecture B context, gradient validation, schema override, Schema-Linked Processing, body-need feeder, developmental timeline, failure modes, individual variation, H9 predictions.

**OUT of scope**: Full Collective-Agent deep drill, consciousness/qualia, religious tradition comparative, neural correlate precise mapping, AI era implications beyond mention.
...
### §3.3 — Parallelism across entities
...
### §3.4 — Why entities need SPM (4 reasons + Architecture B)
...
### §3.6 — PFC = Lawyer in agent processing
...
| **Self-Pattern-Match** | Cơ chế self apply chunks → analyze target (solo forward) | Mechanism (direct) | Forward (prediction) | HOW I try to model |
...
### §4.3 — Self-Pattern-Match (mechanism — v3.0 definition)

**Self-Pattern-Match** = cơ chế self select chunks → apply làm template → analyze/predict agent's state/behavior. Fire 2 functions song song:
...
### §4.4 — Resonance (phenomenon — v3.1 definition)
...
**v3.1 KEY CHANGE**: SPM = NOT prerequisite. SPM = powerful ENHANCER (creates Stream 2).
...
## §5 — Self-Pattern-Match high-level preview (v3.0)

> **Detail**: `Self-Pattern-Match.md v3.0`. Section này = accurate preview + forward pointer.

### §5.1 — One-sentence definition (v3.0)

**Self-Pattern-Match** = solo forward simulation mechanism: select chunks từ self repertoire matching target → fire F1 Compiled (body simulate target state, automatic) + F2 Fresh (PFC chain predict target behavior, deliberate) song song. Per-Agent Valence gates direction. Context-dependent per agent/domain. PFC = Lawyer (narrative bias). 3-cost when Fresh.

### §5.2 — Compiled/Fresh as real axis

```
⭐ v3.0 KEY REFRAME: F1/F2 = Compiled/Fresh spectrum
...
## §6 — Resonance high-level preview (v3.1)

> **Detail**: `By-Product-Gap-Resonance.md v1.0`. Section này = accurate preview + forward pointer.

### §6.1 — Definition (v3.1 — BROADENED)
...
### §7.5 — Compiled/Fresh as 5th dimension (v2.0 addition)

v1.0 described 4 axes. v2.0 adds: **compilation level** of SPM about this target determines processing quality independent of 4 axes.
...
### §9.1 — Master table (v2.0 — 2-stream annotated)

| # | Target | Pattern-Type | Depth | Similarity | Feedback | Quality | 2-Stream |
|---|---|---|---|---|---|---|---|
| 1 | Person hợp tính | Composite | High | Self-similar | Rich | 🟢 | S1+S2 strong |
| 2 | Person khác tính | Verbal + affective | Moderate | Divergent | Rich | 🟡 | S1 mod, S2 effortful |
| 3 | Chó cúp đuôi | Affective only | Thin | Low | Limited | 🟡 | **S1 + Proto-S2** |
| 4 | Mèo stress | Somatic + affective | Thin | Low | Limited | 🟡 Weak | S1 only |
| 5 | Chuột | None | Very thin | Below threshold | None | 🔴 | No Resonance |
| 6 | Cục đá | N/A | N/A | N/A | N/A | N/A | N/A |
| 7 | Đức Mẹ Mode 1 | None | N/A | N/A | None | Schema | S1 (comfort) |
| 8 | Đức Mẹ Mode 2 | Composite imagined | Schema | Schema | Internal | 🟡 Virtual | S1+imagined |
| 9 | Bộ đội lạ | Affective + cognitive | Schema | Moderate | Minimal | 🟡 | S1 (warmth) |
| 10 | Quân giặc | Suppressed | Schema | Hostile | Minimal | 🔴 | Anti-match |
| 11 | Mother day 0 | Composite fresh | Growing | Moderate | Rich | 🟡 Fresh | S1 strong+S2 begin |
| 12 | Mother year 1 | Composite compiled | Deep | High | Rich | 🟢 | S1+S2 compiled |
| 13 | Mother yr 10 suppress | Verbal routine | Blocked | Blocked | Ignored | 🔴 | S1 habituated |
| 14 | Mathematicians | Visual-symbolic | Deep domain | Moderate | Rich | 🟢 Domain | S2 (domain) |
| 15 | Music fans | Affective | Moderate | High affective | Shared | 🟢 Affective | S1+weak S2 |
| 16 | Study group | Verbal-cognitive | Shared problem | Same skill | Rich | 🟢 Domain | S2 (cognitive) |
| 17 | Celebrity parasocial | Composite | Deep media | Medium | **Zero** | 🔴 Distorted | S1 one-sided |
| 18 | Mẹ-bé sơ sinh | Affective+somatic | Growing | Low (age) | Contingent | 🟢 Proto | **S1 CỰC MẠNH** |
...
🟡 2-luồng = framework synthesis. Details: Connection.md §3.3, VP.md §2.

**Mechanism deep-dive → Body-Coupling.md v2.0**: 3 Phase, extension/entanglement/neutral/system, compile_rate, foundational vs additive.
...
- **Grief** = L2 loss = body-base amputation. Intensity ∝ L2 depth.
...
Stage 2 (14-24m): SELF-AWARENESS + SPM BIRTH ★
  ...
  → = Birth of genuine Self-Pattern-Match. Before = only hardware pattern matching.
...
### §14.10 — Summary + framework insight
...
Brief — full treatment in Self-Pattern-Match.md v3.0.
...
### §17.1 — Updates COMPLETED (Phase 1-16)

Most v1.0 recommendations are NOW DONE via ALL REWRITE project:

| Phase | File | Status |
|---|---|---|
| 1 | Inter-Body-Mechanism.md v1.0 | ✅ Created |
| 2 | Body-Feedback-Mechanism.md v2.0 | ✅ Rewritten |
| 3 | Valence-Propagation.md v2.0 | ✅ Rewritten |
| 4 | Body-Base.md v3.1 | ✅ Rewritten |
| 5 | Body-Coupling.md v2.0 | ✅ Rewritten |
| 6 | Self-Pattern-Match.md v3.0 | ✅ Rewritten |
| 7 | By-Product-Gap-Resonance.md v1.0 | ✅ Rewritten |
| 8-11 | Body-Feedback folder (Label, Gap, RSA, BF) | ✅ All rewritten |
| 12 | Feeling.md v3.0 | ✅ Rewritten |
| 13 | Logic-Feeling.md v2.0 | ✅ Full rewrite |
| 14 | Logic-Feeling-Balance.md v2.0 | ✅ Rewritten |
| 15 | Neural-Processing-Flow.md v2.0 | ✅ Rewritten |
| 16 | Agent-Mechanism.md v2.0 | ✅ This file |

### §17.2 — Remaining (Phase 17-20)

- Phase 17: Connection v4.0 (observation parameter rewrite)
- Phase 18: Why-Body-Knows v1.0 (meta file — WHY framework works)
- Phase 19: Collective-Body v2.0 (collective mechanism)
- Phase 20: Terminology propagation + verification pass (~12+ files)

### §17.3 — Old recommendations status

v1.0 §17 recommended updates to Theme E, Empathy-Mirror, Mirror-Neuron, Logic-Feeling. Status:
- Logic-Feeling: ✅ DONE (v2.0 full rewrite Phase 13)
- Empathy-Mirror: integrated via Empathy.md v2.2
- Theme E: superseded by SPM v3.0 + By-Product-Gap-Resonance v1.0
- Mirror-Neuron: cross-ref added in relevant files
...
### §20.1 — Within Agent-Mechanism/ folder

- `Self-Pattern-Match.md v3.0` — solo forward mechanism deep drill
- `By-Product-Gap-Resonance.md v1.0` — emergent mutual phenomenon deep drill
- `backup/Agent-Mechanism-v1.0-backup.md` — previous version

### §20.2 — Core mechanism files

- `Inter-Body-Mechanism.md v1.0` — 8 drill principles, Architecture B, 3-cost, by-product match
- `Body-Base.md v3.1` — 3 Hardware Foundations, Architecture B
...
- Goldman simulation theory
- Hamilton 2013 autism empathy review
- Gazzaniga split-brain confabulation
- Haidt 2001 emotional dog / rational tail
- Nisbett & Wilson 1977 introspective access
- Gailliot 2007 PFC glucose depletion
- Schultz 1997 dopamine prediction error
- Kahneman 2011 fast/slow (reframed as compiled/fresh)
- Klein 1998 recognition-primed decisions
- Tang 2015 meditation training
- Weber-Fechner law (habituation)

(28+ citations, ALL from v1.0 preserved + 8 added from drill)
...
> **Agent-Mechanism.md v2.0 complete**. Integration hub updated.
>
> **Supporting files current**: SPM v3.0, By-Product-Gap-Resonance v1.0 — both already rewritten.
> **Next**: Phase 17 Connection v4.0 (observation parameter).
>
> **END OF Agent-Mechanism.md v2.0**
