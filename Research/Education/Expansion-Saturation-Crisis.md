...
position: |
  Research/Education/ (cạnh Education-Mechanism.md, Education-Arms-Race.md)
  = Intersection của 3 framework streams:
...
dependencies:
  ...
  secondary:
    ...
    - Education-Mechanism.md — learning arc design principles
  observation:
    ...
    - Cortisol-Baseline.md v2.0 — bounded vs unbounded stress
  companion:
...
⭐ FILE NÀY = INTERSECTION CỦA 3 FRAMEWORK STREAMS:
  ...
  ② EDUCATION STREAM:
    Education-Mechanism.md — 8 nguyên lý brain-based cho learning arc design
    Education-Arms-Race.md — positional good, credential inflation, game theory
    Domain-Knowledge-Map.md — WHAT to teach (domain mapping)

  ③ GENERATIONAL SHIFT STREAM:
    Imagine-Final.md — clarity gradient, lifecycle (build→save→background→reload)
    Meaning.md v2.0 — life-level Anchor-Schema (absent/exhausted/disrupted)
    Cortisol-Baseline.md v2.0 — bounded vs unbounded stress
    Boredom.md — dissonance + Imagine-Final unclear = "spinning"
...
🟡 HƯỚNG NGHIỆP MỚI = TÌM PROCESS PHÙ HỢP BODY:
  ...
  ⭐ BẰNG CHỨNG — PROCESS-FIT MODELS ĐANG XUẤT HIỆN:
    ...
    🟢 Research (ScienceDirect 2021):
      ...
      → "Too far removed from natural decision-making"
      → Real decisions: explore → self-explore → role of chance → adapt
...
🟢 DATA — AI ĐANG THAY THẾ ENTRY-LEVEL EXPANSION JOBS:
  ...
  NGÀNH BỊ NẶNG NHẤT (= expansion work):
    ...
    │                      │ 80%+ routine work)            │
    ├──────────────────────┼───────────────────────────────┤
    │ Basic coding         │ GitHub: Copilot handles ~46%  │
  ...
  FRAMEWORK MAPPING:
    → Data entry = EXPANSION (apply known technique to known data)
    → Basic coding = EXPANSION (apply known patterns to known problems)
    → Translation = EXPANSION (apply known rules to known text)
...
⭐ ĐỘ TIN CẬY PER CATEGORY:
  ...
  🟡 FRAMEWORK SYNTHESIS (logic mạnh, chưa test trực tiếp):
    ...
    "Bounded vs unbounded stress" = Cortisol-Baseline application
      → Sapolsky 2004 foundation (🟢)
      → Applied to generational comparison = framework
...
→ Education-Mechanism.md — Learning arc principles
  §2: 8 brain-based principles (Direction > Level, etc.)
  §4: AI-assisted education model
  Kết nối: §3.2 (approach/avoidance tags), §5.2 (AI in education)
...
→ Boredom.md — Drive + no direction = spinning
  Kết nối: §3.3 (graduate boredom), §4.2 (career spinning)

→ Cortisol-Baseline.md v2.0 — Direction > Level
...
→ Body-Feedback-Mechanism.md — Interface loop
  2 sources × 3 dynamics, H10 (5 preconditions)
