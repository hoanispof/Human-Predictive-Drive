...
MỚI (Schema-Drive): Schema phát hiện body CẦN gì → MONG MUỐN cụ thể → DRIVE hành vi đáp ứng.
  ...
  ASYMMETRY quan trọng:
    ...
    Chiều DRIVE (Schema → Body) = phức tạp:
      Schema fire TRƯỚC → kéo ĐỒNG THỜI nhiều hệ thống:
        Hormone (NE tức thì → dopamine/cortisol sustain → energy + motivation)
      ...
      ⚠️ SCHEMA = TRIGGER, HORMONE = ENERGY + SUSTAIN:
        Drive luôn BẮT ĐẦU từ schema fire (schema predict body-need hoặc threat)
        Hormone đến SAU: cung cấp ENERGY (NE tức thì) + SUSTAIN (cortisol/dopamine)
        Hormone KHÔNG khởi động drive — hormone PHỤC VỤ schema
...
EMERGENT METRICS — Observable, KHÔNG set trực tiếp, thay đổi INPUTS → metrics tự đổi:
  ...
  "Cường Độ Phản Ứng" (tên cũ: Amplitude)
    ...
    = EMERGENT từ NHIỀU yếu tố đồng thời:
      → Hardware enzyme: COMT variant → PFC draft clear nhanh/chậm (PFC-Analysis §8)
        ⚠️ DRD4 narrative có lỗ hổng logic — xem PFC-Analysis §8.1③
