---
title: Love Analysis — Tình Yêu Qua Lens Framework v7.8
version: 2.1
created: 2026-03-29
rewritten: 2026-04-26
status: RESEARCH FILE v2.0 — phân tích dựa trên framework v7.8 + established research
scope: |
  RESEARCH FILE: Phân tích tình yêu qua lens framework v7.8.
...
position: |
  ...
  Ngang hàng OCD-Analysis.md, Climate-Cognition.md.
dependencies:
  ...
  - Valence-Propagation.md v1.2 — per-entity valence, §2 Body-Base Extension, chain
  ...
  - PFC-Function.md — 24 functions, HOLD + PROCESS
  ...
  - OCD-Analysis.md — serotonin × obsessive patterns (link limerence)
sources_backup: |
...
# Love Analysis — Tình Yêu Qua Lens Framework v7.8
...
⭐ FILE NÀY TRONG FRAMEWORK:
  ...
  Apply toàn bộ framework v7.8 vào 1 hiện tượng cụ thể: tình yêu.
  ...
  PHÂN BIỆT:
    File NÀY: phân tích HOW love works qua framework lens
...
MECHANISM FILES:
  ...
  → PFC-Function.md — 24 functions, reactive model
...
⭐ STATUS:
  ...
  Version: v2.1 (rewrite + enriched from v1.0 v7.5-era)
  Date: 2026-04-26
  Lines: ~1,800+
  ...
  PENDING:
    → OCD-Analysis.md rewrite (next priority — shares serotonin mechanism)
    → Update Core-v7.8-Draft.md §8 with new observation parameters
```

---

> *Love Analysis v2.1 — "Tình yêu = ❶ Hardware flood + ❷ SPM composite fire + ❸ Valence → Body-Base Extension.
...
> PFC TỰ GIẢM vì body smooth, KHÔNG bị ép tắt → compiled red flags VẪN bảo vệ.
> Serotonin ↓ = AMPLIFIER of uncertainty, NOT root cause — parallel cortisol.
> Root = SPM incomplete + obligation unknown + possible childhood SPM bias.
> Strict parents → SPM threat-biased → adult hyper-monitor = anxious attachment MECHANISM.
