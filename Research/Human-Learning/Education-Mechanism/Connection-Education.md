...
position: |
  ...
  This file BUILDS ON:
    ...
    - Coordination-Node.md v1.2 (prestige, scale transitions)
...
🟡 COMMON MISCONCEPTION: "GOOD FRIENDS = ARE ALIKE"
  ...
  🟢 Gottman Type 2 divorce: 16-year fade from Compiled-Suppress
```
...
⭐ "CAN'T BE CLOSE TO EVERYONE" = ARCHITECTURE, NOT SELFISHNESS:
  ...
  DUNBAR LAYERS = STRUCTURAL SOLUTION:
    🟢 Dunbar 1992 (Social Brain Hypothesis):
      ~5: intimate / support group (Entity-Compiled deep)
      ~15: sympathy group (Entity-Compiled moderate)
...
🟡 TOXIC RELATIONSHIP = COMPILED SUPPRESS OF OWN DRIVES:
  ...
  🟡 Exit cost × compiled suppress = framework synthesis
...
⭐ 4-LAYER MODEL — LOWER LAYER FAILS → EVERYTHING COLLAPSES:
  ...
  LAYER 2 — MODALITY: 3 INDEPENDENT CHANNELS
    ...
    ❷ NON-VERBAL (mixed):
      ...
      = "Respond to bids" → 🟢 Gottman 5:1 positive-to-negative ratio
  ...
  LAYER 3 — AMPLIFICATION: 3 MECHANISMS
    ...
    ❷ PPR — Perceived Partner Responsiveness:
      ...
      ⭐ PERCEPTION > actual behavior (mediated by Self-Pattern-Modeling)
      🟢 Reis & Shaver 1988

    ❸ SECURE BASE EFFECT:
      3 components: Availability + Noninterference + Encouragement
      → Noninterference = anti-compiled-suppress (§4.4)
      → Creates positive spiral: safety → explore → new by-products → resonance
      🟢 Feeney 2007
...
🟢 DUNBAR LAYERS = RESONANCE ARCHITECTURE:
  ...
  (Dunbar 1992, Coordination-Node.md v1.2 §9)

  🟢 ~5:   intimate / support group → DEEP resonance per person
  🟢 ~15:  sympathy group           → MODERATE resonance
  🟢 ~50:  acquaintance band        → SURFACE connection
  🟢 ~150: social group ceiling     → neocortex LIMIT
...
🟡 GOAL: meta-connection + TRUE Entity-Access calibration
  ...
  ⑤ GROUP DYNAMICS:
    ...
    → Prestige vs Dominance recognition (§7.1)
    → "Popular ≠ good" → status ≠ contribution
    → Inclusion/exclusion awareness (§7.4)
...
🟡🔴 PHANTOM RESONANCE IN THE TECHNOLOGY AGE:
  ...
  TECHNOLOGY FILL SPECTRUM:
    (Resonance-Per-Entity.md v1.0 §12)
    Sensory domains: technology CAN substitute (music, visual)
    Social + abstract: technology CANNOT (co-regulation, presence, resonance)
    → Modern resonance frontier = social + abstract
...
```
  ESTABLISHED (🟢):
    ...
    🟢 Attachment theory (Bowlby 1969): secure → prosocial
    🟢 Social Baseline Theory (Coan 2015): proximity = metabolic ↓
    🟢 Social pain = physical pain (Eisenberger 2003): dACC overlap
    ...
    🟢 Prestige vs Dominance (Henrich & Gil-White 2001)
    🟢 Dunbar layers (Dunbar 1992): Social Brain Hypothesis
    ...
    🟢 Self-expansion → satisfaction (Aron 1996, 2000)
    🟢 Active-Constructive Responding (Gable 2006)
    🟢 PPR (Reis & Shaver 1988)
    🟢 Secure Base (Feeney 2007)
    🟢 Maintenance decline (Ogolsky 2013, N=12,273)
    🟢 Loneliness = mortality (Holt-Lunstad 2015)
  ...
  ⚠️ CONFIDENCE PER SECTION:
    ...
    §9 AI era — 🟡🔴 (current fact + future projection)
```
