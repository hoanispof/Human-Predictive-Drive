...
scope: |
  ...
  v2.0 KEY CHANGES: +Body-Feedback Pipeline (§1.3 interoception → Self-Pattern-Modeling foundation),
  +Connection v5.0 (§1.4 Entity-Compiled, Resonance Decline, Bond-Architecture, Entity-Access, 4-Layer),
...
dependencies:
  ...
  pfc-simulation:
    - PFC-Operations.md v1.1 — Hold/Suppress, PFC Budget (§9)
  ...
  agent-entity:
    ...
    - Self-Pattern-Modeling.md v3.1 — 1 mechanism × 3 dimensions, Simulation-Engine APPLICATION
...
3 FILES EDUCATION — ĐỌC CÙNG NHAU:
  ...
  Connection-Education.md = WHO
    → Dạy social interaction qua framework v7.8
  ...
  AI SỬ DỤNG CẢ BA:
    ...
    ④b AI check social interaction context (Connection-Education)
       → Bond formation, Entity-Access, sustainability
    ⑤ Teacher calibrate + Student verify

    = 3 files + AI = generate arc cho BẤT KỲ lá nào trên BẤT KỲ cây nào
```
...
🟢 VẬN ĐỘNG + CƠ THỂ + INTEROCEPTION:
  ...
  TẠI SAO FOUNDATION:
    ...
    → Body-Feedback Pipeline: 2 nguồn (Sensory-Driven + Pattern-Driven) × 3 dynamics
      (Chunk-Shift / Chunk-Miss / Chunk-Gap) → interoception = ĐỌC pipeline này
...
🟢 XÃ HỘI + CẢM XÚC + COLLABORATION:
  ...
  TẠI SAO FOUNDATION:
    ...
    → Empathy = Self-Pattern-Modeling function (Empathy.md v3.0) — cần chunks đủ
    ...
    → Connection = Entity-Compiled formation (40→200h compile 1 entity)
      + Bond-Architecture (1 mechanism × 4 bond types) + Resonance Decline patterns
      (Connection.md v5.0, Bond-Architecture.md v2.0)
    → Entity-Access gradient Mức 0-5: social learning = calibrate
      ai ở mức nào, quan hệ nào cần maintain (Entity-Access.md v1.2)
    → 4-Layer Sustainability: điều kiện → phương thức → amplify → quỹ đạo
      → dạy social = dạy SUSTAIN, không chỉ "kết bạn" (Resonance-Sustainability.md v1.0)
    → → Xem Connection-Education.md cho ứng dụng education chi tiết

  PREREQUISITE:
    → Attachment chunks từ 0-3 (Mother-Opt, Natural-Dev)
    → Entity-Compiled formation: first entity = mẹ (Hardware-Subsidy MAX)
      → expand peers 3-6, teachers 6-12, deep friendships 12+ (Entity-Compiled.md v1.0)
...
🟡 HỌC CÁCH HỌC:
  ...
  TẠI SAO FOUNDATION:
    ...
    → Simulation-Engine: meta-learning = training 1 shared engine
      → improve tất cả applications (Self-Pattern-Modeling, Imagine-Final, Self-Observation)
      → meta-learner giỏi = Simulation-Engine MẠNH (Simulation-Engine.md v1.1)
    → Dissonance-Signal-Architecture: nhận biết dissonance LOẠI NÀO trong learning
  ...
  PREREQUISITE:
    ...
    → PFC Budget: meta-cognition = HIGH PFC operation → tài nguyên hữu hạn
      → không thể meta-learn + domain-learn + social CÙNG LÚC full capacity
      (PFC-Operations.md v1.1 §9)
...
🟡 ERA-SPECIFIC — mới hoàn toàn
  ...
  TẠI SAO ERA-SPECIFIC (2025+):
    → Information overload = NEW cortisol source
      (Cortisol-Baseline.md v2.1 — chronic low-grade)
    ...
    → Dissonance-Signal-Architecture: digital dissonance phần lớn = Evaluative
      (compiled, workable by reframe) → dạy NHẬN BIẾT loại + REFRAME
      (Dissonance-Signal-Architecture.md v1.0)
    → Boredom v2.0: platform design exploit Reward-Habituated + novelty exhaustion
      → "chán → scroll → chán hơn" = boredom loop (Boredom.md v2.0)
      → dạy nhận biết 6 nguồn boredom → self-diagnosis thay vì phản xạ scroll
...
🟡 TIER 3 — KHÔNG PLAN TRƯỚC, OBSERVE + SUPPORT:
  ...
  GAP-DISTRIBUTION-PROFILE:
    → Hardware tendencies → gap distribution emerge → domain direction
      = GAP LANDSCAPE navigation (Gap-Distribution-Profile.md v1.1)
    → 4 trục quan sát: ① Domain Center × ② Origin Balance × ③ Depth × ④ Stability
    → Healthy profile = diverse domains, balanced origin, flexible shift
    → Unhealthy = narrow (chỉ 1 domain), C-dominant (threat-origin), rigid
    → AI observe gap landscape per student → suggest direction, KHÔNG ÉP

  HARDWARE-SUBSIDY:
    → Depth development CẦN Hardware-Subsidy — gia đình, nhà trường, mentor
...
🟡 KHÔNG FIXED STAGES — TÙY CHUNK TÍCH LŨY CỦA TỪNG NGƯỜI:
  ...
  PREREQUISITE LOGIC (không phải age-based):
    ...
    PFC BUDGET CONSTRAINT:
      → Meta-learning, Systems Thinking, AI Literacy = HIGH PFC operations
      → PFC Budget = finite shared resource → KHÔNG thể all high-PFC domains cùng lúc
      → Giai đoạn 6-12: PFC nhỏ → Foundation (less PFC-intensive) DOMINANT = hợp lý
      → Giai đoạn 12+: PFC lớn dần → high-PFC domains MỚI viable full
      (PFC-Operations.md v1.1 §9)
...
⭐ CÁI FILE NÀY CÓ THỂ LÀM:
  ...
  ✅ Kết hợp với Education-Mechanism.md + Connection-Education.md
     → HOW + WHAT + WHO = AI generate arc cho bất kỳ topic
...
→ Chunk.md v2.2 — Prerequisite chains
  §2 Compile, §4 Activation dynamics (prior knowledge effect).


═══════════════════════════════════════════════════════
VALENCE + BODY-FEEDBACK ⭐NEW v2.0
═══════════════════════════════════════════════════════

→ Body-Feedback-Mechanism.md v2.1 — 2 sources × 3 dynamics
  §1.3 Somatic: interoception = ĐỌC pipeline → Self-Pattern-Modeling foundation.

→ Valence-Propagation.md v3.0 — Structural/Current, Hardware-Subsidy
  §1.4: structural valence tags = permanent. §3: Hardware-Subsidy spectrum.

→ Dissonance-Signal-Architecture.md v1.0 — Evaluative/Direct-State
  §1.6: meta-skill biết loại dissonance. §2.6: digital dissonance = primarily Evaluative.

→ Gap-Distribution-Profile.md v1.1 — Per-person gap landscape
  §3: 4 trục quan sát, healthy vs unhealthy profile.

→ Gap-Body-Need.md v1.0 — 3 Satiation Types
  Cyclic / Tonic / Generative → per-domain satiation dynamics.

→ Boredom.md v2.0 — 6 sources, Resonance Decline
  §2.6: platform exploit Reward-Habituated + novelty exhaustion → boredom loop → scroll habit.
...
→ Connection.md v5.0 — 3 Generative Primitives, Resonance Decline
  §1.4: Entity-Compiled formation, resonance decline, 4-Layer Sustainability.

→ Bond-Architecture.md v2.0 — 1 mechanism × 4 bond types
  §1.4: Entity-Compiled reuse per bond, gap clone impossible.

→ Entity-Compiled.md v1.0 — Formation 40→200h
  §1.4: first entity = mẹ → expand peers → teachers → friends.

→ Entity-Access.md v1.2 — Gradient Mức 0-5
  §1.4: social learning = calibrate entity-access per relationship.

→ Self-Pattern-Modeling.md v3.1 — 1 mechanism × 3 dimensions
  §1.3: Self-Pattern-Modeling cần body data (interoception). §1.4: empathy = Self-Pattern-Modeling function.

→ Empathy.md v3.0 — Self-Pattern-Modeling function
  §1.4: empathy foundation, cần self-chunks ĐỦ trước.

→ Resonance-Sustainability.md v1.0 — 4-Layer model
  §1.4: dạy social = dạy SUSTAIN, không chỉ "kết bạn."


═══════════════════════════════════════════════════════
PFC + SIMULATION ⭐NEW v2.0
═══════════════════════════════════════════════════════

→ PFC-Operations.md v1.1 — Hold/Suppress, PFC Budget
  §1.6: meta-cognition = HIGH PFC. §4: PFC Budget constraint on timing.
...
→ Anchor-Schema.md v1.2 — 4 nguồn fill
  Education installs anchor, không chỉ chunks (§1 all domains contribute).


═══════════════════════════════════════════════════════
ERA CONTEXT + OBSERVATION
═══════════════════════════════════════════════════════

→ Observation/Education-Arms-Race.md v1.2
  Societal pressure patterns affecting ALL domains.

→ Observation/Money-Education.md v1.0
  Financial literacy domain — cross-ref with §1.2 Numeracy + §2 Era-specific.

→ Applications/Education/backup/v1.0/Era-Analysis-2025.md
  Era context chi tiết cho §2 (AI revolution, skill shifts).
