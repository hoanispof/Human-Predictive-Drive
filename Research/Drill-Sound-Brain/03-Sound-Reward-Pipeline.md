...
version: 1.1
created: 2026-05-29
updated: 2026-05-29 (v1.1 — §7b window sizes tagged as estimates, Schellenberg 2008 reference added)
status: MECHANISM FILE v1.1
scope: |
  7-step reward mechanism (Dopamine-Is-Not-Reward.md) MAPPED for music listening.
  4 insights formalized. Salimpoor 2011 = DIRECT validation.
  Musical anhedonia = Body-Feedback-Precondition Precondition-2 Chunk-Substrate fail at hardware level.
...
dependencies:
  primary:
    ...
    - Prediction-Error-Is-Not-Reward.md v2.0 — PE + coherence + Body-Feedback-Precondition
    - Reward-Signal-Architecture.md v2.1 — Evaluative/Direct-State, E₀→E₃
  secondary:
    - Body-Feedback.md v3.1 — Body-Feedback-Precondition 5 preconditions
    - Boredom.md v2.0 — prediction-delta = 0
    - 03-Reward.md — Body-Feedback-Precondition preconditions, 7 cases
...
⭐ THESIS:
  ...
  Framework 7-step mechanism (Dopamine-Is-Not-Reward.md §4):
    Step 1: VTA detect delta — ATTENTION
    Step 2: Dopamine cascade — ALERTING
  ...
  Music neuroscience provides STEP-LEVEL validation:
    ...
    Steps 5-6: Salimpoor 2011 NAcc (experience = reward/pleasure)
...
```
⭐⭐ CAUDATE vs NAcc = STEPS 1-3 vs STEPS 5-6:
...
```
⭐⭐ CONNECTIVITY DISCONNECT = PIPELINE BROKEN AT STEP 4:

  🟢 Martínez-Molina et al. 2016 (PNAS):

  FINDING:
    Musical anhedonics (~3-5% population):
      → Auditory cortex: NORMAL (hear music fine)
      → Reward system (NAcc): NORMAL (food/money/sex pleasure fine)
      → CONNECTION auditory cortex ↔ NAcc: REDUCED
    → Domain-specific DISCONNECT, not global deficit

  FRAMEWORK MAPPING:
    Steps 1-3: VTA detect + dopamine cascade + spreading activation → NORMAL
  ...
  = Body-Feedback-Precondition Precondition-2 Chunk-Substrate (CHUNKS BASE ADEQUATE) FAIL AT HARDWARE LEVEL:
    ...
    Anhedonic Precondition-2: compiled music chunks → pathway BROKEN → evaluation NEVER
    → Precondition-2 requirement includes PATHWAY INTEGRITY, not just chunk library
    → = Hardware precondition for Evaluative Reward

  WHY THIS MATTERS FOR FRAMEWORK:
    ① PROVES music reward = multi-step (if single step, can't have partial failure)
    ② PROVES domain-specificity of reward pathways
    ③ PROVES pipeline INTEGRITY required (not just components)
    ④ Consistent with Reward-Signal-Architecture: Evaluative Reward needs
       intact hedonic hotspot pathway (NAcc shell, VP, mOFC)

  ⭐ INSIGHT 10: Musical anhedonia = PROOF of multi-step pipeline.
    If PE = reward (single step), anhedonia would be impossible
    (auditory cortex works fine → PE fires → should feel pleasure).
    Pipeline model predicts: PE fire + pathway broken → no reward. CONFIRMED.

  ADDITIONAL (2025): Motor urge to move CAN remain intact in musical anhedonia
    → Motor entrainment pathway (auditory → motor) ≠ reward pathway (auditory → NAcc)
    → = 2 INDEPENDENT pathways from auditory processing
    → Entrainment ≠ pleasure. Can have rhythm WITHOUT reward.

  🟢 Martínez-Molina 2016: domain-specific disconnect
  🟢 Mas-Herrero 2014: BMRQ development + 3-5% prevalence
```
...
```
⭐⭐ UNCERTAINTY × POSITIVE OUTCOME = MAX PLEASURE:

  🟢 Cheung et al. 2019 (Current Biology 29:4084-4092):

  METHOD:
    Analyzed 80,000 chords from Billboard Hot 100 hits (1958-1991)
    Information-theoretic measures: entropy (uncertainty) + surprise
    Linked to subjective pleasure ratings

  FINDING:
    Musical pleasure = MAX when:
      HIGH uncertainty (many possible next chords)
      + POSITIVE outcome (the actual chord FITS the context)
  ...
  DIRECTLY VALIDATES FRAMEWORK:
    ...
    Cheung 2019 quantifies this EXACT relationship:
      Uncertainty (surprise) ↔ PE (framework Step 1)
      Positive outcome (fit) ↔ Coherence (framework Step 4)
      Max pleasure = BOTH high ↔ PE + coherence = reward

  ALSO:
    🟢 Gold et al. 2019 (PNAS): pleasure = interaction between
    acoustic features (sensory) × schematic expectations (cognitive)
  ...
  🟢 Gold 2019: interaction model, complementary finding
```
...
🟡 MUSIC TẠO CẢ 2 LOẠI REWARD — COMPOUND KHI ACTIVE:
  ...
  │                    │ chills, aesthetic       │ = no Direct-State│
  │                    │ appreciation           │ from exercise)   │
  ├───────────────────┼────────────────────────┼──────────────────┤
  │ Moving to music    │ ✅ Music pleasure       │ ✅ Endorphin      │
  │ (dance, nod, tap)  │ + sync reward          │ from movement    │
  │                    │ (multi-modal match)    │ (Dunbar 2012)    │
  ├───────────────────┼────────────────────────┼──────────────────┤
  │ Playing instrument │ ✅ Music pleasure       │ ✅ Endorphin      │
  │ / singing          │ + creation reward      │ + efference copy │
  │                    │ + sync reward          │ (Autonomy-Hardware §1) │
  ├───────────────────┼────────────────────────┼──────────────────┤
  │ Group music-making │ ✅ ALL above            │ ✅ ALL above      │
  │                    │ + social Evaluative    │ + oxytocin       │
...
```
🟡 VUUST 2018: PRECISION WEIGHTING = Precondition-2 MECHANISM:
...
🟡 GOLDILOCKS ZONES CHO MUSIC (Reward-Calibration.md §1):
  ...
  UNDER-REWARD:
    ...
    → Song too simple for expert → Precondition-4 miss (too predictable)

  MATCH-REWARD (Goldilocks):
    → Enough compiled chunks + enough novelty remaining
    → = Cheung 2019: uncertainty HIGH + outcome POSITIVE
  ...
  WINDOW SIZE ∝ COMPLEXITY (🟡 ESTIMATES — no study compares genres directly):
    ...
    Schellenberg 2008: peak at ~2 exposures (focused/emotional) to ~8 (focused/ambiguous)
...
```
  VALIDATED (🟢):
    → 7-step multi-step model: Salimpoor 2011 (caudate vs NAcc = 2 distinct phases)
    → Body-Feedback-Precondition Precondition-2 hardware requirement: Martínez-Molina 2016 (anhedonia = disconnect)
    → PE + coherence model: Cheung 2019 (80k chords, uncertainty × positive)
    → Active > passive reward: Dunbar 2012 (endorphin release)
    → Dopamine causal role: Ferreri 2019 (bidirectional modulation)

  FRAMEWORK SYNTHESIS (🟡):
    → Full 7-step walkthrough for music (logical, components validated individually)
    → Compound reward scaling (Evaluative + Direct-State + efference copy)
    → Boredom formula applied to music (consistent, not directly tested AS boredom)
    → PCM precision = Precondition-2 mechanism (consistent, Vuust 2018 supports)

  DEBATED (⚠️):
    → Opioid role in SUBJECTIVE music pleasure (Mas-Herrero 2023 challenges)
    → Framework position: opioid = body-level reward (Step 6),
      subjective experience (Step 7) may involve additional mechanisms
    → Does NOT undermine multi-step model — just refines Step 6 vs Step 7

  HYPOTHESIS (🔴):
    → Exact timeline Steps 1→7 for music (framework estimates, not measured)
    → Whether coherence evaluation (Step 4) = single step or distributed process
    → Quantitative threshold for "enough" coherence

  ⭐ STRONGEST VALIDATION POINT:
    Musical anhedonia = strongest test case for multi-step model.
    If PE = reward (single step): anhedonia impossible.
    Multi-step: PE ok + pathway broken → no reward. CONFIRMED.
    = Framework mechanism SURVIVES strongest challenge.
```
...
```
PRIMARY:
  Dopamine-Is-Not-Reward.md v1.2 — 7-step mechanism (source)
  Prediction-Error-Is-Not-Reward.md v2.0 — PE + coherence + Body-Feedback-Precondition (source)
  Reward-Signal-Architecture.md v2.1 — Evaluative/Direct-State (architecture)
  Body-Feedback.md v3.1 — Body-Feedback-Precondition 5 preconditions (definition)

SECONDARY:
  Boredom.md v2.0 — §1 unified formula
  Autonomy-Hardware.md v1.1 — §1 efference copy
  03-Reward.md — Body-Feedback-Precondition preconditions detail, 7 cases
...
WITHIN DRILL:
  02-Sound-Background-Pattern.md — §7 (PCM connects to Background-Pattern precision)
  04-Sound-Social-Resonance.md — builds on reward mechanism for social context
  05-Multi-Modal-Compound.md — extends compound reward (§5 table expanded)
```
...
> Active > passive: compound Evaluative + Direct-State + efference copy.
> Music = rare domain with per-step neuroscience evidence.*
