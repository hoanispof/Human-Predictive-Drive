...
version: 1.2
created: 2026-05-29
updated: 2026-05-29 (v1.2 — I39 UPGRADED: Nakai 2026 confirms shared IFG/BA44 across music×math×language. Startup = analogical.)
status: DRILL v1.1
scope: |
  5 topics drilled:
...
purpose: |
  ...
  THIS file drills DYNAMIC processes:
    ...
    HOW music interacts with other tasks (PFC competition)
...
dependencies:
  ...
  secondary:
    ...
    - Autonomy-Hardware.md v1.1 — efference copy
    - PFC-Operations.md v1.0 — Hold/Suppress, Compiled/Fresh, PFC budget
  evidence:
    - 01-Sound-Brain-Neuroscience.md — §6 (entrainment), §3 (prediction)
    - 06-Music-Architecture-Prediction.md — §2 (BPM), §5 (repetition)
  within_drill:
...
⭐⭐ 4 CLAIMS:
  ...
  CLAIM 1 — Neural entrainment = distributed oscillator sync:
    → Brain has beat-finding CIRCUIT (putamen + SMA), NOT central clock
    → BPM regular → oscillators phase-lock → cross-region coherence
    → = Basis for music-induced coordinated processing

  CLAIM 2 — Music reward = continuous prediction stream with phasic peaks:
  ...
  CLAIM 4 — Music × task performance = PFC budget competition:
    → Familiar music + compiled task = HELP (no PFC competition)
    → Unfamiliar music + fresh task = HURT (both compete for PFC)
    → = Gap-Distribution-Profile PFC budget model applied
```
...
```
🟢 HOW BPM CREATES NEURAL SYNC:

  MECHANISM — 3 layers simultaneously:
  ...
  LAYER 2 — GAMMA OSCILLATIONS (20-60 Hz):
    🟢 Same studies:
      → Fire BEFORE beat onset → ANTICIPATORY
      → ⭐ PERSIST even when expected beat is MISSING ("missing pulse")
      → = Brain GENERATES prediction even without stimulus
      → = TOP-DOWN prediction, not just bottom-up response

  LAYER 3 — STEADY-STATE EVOKED POTENTIALS:
  ...
  ⭐ WHY BPM REGULAR = IMPORTANT:
    ...
    → Regular intervals → oscillators PREDICT next beat PRECISELY
    → Precise prediction → tight phase-locking across regions
    → Tight phase-locking → coherent processing → compound reward

    → Irregular intervals → oscillators STRUGGLE to predict
    → Loose phase-locking → less coherent → less reward
    → = WHY groove inverted-U (Witek 2014):
      Too regular = prediction-delta = 0 → boredom
      Moderate syncopation = manageable prediction challenge
      Too irregular = phase-locking breaks → dissonance
  ...
  ⭐ FRAMEWORK MAPPING:
    ...
    → = User hypothesis VALIDATED at mechanism level

  🟢 Neural entrainment: most robust finding in music neuroscience
  🟡 Cross-region sync → compound reward: framework mapping
```

---

## §3 — Music Reward = Continuous Micro-Rewards
...
🟡 MUSIC = CONTINUOUS PREDICTION STREAM WITH PHASIC REWARD PEAKS:
  ...
  TEMPORAL PATTERN — per-second breakdown:
    ...
    t = 4.0s: Phrase boundary → melodic resolution → LARGER reward
    t = 16s:  Section change → significant prediction event
    t = 60s:  Chorus arrives → BIG prediction confirmed → PEAK micro-reward
    ...
    → = Continuous prediction processing in 3-4 minutes
    → ⚠️ Prediction processing = continuous (Vuust 2022 PCM).
    →   Reward signaling = likely PHASIC with peaks at structural boundaries
    →   (Ferreri 2019: dopamine evidence strongest at peak moments, not tonic).

  HIERARCHICAL — multiple prediction levels SIMULTANEOUS:

    Level 1: Beat-to-beat (500ms) → FAST, SMALL PE → micro-reward
    Level 2: Measure-to-measure (2s) → harmonic change → MEDIUM PE
    Level 3: Phrase-to-phrase (8s) → melodic arc → MEDIUM-LARGE PE
    Level 4: Section-to-section (30s) → verse→chorus → LARGE PE
    Level 5: Whole song arc (3-4 min) → resolution → LARGEST PE

    → 5 levels SIMULTANEOUSLY → compound reward from ALL levels
...
🟡 SAME 7-STEP MECHANISM, DIFFERENT TEMPORAL PATTERN:
  ...
  ├──────────────┼───────────────────┼───────────────────┼───────────────────┤
  │ Duration     │ 3-5 min           │ Hours-days         │ 10-60 min         │
  │              │                   │                   │                   │
  │ Pacing       │ IMPOSED by        │ Self-paced         │ Self-paced        │
  │              │ composer/recording│                   │                   │
  │              │                   │                   │                   │
  │ PE pattern   │ CONTINUOUS stream │ SUSTAINED ramp     │ INTERMITTENT      │
  │              │ (every beat/chord)│ → discrete burst   │ (plot points)     │
  │              │                   │                   │                   │
  │ Reward       │ Continuous stream │ 1 macro-reward     │ Several medium    │
  │ instances    │ with phasic peaks │ (insight moment)   │ rewards (reveals, │
  │              │                   │                   │ resolutions)      │
  │              │                   │                   │                   │
  │ Gap type     │ PREDICTION gap    │ KNOWLEDGE gap      │ NARRATIVE gap     │
  │              │ (auto-fill by ear)│ (requires PFC)     │ (auto-fill by     │
  │              │                   │                   │ reading)          │
  │              │                   │                   │                   │
  │ Modalities   │ 3+ (auditory +   │ 1-2 (visual +     │ 1 (visual-verbal) │
  │              │ motor + emotional)│ verbal)            │                   │
  │              │                   │                   │                   │
  │ DMN          │ BIDIRECTIONAL:    │ SUPPRESSED (task-  │ ACTIVATED         │
  │              │ sad music → DMN ↑ │ positive network   │ (narrative →      │
  │              │ rhythmic → DMN ↓  │ dominant)          │ self-referential) │
  │              │                   │                   │                   │
  │ Body engage  │ Entrainment,      │ Minimal until      │ Minimal (may cry  │
  │              │ chills, movement  │ eureka             │ at climax)        │
  │              │                   │                   │                   │
  │ PFC load     │ LOW (passive)     │ HIGH (active       │ MODERATE (follow  │
  │              │                   │ search + hold)     │ narrative)        │
  │              │                   │                   │                   │
  │ Satiation    │ Tonic ↔ Cyclic    │ Generative         │ Cyclic            │
  │              │                   │                   │                   │
  │ Flow state   │ ABSORPTION:       │ FLOW:              │ IMMERSION:        │
  │              │ passive, time     │ active, skill =    │ passive, empathy  │
  │              │ distortion,       │ challenge, loss    │ + narrative       │
  │              │ entrainment       │ of self-           │ engagement        │
  │              │                   │ consciousness      │                   │
  │              │                   │                   │                   │
  │ Goldilocks   │ Designed by       │ Determined by      │ Determined by     │
  │ control      │ composer/artist   │ problem difficulty  │ author skill      │
  └──────────────┴───────────────────┴───────────────────┴───────────────────┘


  ⭐ KEY DISTINCTIONS:

  MUSIC: "Prediction stream" — continuous processing DELIVERED passively
  ...
  🟢 NAcc/VTA overlap across domains: Tik 2018 (insight), Salimpoor 2011 (music)
  ⚠️ No within-subject study directly compares all 3
```
...
⭐⭐⭐ INSIGHT: Music, mathematics, and startups = THE SAME 1 BRAIN ARCHITECTURE.
  ...
  ═══ WHAT VARIES, WHAT STAYS SAME ═══

  ┌──────────────────────┬──────────┬──────────┬──────────┐
  │ Dimension            │ Music    │ Math     │ Startup  │
  ├──────────────────────┼──────────┼──────────┼──────────┤
  │ Architecture         │ SAME     │ SAME     │ SAME     │
  │ (nested gap-fill)    │          │          │          │
  │                      │          │          │          │
  │ 7-step per level     │ SAME     │ SAME     │ SAME     │
  │ (VTA→evaluate→reward)│          │          │          │
  │                      │          │          │          │
  │ Who designs arc      │ Composer │ Problem  │ Founder  │
  │                      │          │ + solver │ + market │
  │                      │          │          │          │
  │ Timescale            │ Seconds  │ Hours-   │ Years    │
  │                      │          │ days     │          │
  │                      │          │          │          │
  ...
  │                      │          │          │          │
  │ Level sync           │ DESIGNED │ Variable │ Variable │
  │ (fractal coherence)  │ (fractal)│ (can     │ (can     │
  │                      │          │ get stuck)│ misalign)│
  │                      │          │          │          │
  │ Satiation type       │ Cyclic/  │ Genera-  │ Genera-  │
  │                      │ Tonic    │ tive     │ tive     │
  └──────────────────────┴──────────┴──────────┴──────────┘
  ...
  Music = the PUREST FORM of hierarchical prediction architecture BECAUSE:
    ...
    ④ No failure cost → pure prediction satisfaction
    ⑤ Multi-modal × multi-scale → maximum compound channels

  = "Music = brain's hierarchical prediction architecture made audible"
  = Framework contribution: music architecture REVEALS general brain architecture

  ⭐ MUSIC × MATH = SHARED HIERARCHICAL PREDICTION ARCHITECTURE (v1.2):
    🟢 Nakai, Daikoku & Oseki 2026 (Neuroscience Research):
      IFG (inferior frontal gyrus) shared across language, music, AND mathematics.
    ...
    → Shared MECHANISMS confirmed. Domain-specific VARIATIONS exist
      (laterality, anterior-posterior location, modality).
    → "Same architecture, different parameters" = SUPPORTED for music × math × language.
    → Startup comparison = ANALOGICAL (no neuroscience study on startup hierarchy).
      Logical parallel via Progress Principle, but NOT empirically tested as brain architecture.

  🟢/🟡 Unified hierarchy: music × math × language = shared IFG/BA44 mechanisms (Nakai 2026)
  🟡 Startup parallel = analogical, not empirically tested
  🟢 Mini-arc decomposition: Gap-Body-Need §5 (established in framework)
  🟢 Progress Principle: Amabile & Kramer 2011 (Harvard, established)
  🟢 Fractal music structure: Voss & Clarke 1975, Hsü & Hsü 1991
  🟢 PFC budget constraint → forced decomposition: PFC-Operations
```
...
⭐⭐ "NO KNOWLEDGE = NO GAP" — FULL APPLICATION:
  ...
  ═══ PHASE 2: GOLDILOCKS ZONE (peak — timing depends on complexity, see §6) ═══
    ...
    ⚠️ Peak timing: ~2-8 exposures under focused listening (Schellenberg 2008),
       ...
       inverted-U SHAPE (Chmiel & Schubert 2017). Exact peak = f(complexity × mode).
  ...
  ⭐ INVERTED-U ACROSS THE TRAJECTORY:
    ...
    = EXACTLY Zajonc 1968 inverted-U (mere exposure)
    = EXACTLY Cheung 2019 (uncertainty × positive = max pleasure)
    = Framework adds: inverted-U = f(gap formation trajectory)
      Phase 0-1: gap forming → reward rising
      Phase 2: gap optimal → reward peak (Goldilocks)
      Phase 3-4: gap saturating → reward declining OR tonic transition
```
...
🟡 GOLDILOCKS APPLIED TO MUSIC ACROSS COMPLEXITY:
  ...
  UNDER-REWARD:
    → First exposure to new genre → no gap → no direction → no reward
    → Song too simple for expert listener → Precondition-4 miss (too predictable)
    → Wrong satiation type (want peak, getting ambient background)

  MATCH-REWARD (Goldilocks):
    → Enough compiled chunks + enough novelty remaining
    → = Cheung 2019 sweet spot: uncertainty HIGH + outcome POSITIVE
  ...
  ⭐ WINDOW SIZE ∝ SONG/GENRE COMPLEXITY:

    ┌────────────────┬──────────────┬──────────────────────────────┐
    │ Complexity     │ Window       │ Explanation                  │
    ├────────────────┼──────────────┼──────────────────────────────┤
    │ Simple pop     │ ~3-10 listens│ Few chunks to compile →      │
    │ (4 chords,     │ NARROW       │ prediction model completes   │
    │  catchy hook)  │              │ FAST → delta=0 fast          │
    ├────────────────┼──────────────┼──────────────────────────────┤
    │ Rock/alt       │ ~5-20 listens│ More layers → model takes    │
    │ (moderate      │ MEDIUM       │ longer to complete → window  │
    │  complexity)   │              │ wider                        │
    ├────────────────┼──────────────┼──────────────────────────────┤
  ...
  🟡 Window size model: framework application
  🟢 Inverted-U in music exposure: Zajonc 1968, replicated
  🟢 Complexity × repetition × enjoyment: Cheung 2019, Vuust 2018
```
...
```
🟡 PFC BUDGET MODEL APPLIED:

  Gap-Distribution-Profile.md: PFC budget = ~4±1 domains active simultaneously.
  PFC-Operations.md: PFC = universal shared resource.

  MUSIC PROCESSING = DEPENDS ON FAMILIARITY:
    Familiar music → Tonic channel → INVISIBLE → minimal PFC
    Unfamiliar music → requires PFC attention → OCCUPIES budget


  ═══ COMPILED TASK + MUSIC ═══

  COMPILED TASK = prediction model COMPLETE → PFC MINIMAL:
    → Running at steady pace
    → Driving familiar route
    → Routine data entry
    → Practiced motor sequences

  + FAMILIAR MUSIC:
    → Music = Tonic (invisible) → PFC budget: 0 slots occupied
    → Task = Compiled → PFC budget: 0-1 slots occupied
    → Total: ≤ 1 slot → NO COMPETITION → ✅ HELPS
    → Music provides:
      ① Rhythm scaffold → motor entrainment → more efficient movement
      ② Positive valence → body-base reward supplement
      ③ Fill prediction-delta from MONOTONOUS task → prevent boredom
      ④ Arousal modulation (tempo → energy level)

    🟢 Terry et al. 2020 (meta-analysis, k=139 studies):
      Music during exercise: g=0.48 affective valence,
      g=0.31 performance, g=0.22 reduced perceived exertion
      → ESTABLISHED benefit for compiled physical tasks

  + UNFAMILIAR MUSIC:
    → Music = Fresh → PFC budget: 1 slot occupied (processing new patterns)
    → Task = Compiled → PFC budget: 0-1 slots
    → Total: 1-2 slots → LOW competition → 🟡 MODERATE help/neutral
    → Less benefit than familiar, but compiled task still automated


  ═══ FRESH TASK + MUSIC ═══

  FRESH TASK = prediction model BUILDING → PFC NEEDED:
    → Learning new skill
    → Solving novel problem
    → First time driving new route
    → Studying unfamiliar material

  + FAMILIAR MUSIC:
    → Music = Tonic → PFC budget: 0 slots
    → Task = Fresh → PFC budget: 2-3 slots (active learning)
    → Total: 2-3 slots → MANAGEABLE → 🟡 DEPENDS
    → If music truly invisible (very familiar) → minimal interference
    → If music has lyrics → lyrics COMPETE with verbal processing
    → → Instrumental familiar > lyrical familiar for fresh tasks

  + UNFAMILIAR MUSIC:
    → Music = Fresh → PFC budget: 1 slot
    → Task = Fresh → PFC budget: 2-3 slots
    → Total: 3-4 slots → AT LIMIT → ❌ HURTS
    → PFC overloaded → both task AND music suffer
    → = Dual-task interference (established cognitive psychology)
  ...
  ⭐ MATRIX:
    ...
    ├────────────────────┼─────────────────────┼────────────────────┤
    │ Compiled Task      │ ✅ OPTIMAL           │ 🟡 Neutral-mild   │
    │ (running, driving) │ Music scaffolds,     │ help              │
    │                    │ prevents boredom     │                    │
    ├────────────────────┼─────────────────────┼────────────────────┤
    │ Fresh Task         │ 🟡 DEPENDS           │ ❌ WORST           │
    │ (learning, solving)│ Instrumental: ok     │ Both compete       │
    │                    │ Lyrics: interfere    │ for PFC budget     │
    └────────────────────┴─────────────────────┴────────────────────┘

  🟢 Task performance + music: established research (multiple meta-analyses)
  🟡 PFC budget competition model: framework application
  🟡 Bidelman & Feng 2025 (N=96): familiar music ↑ task speed ~170ms
```

---

## §7b — Attention × Music: Effort vs Knowledge (REFINED v1.1)

```
⭐ SINGLE-THREADED CONSCIOUSNESS × MUSIC:

  §7 describes PFC budget BETWEEN music + external task.
  ...
  TYPE B — Accumulated analytical KNOWLEDGE (built-up understanding):
    ...
    🟢 Brattico et al. 2016 (Frontiers in Neuroscience):
      ...
      → Expertise ADDS pleasure pathways, does not remove them

    → Framework mapping: analytical knowledge = compiled chunks
      → More compiled music chunks = richer prediction model
      → Richer model = finer prediction-delta = deeper Goldilocks
  ...
  ⭐ KEY: Mode 2 requires KNOWLEDGE already compiled.
    First-time analysis (effortful) → Mode 1 interference (temporary)
    Compiled analysis (automatic) → Mode 2 enhancement (lasting)
  ...
  🟢 Implicit processing of musical features: established
    (Koelsch et al. 2004 — ERAN/ELAN responses to syntactic violations
     even when NOT attending to harmony)
  🟢 Unconscious auditory scene analysis: Bregman 1990
  ...
  ═══ FRAMEWORK CONNECTIONS ═══
    ...
    2 types of optimal listening:
      TYPE A — Flow/immersion: PFC relaxed, body-feedback dominant
        → Familiar enough → no PFC analysis needed
        → Complex enough → prediction-delta > 0 (not bored)
        → = Goldilocks for casual enjoyment
      TYPE B — Expert appreciation: PFC engaged WITH compiled knowledge
  ...
  🟢 Global Workspace Theory: Baars 1988/1997
  🟡 Transient hypofrontality: Dietrich 2004 (lateral PFC specific — Limb & Braun 2008)
  🟢 Implicit harmony processing: Koelsch et al. 2004
  ⭐ Hou et al. 2024: analytical knowledge INCREASES liking
  🟢 Brattico et al. 2016: expert enhanced pleasure through cognition
  🟢 Limb & Braun 2008: lateral PFC decrease during jazz improvisation
  🟡 2-mode enjoyment model: framework synthesis from converging evidence
```
...
```
🟡/🟢 MUSIC TRAINING TRANSFERS (selectively):

  🟢 MUSICAL TRAINING (active — playing instrument):
    → Moradzadeh et al. 2015 (N=153):
      Musicians: reduced switch costs, better dual-task performance
    → Musical training → enhanced auditory processing
    → Enhanced auditory processing → speech-in-noise perception ↑
    → = TRANSFER via shared auditory-motor circuits

  🟢 PARKINSON'S — Rhythmic Auditory Stimulation:
    → External rhythm → bypass impaired basal ganglia timing
    → Gait improvement: cadence ↑, stride length ↑
    → 🟢 Meta-analyses confirm (Thaut & Hoemberg 2014)
    → = Medical-grade transfer evidence

  🟡 EVERYDAY LISTENING (passive):
    → Heart rate + breathing CAN sync with tempo (established)
    → Music during exercise improves performance (Terry 2020)
    → Familiar music reduces mind-wandering (Bidelman & Feng 2025)
    → BUT: "everyday listeners more rhythmic in life" = limited direct evidence
  ...
  ⭐ FRAMEWORK INTERPRETATION:
    → Transfer = COMPILED CHUNKS from music domain → adjacent domains
...
```
🟡 GAP-BODY-NEED ARCHITECTURE APPLIED:
  ...
  ROAD = Music collective infrastructure:
    ...
    → = Technology fills the routine → frontier SHIFTS:
      ...
      PRE-STREAMING:
        ...
        → = Music gap = ACCESS gap (how to get the music?)
        → Buying album = gap-fill EVENT

      POST-STREAMING:
        → Access = unlimited → listening = default state
        → = Music gap ≠ access (access solved)
        → Music gap = DISCOVERY (what NEW to listen to?)
...
```
  VALIDATED (🟢):
    → Neural entrainment: distributed, with putamen/SMA hub (Grahn 2009)
    → Beta/gamma anticipatory oscillations (Snyder/Large, Fujioka, Nozaradan)
    → Music reward uses same NAcc/VTA as other rewards (Blood & Zatorre 2001)
    → Music during compiled tasks: benefit (Terry 2020, k=139)
    → Musical training → transfer (Moradzadeh 2015, Thaut 2014)
    → Mere exposure inverted-U (Zajonc 1968, replicated in music)

  VALIDATED (v1.1 additions):
    → Analytical knowledge INCREASES liking (Hou 2024) — contradicts old "analysis kills"
    → Expert listeners: additional pleasure channels via cognition (Brattico 2016)
    → Inverted-U: 87.7% of 57 studies confirm shape (Chmiel & Schubert 2017)
    → Peak at ~2-8 exposures under focused listening (Schellenberg 2008)
    → Continuous prediction processing in music (Vuust 2022 PCM)

  FRAMEWORK SYNTHESIS (🟡):
    → Continuous prediction with phasic reward peaks (components validated)
    → Gap-Direction × music trajectory (logical application, inverted-U consistent)
    → PFC budget competition model (logical, cognitive dual-task literature supports)
    → Window size ∝ complexity (logical, Cheung/Vuust support)
    → 2-mode enjoyment model: flow/immersion vs analytical appreciation (converging evidence)
    → ENGINE/ROAD/VEHICLE applied (logical application)
    → Satiation profile transitions (logical, observation-consistent)
  ...
  CORRECTED (v1.1):
    ...
    → ❌ "Hundreds of micro-rewards" → OVERSTATED
    → ✅ Continuous prediction processing with phasic reward peaks (Ferreri 2019)
    → ❌ Peak at "5-30" exposures → WRONG (Schellenberg 2008: ~2-8 focused)
    → ✅ Peak timing = f(complexity × listening mode)

  HYPOTHESIS (🔴):
    → Passive listening history → lasting motor improvement (no direct evidence)
    → Passive listening → better breathing regularity (untested)
    → Passive listening → easier flow WITHOUT music (untested)
    → Exact inverted-U shape per complexity level (not measured)
    → "3-4 min = natural Cyclic unit" (plausible, not tested as satiation)

  ⚠️ KEY CAVEAT:
    → No within-subject study compares music flow vs cognitive flow vs reading flow
    → Transfer effects = much stronger for ACTIVE music than PASSIVE listening
    → PFC budget model = framework application of general cognitive resource theory
    → Reward signaling measured mainly at PEAKS (Salimpoor 2011), tonic component unclear
```
...
WITHIN DRILL:
  02-Sound-Background-Pattern.md — gist, satiation types, snowball
  03-Sound-Reward-Pipeline.md — 7-step, Cheung 2019, PCM
  05-Multi-Modal-Compound.md — coherence condition, Tonic↔Cyclic

RESEARCH CITATIONS:
  Grahn & Rowe 2009 — J Neuroscience — putamen beat perception
  Large, Herrera & Velasco 2015 — Frontiers Systems Neurosci — distributed oscillator
  Snyder & Large 2005 — beta/gamma anticipatory
  Fujioka et al. 2009, 2012 — beta replication
  Nozaradan et al. 2011 — J Neuroscience — steady-state EEG
  Tik et al. 2018 — Human Brain Mapping — insight NAcc activation
  Sridharan, Levitin & Menon 2008 — PNAS — DMN/CEN switching
  Taruffi et al. 2017 — Sci Reports — sad music DMN
  Terry et al. 2020 — meta-analysis k=139 — music exercise
  Moradzadeh et al. 2015 — musical training transfer
  Bidelman & Feng 2025 — familiar music task speed
  Reber et al. 2004 — processing fluency
  Zajonc 1968 — mere exposure
  Howe et al. 2013 — Nature — dopamine ramping
  Witek et al. 2014 — PLOS ONE — groove inverted-U
  Vuust et al. 2022 — Nature Reviews Neuroscience — PCM continuous prediction
  Ferreri et al. 2019 — PNAS — dopamine causal role (phasic evidence)
  Schellenberg et al. 2008 — peak at 2-8 exposures (focused listening)
  Chmiel & Schubert 2017 — 57 studies, 87.7% inverted-U confirmed
  Hou et al. 2024 — PsyCh Journal — analytical knowledge increases liking
  Brattico et al. 2016 — Frontiers Neuroscience — expert enhanced pleasure
  Limb & Braun 2008 — PNAS — PFC during jazz improvisation
```
...
> BPM regular → phase-lock → cross-region coherence → compound reward.
> Music = continuous prediction stream with phasic reward peaks.
> Gap trajectory inverted-U: peak timing = f(complexity × listening mode).
> 2 modes of enjoyment: flow/immersion (PFC relaxed) AND expert appreciation (PFC+knowledge).
> Familiar music + compiled task = optimal. Unfamiliar + fresh = worst.
> Transfer: strong for active music, moderate for passive, weak for lasting.*
