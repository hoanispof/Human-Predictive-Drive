...
> **Trạng thái:** DRAFT v2 — viết mới theo kiến trúc v7.5
> **Ngày:** 2026-03-24
...
IMAGINE OUTPUT → SCHEMA UPDATE:
  ...
  Imagine → draft → body check → compile → schema UPDATE:
    ...
    ⑤ PFC freed → imagine tiếp cho schema khác

  → Imagine = CÔNG XƯỞNG sản xuất schema
  → Mỗi vòng imagine = 1 vòng draft+test+refine
  → Output = schema compiled (vô thức giữ, PFC freed)
  → Pyramidal: schema cũ → imagine → schema mới → imagine tiếp → schema tốt hơn
...
```
→ Architecture-v7.5-Draft.md: Imagine layer trong kiến trúc tổng thể
