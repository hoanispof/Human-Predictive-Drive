...
🟢 Prevalence:
  ...
  Male:female ratio — revised down:
    ...
    → Loomes et al. 2017 (JAACAP 56(6):466-474): meta-analysis
    → High-quality studies: 3.32:1
    → Population screening (not relying on prior diagnosis): 3.25:1
...
🟢 Heritability — VERY high:
  ...
  Twin studies:
    → Bailey et al. 1995 (Psychological Medicine 25(1):63-77):
      MZ concordance 60% autism, 92% broader phenotype
      DZ concordance 0% autism, 10% broader phenotype
    → Tick et al. 2016 (JCPP 57(5):585-595): meta-analysis ALL twin studies
      Heritability: 64-91% depending on prevalence assumptions

  Risk genes identified:
    → Satterstrom et al. 2020 (Cell 180(3):568-584): largest exome study
...
```
🟢 ORIGINAL model — underconnectivity:
  → Just et al. 2004 (Brain 127(8):1811-1821):
    Functional connectivity between cortical areas consistently LOWER in ASD
    Proposed frontal-posterior underconnectivity

🟡 REFINED — local over + long-range under:
  → Courchesne & Pierce 2005 (Current Opinion Neurobiology 15(2)):
    Frontal cortex EXCESSIVELY connected locally
    POORLY synchronized with distant regions

🟢 CURRENT — IDIOSYNCRATIC patterns:
  → Hahamy et al. 2015 (Nature Neuroscience 18(2):302-309):
    ASD brains show IDIOSYNCRATIC distortions of connectivity
    Increased INTER-INDIVIDUAL VARIABILITY is the core feature
    Previous over/under findings = this heterogeneity mischaracterized
...
🟡 Amygdala:
  → Faster growth 6-24 months (Cohen's d = 0.56 by 12 months)
  → Right amygdala significantly larger in ASD (Hedges' g = 0.44)
...
🟡 GABA — reduced in specific regions:
  → Puts et al. 2017 (Autism Research 10(4):608-619):
    GABA reduced in SENSORIMOTOR cortex (MRS MEGA-PRESS)
    Correlated with tactile discrimination thresholds
    Region-specific (occipital GABA = normal)
    → Links to sensory processing differences (§2)
...
```
🟢 Bishop et al. 2013 (JADD 43(6):1287-1297, N=1,825):

  RRBs = NOT 1 thing — 2 GENUINELY DISTINCT constructs:

  FACTOR 1: RSM (Repetitive Sensory-Motor)
    → Stereotyped movements, hand flapping, spinning
    → Sensory seeking behaviors
    → CORRELATED with IQ (lower IQ → more RSM)
    → Likely SENSORIMOTOR mechanism
    → Framework: body-feedback REGULATION (§10 stimming)

  FACTOR 2: IS (Insistence on Sameness)
    → Resistance to change, routines, rituals
    → Insistence on predictability
    → INDEPENDENT of IQ
    → Likely EXECUTIVE/ANXIETY mechanism
...
🟡 PATH ② → ①: Severe ADHD + labeling → self-fulfilling
  ...
  Hornstra et al. 2023 (Educational Psychology Review): meta-analysis
    ...
    → → Produce negative self-fulfilling prophecies
    → = Label → lower expectations → less input → confirm label
```
...
🟢 RESEARCH — sensory processing differences extremely common:
  ...
  Ben-Sasson et al. 2009 (JADD 39(1):1-11): meta-analysis
    → 42-96% across studies (depends on measure)
    → Updated 2019: 55 studies confirm — ROBUST finding

  Marco et al. 2011 (Pediatric Research 69(5)):
    → 45-95% estimate range
    → Sensory processing = one of earliest observable markers

  Baranek et al. 2006 (J Child Psych & Psychiatry 47(6)):
...
```
🟢 Green et al. 2015 (JAMA Psychiatry 72(8):778-786):

  Study: fMRI sensory processing in ASD with/without SOR
  Finding: ASD+SOR group showed DECREASED NEURAL HABITUATION
  ...
  FRAMEWORK MAPPING:
    ...
    → Typical: sensory input → body-feedback → HABITUATE → signal fades
    → ASD+SOR: sensory input → body-feedback → NO HABITUATION → ACCUMULATES
...
```
🟢 Green et al. 2013 (JAACAP 52(11):1158-1172):

  Finding: ASD children with SOR showed HYPERACTIVATION in:
    → Primary sensory cortices (auditory, visual, somatosensory)
    → Amygdala
    → Hippocampus
    → Orbitofrontal cortex (OFC)
  Even to MILDLY AVERSIVE stimuli (scratchy fabric, moderately loud sounds)

  CRITICALLY:
    → Amygdala activation correlated with PARENT-RATED SOR severity
    → Correlation held EVEN CONTROLLING FOR ANXIETY
    → = Sensory overreactivity is NOT JUST ANXIETY — distinct mechanism
...
🟢 Foss-Feig et al. 2010 (Experimental Brain Research 203(2):381-389):
  ...
  WHAT THIS MEANS:
    → ASD brain treats 2 separate events as "together" even when
      separated by up to 600ms (instead of 300ms)
    → OR: ASD brain FAILS to separate events that occur within 600ms
    → = More events get "bound" together → more cross-modal interference
    → = Harder to parse rapid sequences (like speech + lip movement)
```

### §3.2 — Speech-specific deficit
...
🟢 Stevenson et al. 2014 (Journal of Neuroscience 34(3):691-697):
  ...
  Perceptual binding strength correlated with:
    ...
    → = Foundation matters — low-level timing → high-level integration

  IMPLICATION:
    → Speech = inherently audiovisual (lip movement + sound + facial expression)
    → Wider TBW for speech → audiovisual speech binding DIFFERENT
    → = Difficulty following conversation in group (multiple audio-visual streams)
    → = Preference for text-based communication (removes temporal binding requirement)
    → = Consistent with Sasson 2017: bias disappears with TEXT-only

🟢 Baum, Stevenson & Wallace 2015 (Progress in Neurobiology 134:140-160):
  Comprehensive review: ROBUST and CONSISTENT multisensory integration impairment
  Neuroimaging: failure to activate large-scale cortical networks
  Reduced long-range functional connectivity supports integration differences
```

### §3.3 — Framework: Social = inherently multi-modal

```
🟡 07-Social-Recognition-Arc.md §1.1:

  "There is no meaningful single-channel social event."

  Social event = MULTIPLE channels fire SIMULTANEOUSLY:
    → Visual (face + posture + gesture)
    → Auditory (voice + intonation + content)
    → Tactile (handshake, hug, proximity)
    → Interoceptive (emotional state mirroring)
    → Olfactory (pheromones, familiar scent)

  4 binding mechanisms (07-Social-Recognition-Arc §6):
    ① Temporal co-occurrence + Hebbian binding
    ② Multisensory neurons (superior colliculus + STS)
  ...
  IF 1+ mechanism DIFFERENT (not absent — DIFFERENT):
    ...
    → = NOT "social brain broken" — integration parameters DIFFERENT
```

### §3.4 — From integration to social: the cascade

```
🟡 FRAMEWORK SYNTHESIS:

  STEP 1: TBW wider (~600ms vs ~300ms)
    → Brain binds more events into single percept
    → OR fails to separate rapidly alternating events

  STEP 2: Social events = rapid multi-modal streams
    → Conversation: words at ~150ms intervals + facial expressions at ~200ms
    → Group conversation: multiple speakers overlapping
    → Each stream requires temporal binding PRECISION

  STEP 3: Wider TBW → DIFFERENT social chunk compilation
    → Nuances that depend on precise timing → missed or blurred
    → "Sarcasm" = requires detecting MISMATCH between tone (audio) and content
  ...
  → "Social deficit" reframe: INTEGRATION DIFFERENCE → COMPILATION DIFFERENCE
    → DOWNSTREAM cascade, not primary "social brain" failure
```

### §3.5 — Framework prediction (testable)

```
🔴 HYPOTHESIS:

  P2: TBW magnitude → predict social difficulty magnitude
    → Wider TBW → more integration differences → more social cascade
    → Testable: EEG/MEG TBW measurement per-individual
      → correlate with standardized social communication measures
    → If confirmed: TBW could become INDIVIDUAL assessment tool
      (not just group-level finding)
    → Per-individual: know TBW → predict WHICH social situations most difficult
      (rapid group conversation >> slow 1-on-1 >> text-only)
```
...
### §4.1 — Hypo-priors: world "too real"

```
🟢 Pellicano & Burr 2012 (TiCS 16(10):504-510):

  CORE CLAIM:
    → Typical brain: strong PRIORS (predictions based on past experience)
      → Perception = prior + sensory input (Bayesian brain)
      → Strong priors → SMOOTH experience (fill in gaps, predict next event)
    → Autistic brain: WEAKER PRIORS
      → Sensory data DOMINATES → world perceived "more raw"
      → Less smoothing → more detail perceived → BUT more overwhelming

  EXPLAINS:
    → Enhanced perceptual abilities (§9): weaker priors = less top-down smoothing
      → SEE MORE DETAIL (not filtered by expectation)
    → Sensory sensitivity: raw data without dampening = LOUDER, BRIGHTER, MORE
    → Resistance to optical illusions (some studies)
    → Difficulty with social prediction (social = heavily prior-dependent)

  FRAMEWORK MAPPING:
    → prediction-delta = difference between PREDICTED and ACTUAL
    → Weak priors → fewer/weaker predictions → MORE prediction-delta per event
    → = Body-feedback fires MORE OFTEN, for MORE things
    → = Consistent with sensory overload (§2) and reduced habituation (§2.3)
```

### §4.2 — HIPPEA: inflexible precision of prediction errors

```
🟢 Van de Cruys et al. 2014 (Psychological Review 121(4):649-675):

  CORE CLAIM:
    → Typical brain: FLEXIBLY adjusts precision of prediction errors
      → "Noisy context" → turn DOWN precision (tolerate more mismatch)
      → "Precise context" → turn UP precision (catch small errors)
    → Autistic brain: prediction errors given inflexibly HIGH precision
      → EVERY mismatch treated as IMPORTANT → cannot ignore minor deviations
      → = OVERFITTING to details

  EXPLAINS:
    → Insistence on sameness: changing routine = prediction error
      → HIGH precision → error treated as SIGNIFICANT → distress
...
### §4.3 — Aberrant Precision: WHERE matters

```
🟢 Lawson, Rees & Friston 2014 (Frontiers in Human Neuroscience 8:302):

  CORE CLAIM:
    → Precision allocation ATYPICAL across cortical hierarchy
    → NOT uniformly hypo or hyper — DIFFERENT at different levels
    → Low-level sensory: TOO MUCH precision (over-weighted sensory details)
    → High-level social/abstract: TOO LITTLE precision (weak top-down models)
    → = BRIDGE between Pellicano (weak high-level priors)
        and Van de Cruys (inflexible low-level precision)

  EXPLAINS:
    → BOTH enhanced perception (low-level precision high)
        AND social difficulty (high-level precision low)
    → Simultaneously: SEE details + MISS social gist
...
### §4.5 — Insistence on Sameness = prediction-delta REDUCTION
...
🟢 Bird & Cook 2013 (Translational Psychiatry 3:e285):
  ...
  LANDMARK STUDY:
    → Measured: emotion recognition, empathy, autism traits, alexithymia
    → CONTROLLING FOR ALEXITHYMIA:
      → Autism traits = NO independent effect on emotion recognition
      → Alexithymia = SOLE predictor of emotion recognition difficulty
    → = "Empathy deficit in autism" = ARTIFACT of alexithymia co-occurrence
    → = Autism WITHOUT alexithymia → TYPICAL empathy responses

🟢 Cook, Brewer, Shah & Bird 2013 (Psychological Science 24(5):723-732):
  → Facial emotion recognition:
    → Alexithymia predicted difficulty, autism did NOT
    → Confirmed Bird & Cook 2013 with different paradigm

🟢 Shah et al. 2016 (Cortex 81:215-220):
  ...
  SUMMARY:
    → "Empathy deficit in autism" → WRONG
    → "Empathy difficulty in alexithymia (which co-occurs ~50% with autism)" → CORRECT
    → 50% autistic WITH alexithymia → difficulty (but caused by alexithymia)
    → 50% autistic WITHOUT alexithymia → TYPICAL empathy
...
🟢 Crompton et al. 2025 (Nature Human Behaviour 9(7):1488-1500):
  → REGISTERED REPORT replication across 3 independent sites
  → Confirmed 2020 findings with stronger methodology
  → = Double empathy: ROBUST, REPLICATED finding
  ...
  → Solution = MUTUAL understanding (both sides learn each other's patterns)
```

### §5.5 — Thin-slice bias by neurotypical

```
🟢 Sasson et al. 2017 (Scientific Reports 7:40700):

  STUDY DESIGN:
    → Neurotypical participants watched brief clips of autistic + neurotypical adults
    → Rated: likability, attractiveness, desirability as friend

  RESULTS:
    → Neurotypical formed NEGATIVE impressions of autistic adults within SECONDS
    → Rated: less likable, less attractive, less desired as friend
    → = THIN-SLICE JUDGMENT — before any meaningful interaction

  CRITICAL FINDING:
    → Same content presented as TEXT-ONLY (transcript without video):
    → BIAS DISAPPEARED
    → = Neurotypical negative judgment based on PRESENTATION STYLE, not CONTENT
...
### §5.6 — Face processing: ACTIVE avoidance for self-protection

```
🟢 Hadjikhani et al. 2017 (Scientific Reports 7:3163):

  EXPERIMENTAL SETUP:
    → Autistic + neurotypical viewed faces in 2 conditions:
      (1) FREE VIEWING (look wherever they want)
      (2) CONSTRAINED GAZE (must look at eyes)

  RESULTS:
    → Free viewing: autistic activation = EQUIVALENT to neurotypical
      (when allowed to look where comfortable)
    → Constrained gaze: amygdala + superior colliculus HYPERACTIVATION
      = SUBCORTICAL arousal response

  INTERPRETATION:
    → Eye avoidance is NOT "indifference to faces"
    → Eye avoidance is NOT "failure to detect social stimuli"
    → Eye avoidance = ACTIVE SELF-PROTECTION from subcortical hyperarousal
    → Eye contact → amygdala fires STRONGLY → body-feedback OVERLOAD
    → Avoidance = REDUCE body-feedback load (same logic as §2.6)
...
🟢 Gernsbacher & Yergeau 2019 (Archives of Scientific Psychology 7(1):102-118):
  ...
  KEY FINDINGS:
    ...
    → Some autistic adults OUTPERFORM neurotypical on advanced ToM tasks

  CURRENT CONSENSUS:
    → NO consensus that ToM is inherently deficient in ALL autistic individuals
    → Language ability, alexithymia, IQ all confound
    → ToM = DIVERSE in autism (some impaired, some typical, some superior)
...
### §5.8 — Social motivation: not pure deficit
...
🟡 Chevallier et al. 2012 (TiCS 16(4):231-239):
  ...
  Current status: likely BOTH motivation AND ability AND context:
    ...
    → Some autistic people: LOW social motivation (genuine, not deficit — PREFERENCE)
    → Some: social motivation CONTEXT-DEPENDENT
      (high with autistic peers, low with neurotypical strangers)
...
🟡 FRAMEWORK DEFINITION:
  ...
  Masking = LEARN AND EXECUTE neurotypical social scripts MANUALLY:
    ...
    → Suppress stimming (despite body-feedback regulation need)
    → Mirror neurotypical conversational patterns (despite different processing)
    → "Look normal" across multiple channels simultaneously

  = COMPILATION FOR PERFORMANCE, not understanding
  = Autistic person compiles "how to ACT neurotypical" chunks
    WITHOUT the underlying neurotypical processing that makes these behaviors natural
  = Like learning a foreign language's PRONUNCIATION without SPEAKING the language
```

### §6.2 — Hull et al. 2017: 3-stage model

```
🟢 Hull et al. 2017 (JADD 47(8):2519-2534):

  Qualitative study → identified 3-stage process:

  STAGE 1: MOTIVATIONS
    → Fitting in, avoiding bullying, maintaining employment
    → "I mask because the CONSEQUENCES of not masking are worse"
    → = Cost-benefit calculation: masking cost < social punishment cost

  STAGE 2: TECHNIQUES
    → Masking: hiding autistic traits (suppress stim, force eye contact)
    → Compensation: learn explicit social rules (study body language, scripts)
    → "I watch how others do it and copy" — explicit learning of implicit rules

  STAGE 3: CONSEQUENCES
    → Exhaustion (most reported): "like acting all day every day"
...
```
🟢 Lai et al. 2017 (Autism 21(6):690-702):

  → Autistic women scored SIGNIFICANTLY higher on camouflaging:
    Cohen's d = 0.98 = LARGE effect size
  → 3/4 women vs 1/4 men: masking IMPACTED diagnosis access
  → = Women develop more sophisticated masking → look "less autistic"
    → delayed/missed diagnosis → delayed support → accumulated cost

🟢 Hull et al. 2020 (Autism 24(2)):
  → Confirmed: autistic females > autistic males on Masking + Assimilation subscales
  ...
  FRAMEWORK:
    ...
    → = Lai 2015: under-diagnosis in females = partly masking-driven
```

### §6.4 — Mental health cost: systematic evidence

```
🟢 Cage & Troxell-Whitman 2019 (JADD 49(5):1899-1911):
  → Higher camouflaging → lower wellbeing
  → Moderated by autistic identity: strong autistic identity = buffer

🟢 Cook et al. 2021 (Clinical Psychology Review 89:102080):
  → Systematic review: ALL evidence cross-sectional (cannot prove causation)
  → BUT consistent finding: higher masking → worse mental health
  → Anxiety, depression, suicidal ideation all associated with higher masking
  → Exhaustion/burnout = most commonly reported consequence

  CONSISTENT ACROSS STUDIES:
    → Masking ↑ → wellbeing ↓
    → Masking ↑ → anxiety ↑
    → Masking ↑ → depression ↑
    → Masking ↑ → identity confusion ↑
    → = CONVERGENT evidence even without RCT
```

### §6.5 — Framework mechanism: chronic mismatch

```
🟡 FRAMEWORK SYNTHESIS:

  Masking = CHRONIC PREDICTION-DELTA MISMATCH:

    Internal state: body-feedback says X (overwhelmed, confused, in pain)
    External performance: behavior shows Y (calm, attentive, "normal")
    = CONTINUOUS MISMATCH between body state and output

  Body-Feedback-Mechanism.md §4: Compound dynamics
    → Multiple simultaneous mismatches = COMPOUND body-feedback
    → Masking = MULTIPLE compounds:
      ① Suppress stimming = body-feedback regulation removed
      ② Force eye contact = add amygdala activation
      ③ Monitor social performance = PFC load added
      ④ Internal/external mismatch = continuous dissonance
  ...
  → Long-term: accumulated cost → mental health cascade (§6.4)
```

### §6.6 — Late diagnosis via masking

```
🟡 Lai et al. 2015 (JAACAP 54(1):11-24):

  "Female autism phenotype":
    → More social motivation (or better masking of low motivation)
    → Better surface-level social skills (compiled, not intuitive)
    → More internalizing (anxiety, depression instead of externalizing)
    → Interests appear more "typical" (animals, fiction vs trains, numbers)
  ...
  Framework:
    ...
    → Duration and difficulty of re-compilation ∝ years of masking
```
...
### §7.1 — Prevalence: ~50% autism vs ~5% general

```
🟢 Kinnaird et al. 2019 (European Psychiatry 55:80-89): meta-analysis

  → Alexithymia prevalence in autism: ~50%
  → Alexithymia prevalence in general population: ~5%
  → = ~10× HIGHER in autism — significant co-occurrence

  BUT:
    → 50% autistic people DO NOT have alexithymia
    → 5% non-autistic people DO have alexithymia
    → = 2 INDEPENDENT constructs that CO-OCCUR at high rate
    → = NOT "autism feature" — SEPARATE condition that OVERLAPS
```

### §7.2 — Alexithymia predicts, NOT autism

```
🟢 Bird & Cook 2013 (Translational Psychiatry 3:e285):
  → Controlling alexithymia: NO independent autism effect on emotion recognition
  → = "Emotion recognition deficit in autism" = artifact of alexithymia co-occurrence

🟢 Cook, Brewer, Shah & Bird 2013 (Psychological Science 24(5):723-732):
  → Facial emotion recognition:
    Alexithymia = predictor. Autism = NOT predictor.
  → Confirmed with different paradigm.

🟢 Shah et al. 2016 (Cortex 81:215-220):
  → Heartbeat detection task (interoceptive accuracy):
    Alexithymia predicted POOR accuracy. Autism did NOT.
  → = Body-signal reading difficulty = alexithymia, not autism.

  IMPLICATIONS:
    → Decades of research on "empathy deficit in autism" may have been
      measuring ALEXITHYMIA in autistic samples without controlling for it
    → When controlled: autism effect DISAPPEARS
    → = Critical methodological point for ALL autism empathy research
```

### §7.3 — Self-to-other model

```
🟢 Bird & Viding 2014 (Neuroscience & Biobehavioral Reviews 47:520-532):
  ...
  ALEXITHYMIA disrupts Step 1:
    → Cannot clearly READ own body-feedback
    → → Cannot clearly MAP to other person (Step 2 degraded)
    → → Empathic response ATTENUATED or absent

  AUTISM does NOT inherently impair Step 1:
    → Autistic without alexithymia: Step 1 intact → empathy intact
    → Autistic WITH alexithymia: Step 1 disrupted → empathy attenuated
...
### §7.4 — Framework mapping: Feeling.md + Somatic-Articulation-Loop
...
🟡 FRAMEWORK SYNTHESIS:
  ...
  PRACTICAL IMPLICATION:
    → Autistic + alexithymic: "Are you ok?" → "I don't know" = GENUINE
    → Body is NOT ok → but PFC cannot parse the signals
    → = Not avoidance, not deception — genuine difficulty READING own body
```

### §7.5 — Separation from autism: independent construct
...
🟢 SUMMARY OF INDEPENDENCE:
  ...
  ├──────────────────────┼────────────────────────┼──────────────────────┤
  │ WITH alexithymia     │ ~50% of autistic       │ ~5% of general pop   │
  │ (~50% / ~5%)         │ → empathy DIFFICULTY   │ → empathy DIFFICULTY │
  │                      │ → intero DIFFICULTY    │ → intero DIFFICULTY  │
  ├──────────────────────┼────────────────────────┼──────────────────────┤
  │ WITHOUT alexithymia  │ ~50% of autistic       │ ~95% of general pop  │
  │ (~50% / ~95%)        │ → empathy TYPICAL      │ → empathy TYPICAL    │
  │                      │ → intero TYPICAL       │ → intero TYPICAL     │
  └──────────────────────┴────────────────────────┴──────────────────────┘

  → Pattern is CLEAR: alexithymia drives empathy/intero difficulty
  → Autism presence/absence does NOT change the pattern
  → = 2 independent dimensions that CO-OCCUR at high rate
  → = Research must ALWAYS control for alexithymia when studying autism + empathy

  → WHY co-occur? Unknown. Possibilities:
    → Shared genetic factors
    → Sensory processing cascade → affects interoception → alexithymia develops
    → Early development: if body-feedback OVERWHELMING → learn to SUPPRESS reading
...
🟢 Special Interests — near-universal and POSITIVE:
  ...
  Prevalence:
    ...
    → = Near-UNIVERSAL feature of autism

  Positive impact:
    → Patten Koenig & Williams 2017 (OTMH 33(2):129-140):
      92% report Special Interests as CALMING / anxiety-alleviating
      86% incorporate Special Interests into job or education
      = Special Interests = RESOURCE, not pathology

    → Grove et al. 2018 (Autism Research 11(5):766-775):
      Special Interests positively associated with HIGHER subjective wellbeing
      Motivation for engagement = key predictor
      BUT: very high intensity → negatively associated (inverted-U)
      = Sweet spot: engaged + passionate, not overwhelmed by intensity

  "Restricted interest" (DSM-5 B3) = PROBLEMATIC framing:
    → "Restricted" implies limitation
    → Framework sees: CONCENTRATED gap-direction → deep expertise
    → "Intense interest" more accurate than "restricted interest"
```

### §8.2 — Framework: Gap-Direction concentrated
...
🟡 Gap-Direction.md v1.0:
  ...
  AUTISTIC SPECIAL INTERESTS through Gap-Direction:
    ...
    → = Few DEEP gaps → attention concentrated → extreme depth

  MECHANISM:
    → Sensory profile (§2) → certain inputs compile RICHER chunks
    → Richer chunks in domain → MORE surrounding structure → MORE defined gaps
    → More defined gaps → STRONGER gap-direction → STRONGER pull
    → Stronger pull → MORE time in domain → MORE chunks → DEEPER gaps
...
### §8.3 — Monotropism: attention tunneling

```
🟡 Murray, Lesser & Lawson 2005 (Autism 9(2):139-156):

  THEORY (by autistic researchers):
    → Monotropism: attention system allocates resources to FEW interests
    → vs Polytropism: attention distributed across MANY interests
...
🟢 Dwyer et al. 2024 (Neurodiversity 2(3)):
  → 492 autistic adults: empirical support for monotropism
  → Monotropic processing = sustained engagement + overwhelm vulnerability
  → CONSISTENT with self-report data

  FRAMEWORK MAPPING:
    → Gap-Direction.md: concentrated gaps → focused attention → deep compile
    → Monotropism = DESCRIPTION of the attention allocation PATTERN
    → Gap-Direction = MECHANISM explaining WHY pattern exists
    → = Monotropism OBSERVED at behavioral level
    → = Gap-Direction EXPLAINS at mechanism level
...
🟡 FRAMEWORK DISTINCTION (OCD-Analysis.md v2.1):
  ...
  │ Voluntary       │ Ego-dystonic (unwant)│ Ego-syntonic (wanted)│
  │ Function        │ REDUCES anxiety      │ BUILDS knowledge     │
...
```
🟢 Bross et al. 2022 (Journal of Vocational Rehabilitation 57(2):121-140):

  → Special Interests = resource for employment + wellbeing WHEN SUPPORTED
  → Autistic adults who incorporate Special Interests into work:
    → Higher job satisfaction
    → Better performance in Special Interests-related tasks
  ...
  FRAMEWORK IMPLICATION:
    → Support Special Interests → support coping + reward + identity
    → Special Interests → job match = Gap-Direction match → sustainable performance
    → Special Interests → education path = motivated learning → genuine compilation
    → = Architecture FEATURE enabling deep mastery in specific domains
```
...
### §9.1 — Enhanced Perceptual Functioning model

```
🟢 Mottron et al. 2006 (JADD 36(1):27-43):

  8 PRINCIPLES of autistic perception:

  ① Default superiority in low-level perceptual operations
  ② Locally oriented visual AND auditory processing
  ③ Enhanced discrimination abilities
  ④ Perception autonomous from higher-order operations
  ⑤ Perception serves pattern detection across modalities
  ⑥ Enhanced role of perceptual processes in intelligence
  ⑦ Expertise effects amplified through perceptual processing
  ⑧ Neural resources allocated MORE to perceptual processing

  CORE CLAIM:
    → Autistic perception = ENHANCED at low level, not deficit at high level
    → Superior pitch discrimination, visual search, pattern detection
    → = Gain HIGHER in specific processing streams (§2.5 cascade)
    → "Deficit" narrative FLIPPED: what looks like weakness in social = TRADE-OFF
      for genuine strength in perceptual processing
```

### §9.2 — IQ measurement bias: Raven's Matrices

```
🟢 Dawson et al. 2007 (Psychological Science):

  FINDING:
    → Autistic participants scored UP TO 30 PERCENTILE POINTS higher
      on Raven's Progressive Matrices vs Wechsler (WISC/WAIS)
    → Raven's = non-verbal pattern recognition (matrix completion)
    → Wechsler = verbal + performance composite

  IMPLICATION:
    → Standard IQ tests (Wechsler) UNDERESTIMATE autistic intelligence
    → Verbal component penalizes language differences
    → Social subtests penalize social processing differences
    → Raven's removes these confounds → reveals HIGHER pattern recognition
    → "Intellectual disability" prevalence in autism may be INFLATED
      by measurement tools biased toward neurotypical processing style

  FRAMEWORK:
    → Gain HIGH in pattern detection → better at Raven's
    → Language/social processing DIFFERENT → worse at Wechsler verbal
    → = NOT "lower IQ" — DIFFERENT cognitive profile measured by wrong tool
```

### §9.3 — Detail-focused cognitive STYLE (not deficit)

```
🟢 Happé & Frith 2006 (JADD 36(1):5-25):

  ORIGINAL "Weak Central Coherence" (WCC) theory:
    → Frith 1989: autistic processing = local detail at expense of global gist
    → = "Failure" to see the big picture

  UPDATED 2006:
    → Reframed from DEFICIT to COGNITIVE STYLE
    → "Detail-focused processing PREFERENCE"
    → When INSTRUCTED → CAN extract global meaning
    → = NOT inability for global — PREFERENCE for local
...
### §9.4 — Framework integration: why strengths emerge FROM configuration
...
🟡 FRAMEWORK SYNTHESIS:
  ...
  DETAIL PREFERENCE (§9.3) → local feature extraction
  ...
  NEURODIVERSITY HIRING (practical):
    → Pattern recognition for code review, QA, data analysis
    → Genuine advantage (not tokenism) IF role matches strengths
    → Gap-Direction.md: Special Interests→job match = sustainable performance
    → = Architecture FEATURE, not accommodation
```
...
🟡 FRAMEWORK ANALYSIS:
  ...
  MELTDOWN defined:
    → Body-feedback OVERLOAD → PFC goes OFFLINE → motor/emotional DISCHARGE
    → Crying, screaming, hitting, throwing, running
    → INVOLUNTARY — person cannot "choose" to stop

  MELTDOWN ≠ TANTRUM:
    → Tantrum = GOAL-DIRECTED (want something → escalate to get it)
    → Meltdown = OVERFLOW (body-feedback exceeds PFC capacity → discharge)
    → Tantrum: audience changes behavior. Remove audience → often stops.
    → Meltdown: audience irrelevant. Remove audience → continues.
    → = Fundamentally different mechanism — same behavioral surface
...
### §10.2 — Shutdown: WITHDRAWAL, not avoidance

```
🟡 FRAMEWORK ANALYSIS:

  SHUTDOWN defined:
    → Body-feedback OVERLOAD → PFC WITHDRAWAL → freeze/collapse
    → Going silent, becoming non-verbal, "spacing out," lying down
    → INVOLUNTARY — person cannot "choose" to engage

  SHUTDOWN = DIFFERENT EXPRESSION, SAME CAUSE:
    → Same trigger: body-feedback exceeds PFC capacity
    → Different output: discharge (meltdown) vs withdrawal (shutdown)
    → May depend on:
      → Individual nervous system profile (fight/flight vs freeze)
      → Context (public → shutdown, private → meltdown)
      → Current arousal level (moderate → meltdown, extreme → shutdown)

  PFC-Configuration.md:
    → Mode ④ Disconnected: PFC partially offline
    → Mode ⑤ Dissociated: PFC withdrawal, emotional numbness
    → Shutdown ≈ Mode ⑤ — defense mechanism, not "not caring"
```

### §10.3 — Neuroscience evidence

```
🟢 Green et al. 2013 (JAACAP 52(11)):
  → Amygdala HYPERACTIVATION to mildly aversive sensory stimuli
  → Even controlling for anxiety → sensory over-reactivity = DISTINCT
  → Severity correlated with parent-reported sensory difficulties

🟢 Green et al. 2015 (JAMA Psychiatry 72(8)):
  → REDUCED NEURAL HABITUATION in ASD+SOR
  → Brain does NOT "get used to" repeated stimuli
  → = CUMULATIVE: each additional input ADDS to load (no decay)
  → = "Fine in the morning, meltdown by afternoon" = ACCUMULATION, not trigger

  FRAMEWORK:
    → Reduced habituation + amplified prediction-delta + sensory gain high
    → = TRIPLE load on body-feedback system
    → Throughout the day: load ACCUMULATES (no habituation to reduce)
    → Final straw: seemingly minor event → capacity EXCEEDED → meltdown/shutdown
    → Observers: "They overreacted to nothing" → WRONG — overreacted to EVERYTHING
      accumulated over HOURS, final event was just the ONE that tipped the scale
```

### §10.4 — Stimming = REGULATION mechanism

```
🟢 Kapp et al. 2019 (Autism 23(7):1782-1792):

  Study: autistic adults' OWN perspectives on stimming
  → Self-regulatory mechanism (soothing, communicating, processing)
  → Suppressing stimming = EFFORTFUL + DISTRESSING
  → Stimming serves MULTIPLE functions:
    → Self-soothing (reduce body-feedback load)
    → Communication (express internal state)
    → Processing aid (rhythmic movement supports thinking)
    → Enjoyment (pleasant sensory input)

  FRAMEWORK:
    → Fidgeting-Analysis.md v1.0: fidgeting = body-feedback regulation MILD
    → Stimming = body-feedback regulation STRONGER
      (same mechanism, higher regulation NEED due to higher sensory load)
    → Stimming = body-feedback DRAIN mechanism:
      → Rhythmic movement → predictable sensory input → prediction-delta LOW
      → = Creates "safe" sensory stream that reduces cumulative load
      → = Like background music that masks unpredictable noise

  SUPPRESSING STIMMING:
    → Remove regulation mechanism WHILE load continues building
    → = INCREASES risk of meltdown/shutdown (no drain + accumulating load)
    → = Forcing "quiet hands" = removing fire extinguisher during fire
    → Correct approach: allow stimming + reduce ENVIRONMENTAL load
```
...
🟡 FRAMEWORK ANALYSIS:
  ...
  Body-feedback must DRAIN before PFC re-engages:
    ...
    → Body-first invariant (Feeling.md): body state → PFC capacity
      NOT PFC decision → body state

  WHAT HELPS:
    → Low-stimulation environment (reduce INPUT)
    → Time (body-feedback naturally drains if no new input)
    → Predictable, safe context (reduce prediction-delta)
    → Familiar sensory input (stimming, comfort objects, familiar textures)
    → NO additional demands (adding demands = adding load to already-overloaded system)

  WHAT HARMS:
    → Forcing engagement DURING overload → extends recovery
    → Demanding eye contact → adds amygdala activation
    → Bright/loud environment → adds sensory input
    → Social pressure ("calm down") → adds social prediction demand
    → "Processing" the event immediately → PFC not yet online
    → = All of above = ADDING LOAD to system that needs DRAINING
```
...
```
🟢 Earliest observable differences (meta-analyses):

  → Reduced joint attention (initiation + response) by 12-18 months
  → Delayed/absent pointing (both proto-imperative + proto-declarative)
  → Social smile timing may differ
  → Gaze patterns different (Farroni 2002 cross-ref: 03-Visual-System-Arc.md)
  → Response to name inconsistent
  → Less interest in faces (BUT: Hadjikhani 2017 — may be ACTIVE AVOIDANCE, not disinterest)

  07-Social-Recognition-Arc.md developmental events:
    → E12 Social smile (6-8 weeks): timing may differ
    → E14 Gaze following (6 months): reduced or different trajectory
    → E18 Joint attention (9-12 months): key divergence point
    → E19 Social referencing (9-12 months): reduced
    → Proto-declarative pointing (12-14 months): delayed or absent

  FRAMEWORK:
    → Early signs = DOWNSTREAM of sensory + integration cascade
    → Wider TBW (§3) from birth → social chunks compile DIFFERENTLY
    → Joint attention = multi-modal social event → integration difference → delay
    → "Early intervention" = support sensory + communication, not "fix" social
```

### §11.3 — Language: wide range within autism

```
🟡 Language development spectrum WITHIN autism:

  NON-VERBAL:
    → ~25-30% autistic children non-verbal or minimally verbal by age 5
    → Non-verbal ≠ non-thinking (critical distinction)
    → AAC (Augmentative Alternative Communication) can reveal rich inner world
    → Some remain non-verbal lifelong

  TYPICAL LANGUAGE TIMELINE:
    → Many autistic children reach language milestones on typical timeline
    → But PRAGMATICS (social use of language) often different
  ...
  ECHOLALIA:
    ...
    → NOT meaningless — often FUNCTIONAL:
      → Delayed echolalia = retrieving relevant phrase from chunk library
      → = Communication attempt using compiled audio chunks
    → Framework: chunk retrieval for communication when spontaneous generation difficult
```

### §11.4 — Adolescence: increased mental health risk

```
🟢 Research evidence:

  → Depression: ~4× higher than neurotypical peers (Hollocks 2019)
  → Anxiety: 39.6% (van Steensel 2011)
  → Risk increases through adolescence because:

  FRAMEWORK ANALYSIS:
    → Social demands INCREASE dramatically (middle/high school)
    → Peer relationships become MORE complex (subtle social rules)
...
```
🟢 Howlin et al. 2004 (JCPP 45(2):212-229):
  → 68 autistic adults followed up:
    Only 12% "very good" outcome (independent, employed)
    46% "poor" outcome (dependent, limited autonomy)
  → BUT: pre-2004 services and awareness — likely OUTDATED for current cohort
  → Intellectual ability = most consistent predictor

🟢 Tafolla et al. 2025 (Annual Review Clinical Psychology 21:193-220):
  → Lifespan review: cognitive ability MOST PREDICTIVE of trajectory
  → Emphasis: support across lifespan, not just childhood
  → Adult services = major gap worldwide

🟢 Aging — MASSIVE knowledge gap:
  → Happé & Charlton 2012 (Gerontology 58(1):70-78):
    Almost NO research on older autistic adults
    First large cohort now entering old age (diagnosis became common ~1990s)
  → Stewart & Happé 2025 (Annual Rev Dev Psych 7:461-484):
    Updated review — still limited data
    Open questions: cognitive aging trajectory? Dementia risk? Quality of life?
    → Framework: unknown whether cognitive reserve patterns differ in autism

  → = CRITICAL research gap — large population aging WITHOUT evidence base
```

### §11.6 — Late diagnosis: identity re-compile

```
🟡 Lai et al. 2015 (JAACAP 54(1):11-24):

  Female autism phenotype → under-diagnosis:
    → Better surface social skills (masking §6)
    → More internalizing (anxiety > externalizing)
    → Interests appear more "typical"
  ...
  PARALLEL with ADHD (ADHD-Observation.md §10.4):
    ...
    → Duration of re-compilation ∝ years of masking

  → Late diagnosis = NOT just "getting a label"
  → = FUNDAMENTAL re-compilation of self-chunks built over decades
```

### §11.7 — "Optimal Outcome" + Labeling Paradox
...
### §12.2 — 2D model: Configuration × Tuning
...
🟡 FRAMEWORK COMPARISON TABLE:
  ...
  BOTH have co-occurring:
    ...
    → Sensory processing differences (though different profile)
    → Social difficulty (though different cause)
    → Mental health risk (though different pathway)
```
...
```
🟢 van Steensel et al. 2011: meta-analysis
  → 39.6% autistic children meet criteria for at least 1 anxiety disorder
  → Specific phobia: 29.8%
  → OCD: 17.4%
  → Social anxiety: 16.6%
  → Generalized anxiety: 15.4%

  FRAMEWORK:
    → Prediction-delta LARGER (§4) → world less predictable → anxiety NATURAL response
    → Social environment unpredictable (§5) → social anxiety LOGICAL
    → Sensory overload possible anytime (§2) → anticipatory anxiety → phobia
    → = NOT "comorbidity" — CASCADE from same architecture under neurotypical-designed environment
```

### §13.2 — Depression: 23% current, 37% lifetime

```
🟢 Hollocks et al. 2019 (Psychological Medicine 49(4):559-572): meta-analysis (~26K)
  → Current depression: 23%
  → Lifetime depression: 37%
  → Risk INCREASES adolescence → adulthood

  FRAMEWORK:
    → Masking (§6) → chronic cortisol → depression pathway
    → Social withdrawal → reward system UNUSED → anhedonia
    → Avoidance of overloading environments → life NARROWS → meaning dissolves
    → "I'm broken" self-chunks (pre-diagnosis) → depression consistent with identity
    → = Multiple CONVERGENT pathways to depression, all architecture-linked
```

### §13.3 — Epilepsy + GI + Sleep

```
🟢 Epilepsy:
  → Tuchman & Rapin 2002 (Lancet Neurology 1(6)):
    ~20-30% autistic individuals have epilepsy
    Higher in lower-IQ, peaks in adolescence/young adulthood
  → Framework: E/I imbalance (§1.5) → seizure threshold affected

🟢 Gastrointestinal:
  → McElhanon et al. 2014 (Pediatrics 133(5):872-883): meta-analysis
    OR = 4.42 (autistic children ~4.4× more likely to have GI symptoms)
    Constipation, diarrhea, abdominal pain
  → Framework: interoceptive sensitivity differences + stress pathway
    Gut-brain axis × chronic body-feedback load

🟢 Sleep:
  → Richdale & Schreck 2009:
    Higher onset insomnia, night waking, reduced duration
    Possible melatonin abnormalities
  → Framework: sensory overload carry-over → body-feedback not fully drained by bedtime
    Prediction-delta not resolved → mind active → onset difficulty
    Cross-ref: Alzheimer-Analysis.md §11 (sleep and brain clearance)
```

### §13.4 — Framework: cascade, not comorbidity

```
🟡 FRAMEWORK SYNTHESIS:

  "Comorbidity" implies: separate, independent conditions co-occurring by chance.
  Framework OBSERVES: CASCADING from same architecture under load.

  ┌────────────────────────────────────────────────────────────┐
  │ Configuration (sensory + integration + predictive)         │
  │    ↓                                                       │
  ...
  │    ↓                                                       │
  │ Sleep disruption → daytime capacity reduced                │
  │    ↓                                                       │
  │ GI (gut-brain × stress) + more sensory sensitivity        │
  │    ↓                                                       │
  │ MORE load → MORE anxiety → CYCLE                          │
  └────────────────────────────────────────────────────────────┘

  → Treating "comorbidities" separately = missing ROOT
  → Supporting SENSORY environment + reducing MASKING demand
    → reduces LOAD → reduces cascade → reduces "comorbidities"
  → = One root (architecture + environment mismatch) → many branches
```
...
P2: TBW → SOCIAL (§3)
  ...
  Wider TBW → more integration difference → more social cascade
  🔴 Hypothesis — testable via EEG/MEG + standardized social measures
...
P5: ALEXITHYMIA → EMPATHY (§7)
  ...
  Controlling alexithymia → autistic empathy = typical
  🟡 Supported (Bird & Cook 2013, Shah 2016)
...
P8: MONOTROPISM → Special Interests + OVERWHELM (§8.3)
  Monotropic index → predict Special Interests intensity + overwhelm risk
  🔴 Hypothesis — Dwyer 2024 beginning empirical support

ALL P1-P8 = NEED EMPIRICAL VALIDATION PER-INDIVIDUAL
Framework generates predictions — DOES NOT confirm them.
```
...
### §15.1 — Confidence markers per section

```
🟢 ESTABLISHED RESEARCH (framework ENDORSES):
  → Sensory prevalence 95% (Tomchek & Dunn 2007)
  → TBW ~300ms → ~600ms (Foss-Feig 2010, Stevenson 2014)
  → Reduced habituation (Green 2015 JAMA Psychiatry)
  → Amygdala hyperactivation (Green 2013)
  → Double empathy (Milton 2012, Crompton 2020, Crompton 2025 Nat HB)
  → Alexithymia ~50% (Kinnaird 2019 meta)
  → Alexithymia NOT autism predicts empathy (Bird & Cook 2013, Shah 2016)
  → Eye avoidance = self-protection (Hadjikhani 2017)
  → Thin-slice bias (Sasson 2017)
  → Masking + gender (Hull 2017, Lai 2017 d=0.98)
  → Masking → mental health (Cook 2021 systematic review)
  → Heritability 64-91% (Tick 2016). 102 risk genes (Satterstrom 2020)
  → Idiosyncratic connectivity (Hahamy 2015 Nat Neurosci)
  → EPF 8 principles (Mottron 2006)
  → Raven's +30 percentile (Dawson 2007)
  → RRB 2-factor (Bishop 2013 N=1,825)
  → Stimming = self-regulation (Kapp 2019)
  → Special Interests wellbeing (Grove 2018), 92% calming (Patten Koenig 2017)
  → Serotonin hyperserotonemia >25% (Schain 1961, 60+ years)
  → 1 in 31 US (Shaw 2025 CDC), 3.4:1 sex ratio
...
🟡 FRAMEWORK SYNTHESIS (logically consistent, partially supported):
  ...
  → Stimming = body-feedback DRAIN mechanism
  → Late diagnosis = identity chunk re-compilation
  → 2D neurodiversity model (Configuration × Tuning)
  → Social difficulty = DOWNSTREAM of sensory + integration cascade

🔴 HYPOTHESIS (framework-derived, NEED validation):
...
Framework MAY BE WRONG about:
  ...
  ? Monotropism as attention mechanism (emerging, needs more replication)
  ? Masking → cortisol → burnout causal chain (all evidence cross-sectional)
  ? RRB 2-factor mapping to prediction-delta vs body-feedback
```

### §15.3 — Open questions

```
Q1: Which predictive coding mechanism per-individual?
    → Can EEG/fMRI distinguish Pellicano vs Van de Cruys vs Lawson?
    → If yes → different support strategies per-type

Q2: Can TBW predict social outcomes?
    → Testable: TBW measurement + longitudinal social assessment
    → Could become individual assessment tool

Q3: What determines alexithymia co-occurrence?
    → Shared genetics? Sensory cascade → affects interoception?
    → Early body-feedback overwhelming → learn to suppress reading?

Q4: Optimal support model per-individual configuration?
    → 1 in 31 = population-level challenge
    → Per-individual assessment → per-individual support plan
    → Framework could inform: sensory profile → predict support needs

Q5: Aging trajectory in autism?
    → Cognitive reserve patterns? Dementia risk?
    → Quality of life across lifespan? (Happé & Charlton 2012 — massive gap)

Q6: Monotropism → measurable attention allocation index?
    → Dwyer 2024: empirical support growing
    → Can monotropic processing be QUANTIFIED?

Q7: 4+ biological subtypes → match framework dimensions?
    → Princeton 2025: biologically distinct subtypes
    → Do subtypes map to sensory gain profiles? TBW? Predictive coding type?

Q8: Masking → burnout causal chain?
    → All evidence cross-sectional (Cook 2021)
    → RCT ethically complex (cannot randomly assign "more masking")
    → Longitudinal needed: masking trajectory → cortisol → outcomes
...
### §16.2 — Neurodegeneration cluster (comparison)
...
### §16.4 — Body-feedback + Sensory mechanism
...
### §16.5 — Development + Behavior

```
  → Child-Development-Mechanism.md v1.0 — 4+1 channel compile
     §11 Developmental trajectory: same architecture, different rates
  ...
  → OCD-Analysis.md v2.1 — "done" pipeline failure
     §8.4 Special Interests vs OCD distinction (gap fill vs loop)
  → Fidgeting-Analysis.md v1.0 — body-feedback regulation mild
     §10.4 Stimming = same mechanism, higher need
```
...
```
🟢 Primary research (sensory + integration):
  Tomchek SP, Dunn W. (2007) AJOT 61(2):190-200. Sensory prevalence 95%.
  Ben-Sasson A et al. (2009) JADD 39(1):1-11. Sensory meta-analysis.
  Marco EJ et al. (2011) Pediatric Research 69(5). Sensory review.
  Baranek GT et al. (2006) JCPP 47(6). Early sensory features.
  Tavassoli T et al. (2014) Autism 18(4):428-432. Mixed sensory patterns.
  Green SA et al. (2013) JAACAP 52(11):1158-1172. Amygdala SOR fMRI.
  Green SA et al. (2015) JAMA Psychiatry 72(8):778-786. Reduced habituation.
  Foss-Feig JH et al. (2010) Exp Brain Res 203(2):381-389. TBW ~600ms.
  Stevenson RA et al. (2014) J Neurosci 34(3):691-697. TBW speech-specific.
  Baum SH, Stevenson RA, Wallace MT. (2015) Prog Neurobiol 134:140-160.

🟢 Predictive coding:
  Pellicano E, Burr D. (2012) TiCS 16(10):504-510. Hypo-priors.
  Van de Cruys S et al. (2014) Psych Review 121(4):649-675. HIPPEA.
  Lawson RP, Rees G, Friston KJ. (2014) Front Hum Neurosci 8:302. Aberrant precision.

🟢 Social cognition + empathy:
  Milton DEM. (2012) Disability & Society 27(6):883-887. Double empathy.
  Crompton CJ et al. (2020) Autism 24(7):1704-1712. Information transfer.
  Crompton CJ et al. (2025) Nat Hum Behav 9(7):1488-1500. Registered replication.
  Bird G, Cook R. (2013) Transl Psychiatry 3:e285. Alexithymia not autism.
  Cook R, Brewer R, Shah P, Bird G. (2013) Psych Sci 24(5):723-732. Facial recognition.
  Shah P et al. (2016) Cortex 81:215-220. Interoception alexithymia.
  Bird G, Viding E. (2014) Neurosci Biobehav Rev 47:520-532. Self-to-other model.
  Kinnaird E et al. (2019) Eur Psychiatry 55:80-89. Alexithymia 50% meta.
  Sasson NJ et al. (2017) Sci Reports 7:40700. Thin-slice bias.
  Hadjikhani N et al. (2017) Sci Reports 7:3163. Eye avoidance active.
  Gernsbacher MA, Yergeau M. (2019) Arch Sci Psych 7(1):102-118. ToM critique.
  Chevallier C et al. (2012) TiCS 16(4):231-239. Social motivation.

🟢 Masking:
  Hull L et al. (2017) JADD 47(8):2519-2534. 3-stage model.
  Hull L et al. (2020) Autism 24(2). Gender differences.
  Lai MC et al. (2017) Autism 21(6):690-702. Camouflaging d=0.98.
  Cage E, Troxell-Whitman Z. (2019) JADD 49(5):1899-1911. Masking wellbeing.
  Cook J et al. (2021) Clin Psych Rev 89:102080. Masking systematic review.

🟢 Genetics + neuroscience:
  Bailey A et al. (1995) Psychological Medicine 25(1):63-77. Twin study.
  Tick B et al. (2016) JCPP 57(5):585-595. Heritability meta-analysis.
  Satterstrom FK et al. (2020) Cell 180(3):568-584. 102 risk genes.
  Gaugler T et al. (2014) Nat Genet 46:881-885. Common variation primary.
  Grove J et al. (2019) Nat Genet 51(3):431-444. GWAS meta-analysis.
  Hahamy A et al. (2015) Nat Neurosci 18(2):302-309. Idiosyncratic connectivity.
  Just MA et al. (2004) Brain 127(8):1811-1821. Underconnectivity.
  Courchesne E, Pierce K. (2005) Curr Opin Neurobiol 15(2):225-230. Local over.
  Courchesne E et al. (2003) JAMA 290(3):337-344. Early brain overgrowth.

🟢 Neurotransmitters + E/I:
  Schain RJ, Freedman DX. (1961) J Pediatrics 58:315-320. Serotonin.
  Cook EH, Leventhal BL. (1996) Curr Opin Pediatrics 8(4):348-354. 5-HT review.
  Puts NAJ et al. (2017) Autism Research 10(4):608-619. GABA reduced.
  Rubenstein JLR, Merzenich MM. (2003) Genes Brain Behav 2(5):255-267. E/I balance.
  Nelson SB, Valakh V. (2015) Neuron 87(4):684-698. E/I homeostasis.

🟢 Prevalence + demographics:
  Shaw KA et al. (2025) MMWR 74(SS-2):1-22. 1 in 31 (2022 data).
  Zeidan J et al. (2022) Autism Research 15(5):778-790. Global prevalence.
...
🟢 Special Interests + strengths:
  ...
  Mottron L et al. (2006) JADD 36(1):27-43. EPF 8 principles.
  Dawson M et al. (2007) Psychological Science. Raven's +30 percentile.
  Happé F, Frith U. (2006) JADD 36(1):5-25. Cognitive style not deficit.
  Murray D et al. (2005) Autism 9(2):139-156. Monotropism.
  Dwyer P et al. (2024) Neurodiversity 2(3). Monotropism empirical.

🟢 RRBs + stimming:
  Bishop SL et al. (2013) JADD 43(6):1287-1297. 2-factor RRB model.
  Kapp SK et al. (2019) Autism 23(7):1782-1792. Stimming self-regulation.

🟢 Co-occurring + development:
  Rommelse NNJ et al. (2010) Eur Child Adolesc Psychiatry 19(3):281-295.
  van Steensel FJA et al. (2011). Anxiety meta-analysis 39.6%.
  Hollocks MJ et al. (2019) Psych Medicine 49(4):559-572. Depression meta.
  Tuchman R, Rapin I. (2002) Lancet Neurology 1(6). Epilepsy.
  McElhanon BO et al. (2014) Pediatrics 133(5):872-883. GI meta-analysis.
  Howlin P et al. (2004) JCPP 45(2):212-229. Adult outcomes.
  Tafolla SA et al. (2025) Ann Rev Clin Psych 21:193-220. Lifespan.
  Happé F, Charlton RA. (2012) Gerontology 58(1):70-78. Aging gap.
  Stewart GR, Happé F. (2025) Ann Rev Dev Psych 7:461-484. Aging update.

🟢 Quasi-autism + deprivation (§1.9):
  Rutter M et al. (1999) JCPP 40(4):537-549. Coined "quasi-autism", ERA study.
  Rutter M, Kreppner J et al. (2007) JCPP 48(12):1200-1207. ERA follow-up age 11.
  Rodriguez-Perez M et al. (2023) JCPP 64(9):1292-1302. QA adult outcome.
  Nelson CA et al. (2007) Science 318(5858):1937-1940. BEIP RCT foster care.

🟢 Labeling + misdiagnosis (§1.9):
  Miodovnik A et al. (2015) J Pediatrics 166(6). ADHD→ASD delayed diagnosis.
  Hornstra J et al. (2023) Educational Psychology Review. Golem effect meta.

🟢 Optimal outcome + labeling paradox (§11.7):
  Fein D et al. (2013) JCPP. Optimal outcome 4-13%, no longer meet criteria.
  Cooper K et al. (2023) PMC. Autistic identity → lower depression.
  AMA Journal of Ethics. (2015) "The Autism Paradox."
  Cutler E. (2006) A Thorn in My Pocket. Temple Grandin mother's memoir.
```
