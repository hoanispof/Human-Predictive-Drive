...
ADHD-Observation.md ALREADY COVERS (reference — not repeated):
  ...
  + Emotional dysregulation + RSD + Anxiety chain (§5)
  + Time perception: temporal myopia (§6)
  + Hyperfocus × gap-direction (§7)
  + DMN interference (§8)
  + Inverted-U severity × outcome (§9)
  + Lifespan trajectory (§10)
  + Medication mechanism (§11)
...
THIS FILE ADDS (new value):
  ...
  + Background-Pattern accumulation: 3 Background-Patterns + cortisol CORRECTED mechanism
  + Satiation profile: generative bias + anticipation/consummation split
  + 3D cost-benefit: severity × fit × support (with subclinical creativity data)
  + Self-Created Threat calibration ADHD-specific
  + Late diagnosis dynamics: demographics + grief model
  + Collective role analysis (flagged as conceptual)
```
  ...
  ① BALANCING SELECTION (hypothesis):
    ...
    Status: Thagaard 2016 systematic review:
      "evolutionary accounts not subjected to empirical test,
       remain HYPOTHETICAL"
    → Framework: present as hypothesis, NOT confirmed

  ② HUNTER-GATHERER ADVANTAGE (hypothesis):
...
🟢 RESEARCH (multiple studies, high confidence):
  ...
  ⓐ GAZE CUEING SELECTIVE — eyes fail, arrows intact:
    🟢 Psychiatry Research 2016:
      ...
      → = NOT "can't pay attention" — "can't AUTO-READ social gaze"

  ⓑ ALPHA MODULATION INVERSE (neural marker):
    🟢 PMC6969336 (2019, n=45, EEG):
  ...
  FRAMEWORK INTERPRETATION:
    → "Micro-cue miss" (§2.1) = NOT just "distracted"
...
🟢 RESEARCH (meta-analyses, high confidence):
  ...
  🟢 Social cognition meta-analysis (49 studies, n=2,449):
    ...
    → ADHD adults: deficit NON-SIGNIFICANT
    → = Deficit SHRINKS dramatically with age

  🟢 Sells 2023 (Cognition & Emotion, 21 studies):
    → Visual emotion recognition: medium deficit overall
    → Children: LARGER deficit
    → Adults: MODERATE deficit (smaller than children)
  ...
  ⚠️ NUANCE:
    ...
    → = Quality of compiled chunks matters (§2.4)
```
...
🟡 FRAMEWORK SYNTHESIS (applying Compile-Taxonomy.md):
  ...
  ⚠️ Distribution percentages = HYPOTHESIS, not measured.
  Focus on PATHWAY distinction, not exact percentages.
```

### §2.5 — Passive → Active Avoidance: 2-Phase Model
...
### §2.7 — Positive Illusory Bias: Protective But Fragile

```
🟢 RESEARCH:

  POSITIVE ILLUSORY BIAS (PIB) — well-documented in ADHD:
    → ADHD children overestimate OWN competence vs objective measures
    → = Self-model MORE POSITIVE than reality
    → Occurs across: academic, social, behavioral domains

  Framework mapping:
    → PIB = protective self-model in early development
    → ADHD child: failure signals partially FILTERED (threshold → less pain)
    → + Remaining positive chunks → "I'm good at this" persists
...
🟢 RESEARCH:
  ...
  🟢 ADHD camouflaging 2024 (PMC11528950):
    → ADHD camouflaging > neurotypical but < autistic
    → = Masking HIERARCHY:

    Autism masking > ADHD masking > neurotypical social effort
...
🟢 RESEARCH (meta-analyses, CRITICAL CORRECTION):
  ...
  🟢 Cortisol reactivity meta-analysis 2018 (PMC5837926):
    → Cortisol REACTIVITY: overall r ≈ 0 (heterogeneous results)
    → Some studies: GREATER reactivity to acute stress
...
```
🟡 FRAMEWORK SYNTHESIS (mechanism corrected):

  CORRECTED LOOP (accounting for LOWER baseline + HIGHER reactivity):

  STEP 1: Background-Pattern [social = threat] COMPILED (from childhood)
    ↓
  STEP 2: Enter social situation → Background-Pattern fires → PERCEIVED threat
    → High reactivity → cortisol SPIKE (greater than neurotypical spike)
    ↓
  STEP 3: PFC already UNDER-FUELED (dopamine deficit)
    + cortisol spike → PFC FURTHER WEAKENED (Arnsten 2009)
    ↓
  STEP 4: Weakened PFC → LESS regulation → MORE social errors
    ↓
  STEP 5: Social errors → CONFIRM Background-Pattern → compile DEEPER
    ↓
  STEP 6: Deeper Background-Pattern → more social situations trigger spike → repeat

  = "Spike-crash cascade" (revised from "double-deficit cascade")
  → ADHD PFC: dopamine deficit (chronic) + cortisol spike (phasic)
  → = Two DIFFERENT temporal patterns compounding
...
### §5.2 — Anticipation vs Consummation: Feast-or-Famine Explained

```
🟢 RESEARCH (fMRI, high confidence):

  🟢 Plichta & Scheres 2014 (NeuroImage):
    → Ventral striatum (VS) hyporesponsive during ANTICIPATION
    → Effect size: d = 0.48-0.58 (medium)

  🟢 PLOS ONE 2014:
    → Anticipation: DECREASED response
    → Consummation (delivery): NORMAL or even INCREASED response

  FRAMEWORK INTERPRETATION — "FEAST-OR-FAMINE" EXPLAINED:
    → ADHD: CAN'T anticipate reward → no initiation signal
    → ADHD: reward ARRIVES → strong response → hyperfocus engaged
    → = "Can't START (anticipation deficit) but once started → CAN'T STOP"

  Mapping to daily experience:
    → "I KNOW I should start this task" → but NO anticipatory reward
    → = Activation energy MISSING (not motivation — ACTIVATION)
    → Once task PRODUCES reward (progress visible, novelty found):
...
🟡 FRAMEWORK SYNTHESIS + 🟢 RESEARCH (CRITICAL CORRECTION):
  ...
  ADHD × Addiction — SHARED pathway:
    ...
    → Stimulant medication (carefully managed) → REDUCES addiction risk
      (paradoxical — treats underlying deficit → less self-medication need)
```
...
🟢 RESEARCH (meta-analyses, high confidence):
  ...
  🟢 Tran 2026 (meta-analysis, 17,000+ individuals):
    ...
    → = ADHD traits help START ventures but HURT sustaining them

  🟢 Tran 2026 (creativity):
    → Subclinical ADHD: creativity benefit g = 0.36
    → Clinical ADHD: NO creativity benefit
    → = CRITICAL distinction

  🟢 Frontiers 2022 + ScienceDirect 2026:
    → Inverted-U confirmed: quadratic > linear fit
...
🟡 FRAMEWORK SYNTHESIS (applying Self-Created-Threat.md):
  ...
  Phase 3 (Intentional Self-Creation):
    ...
    → = Narrow optimal window between "enough push" and "too much"
```
...
🟡 FRAMEWORK-DERIVED (not empirically tested):
  ...
  "ANXIETY FLOOR" REFRAMED (§4.2 correction):
...
🟡 FRAMEWORK SYNTHESIS + 🟢 RESEARCH:
  ...
  CHILDHOOD (5-12):
    → Energy HIGH, masking cost LOW (environment more tolerant)
    → Background-Patterns BEGIN compiling (§4.1)
...
### §8.2 — Late Diagnosis Demographics

```
🟢 RESEARCH (large databases):

  GENDER GAP IN DIAGNOSIS:
    🟢 EurekAlert 2024 (large health databases):
      → Women diagnosed ~5 years later: 28.96 vs 24.13 average age
    🟢 PMC10173330:
  ...
  4-STEP POST-DIAGNOSIS PROCESS (framework-informed):
    ...
    Step 2: GRIEF → "what could I have been?" (weeks-months)
    Step 3: RECOMPILE → old Background-Pattern [defective] vs new [differently tuned] (months-years)
    Step 4: OPTIMIZATION → informed environment + strategy choices (ongoing)

  RISK AT EACH STEP:
    1: Over-identify ("everything is ADHD") → lose agency
    2: Stuck in grief → rumination loop
    3: Impatient → give up recompilation
    4: Over-optimize → life becomes "ADHD management" not living
```
...
```
🔴 CONCEPTUAL (plausible but untested):

  COMPLEMENTARY PAIRING (ADHD × neurotypical):
  ...
  WHY PAIRING > INDIVIDUAL (in theory):
    → ADHD alone: vision without execution → starts but doesn't finish
    → Neurotypical alone: execution without vision → finishes but may miss direction
...
### §9.2 — Double Empathy Extension

```
🔴 HYPOTHESIS (plausible, no ADHD-specific data):

  Milton 2012 "Double Empathy Problem" (Autism literature):
    → Communication difficulty = MUTUAL, not one-sided
    → Neurotype-matching predicts rapport (Crompton 2020, Morrison 2020)
    → = Autistic people communicate well WITH other autistic people
  ...
  DISTINCTION FROM AUTISM DOUBLE EMPATHY:
    ...
    → ADHD: SIGNAL difference (same processing, different THRESHOLD filter)
    → = ADHD version: easier to bridge (provide explicit cues → works)
    → = Autism version: harder to bridge (processing itself different)

  ⚠️ No ADHD-specific "double empathy" studies found.
  Framework extension based on mechanism parallel, NOT evidence.
```
...
```
  FRAMEWORK DOES NOT CLAIM:
    ✗ "ADHD is a superpower" — severity matters, severe = real impairment
    ✗ "Right environment fixes everything" — severe ADHD may need
      medical support regardless of environment
    ✗ "Evolutionary advantage confirmed" — hypotheses untested
    ✗ "ADHD and addiction are distinct" — shared vulnerability

  FRAMEWORK DOES CLAIM:
    ✓ Trade-off is REAL and MEASURABLE (inverted-U confirmed)
    ✓ Environment MODULATES outcome significantly
    ✓ Social compilation is DELAYED, not absent (meta-analysis confirmed)
    ✓ Masking has measurable cost (executive function → burnout mediation confirmed)
    ✓ Gaze cueing impairment is SELECTIVE, not general (replicated)
    ✓ Cortisol profile = DYSREGULATED, not simply elevated
    ✓ Subclinical creativity benefit EXISTS (g=0.36), clinical does NOT
    ✓ Gender diagnosis gap = documented (women +5 years)
```
...
🟡 FRAMEWORK SYNTHESIS (logically consistent, partially supported):
  ...
  → 3 Compiled Quality types applied to social (genuine/schema/threat)
  → 5 detection signals
  → Passing Tax (PFC budget split concept)
  → 3 Background-Patterns in ADHD
  → Generative satiation bias
  → 3D cost-benefit model (severity × fit × support)
  → Self-Created Threat narrower window for ADHD
  → Post-diagnosis 4-step process
  → Passive → Active avoidance 2-phase model

🔴 HYPOTHESIS (framework-derived, needs validation):
  → DRD4-7R positive selection (contested — Naka 2011)
  → Frequency-dependent equilibrium mechanism
  → Compiled suppress trajectory (longitudinal prediction)
...
CANNOT:
  ...
  - Predict individual outcomes from group-level data
  - Confirm evolutionary advantage hypothesis
  - Quantify PFC budget allocation

MAY BE WRONG ABOUT:
  ? Compiled suppress trajectory (no longitudinal data)
  ? 2-phase passive → active avoidance model
  ? Frequency-dependent equilibrium
  ? 90:10 pull:push ratio
  ? Subclinical prevalence estimates
```
