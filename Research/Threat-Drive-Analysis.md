...
```
🟡 3 nguồn KHÁC NHAU — cùng output cortisol, khác cơ chế:
...
```
🟡 (Reference: UD-Parenting.md §8.5, §8.6)
  ...
  Cơ chế ở trẻ:
    Nguồn 2 (Social threat): bố/mẹ quát = social pain
    ...
    Nguồn 3 (Anticipation): "nếu không học → tương lai tệ"
    → Trẻ nhỏ PFC YẾU → KHÔNG imagine xa được → KHÔNG hiểu threat
  ...
  Thay thế:
    ...
    Novelty-Drive: "cái này hay lắm, thử xem" → dopamine
    Experience-Drive: "làm xong sẽ feel tốt lắm" → opioid predict
    Connection-Drive: "mình làm cùng nhé" → oxytocin
...
```
→ Core-v7-UD.md: Schema = trigger, Hormone = sustain (Drive ↔ Satisfy loop)
...
→ Drive-Optimization.md §9: Pressure + Challenge + Autonomy (Pressure ≈ Threat-Drive nhẹ)
→ UD-Parenting.md §8.5-8.6: Pressure by age + Realtime calibration
→ Reward-Dual-System.md §6: Imagination → body respond 20-60%
→ Addiction-Analysis.md: Dopamine hijack = exception (hormone drive trực tiếp)
